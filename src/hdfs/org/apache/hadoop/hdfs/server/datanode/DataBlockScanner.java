...
class DataBlockScanner implements Runnable {
    ...
    static class BlockScanInfo implements Comparable<BlockScanInfo> {
        ...
        public boolean equals(Object other) {
            return other instanceof BlockScanInfo && compareTo((BlockScanInfo) other) == 0;
        }
    ...
    private synchronized long getNewBlockScanTime() {
        /*
         * If there are a lot of blocks, this returns a random time with in the
         * scan period. Otherwise something sooner.
         */
        long period = Math.min(scanPeriod, Math.max(blockMap.size(), 1) * 600 * 1000L);
        return System.currentTimeMillis() - scanPeriod + random.nextInt((int) period);
    }
    ...
    private synchronized void updateScanStatus(Block block, ScanType type, boolean scanOk) {
        ...
        if (info != null) {
            ...
            // It might already be removed. Thats ok, it will be caught next
            // time.
            info = new BlockScanInfo(block);
        }
    ...
    private void handleScanFailure(Block block) {
        ...
        try {
            ...
            /*
             * One common reason is that NameNode could be in safe mode. Should
             * we keep on retrying in that case?
             */
            LOG.warn("Failed to report bad block " + block + " to namenode : " + " Exception : "
    ...
    static private class LogEntry {
        ...
        /**
         * The format consists of single line with multiple entries. each entry
         * is in the form : name="value". This simple text and easily extendable
         * and easily parseable with a regex.
         */
        private static Pattern entryPattern = Pattern.compile("\\G\\s*([^=\\p{Space}]+)=\"(.*?)\"\\s*");

        static String newEnry(Block block, long time) {
            return "date=\"" + dateFormat.format(new Date(time)) + "\"\t " + "time=\"" + time + "\"\t " + "genstamp=\""
                    + block.getGenerationStamp() + "\"\t " + "id=\"" + block.getBlockId() + "\"";
        }
    ...
    private void verifyBlock(Block block) {
        ...
        /*
         * In case of failure, attempt to read second time to reduce transient
         * errors. How do we flush block data from kernel buffers before the
         * second read?
         */
        for (int i = 0; i < 2; i++) {
            boolean second = (i > 0);
            ...
            try {
                ...
                LOG.warn((second ? "Second " : "First ") + "Verification failed for " + block + ". Exception : "
                        + StringUtils.stringifyException(e));
    ...
    private boolean assignInitialVerificationTimes() {
        ...
        /*
         * Initially spread the block reads over half of MIN_SCAN_PERIOD so that
         ...
         * have lastScanTime of <= 0 . Loop until first entry has
    ...
    synchronized void printBlockReport(StringBuilder buffer, boolean summaryOnly) {
        ...
        double pctProgress = (totalBytesToScan == 0) ? 100 : (totalBytesToScan - bytesLeft) * 10000.0
                / totalBytesToScan / (100 - pctPeriodLeft + 1e-10);
     ...
     * times of the blocks. It rolls the current file when it is too big etc. If
     * there is an error while writing, it stops updating with an error message.
     */
    private static class LogFileHandler {
        ...
        private static final long minRollingPeriod = 6 * 3600 * 1000L; // 6
                                                                       // hours
        ...
        /**
         * Opens the log file for appending. Note that rolling will happen only
         * after "updateLineCount()" is called. This is so that line count could
         * be updated in a separate thread without delaying start up.
         * 
         * @param dir
         ...
         *            prefix of the file.
         * @param maxNumLines
         *            max lines in a file (its a soft limit).
         * @throws IOException
         */
        ...
        /**
         * Append "\n" + line. If the log file need to be rolled, it will done
         * after appending the text. This does not throw IOException when there
         * is an error while appending. Currently does not throw an error even
         * if rolling fails (may be it should?). return true if append was
         * successful.
         */
        ...
        /**
         * This is used to read the lines in order. If the data is not read
         * completely (i.e, untill hasNext() returns false), it needs to be
         * explicitly
         */
        private class Reader implements Iterator<String>, Closeable {
            ...
            public String next() {
                ...
                try {
                    ...
                    LOG.info("Could not reade next line in LogHandler : " + StringUtils.stringifyException(e));
                }
                return curLine;
            }
    ...
    public static class Servlet extends HttpServlet {
        ...
        public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
            ...
            DataBlockScanner blockScanner = (DataBlockScanner) getServletContext()
                    .getAttribute("datanode.blockScanner");
