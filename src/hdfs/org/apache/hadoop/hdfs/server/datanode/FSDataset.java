...
/**************************************************
 * FSDataset manages a set of data blocks. Each block has a unique name and an
 * extent on disk.
 * 
 ***************************************************/
public class FSDataset implements FSConstants, FSDatasetInterface {
    ...
    /**
     * A node type that can be built into a tree reflecting the hierarchy of
     * blocks on the local disk.
     */
    class FSDir {
        File dir;
        int numBlocks = 0;
     ...
     */
        ...
        /**
         * Find the metadata file for the specified block file. Return the
         * generation stamp from the name of the metafile.
         */
        ...
        void getVolumeMap(HashMap<Block, DatanodeBlockInfo> volumeMap, FSVolume volume) {
            ...
            for (int i = 0; i < blockFiles.length; i++) {
                if (Block.isBlockFilename(blockFiles[i])) {
                    long genStamp = getGenerationStampFromFile(blockFiles, blockFiles[i]);
                    volumeMap.put(new Block(blockFiles[i], blockFiles[i].length(), genStamp), new DatanodeBlockInfo(
                            volume, blockFiles[i]));
                }
            }
        }
        ...
        private boolean clearPath(File f, String[] dirNames, int idx) {
            ...
            if (dirNames != null) {
                ...
                return (childIdx >= 0 && childIdx < children.length) ? children[childIdx].clearPath(f, dirNames,
                        idx + 1) : false;
            }
    ...
    class FSVolume {
        ...
        /**
         * Temporary files. They get moved to the real block directory either
         * when the block is finalized or the datanode restarts.
         */
        ...
        /**
         * Recover detached files on datanode restart. If a detached block does
         * not exist in the original directory, then it is moved to the original
         * directory.
         */
    ...
    private BlockWriteStreams createBlockWriteStreams(File f, File metafile) throws IOException {
        return new BlockWriteStreams(new FileOutputStream(new RandomAccessFile(f, "rw").getFD()), new FileOutputStream(
     ...
     * This ensures that modifying this block does not modify data in any
     * existing snapshots.
     * 
     * @param block
     *            Block
     * @param numLinks
     *            Detach if the number of links exceed this value
     * @throws IOException
    ...
    static private <T> void updateBlockMap(Map<Block, T> blockmap, Block oldblock, Block newblock) throws IOException {
    ...
    /**
     * Try to update an old block to a new block. If there are ongoing create
     * threads running for the old block, the threads will be returned without
     * updating the block.
     * 
    ...
    private synchronized List<Thread> tryUpdateBlock(Block oldblock, Block newblock) throws IOException {
        ...
        File tmpMetaFile = new File(oldMetaFile.getParent(), oldMetaFile.getName() + "_tmp"
        ...
        if (oldgs > newblock.getGenerationStamp()) {
            throw new IOException("Cannot update block (id=" + newblock.getBlockId() + ") generation stamp from "
                    + oldgs + " to " + newblock.getGenerationStamp());
        }

        // update length
        if (newblock.getNumBytes() > oldblock.getNumBytes()) {
            throw new IOException("Cannot update block file (=" + blockFile + ") length from " + oldblock.getNumBytes()
                    + " to " + newblock.getNumBytes());
        }
        if (newblock.getNumBytes() < oldblock.getNumBytes()) {
    ...
    /**
     * Start writing to a block file If isRecovery is true and the block
     * pre-exists, then we kill all volumeMap.put(b, v); volumeMap.put(b, v);
        ...
        synchronized (this) {
            ...
            if (!isRecovery) {
                ...
                if (!oldmeta.renameTo(newmeta)) {
                    throw new IOException("Block " + b + " reopen failed. " + " Unable to move meta file  " + oldmeta
                            + " to tmp dir " + newmeta);
                }
    ...
    public void setChannelPosition(Block b, BlockWriteStreams streams, long dataOffset, long ckOffset)
        ...
        if (size < dataOffset) {
            String msg = "Trying to change block file offset of block " + b + " to " + dataOffset
                    + " but actual size of file is " + size;
    ...
    public void validateBlockMetadata(Block b) throws IOException {
        ...
        if (b.getNumBytes() != f.length()) {
            throw new IOException("Block " + b + " length is " + b.getNumBytes() + " does not match block file length "
                    + f.length());
        }
    ...
    /**
     * We're informed that a block is no longer valid. We could lazily
     * garbage-collect the block, but why bother? just get rid of it.
     */
    public void invalidate(Block invalidBlks[]) throws IOException {
        ...
        for (int i = 0; i < invalidBlks.length; i++) {
            ...
            synchronized (this) {
                ...
                if (f == null) {
                    DataNode.LOG.warn("Unexpected error trying to delete block " + invalidBlks[i]
                            + ". Block not found in blockMap." + ((v == null) ? " " : " Block found in volumeMap."));
                    error = true;
                    continue;
                }
                if (v == null) {
