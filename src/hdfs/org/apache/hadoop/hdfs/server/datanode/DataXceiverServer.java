...
class DataXceiverServer implements Runnable, FSConstants {
    ...
    /**
     * Maximal number of concurrent xceivers per node. Enforcing the limit is
     * required in order to avoid data-node running out of memory.
     */
    ...
    static class BlockBalanceThrottler extends BlockTransferThrottler {
         ...
         * 
         * Return true if the thread quota is not exceeded and the counter is
     ...
     * enough space. For now we set it to be the default block size set in the
     * server side configuration, which is not ideal because the default block
     * size should be a client-size configuration. A better solution is to
     * include in the header the estimated block size, i.e. either the actual
     * block size or the default block size.
     */
   ...
   */
    ...
    void kill() {
        assert datanode.shouldRun == false : "shoudRun should be set to false before killing";
        try {
            this.ss.close();
        } catch (IOException ie) {
            LOG.warn(datanode.dnRegistration + ":DataXceiveServer.kill(): " + StringUtils.stringifyException(ie));
        }
