...
class DataXceiver implements Runnable, FSConstants {
    ...
    public void run() {
        ...
        try {
            ...
            if (curXceiverCount > dataXceiverServer.maxXceiverCount) {
                throw new IOException("xceiverCount " + curXceiverCount + " exceeds the limit of concurrent xcievers "
    ...
    private void readBlock(DataInputStream in) throws IOException {
        ...
        final String clientTraceFmt = clientName.length() > 0 && ClientTraceLog.isInfoEnabled() ? String.format(
                DN_CLIENTTRACE_FORMAT, localAddress, remoteAddress, "%d", "HDFS_READ", clientName,
                datanode.dnRegistration.getStorageID(), block) : datanode.dnRegistration + " Served block " + block
                + " to " + s.getInetAddress();
        try {
            try {
    ...
    private void writeBlock(DataInputStream in) throws IOException {
        ...
        try {
            ...
            blockReceiver = new BlockReceiver(block, in, s.getRemoteSocketAddress().toString(), s
            ...
            if (targets.length > 0) {
                ...
                try {
                    ...
                    mirrorOut = new DataOutputStream(new BufferedOutputStream(NetUtils.getOutputStream(mirrorSock,
                            writeTimeout), SMALL_BUFFER_SIZE));
            ...
            if (client.length() == 0) {
                ...
                LOG.info("Received block " + block + " src: " + remoteAddress + " dest: " + localAddress + " of size "
                        + block.getNumBytes());
            }
    ...
    void getBlockChecksum(DataInputStream in) throws IOException {
        ...
        try {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("block=" + block + ", bytesPerCRC=" + bytesPerCRC + ", crcPerBlock=" + crcPerBlock + ", md5="
                        + md5);
            }
    ...
    private void replaceBlock(DataInputStream in) throws IOException {
        /* read header */
        long blockId = in.readLong();
        Block block = new Block(blockId, dataXceiverServer.estimateBlockSize, in.readLong()); // block
                                                                                              // id
                                                                                              // &
                                                                                              // generation
                                                                                              // stamp
        ...
        try {
            ...
            proxyOut.writeShort(DataTransferProtocol.DATA_TRANSFER_VERSION); // transfer
                                                                             // version
