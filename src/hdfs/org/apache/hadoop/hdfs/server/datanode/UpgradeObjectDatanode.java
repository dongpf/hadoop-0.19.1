...
/**
 * Base class for data-node upgrade objects. Data-node upgrades are run in
 * separate threads.
 */
public abstract class UpgradeObjectDatanode extends UpgradeObject implements Runnable {
     ...
     * 
     * The default implementation checks whether the data-node missed the
     * upgrade and throws an exception if it did. This leads to the data-node
     * shutdown.
     * 
     * Data-nodes usually start distributed upgrade when the name-node replies
     * to its heartbeat with a start upgrade command. Sometimes though, e.g.
     * when a data-node missed the upgrade and wants to catchup with the rest of
     * the cluster, it is necessary to initiate the upgrade directly on the
     * data-node, since the name-node might not ever start it. An override of
     * this method should then return true. And the upgrade will start after
     * data-ndoe registration but before sending its first heartbeat.
     * 
     * @param nsInfo
     *            name-node versions, verify that the upgrade object can talk to
     *            this name-node version if necessary.
     * 
     * @throws IOException
     * @return true if data-node itself should start the upgrade or false if it
     *         should wait until the name-node starts the upgrade.
     */
    boolean preUpgradeAction(NamespaceInfo nsInfo) throws IOException {
        ...
        // Missed the upgrade. Report problem to the name-node and throw
        // exception
    ...
    public void run() {
        ...
        if (getUpgradeStatus() < 100) {
            DataNode.LOG.info("\n   Distributed upgrade for DataNode version " + getVersion() + " to current LV "
     ...
     * 
     * Data-nodes finish upgrade at different times. The data-node needs to
     * re-confirm with the name-node that the upgrade is complete while other
     * nodes are still upgrading.
     */
