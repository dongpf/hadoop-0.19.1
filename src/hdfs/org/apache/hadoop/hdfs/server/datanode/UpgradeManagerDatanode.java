 ...
 * 
 * Distributed upgrades for a data-node are performed in a separate thread. The
 * upgrade starts when the data-node receives the start upgrade command from the
 * namenode. At that point the manager finds a respective upgrade object and
 * starts a daemon in order to perform the upgrade defined by the object.
 */
class UpgradeManagerDatanode extends UpgradeManager {
    ...
    synchronized void initializeUpgrade(NamespaceInfo nsInfo) throws IOException {
        ...
        DataNode.LOG.info("\n   Distributed upgrade for DataNode " + dataNode.dnRegistration.getName() + " version "
                + getUpgradeVersion() + " to current LV " + FSConstants.LAYOUT_VERSION + " is initialized.");
    ...
    public synchronized boolean startUpgrade() throws IOException {
        if (upgradeState) { // upgrade is already in progress
            assert currentUpgrades != null : "UpgradeManagerDatanode.currentUpgrades is null.";
            UpgradeObjectDatanode curUO = (UpgradeObjectDatanode) currentUpgrades.first();
            curUO.startUpgrade();
            return true;
        }
        if (broadcastCommand != null) {
            if (broadcastCommand.getVersion() > this.getUpgradeVersion()) {
                ...
                assert currentUpgrades == null : "UpgradeManagerDatanode.currentUpgrades is not null.";
                assert upgradeDaemon == null : "UpgradeManagerDatanode.upgradeDaemon is not null.";
        ...
        if (currentUpgrades == null) {
            DataNode.LOG.info("\n   Distributed upgrade for DataNode version " + getUpgradeVersion()
                    + " to current LV " + FSConstants.LAYOUT_VERSION + " cannot be started. "
    ...
    synchronized void processUpgradeCommand(UpgradeCommand command) throws IOException {
        assert command.getAction() == UpgradeCommand.UC_ACTION_START_UPGRADE : "Only start upgrade action can be processed at this time.";
        this.upgradeVersion = command.getVersion();
        // Start distributed upgrade
        if (startUpgrade()) // upgrade started
            return;
        throw new IOException("Distributed upgrade for DataNode " + dataNode.dnRegistration.getName() + " version "
                + getUpgradeVersion() + " to current LV " + FSConstants.LAYOUT_VERSION + " cannot be started. "
                + "The upgrade object is not defined.");
    }
    ...
    public synchronized void completeUpgrade() throws IOException {
        ...
        upgradeDaemon = null;
        DataNode.LOG.info("\n   Distributed upgrade for DataNode " + dataNode.dnRegistration.getName() + " version "
                + getUpgradeVersion() + " to current LV " + FSConstants.LAYOUT_VERSION + " is complete.");
    }
