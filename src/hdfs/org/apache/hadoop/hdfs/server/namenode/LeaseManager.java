 ...
 * 
 * 2.3) p obtains a new generation stamp form the namenode 2.4) p get the block
...
public class LeaseManager {
     ...
     * periodically checks in. If the client dies and allows its lease to
    ...
    class Lease implements Comparable<Lease> {
        ...
        /**
         * @return the path associated with the pendingFile and null if not
         *         found.
         */
    ...
    synchronized void changeLease(String src, String dst, String overwrite, String replaceBy) throws IOException {
        if (LOG.isDebugEnabled()) {
            LOG.debug(getClass().getSimpleName() + ".changelease: " + " src=" + src + ", dest=" + dst + ", overwrite="
                    + overwrite + ", replaceBy=" + replaceBy);
        }

        for (Map.Entry<String, Lease> entry : findLeaseWithPrefixPath(src, sortedLeasesByPath)) {
            final String oldpath = entry.getKey();
            final Lease lease = entry.getValue();
            final String newpath = oldpath.replaceFirst(java.util.regex.Pattern.quote(overwrite), replaceBy);
            if (LOG.isDebugEnabled()) {
    ...
    static private List<Map.Entry<String, Lease>> findLeaseWithPrefixPath(String prefix,
            SortedMap<String, Lease> path2lease) {
        if (LOG.isDebugEnabled()) {
    ...
    private synchronized void checkLeases() {
        for (; sortedLeases.size() > 0;) {
            ...
            // internalReleaseLease() removes paths corresponding to empty
            // files,
