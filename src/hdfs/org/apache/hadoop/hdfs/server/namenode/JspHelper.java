...
public class JspHelper {
    ...
    public static final UnixUserGroupInformation webUGI = UnixUserGroupInformation.createImmutable(conf
            .getStrings(WEB_UGI_PROPERTY_NAME));

    public static int defaultChunkSizeToView = conf.getInt("dfs.default.chunk.view.size", 32 * 1024);
    ...
    public JspHelper() {
        ...
        UnixUserGroupInformation.saveToConf(conf, UnixUserGroupInformation.UGI_PROPERTY_NAME, webUGI);
    }
    ...
    public DatanodeInfo bestNode(LocatedBlock blk) throws IOException {
        ...
        while (s == null) {
            ...
            InetSocketAddress targetAddr = NetUtils.createSocketAddr(chosenNode.getHost() + ":"
    ...
    public void streamBlockInAscii(InetSocketAddress addr, long blockId, long genStamp, long blockSize,
            long offsetIntoBlock, long chunkSizeToView, JspWriter out) throws IOException {
        ...
        DFSClient.BlockReader blockReader = DFSClient.BlockReader.newBlockReader(s, addr.toString() + ":" + blockId,
                blockId, genStamp, offsetIntoBlock, amtToRead, conf.getInt("io.file.buffer.size", 4096));
    ...
    public void addTableRow(JspWriter out, String[] columns, int row) throws IOException {
        ...
        for (int i = 0; i < columns.length; i++) {
            if (row / 2 * 2 == row) {// even
                out.print("<td style=\"vertical-align: top;background-color:LightGrey;\"><B>" + columns[i]
                        + "</B><br></td>");
            } else {
                out.print("<td style=\"vertical-align: top;background-color:LightBlue;\"><B>" + columns[i]
                        + "</B><br></td>");
    ...
    public String getInodeLimitText() {
        ...
        str += ".  Heap Size is " + FsShell.byteDesc(totalMemory) + " / " + FsShell.byteDesc(maxMemory) + " (" + used
                + "%) <br>";
        return str;
    }
    ...
    public void sortNodeList(ArrayList<DatanodeDescriptor> nodes, String field, String order) {
        ...
        class NodeComapare implements Comparator<DatanodeDescriptor> {
            static final int FIELD_NAME = 1, FIELD_LAST_CONTACT = 2, FIELD_BLOCKS = 3, FIELD_CAPACITY = 4,
                    FIELD_USED = 5, FIELD_PERCENT_USED = 6, FIELD_NONDFS_USED = 7, FIELD_REMAINING = 8,
                    FIELD_PERCENT_REMAINING = 9, SORT_ORDER_ASC = 1, SORT_ORDER_DSC = 2;
    ...
    public static void printPathWithLinks(String dir, JspWriter out, int namenodeInfoPort) throws IOException {
        try {
            ...
            out.print("<a href=\"browseDirectory.jsp" + "?dir=" + Path.SEPARATOR + "&namenodeInfoPort="
                    + namenodeInfoPort + "\">" + Path.SEPARATOR + "</a>");
            tempPath.append(Path.SEPARATOR);
            for (int i = 0; i < parts.length - 1; i++) {
                if (!parts[i].equals("")) {
                    tempPath.append(parts[i]);
                    out.print("<a href=\"browseDirectory.jsp" + "?dir=" + tempPath.toString() + "&namenodeInfoPort="
                            + namenodeInfoPort);
