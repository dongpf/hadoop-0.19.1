...
/**
 * 
 * This is the JMX management interface for getting runtime statistics of the
 * name node. Many of the statistics are sampled and averaged on an interval
 * which can be specified in the config file.
 * <p>
 * For the statistics that are sampled and averaged, one must specify a metrics
 * context that does periodic update calls. Most do. The default Null metrics
 * context however does NOT. So if you aren't using any other metrics context
 * then you can turn on the viewing and averaging of sampled metrics by
 * specifying the following two lines in the hadoop-meterics.properties file:
 * 
 * <pre>
