...
public class FSNamesystem implements FSConstants, FSNamesystemMBean {
    ...
    private long pendingReplicationBlocksCount = 0L, underReplicatedBlocksCount = 0L,
    ...
    private void initialize(NameNode nn, Configuration conf) throws IOException {
        ...
        this.dnthread = new Daemon(new DecommissionManager(this).new Monitor(conf.getInt(
                "dfs.namenode.decommission.interval", 30), conf.getInt("dfs.namenode.decommission.nodes.per.interval",
                5)));
        dnthread.start();
        ...
        InetSocketAddress datanodeSslPort = NetUtils.createSocketAddr(conf.get("dfs.datanode.https.address", infoHost
                + ":" + 50475));
    ...
    private void setConfigurationParameters(Configuration conf) throws IOException {
        ...
        this.defaultPermission = PermissionStatus.createImmutable(fsOwner.getUserName(), supergroup, new FsPermission(
                filePermission));
        ...
        this.heartbeatRecheckInterval = conf.getInt("heartbeat.recheck.interval", 5 * 60 * 1000); // 5
                                                                                                  // minutes
    ...
    NamespaceInfo getNamespaceInfo() {
        return new NamespaceInfo(dir.fsImage.getNamespaceID(), dir.fsImage.getCTime(), getDistributedUpgradeVersion());
    }
    ...
    synchronized BlocksWithLocations getBlocks(DatanodeID datanode, long size) throws IOException {
        ...
        if (node == null) {
            NameNode.stateChangeLog.warn("BLOCK* NameSystem.getBlocks: " + "Asking for blocks from an unrecorded node "
                    + datanode.getName());
            throw new IllegalArgumentException("Unexpected exception.  Got getBlocks message for datanode "
    ...
    private long addBlock(Block block, List<BlockWithLocations> results) {
        ArrayList<String> machineSet = new ArrayList<String>(blocksMap.numNodes(block));
        ...
        if (machineSet.size() == 0) {
            ...
            results.add(new BlockWithLocations(block, machineSet.toArray(new String[machineSet.size()])));
    ...
    LocatedBlocks getBlockLocations(String clientMachine, String src, long offset, long length) throws IOException {
        ...
        if (blocks != null) {
            // sort the blocks
            DatanodeDescriptor client = host2DataNodeMap.getDatanodeByHost(clientMachine);
    ...
    public LocatedBlocks getBlockLocations(String src, long offset, long length, boolean doAccessTime)
        ...
        final LocatedBlocks ret = getBlockLocationsInternal(src, dir.getFileINode(src), offset, length,
                Integer.MAX_VALUE, doAccessTime);
        if (auditLog.isInfoEnabled()) {
            logAuditEvent(UserGroupInformation.getCurrentUGI(), Server.getRemoteIp(), "open", src, null, null);
        }
        return ret;
    }

    private synchronized LocatedBlocks getBlockLocationsInternal(String src, INodeFile inode, long offset, long length,
            int nrBlocksToReturn, boolean doAccessTime) throws IOException {
        if (inode == null) {
            return null;
        }
        ...
        do {
            ...
            if (numCorruptNodes != numCorruptReplicas) {
                LOG.warn("Inconsistent number of corrupt replicas for " + blocks[curBlk] + "blockMap has "
    ...
    public synchronized void setTimes(String src, long mtime, long atime) throws IOException {
        ...
        if (inode != null) {
            ...
            if (auditLog.isInfoEnabled()) {
                final FileStatus stat = dir.getFileInfo(src);
                logAuditEvent(UserGroupInformation.getCurrentUGI(), Server.getRemoteIp(), "setTimes", src, null, stat);
            }
        } else {
    ...
    public boolean setReplication(String src, short replication) throws IOException {
        ...
        if (status && auditLog.isInfoEnabled()) {
            logAuditEvent(UserGroupInformation.getCurrentUGI(), Server.getRemoteIp(), "setReplication", src, null, null);
        }
        return status;
    }
    ...
    void startFile(String src, PermissionStatus permissions, String holder, String clientMachine, boolean overwrite,
        ...
        if (auditLog.isInfoEnabled()) {
            final FileStatus stat = dir.getFileInfo(src);
            logAuditEvent(UserGroupInformation.getCurrentUGI(), Server.getRemoteIp(), "create", src, null, stat);
        }
    }

    private synchronized void startFileInternal(String src, PermissionStatus permissions, String holder,
            String clientMachine, boolean overwrite, boolean append, short replication, long blockSize)
            throws IOException {
        if (NameNode.stateChangeLog.isDebugEnabled()) {
            NameNode.stateChangeLog.debug("DIR* NameSystem.startFile: src=" + src + ", holder=" + holder
                    + ", clientMachine=" + clientMachine + ", replication=" + replication + ", overwrite=" + overwrite
                    + ", append=" + append);
        }
        ...
        try {
            ...
            if (append) {
                if (myFile == null) {
                    throw new FileNotFoundException("failed to append to non-existent file " + src + " on client "
                            + clientMachine);
                ...
                if (overwrite) {
                    ...
                    throw new IOException("failed to create file " + src + " on client " + clientMachine
            ...
            if (append) {
                ...
                INodeFileUnderConstruction cons = new INodeFileUnderConstruction(node.getLocalNameBytes(),
                        node.getReplication(), node.getModificationTime(), node.getPreferredBlockSize(),
                        node.getBlocks(), node.getPermissionStatus(), holder, clientMachine, clientNode);
                ...
                INodeFileUnderConstruction newNode = dir.addFile(src, permissions, replication, blockSize, holder,
                ...
                if (NameNode.stateChangeLog.isDebugEnabled()) {
                    NameNode.stateChangeLog.debug("DIR* NameSystem.startFile: " + "add " + src + " to namespace for "
                            + holder);
                }
            }
        } catch (IOException ie) {
    ...
    LocatedBlock appendFile(String src, String holder, String clientMachine) throws IOException {
        ...
        startFileInternal(src, null, holder, clientMachine, false, true, (short) maxReplication, (long) 0);
        getEditLog().logSync();
        ...
        if (auditLog.isInfoEnabled()) {
            logAuditEvent(UserGroupInformation.getCurrentUGI(), Server.getRemoteIp(), "append", src, null, null);
        }
        return lb;
    }
    ...
    private void checkLease(String src, String holder, INode file) throws IOException {
        ...
        if (file == null || file.isDirectory()) {
            Lease lease = leaseManager.getLease(new StringBytesWritable(holder));
            throw new LeaseExpiredException("No lease on " + src + " File does not exist. "
                    + (lease != null ? lease.toString() : "Holder " + holder + " does not have any open files."));
        }
        if (!file.isUnderConstruction()) {
            Lease lease = leaseManager.getLease(new StringBytesWritable(holder));
            throw new LeaseExpiredException("No lease on " + src + " File is not open for writing. "
                    + (lease != null ? lease.toString() : "Holder " + holder + " does not have any open files."));
        }
        INodeFileUnderConstruction pendingFile = (INodeFileUnderConstruction) file;
        if (holder != null && !pendingFile.getClientName().equals(holder)) {
            throw new LeaseExpiredException("Lease mismatch on " + src + " owned by " + pendingFile.getClientName()
    ...
    private synchronized CompleteFileStatus completeFileInternal(String src, String holder) throws IOException {
        ...
        if (fileBlocks == null) {
            NameNode.stateChangeLog.warn("DIR* NameSystem.completeFile: " + "failed to complete " + src
                    + " because dir.getFileBlocks() is null " + " and pendingFile is "
                    + ((pendingFile == null) ? "null" : ("from " + pendingFile.getClientMachine())));
        ...
        if (NameNode.stateChangeLog.isDebugEnabled()) {
            NameNode.stateChangeLog.debug("DIR* NameSystem.completeFile: " + src + " blocklist persisted");
        }
    ...
    private void checkReplicationFactor(INodeFile file) {
        ...
        for (int i = 0; i < nrBlocks; i++) {
            ...
            if (number.liveReplicas() < numExpectedReplicas) {
                neededReplications.add(pendingBlocks[i], number.liveReplicas(), number.decommissionedReplicas,
                        numExpectedReplicas);
            }
        }
    }
    ...
    private Block allocateBlock(String src, INode[] inodes) throws IOException {
        Block b = null;
        do {
            b = new Block(FSNamesystem.randBlockId.nextLong(), 0, getGenerationStamp());
    ...
    private void addToInvalidates(Block b, DatanodeInfo n) {
        ...
        NameNode.stateChangeLog.info("BLOCK* NameSystem.addToInvalidates: " + b.getBlockName()
                + " is added to invalidSet of " + n.getName());
    }
    ...
    public synchronized void markBlockAsCorrupt(Block blk, DatanodeInfo dn) throws IOException {
        ...
        if (node == null) {
            throw new IOException("Cannot mark block" + blk.getBlockName() + " as corrupt because datanode "
        ...
        if (!blocksMap.contains(blk, node)) {
            ...
            NameNode.stateChangeLog.info("BLOCK NameSystem.markBlockAsCorrupt: " + "block " + blk
                    + " could not be marked " + "as corrupt as it does not exists in " + "blocksMap");
        } else {
    ...
    public synchronized void invalidateBlock(Block blk, DatanodeInfo dn) throws IOException {
        ...
        DatanodeDescriptor node = getDatanode(dn);
        if (node == null) {
            throw new IOException("Cannot invalidate block " + blk + " because datanode " + dn.getName()
                    + " does not exist.");
        }
    ...
    public boolean renameTo(String src, String dst) throws IOException {
        ...
        if (status && auditLog.isInfoEnabled()) {
            ...
            logAuditEvent(UserGroupInformation.getCurrentUGI(), Server.getRemoteIp(), "rename", src, dst, stat);
        }
        return status;
    }
    ...
    private synchronized boolean renameToInternal(String src, String dst) throws IOException {
        ...
        if (isPermissionEnabled) {
            ...
            // but for now,
            String actualdst = dir.isDir(dst) ? dst + Path.SEPARATOR + new Path(src).getName() : dst;
    ...
    public boolean delete(String src, boolean recursive) throws IOException {
        ...
        getEditLog().logSync();
        if (status && auditLog.isInfoEnabled()) {
            logAuditEvent(UserGroupInformation.getCurrentUGI(), Server.getRemoteIp(), "delete", src, null, null);
        }
        return status;
    }
     ...
     */
    synchronized boolean deleteInternal(String src, boolean enforceSafeMode, boolean enforcePermission)
            throws IOException {
        if (NameNode.stateChangeLog.isDebugEnabled()) {
    ...
    public boolean mkdirs(String src, PermissionStatus permissions) throws IOException {
        ...
        if (status && auditLog.isInfoEnabled()) {
            final FileStatus stat = dir.getFileInfo(src);
            logAuditEvent(UserGroupInformation.getCurrentUGI(), Server.getRemoteIp(), "mkdirs", src, null, stat);
        }
        return status;
    }
    ...
    void fsync(String src, String clientName) throws IOException {
        ...
        NameNode.stateChangeLog.info("BLOCK* NameSystem.fsync: file " + src + " for " + clientName);
        synchronized (this) {
            if (isInSafeMode()) {
     ...
     *            The filename
     * @param holder
     *            The datanode that was creating the file
     */
    void internalReleaseLease(Lease lease, String src) throws IOException {
        ...
        if (iFile == null) {
            final String message = "DIR* NameSystem.internalReleaseCreate: " + "attempt to release a create lock on "
    ...
    synchronized void commitBlockSynchronization(Block lastblock, long newgenerationstamp, long newlength,
        ...
        if (!iFile.isUnderConstruction()) {
            throw new IOException("Unexpected block (=" + lastblock + ") since the file (=" + iFile.getLocalName()
        ...
        if (deleteblock) {
            ...
            // There should be no locations in the blocksMap till now because
            // the
        ...
        finalizeINodeFileUnderConstruction(src, pendingFile);
        getEditLog().logSync();
        LOG.info("commitBlockSynchronization(newblock=" + lastblock + ", file=" + src + ", newgenerationstamp="
    ...
    /**
     * Get a listing of all files at 'src'. The Object[] array exists so we can
     * return file attributes (soon to be implemented)
     */
    public FileStatus[] getListing(String src) throws IOException {
        ...
        if (auditLog.isInfoEnabled()) {
            logAuditEvent(UserGroupInformation.getCurrentUGI(), Server.getRemoteIp(), "listStatus", src, null, null);
        }
    ...
    public synchronized void registerDatanode(DatanodeRegistration nodeReg) throws IOException {
        ...
        DatanodeID dnReg = new DatanodeID(dnAddress + ":" + nodeReg.getPort(), nodeReg.getStorageID(),
                nodeReg.getInfoPort(), nodeReg.getIpcPort());
        nodeReg.updateRegInfo(dnReg);

        NameNode.stateChangeLog.info("BLOCK* NameSystem.registerDatanode: " + "node registration from "
                + nodeReg.getName() + " storage " + nodeReg.getStorageID());
        ...
        if (nodeS != null) {
            if (nodeN == nodeS) {
                ...
                NameNode.stateChangeLog
                        .info("BLOCK* NameSystem.registerDatanode: " + "node " + nodeS.getName() + " is replaced by "
                                + nodeReg.getName() + " with the same storageID " + nodeReg.getStorageID());
            }
            // update cluster map
        ...
        if (nodeReg.getStorageID().equals("")) {
            ...
            NameNode.stateChangeLog.debug("BLOCK* NameSystem.registerDatanode: " + "new storageID "
        ...
        synchronized (neededReplications) {
            ...
            for (int blkCnt = 0; blkCnt < blocksToProcess; blkCnt++, replIndex++) {
                ...
                DatanodeDescriptor targets[] = replicator.chooseTarget(
                        Math.min(requiredReplication - numEffectiveReplicas, maxTargets), srcNode, containingNodes,
                        null, block.getNumBytes());
                if (targets.length == 0)
                    continue;
                ...
                if (NameNode.stateChangeLog.isInfoEnabled()) {
                    ...
                    NameNode.stateChangeLog.info("BLOCK* ask " + srcNode.getName() + " to replicate " + block + " to "
                            + targetList);
    ...
    void processPendingReplications() {
        ...
        if (timedOutItems != null) {
            ...
            /*
             * If we know the the target datanodes where the replication
    ...
    synchronized public void removeDatanode(DatanodeID nodeID) throws IOException {
        ...
        if (nodeInfo != null) {
            ...
            NameNode.stateChangeLog.warn("BLOCK* NameSystem.removeDatanode: " + nodeID.getName() + " does not exist");
        }
    }
    ...
    void unprotectedAddDatanode(DatanodeDescriptor nodeDescr) {
        ...
        NameNode.stateChangeLog.debug("BLOCK* NameSystem.unprotectedAddDatanode: " + "node " + nodeDescr.getName()
    ...
    void wipeDatanode(DatanodeID nodeID) throws IOException {
        ...
        NameNode.stateChangeLog.debug("BLOCK* NameSystem.wipeDatanode: " + nodeID.getName() + " storage " + key
    ...
    public synchronized void processReport(DatanodeID nodeID, BlockListAsLongs newReport) throws IOException {
        ...
        for (Block b : toInvalidate) {
            NameNode.stateChangeLog.info("BLOCK* NameSystem.processReport: block " + b + " on " + node.getName()
                    + " size " + b.getNumBytes() + " does not belong to any file.");
            addToInvalidates(b, node);
        }
    ...
    synchronized Block addStoredBlock(Block block, DatanodeDescriptor node, DatanodeDescriptor delNodeHint) {
        ...
        if (storedBlock == null || storedBlock.getINode() == null) {
            ...
            NameNode.stateChangeLog.info("BLOCK* NameSystem.addStoredBlock: " + "addStoredBlock request received for "
                    + block + " on " + node.getName() + " size " + block.getNumBytes()
        ...
        if (block != storedBlock) {
            if (block.getNumBytes() >= 0) {
                ...
                if (cursize == 0) {
                    ...
                    LOG.warn("Inconsistent size for block " + block + " reported from " + node.getName()
                            + " current size is " + cursize + " reported size is " + block.getNumBytes());
                    try {
                        if (cursize > block.getNumBytes()) {
                            ...
                            //
                            storedBlock = blocksMap.getStoredBlock(block); // extra
                                                                           // look
                                                                           // up!
                            if (storedBlock == null) {
                                LOG.warn("Block " + block + " reported from " + node.getName()
        ...
        if (added) {
            ...
            //
            if (!isInSafeMode()) {
                NameNode.stateChangeLog.info("BLOCK* NameSystem.addStoredBlock: " + "blockMap updated: "
                        + node.getName() + " is added to " + block + " size " + block.getNumBytes());
            }
        } else {
            NameNode.stateChangeLog.warn("BLOCK* NameSystem.addStoredBlock: "
                    + "Redundant addStoredBlock request received for " + block + " on " + node.getName() + " size "
                    + block.getNumBytes());
        }
    ...
    void invalidateCorruptReplicas(Block blk) {
        ...
        for (Iterator<DatanodeDescriptor> it = nodes.iterator(); it.hasNext();) {
            ...
            try {
                ...
                NameNode.stateChangeLog.info("NameNode.invalidateCorruptReplicas " + "error in deleting bad block "
                        + blk + " on " + node + e);
                gotException = true;
            }
        }
    ...
    public synchronized void blockReceived(DatanodeID nodeID, Block block, String delHint) throws IOException {
        ...
        if (node == null) {
            NameNode.stateChangeLog.warn("BLOCK* NameSystem.blockReceived: " + block
                    + " is received from an unrecorded node " + nodeID.getName());
            throw new IllegalArgumentException("Unexpected exception.  Got blockReceived message from node " + block
    ...
    private synchronized ArrayList<DatanodeDescriptor> getDatanodeListForReport(DatanodeReportType type) {
    ...
    public synchronized DatanodeInfo[] datanodeReport(DatanodeReportType type) throws AccessControlException {
        checkSuperuserPrivilege();
   ...
   */
    ...
    private boolean inHostsList(DatanodeID node, String ipAddr) {
        ...
        return (hostsList.isEmpty() || (ipAddr != null && hostsList.contains(ipAddr))
                || hostsList.contains(node.getHost()) || hostsList.contains(node.getName()) || ((node instanceof DatanodeInfo) && hostsList
    ...
    public String randomDataNode() {
        ...
        if (size != 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (d != null && !d.isDecommissioned() && !isDatanodeDead(d) && !d.isDecommissionInProgress()) {
    ...
    class SafeModeInfo {
        ...
        synchronized void leave(boolean checkForUpgrades) {
            ...
            reached = -1;
            safeMode = null;
            NameNode.stateChangeLog.info("STATE* Network topology has " + clusterMap.getNumOfRacks() + " racks and "
        ...
        String getTurnOffTip() {
            ...
            if (blockTotal < 0)
                return leaveMsg + ".";
            String safeBlockRatioMsg = String.format("The ratio of reported blocks %.4f has "
                    + (reached == 0 ? "not " : "") + "reached the threshold %.4f. ", getSafeBlockRatio(), threshold)
                    + leaveMsg;
            if (reached == 0 || isManual()) // threshold is not reached or
                                            // manual
                return safeBlockRatioMsg + ".";
            // extension period is in progress
            return safeBlockRatioMsg + " in " + Math.abs(reached + extension - now()) / 1000 + " seconds.";
        }
        ...
        public String toString() {
            String resText = "Current safe block ratio = " + getSafeBlockRatio() + ". Target threshold = " + threshold
        ...
        boolean isConsistent() throws IOException {
            ...
            for (Iterator<Collection<Block>> it = recentInvalidateSets.values().iterator(); it.hasNext();) {
    ...
    synchronized void leaveSafeMode(boolean checkForUpgrades) throws SafeModeException {
        ...
        if (getDistributedUpgradeState())
            throw new SafeModeException("Distributed upgrade is in progress", safeMode);
    ...
    private void checkSuperuserPrivilege() throws AccessControlException {
        if (isPermissionEnabled) {
            PermissionChecker pc = new PermissionChecker(fsOwner.getUserName(), supergroup);
            if (!pc.isSuper) {
     ...
     * .
     */
    private PermissionChecker checkPermission(String path, boolean doCheckOwner, FsAction ancestorAccess,
            FsAction parentAccess, FsAction access, FsAction subAccess) throws AccessControlException {
        PermissionChecker pc = new PermissionChecker(fsOwner.getUserName(), supergroup);
        if (!pc.isSuper) {
            dir.waitForReady();
    ...
    //
    void changeLease(String src, String dst, FileStatus dinfo) throws IOException {
        String overwrite;
        String replaceBy;
