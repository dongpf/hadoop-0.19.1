...
/**
 * Stores information about all corrupt blocks in the File System. A Block is
 * considered corrupt only if all of its replicas are corrupt. While reporting
 * replicas of a Block, we hide any corrupt copies. These copies are removed
 * once Block is found to have expected number of good replicas. Mapping: Block
 * -> TreeSet<DatanodeDescriptor>
 */
    ...
    public void addToCorruptReplicasMap(Block blk, DatanodeDescriptor dn) {
        ...
        if (!nodes.contains(dn)) {
            ...
            NameNode.stateChangeLog
                    .info("BLOCK NameSystem.addToCorruptReplicasMap: " + "duplicate requested for "
                            + blk.getBlockName() + " to add as corrupt " + "on " + dn.getName() + " by "
                            + Server.getRemoteIp());
        }
        if (NameNode.getNameNodeMetrics() != null) {
            NameNode.getNameNodeMetrics().numBlocksCorrupted.set(corruptReplicasMap.size());
        }
    }
