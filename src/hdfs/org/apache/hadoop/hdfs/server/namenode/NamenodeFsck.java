 ...
 * <p>
 * The tool scans all files and directories, starting from an indicated root
 * path. The following abnormal conditions are detected and handled:
 * </p>
 * <ul>
 ...
 * {@link #FIXING_MOVE}). Remaining data blocks are saved as a block chains,
 * representing longest consecutive series of valid blocks.</li>
...
public class NamenodeFsck {
     ...
     * @param pmap
     *            key=value[] map that is passed to the http servlet as url
     *            parameters
     * @param response
     *            the object into which this servelet writes the url contents
     * @throws IOException
     */
    public NamenodeFsck(Configuration conf, NameNode nn, Map<String, String[]> pmap, HttpServletResponse response)
            throws IOException {
        this.conf = conf;
        this.nn = nn;
    ...
    public void fsck() throws IOException {
        try {
            ...
            if (files != null) {
                ...
                out.println(res);
                // DFSck client scans for the string HEALTHY/CORRUPT to check
                // the status
                // of file system and return appropriate code. Changing the
                // output string
    ...
    private void check(FileStatus file, FsckResult res) throws IOException {
        ...
        if (isOpen && !showOpenFiles) {
            // We collect these stats about open files to report with default
            // options
        ...
        for (LocatedBlock lBlk : blocks.getLocatedBlocks()) {
            ...
            if (locs.length < targetFileReplication && locs.length > 0) {
                ...
                out.println(" Under replicated " + block + ". Target Replicas is " + targetFileReplication
                        + " but found " + locs.length + " replica(s).");
            }
            // verify block placement policy
            int missingRacks = ReplicationTargetChooser.verifyBlockPlacement(lBlk, targetFileReplication,
            ...
            if (missingRacks > 0) {
                ...
                out.println(" Replica placement policy is violated for " + block
        ...
        try {
            ...
            try {
                ...
                for (LocatedBlock lBlk : blocks.getLocatedBlocks()) {
                    ...
                    // copy the block. It's a pity it's not abstracted from
                    // DFSInputStream ...
                    try {
     ...
     * which is bad. Both places should be refactored to provide a method to
     * copy blocks around.
     */
    private void copyBlock(DFSClient dfs, LocatedBlock lblock, OutputStream fos) throws Exception {
        ...
        while (s == null) {
            ...
            try {
                ...
                blockReader = DFSClient.BlockReader
                        .newBlockReader(s, targetAddr.toString() + ":" + block.getBlockId(), block.getBlockId(),
     ...
     * 
     * Pick the best node from which to stream the data. That's the local one,
     * if available.
     */
    Random r = new Random();
    ...
    public static class FsckResult {
        ...
        private long numMisReplicatedBlocks = 0L; // blocks that do not satisfy
        ...
        /**
         * Return the number of under-replicated blocks. Note: missing blocks
        ...
        /**
         * Return total number of files opened for write encountered during this
         * scan.
         */
         ...
         * replicated blocks are counted. Note: this values comes from the
         * current Configuration supplied for the tool, so it may be different
         * from the value in DFS Configuration.
         */
