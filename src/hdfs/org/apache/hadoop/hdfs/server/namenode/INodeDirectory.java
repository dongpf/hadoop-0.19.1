...
public class INodeDirectory extends INode {
   ...
   */
    ...
    int getExistingPathINodes(byte[][] components, INode[] existing) {
        assert compareBytes(this.name, components[0]) == 0 : "Incorrect name " + getLocalName() + " expected "
                + components[0];
    ...
    <T extends INode> T addChild(final T node, boolean inheritPermission) {
        if (inheritPermission) {
            ...
            if (!p.getUserAction().implies(FsAction.WRITE_EXECUTE)) {
                p = new FsPermission(p.getUserAction().or(FsAction.WRITE_EXECUTE), p.getGroupAction(),
                        p.getOtherAction());
            }
