...
/**
 * 
 * This class is for maintaining the various FSNamesystem status metrics and
 * publishing them through the metrics interfaces. This also registers the JMX
 * MBean for RPC.
 * <p>
...
public class FSNamesystemMetrics implements Updater {
     ...
     * periodically, e.g. every 5 seconds. We set the metrics value within this
     * function before pushing it out. FSNamesystem updates its own local
     * variables which are light weight compared to Metrics counters.
     * 
     ...
     * values fit in int value. Metrics related to DFS capacity are stored in
     * bytes which do not fit in int, so they are rounded to GB
     */
