...
public class SecondaryNameNode implements Runnable {
    ...
    private void initialize(Configuration conf) throws IOException {
        ...
        String infoAddr = NetUtils.getServerAddress(conf, "dfs.secondary.info.bindAddress", "dfs.secondary.info.port",
    ...
    private void putFSImage(CheckpointSignature sig) throws IOException {
        String fileid = "putimage=1&port=" + infoPort + "&machine=" + InetAddress.getLocalHost().getHostAddress()
                + "&token=" + sig.toString();
        ...
        try {
            if ("-checkpoint".equals(cmd)) {
                ...
                if (size >= checkpointSize || argv.length == 2 && "force".equals(argv[i])) {
                    ...
                    System.err.println("EditLog size " + size + " bytes is " + "smaller than configured checkpoint "
