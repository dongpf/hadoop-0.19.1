...
public class FSEditLog {
    ...
    static private class EditLogFileOutputStream extends EditLogOutputStream {
        ...
        public void close() throws IOException {
            ...
            if (bufSize != 0) {
                throw new IOException("FSEditStream has " + bufSize + " bytes still to be flushed and cannot "
                        + "be closed.");
            }
            bufCurrent.close();
            bufReady.close();
         ...
         * New data can be still written to the stream while flushing is
         * performed.
         */
        @Override
         ...
         * as it accumulates new log records while readyBuffer will be flushed
         * and synced.
         */
        @Override
        ...
        private void preallocate() throws IOException {
            ...
            if (position + 4096 >= fc.size()) {
                ...
                FSNamesystem.LOG.debug("Edit log size is now " + fc.size() + " written " + written + " bytes "
                        + " at offset " + newsize);
            }
        }
    ...
    /**
     * If there is an IO Error on any log operations, remove that directory from
     * the list of directories. If no more directories remain, then exit.
     */
    ...
    synchronized void processIOError(StorageDirectory sd) {
        ...
        for (int idx = 0; idx < editStreams.size(); idx++) {
            File parentStorageDir = ((EditLogFileOutputStream) editStreams.get(idx)).getFile().getParentFile()
                    .getParentFile();
    ...
    /**
     * The specified streams have IO errors. Remove them from logging new
     * transactions.
     */
    ...
    static int loadFSEdits(EditLogInputStream edits) throws IOException {
        ...
        try {
            ...
            in.mark(4);
            // If edits log is greater than 2G, available method will return
            // negative
            ...
            if (available) {
                ...
                if (logVersion < FSConstants.LAYOUT_VERSION) // future version
                    throw new IOException("Unexpected version of the file system log file: " + logVersion
            ...
            while (true) {
                ...
                case OP_CLOSE: {
                    ...
                    // clientname, clientMachine and block locations of last
                    // block.
                    ...
                    // The open lease transaction re-creates a file if
                    // necessary.
                    ...
                    INodeFile node = (INodeFile) fsDir.unprotectedAddFile(path, permissions, blocks, replication,
                    ...
                    if (opcode == OP_ADD) {
                        ...
                        INodeFileUnderConstruction cons = new INodeFileUnderConstruction(node.getLocalNameBytes(),
                                ...
                                node.getBlocks(), node.getPermissionStatus(), clientName, clientMachine, null);
                ...
                case OP_MKDIR: {
                    ...
                    // However, currently this is not being updated/used because
                    // of
                    // performance reasons.
                    if (logVersion <= -17) {
                        atime = readLong(in);
        ...
        FSImage.LOG.info("Edits file " + edits.getName() + " of size " + edits.length() + " edits # " + numEdits
                + " loaded in " + (FSNamesystem.now() - startTime) / 1000 + " seconds.");

        if (FSImage.LOG.isDebugEnabled()) {
            FSImage.LOG.debug("numOpAdd = " + numOpAdd + " numOpClose = " + numOpClose + " numOpDelete = "
    ...
    private void printStatistics(boolean force) {
        ...
        buf.append("Number of transactions: " + numTransactions + " Total time for transactions(ms): "
    ...
    /**
     * Add open lease record to edit log. Records the block locations of the
     * last block.
     */
    public void logOpenFile(String path, INodeFileUnderConstruction newNode) throws IOException {

        UTF8 nameReplicationPair[] = new UTF8[] { new UTF8(path), FSEditLog.toLogReplication(newNode.getReplication()),
                FSEditLog.toLogLong(newNode.getModificationTime()), FSEditLog.toLogLong(newNode.getAccessTime()),
                FSEditLog.toLogLong(newNode.getPreferredBlockSize()) };
        logEdit(OP_ADD, new ArrayWritable(UTF8.class, nameReplicationPair),
                new ArrayWritable(Block.class, newNode.getBlocks()), newNode.getPermissionStatus(),
                new UTF8(newNode.getClientName()), new UTF8(newNode.getClientMachine()));
    }
    ...
    public void logMkDir(String path, INode newNode) {
        UTF8 info[] = new UTF8[] { new UTF8(path), FSEditLog.toLogLong(newNode.getModificationTime()),
                FSEditLog.toLogLong(newNode.getAccessTime()) };
        logEdit(OP_MKDIR, new ArrayWritable(UTF8.class, info), newNode.getPermissionStatus());
    }

    /**
     * Add rename record to edit log TODO: use String parameters until just
     ...
     * 
     * @param src
     *            the string representation of the path to a directory
     * @param quota
     *            the directory size limit
     */
    ...
    /**
     * Closes the current edit log and opens edits.new. Returns the lastModified
     * time of the edits log.
     */
    ...
    /**
     * Removes the old edit log and renamed edits.new as edits. Reopens the
     * edits file.
     */
    ...
    /**
     * A class to read in blocks stored in the old format. The only two fields
     * in the block were blockid and length.
     */
    ...
    static private DatanodeDescriptor[] readDatanodeDescriptorArray(DataInput in) throws IOException {
