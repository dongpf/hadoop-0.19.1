...
class PermissionChecker {
    ...
    /**
     * Check whether current user have permissions to access the path. Traverse
     * is always checked.
     * 
     * Parent path means the parent directory for the path. Ancestor path means
     * the last (the closest) existing ancestor directory of the path. Note that
     * if the parent path exists, then the parent path and the ancestor path are
     * the same.
     * 
     * For example, suppose the path is "/foo/bar/baz". No matter baz is a file
     * or a directory, the parent path is "/foo/bar". If bar exists, then the
     * ancestor path is also "/foo/bar". If bar does not exist and foo exists,
     * then the ancestor path is "/foo". Further, if both foo and bar do not
     * exist, then the ancestor path is "/".
     * 
     * @param doCheckOwner
     ...
     * @param subAccess
     *            If path is a directory, it is the access required of the path
     *            and all the sub-directories. If path is not a directory, there
     *            is no effect.
     * @return a PermissionChecker object which caches data for later use.
     * @throws AccessControlException
     */
    void checkPermission(String path, INodeDirectory root, boolean doCheckOwner, FsAction ancestorAccess,
