...
class FSDirectory implements FSConstants, Closeable {
    ...
    FSDirectory(FSImage fsImage, FSNamesystem ns, Configuration conf) {
        rootDir = new INodeDirectoryWithQuota(INodeDirectory.ROOT_NAME, ns.createFsOwnerPermissions(new FsPermission(
                (short) 0755)), Integer.MAX_VALUE, -1);
     ...
     */
    INodeFileUnderConstruction addFile(String path, PermissionStatus permissions, short replication,
            long preferredBlockSize, String clientName, String clientMachine, DatanodeDescriptor clientNode,
            long generationStamp) throws IOException {
        waitForReady();

        // Always do an implicit mkdirs for parent directory tree.
        long modTime = FSNamesystem.now();
        if (!mkdirs(new Path(path).getParent().toString(), permissions, true, modTime)) {
            return null;
        }
        INodeFileUnderConstruction newNode = new INodeFileUnderConstruction(permissions, replication,
        ...
        if (newNode == null) {
            NameNode.stateChangeLog
                    .info("DIR* FSDirectory.addFile: " + "failed to add " + path + " to the file system");
            return null;
        }
    ...
    /**
   */
    INode unprotectedAddFile(String path, PermissionStatus permissions, Block[] blocks, short replication,
        ...
        if (blocks == null)
            ...
            newNode = new INodeFile(permissions, blocks.length, replication, modificationTime, atime,
                    preferredBlockSize);
    ...
    INodeDirectory addToParent(String src, INodeDirectory parentINode, PermissionStatus permissions, Block[] blocks,
            short replication, long modificationTime, long atime, long nsQuota, long dsQuota, long preferredBlockSize) {
    ...
    Block addBlock(String path, INode[] inodes, Block block) throws IOException {
        ...
        synchronized (rootDir) {
            ...
            NameNode.stateChangeLog.debug("DIR* FSDirectory.addFile: " + path + " with " + block
    ...
    void persistBlocks(String path, INodeFileUnderConstruction file) throws IOException {
        ...
        synchronized (rootDir) {
            ...
            NameNode.stateChangeLog.debug("DIR* FSDirectory.persistBlocks: " + path + " with "
                    + file.getBlocks().length + " blocks is persisted to the file system");
        }
    }
    ...
    void closeFile(String path, INodeFile file) throws IOException {
        ...
        synchronized (rootDir) {
            ...
            if (NameNode.stateChangeLog.isDebugEnabled()) {
                NameNode.stateChangeLog.debug("DIR* FSDirectory.closeFile: " + path + " with "
                        + file.getBlocks().length + " blocks is persisted to the file system");
            }
        }
    }
    ...
    boolean removeBlock(String path, INodeFileUnderConstruction fileNode, Block block) throws IOException {
        ...
        synchronized (rootDir) {
            ...
            // If block is removed from blocksMap remove it from
            // corruptReplicasMap
    ...
    boolean unprotectedRenameTo(String src, String dst, long timestamp) throws QuotaExceededException {
        synchronized (rootDir) {
            ...
            if (srcInodes[srcInodes.length - 1] == null) {
                NameNode.stateChangeLog.warn("DIR* FSDirectory.unprotectedRenameTo: " + "failed to rename " + src
            ...
            if (srcChild == null) {
                NameNode.stateChangeLog.warn("DIR* FSDirectory.unprotectedRenameTo: " + "failed to rename " + src
                        + " to " + dst + " because the source can not be removed");
                return false;
            }
            ...
            if (dstInodes[dstInodes.length - 1] != null) { // check if
                                                           // destination exists
                NameNode.stateChangeLog.warn("DIR* FSDirectory.unprotectedRenameTo: " + "failed to rename " + src
                        + " to " + dst + " because destination exists");
            } else if (dstInodes[dstInodes.length - 2] == null) { // check if
                                                                  // its parent
                                                                  // exists
                NameNode.stateChangeLog.warn("DIR* FSDirectory.unprotectedRenameTo: " + "failed to rename " + src
                        + " to " + dst + " because destination's parent does not exists");
            } else {
    ...
    Block[] unprotectedSetReplication(String src, short replication, int[] oldReplication) throws IOException {
        ...
        synchronized (rootDir) {
            ...
            // check disk quota
            long dsDelta = (replication - oldReplication[0]) * (fileNode.diskspaceConsumed() / oldReplication[0]);
     ...
     * @throws IOException
     *             if it is a directory or does not exist.
     */
     ...
     * @param deletedBlocks
     *            the place holder for the blocks to be removed
    ...
    INode unprotectedDelete(String src, long modificationTime) {
        ...
        synchronized (rootDir) {
            ...
            if (targetNode == null) { // non-existent src
                NameNode.stateChangeLog.debug("DIR* FSDirectory.unprotectedDelete: " + "failed to remove " + src
                        + " because it does not exist");
                return null;
            } else if (inodes.length == 1) { // src is the root
                NameNode.stateChangeLog.warn("DIR* FSDirectory.unprotectedDelete: " + "failed to remove " + src
     ...
     */
    private void replaceNode(String path, INodeFile oldnode, INodeFile newnode, boolean updateDiskspace)
            throws IOException {
        synchronized (rootDir) {
     ...
     * 
     * This function is admittedly very inefficient right now. We'll make it
     * better later.
     */
     ...
     */
    void updateSpaceConsumed(String path, long nsDelta, long dsDelta) throws QuotaExceededException,
     ...
     *            parent or not. The automatically created ones always inherit
     *            its permission from its parent
     * @param now
     *            creation time
   ...
   */
    INode unprotectedMkdir(String src, PermissionStatus permissions, long timestamp) throws QuotaExceededException {
        ...
        synchronized (rootDir) {
            rootDir.getExistingPathINodes(components, inodes);
            unprotectedMkdir(inodes, inodes.length - 1, components[inodes.length - 1], permissions, false, timestamp);
     ...
     */
    private void unprotectedMkdir(INode[] inodes, int pos, byte[] name, PermissionStatus permission,
            boolean inheritPermission, long timestamp) throws QuotaExceededException {
        inodes[pos] = addChild(inodes, pos, new INodeDirectory(name, permission, timestamp), inheritPermission);
    }
     ...
     */
    private <T extends INode> T addChild(INode[] pathComponents, int pos, T child, boolean inheritPermission)
            throws QuotaExceededException {
     ...
     * [0, pos-1]. QuotaExceededException is thrown if it violates quota limit
     */
    private <T extends INode> T addChild(INode[] pathComponents, int pos, T child, long childDiskspace,
    ...
    private INode removeChild(INode[] pathComponents, int pos) throws QuotaExceededException {
        ...
        if (removedNode != null) {
            ...
            updateCount(pathComponents, pos, -counts.getNsCount(), -counts.getDsCount());
        }
        return removedNode;
    }
   ...
   */
    ...
    void updateCountForINodeWithQuota() {
        updateCountForINodeWithQuota(rootDir, new INode.DirCounts(), new ArrayList<INode>(50));
    }
     ...
     */
    private static void updateCountForINodeWithQuota(INodeDirectory dir, INode.DirCounts counts,
            ArrayList<INode> nodesInPath) {
        long parentNamespace = counts.nsCount;
        long parentDiskspace = counts.dsCount;

        counts.nsCount = 1L;// for self. should not call
                            // node.spaceConsumedInTree()
        counts.dsCount = 0L;

        /*
         * We don't need nodesInPath if we could use 'parent' field in INode.
         * using 'parent' is not currently recommended.
         */
        nodesInPath.add(dir);
        ...
        if (dir.isQuotaSet()) {
            ...
            if ((dir.getNsQuota() >= 0 && counts.nsCount > dir.getNsQuota())
                ...
                // can only happen because of a software bug. the bug should be
                // fixed.
                ...
                NameNode.LOG.warn("Unexpected quota violation in image for " + path + " (Namespace quota : "
                        + dir.getNsQuota() + " consumed : " + counts.nsCount + ")" + " (Diskspace quota : "
                        + dir.getDsQuota() + " consumed : " + counts.dsCount + ").");
            }
        }
     ...
     * @throws QuotaExceededException
     *             if the directory tree size is greater than the given quota
     */
    INodeDirectory unprotectedSetQuota(String src, long nsQuota, long dsQuota) throws FileNotFoundException,
            QuotaExceededException {
        // sanity check
        if ((nsQuota < 0 && nsQuota != FSConstants.QUOTA_DONT_SET && nsQuota < FSConstants.QUOTA_RESET)
                || (dsQuota < 0 && dsQuota != FSConstants.QUOTA_DONT_SET && dsQuota < FSConstants.QUOTA_RESET)) {
            throw new IllegalArgumentException("Illegal value for nsQuota or " + "dsQuota : " + nsQuota + " and "
                    + dsQuota);
        }
        ...
        if (targetNode == null) {
            ...
            if (dirNode instanceof INodeDirectoryWithQuota) {
                ...
                // a non-quota directory; so replace it with a directory with
                // quota
    ...
    private static FileStatus createFileStatus(String path, INode node) {
        ...
        return new FileStatus(node.isDirectory() ? 0 : node.computeContentSummary().getLength(), node.isDirectory(),
                node.isDirectory() ? 0 : ((INodeFile) node).getReplication(), node.isDirectory() ? 0
                        : ((INodeFile) node).getPreferredBlockSize(), node.getModificationTime(), node.getAccessTime(),
                node.getFsPermission(), node.getUserName(), node.getGroupName(), new Path(path));
    }
}
