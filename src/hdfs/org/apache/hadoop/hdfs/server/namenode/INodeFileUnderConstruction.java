...
public class INodeFileUnderConstruction extends INodeFile {
    ...
    INodeFileUnderConstruction(PermissionStatus permissions, short replication, long preferredBlockSize, long modTime,
    ...
    public INodeFileUnderConstruction(byte[] name, short blockReplication, long modificationTime,
            long preferredBlockSize, BlockInfo[] blocks, PermissionStatus perm, String clientName,
    ...
    INodeFile convertToInodeFile() {
        INodeFile obj = new INodeFile(getPermissionStatus(), getBlocks(), getReplication(), getModificationTime(),
                getModificationTime(), getPreferredBlockSize());
        return obj;

    }

    /**
     * remove a block from the block list. This block should be the last one on
     * the list.
     */
    ...
    void assignPrimaryDatanode() {
        ...
        for (int i = 1; i <= targets.length; i++) {
            ...
            if (targets[j].isAlive) {
                ...
                NameNode.stateChangeLog.info("BLOCK* " + blocks[blocks.length - 1] + " recovery started, primary="
                        + primary);
                return;
            }
        }
