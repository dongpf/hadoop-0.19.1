 ...
 * 
 * Each upgrade object corresponds to a layout version, which is the latest
 * version that should be upgraded using this object. That is all components
 * whose layout version is greater or equal to the one returned by
 * {@link #getVersion()} must be upgraded with this object.
 */
public interface Upgradeable extends Comparable<Upgradeable> {
     ...
     * 
     * 100% means that the upgrade is completed. Any value < 100 means it is not
     * complete.
     * 
    ...
    /**
     * Prepare for the upgrade. E.g. initialize upgrade data structures and set
     * status to 0.
     * 
     * Returns an upgrade command that is used for broadcasting to other cluster
     * components. E.g. name-node informs data-nodes that they must perform a
     * distributed upgrade.
     * 
    ...
    /**
     * Complete upgrade. E.g. cleanup upgrade data structures or write metadata
     * to disk.
     * 
     * Returns an upgrade command that is used for broadcasting to other cluster
     * components. E.g. data-nodes inform the name-node that they completed the
     * upgrade while other data-nodes are still upgrading.
     * 
     * @throws IOException
     */
     ...
     * 
     * @param details
     *            true if upgradeStatus details need to be included, false
     *            otherwise
