 ...
 * <p>
 * Local storage information is stored in a separate file VERSION. It contains
 * type of the node, the storage layout version, the namespace id, and the fs
 * state creation time.
 * <p>
 * Local storage can reside in multiple directories. Each directory should
 * contain the same VERSION file as the others. During startup Hadoop servers
 * (name-node and data-nodes) read their local storage information from them.
 * <p>
 * The servers hold a lock for each storage directory while they run so that
 * other nodes were not able to startup sharing the same storage. The locks are
 * released when the servers stop (normally or abnormally).
 * 
 */
public abstract class Storage extends StorageInfo {
    ...
    /*
     * this should be removed when LAST_UPGRADABLE_LV goes beyond -13. any
     * upgrade code that uses this constant should also be removed.
     */
    ...
    /**
     * An interface to denote storage directory type Implementations can define
     * a type for storage directory by implementing this interface.
     */
    ...
    /**
     * Return iterator based on Storage Directory Type This iterator selects
     * entires of storageDirs of type dirType and returns them via the Iterator
     */
    ...
    public class StorageDirectory {
         ...
         * 
         * @throws IOException
         *             if file cannot be read or contains inconsistent data
         */
         ...
         * directory.
         * 
         * This does not fully format storage directory. It cannot write the
         * version file since it should be written last after all other storage
         * type dependent files are written. Derived storage is responsible for
         * setting specific storage values and writing the version file to disk.
         * 
         * @throws IOException
         */
         ...
         * @throws {@link InconsistentFSStateException} if directory state is
         *         not consistent and cannot be recovered
         */
        public StorageState analyzeStorage(StartupOption startOpt) throws IOException {
            ...
            // # of temp dirs == 1 should either recover or complete a
            // transition
            if (hasCheckpointTmp) {
        ...
        public void doRecover(StorageState curState) throws IOException {
            ...
            case COMPLETE_FINALIZE: // rm finalized.tmp
                ...
                return;
            case COMPLETE_CHECKPOINT: // mv lastcheckpoint.tmp ->
                                      // previous.checkpoint
         ...
         * 
         * <p>
         * Locking is not supported by all file systems. E.g., NFS does not
         * consistently support exclusive locks.
         * 
         * <p>
         * If locking is supported we guarantee exculsive access to the storage
         * directory. Otherwise, no guarantee is given.
         * 
         * @throws IOException
         *             if locking fails
         */
    ...
    /**
     * Checks if the upgrade from the given old version is supported. If no
     * upgrade is supported, it throws IncorrectVersionException.
     * 
     * @param oldVersion
     */
    ...
    /**
     * Get common storage fields. Should be overloaded if additional fields need
     * to be get.
     * 
     * @param props
     * @throws IOException
    ...
    /**
     * Set common storage fields. Should be overloaded if additional fields need
     * to be set.
     * 
     * @param props
     * @throws IOException
    ...
    public static String getRegistrationID(StorageInfo storage) {
        return "NS-" + Integer.toString(storage.getNamespaceID()) + "-" + Integer.toString(storage.getLayoutVersion())
