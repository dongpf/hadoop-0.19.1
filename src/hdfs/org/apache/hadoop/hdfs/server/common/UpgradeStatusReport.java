...
/**
 * Base upgrade upgradeStatus class. Overload this class if specific status
...
public class UpgradeStatusReport implements Writable {
    ...
    /**
     * Get upgradeStatus data as a text for reporting. Should be overloaded for
     * a particular upgrade specific upgradeStatus data.
     * 
     * @param details
    ...
    public String getStatusText(boolean details) {
        return "Upgrade for version "
                + getVersion()
                + (upgradeStatus < 100 ? " is in progress. Status = " + upgradeStatus + "%" : " has been completed."
                        + "\nUpgrade is " + (finalized ? "" : "not ") + "finalized.");
    }
