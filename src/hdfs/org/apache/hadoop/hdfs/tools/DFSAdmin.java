...
public class DFSAdmin extends FsShell {
    ...
    public void report() throws IOException {
        if (fs instanceof DistributedFileSystem) {
            ...
            System.out.println("Datanodes available: " + live.length + " (" + (live.length + dead.length) + " total, "
                    + dead.length + " dead)\n");
    ...
    /**
     * Safe mode maintenance command. Usage: java DFSAdmin -safemode [enter |
     * leave | get]
     * 
     * @param argv
    ...
    /**
     * Command to request current distributed upgrade status, a detailed status,
     * or to force the upgrade to proceed.
     * 
    ...
    public int upgradeProgress(String[] argv, int idx) throws IOException {
        ...
        String statusText = (status == null ? "There are no upgrades in progress." : status
                .getStatusText(action == UpgradeAction.DETAILED_STATUS));
        System.out.println(statusText);
        return 0;
    }

    /**
     * Dumps DFS data structures into specified file. Usage: java DFSAdmin
     * -metasave filename
     * 
     * @param argv
     *            List of of command line parameters.
     ...
     *            The index of the command that is being processed.
     * @exception IOException
     *                if an error accoured wile accessing the file or path.
     */
