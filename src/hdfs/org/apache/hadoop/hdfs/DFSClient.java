...
public class DFSClient implements FSConstants, java.io.Closeable {
  ...
  /**
   * Convert an HdfsLocatedFileStatus with its block locations to a LocatedFileStatus
   * @param stat an HdfsLocatedFileStatus with its block locations
   * @return a LocatedFileStatus object
   */
  private static LocatedFileStatus toLocatedFileStatus(
      HdfsLocatedFileStatus stat) {
    if (stat == null) {
      return null;
    }
    return new LocatedFileStatus(stat.getLen(),
        ...
        stat.getPath(),
        DFSUtil.locatedBlocks2Locations(stat.getBlockLocations()));
  }
