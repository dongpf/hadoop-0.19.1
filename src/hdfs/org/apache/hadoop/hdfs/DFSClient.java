...
/********************************************************
 * DFSClient can connect to a Hadoop Filesystem and perform basic file tasks. It
 * uses the ClientProtocol to communicate with a NameNode daemon, and connects
 * directly to DataNodes to read/write block data.
 * 
 * Hadoop DFS users should obtain an instance of DistributedFileSystem, which
 * uses DFSClient to handle filesystem tasks.
 * 
 ********************************************************/
public class DFSClient implements FSConstants, java.io.Closeable {
    ...
    public static ClientProtocol createNamenode(InetSocketAddress nameNodeAddr, Configuration conf) throws IOException {
        try {
            return createNamenode(createRPCNamenode(nameNodeAddr, conf, UnixUserGroupInformation.login(conf, true)));
        } catch (LoginException e) {
            throw (IOException) (new IOException().initCause(e));
        }
    }

    private static ClientProtocol createRPCNamenode(InetSocketAddress nameNodeAddr, Configuration conf,
            UnixUserGroupInformation ugi) throws IOException {
        return (ClientProtocol) RPC.getProxy(ClientProtocol.class, ClientProtocol.versionID, nameNodeAddr, ugi, conf,
                NetUtils.getSocketFactory(conf, ClientProtocol.class));
    }

    private static ClientProtocol createNamenode(ClientProtocol rpcNamenode) throws IOException {
        RetryPolicy createPolicy = RetryPolicies.retryUpToMaximumCountWithFixedSleep(5, LEASE_SOFTLIMIT_PERIOD,
        ...
        return (ClientProtocol) RetryProxy.create(ClientProtocol.class, rpcNamenode, methodNameToPolicyMap);
    }

    static ClientDatanodeProtocol createClientDatanodeProtocolProxy(DatanodeID datanodeid, Configuration conf)
            throws IOException {
    ...
    public DFSClient(InetSocketAddress nameNodeAddr, Configuration conf, FileSystem.Statistics stats)
        ...
        this.maxBlockAcquireFailures = conf.getInt("dfs.client.max.block.acquire.failures", MAX_BLOCK_ACQUIRE_FAILURES);
     ...
     * 
     *             getHints() returns a list of hostnames that store data for a
     *             specific file region. It returns a set of hostnames for every
     *             block within the indicated region.
     * 
     ...
     * 
     * getBlockLocations() returns a list of hostnames that store data for a
     * specific file region. It returns a set of hostnames for every block
     * within the indicated region.
     * 
     * This function is very useful when writing code that considers
     * data-placement when performing operations. For example, the MapReduce
     * system tries to schedule tasks on the same machines as the data-block the
     * task processes.
     */
    ...
    /**
     * Create an input stream that obtains a nodelist from the namenode, and
     * then reads from all the right places. Creates inner subclass of
     * InputStream that does the right out-of-band work.
     */
    DFSInputStream open(String src, int buffersize, boolean verifyChecksum, FileSystem.Statistics stats)
            throws IOException {
        checkOpen();
    ...
    /**
     * Create a new dfs file with the specified block replication and return an
    ...
    /**
     * Create a new dfs file with the specified block replication with
     * write-progress reporting and return an output stream for writing into the
     * file.
     * 
     * @param src
     *            stream name
     ...
     *      long)
     */
    public OutputStream create(String src, FsPermission permission, boolean overwrite, short replication,
            long blockSize, Progressable progress, int buffersize) throws IOException {
        checkOpen();
        if (permission == null) {
     ...
     *            buffer size
     * @param progress
     *            for reporting write-progress
     * @return an output stream for writing into the file
     * @throws IOException
     * @see {@link ClientProtocol#append(String, String)}
     */
    OutputStream append(String src, int buffersize, Progressable progress) throws IOException {
        checkOpen();
        ...
        OutputStream result = new DFSOutputStream(src, buffersize, progress, lastBlock, stat, conf.getInt(
    ...
    /**
     * Delete file or directory. See {@link ClientProtocol#delete(String)}.
     */
    @Deprecated
    ...
    /**
     * delete file or directory. delete contents of the directory if non empty
   ...
   */
     ...
     * 
     * @see ClientProtocol#getHdfsLocatedListing(String)
     */
    public LocatedFileStatus[] listPathWithLocations(String src) throws IOException {
        checkOpen();
        try {
            HdfsLocatedFileStatus[] listing = namenode.getHdfsLocatedListing(src);
            LocatedFileStatus[] result = new LocatedFileStatus[listing.length];
            for (int i = 0; i < listing.length; i++) {
                result[i] = toLocatedFileStatus(listing[i]);
            }
            return result;
        } catch (RemoteException re) {
            throw re.unwrapRemoteException(AccessControlException.class);
    ...
    /**
     * Convert an HdfsLocatedFileStatus with its block locations to a
     * LocatedFileStatus
     * 
     * @param stat
     *            an HdfsLocatedFileStatus with its block locations
     * @return a LocatedFileStatus object
     */
    private static LocatedFileStatus toLocatedFileStatus(HdfsLocatedFileStatus stat) {
        if (stat == null) {
            return null;
        }
        return new LocatedFileStatus(stat.getLen(), stat.isDir(), stat.getReplication(), stat.getBlockSize(),
                stat.getModificationTime(), stat.getAccessTime(), stat.getPermission(), stat.getOwner(),
                stat.getGroup(), stat.getPath(), DFSUtil.locatedBlocks2Locations(stat.getBlockLocations()));
    }
    ...
    public static MD5MD5CRC32FileChecksum getFileChecksum(String src, ClientProtocol namenode,
        ...
        for (int i = 0; i < locatedblocks.size(); i++) {
            ...
            for (int j = 0; !done && j < datanodes.length; j++) {
                ...
                DataOutputStream out = new DataOutputStream(new BufferedOutputStream(NetUtils.getOutputStream(sock),
                ...
                try {
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("write to " + datanodes[j].getName() + ": " + DataTransferProtocol.OP_BLOCK_CHECKSUM
                                + ", block=" + block);
                    }
   ...
   */
    ...
    /**
     * Enter, leave or get safe mode. See
     * {@link ClientProtocol#setSafeMode(FSConstants.SafeModeAction)} for more
     * details.
     * 
    ...
    /**
     * Refresh the hosts and exclude files. (Rereads them.) See
     * {@link ClientProtocol#refreshNodes()} for more details.
     * 
    ...
    /**
     * Dumps DFS data structures into specified file. See
     * {@link ClientProtocol#metaSave(String)} for more details.
     * 
   ...
   */
    ...
    void setQuota(String src, long namespaceQuota, long diskspaceQuota) throws IOException {
        // sanity check
        if ((namespaceQuota <= 0 && namespaceQuota != FSConstants.QUOTA_DONT_SET && namespaceQuota != FSConstants.QUOTA_RESET)
                || (diskspaceQuota <= 0 && diskspaceQuota != FSConstants.QUOTA_DONT_SET && diskspaceQuota != FSConstants.QUOTA_RESET)) {
            throw new IllegalArgumentException("Invalid values for quota : " + namespaceQuota + " and "
                    + diskspaceQuota);
    ...
    /**
     * Pick the best node from which to stream the data. Entries in <i>nodes</i>
     * are already in the priority order
     */
    ...
    class LeaseChecker implements Runnable {
        ...
        /**
         * Periodically check in with the namenode and renew all the leases when
         * the lease period is half over.
         */
        public void run() {
            long lastRenewed = 0;
    ...
    /**
     * This is a wrapper around connection to datadone and understands checksum,
     * offset etc
     */
    public static class BlockReader extends FSInputChecker {
        ...
        private boolean sentChecksumOk = false; // temp : to be removed in
                                                // 0.20.0
        ...
        /*
         * same interface as inputStream java.io.InputStream#read() used by
         * DFSInputStream#read() This violates one rule when there is a checksum
         * error: "Read should not modify user buffer before successful read"
         * because it first reads the data to user buffer and then checks the
         * checksum.
         */
        @Override
        public synchronized int read(byte[] buf, int off, int len) throws IOException {
            ...
            if (gotEOS && !eosBefore && nRead >= 0 && needChecksum()) {
                if (sentChecksumOk) {
                    // this should not happen; log the error for the debugging
                    // purpose
        ...
        public boolean seekToNewSource(long targetPos) throws IOException {
             ...
             * case of pread(), it just tries a different replica without
             * seeking.
             */
            return false;
        }
        ...
        /**
         * Makes sure that checksumBytes has enough capacity and limit is set to
         * the number of checksum bytes needed to be read.
         */
        ...
        protected synchronized int readChunk(long pos, byte[] buf, int offset, int len, byte[] checksumBuf)
            ...
            if (dataLeft <= 0) {
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug("DFSClient readChunk got seqno " + seqno + " offsetInBlock " + offsetInBlock
                            + " lastPacketInBlock " + lastPacketInBlock + " packetLen " + packetLen);
                }

                int dataLen = in.readInt();

                // Sanity check the lengths
                if (dataLen < 0 || ((dataLen % bytesPerChecksum) != 0 && !lastPacketInBlock)
                        || (seqno != (lastSeqNo + 1))) {
                    throw new IOException("BlockReader: error in packet header" + "(chunkOffset : " + chunkOffset
                            + ", dataLen : " + dataLen + ", seqno : " + seqno + " (last: " + lastSeqNo + "))");
                }
        ...
        private BlockReader(String file, long blockId, DataInputStream in, DataChecksum checksum,
                boolean verifyChecksum, long startOffset, long firstChunkOffset, Socket dnSock) {
            super(new Path("/blk_" + blockId + ":of:" + file)/*
                                                              * too non
                                                              * path-like?
                                                              */, 1, verifyChecksum,
                    checksum.getChecksumSize() > 0 ? checksum : null, checksum.getBytesPerChecksum(), checksum
                            .getChecksumSize());
        ...
        public static BlockReader newBlockReader(Socket sock, String file, long blockId, long genStamp,
                long startOffset, long len, int bufferSize) throws IOException {
            return newBlockReader(sock, file, blockId, genStamp, startOffset, len, bufferSize, true);
        }

        /** Java Doc required */
        public static BlockReader newBlockReader(Socket sock, String file, long blockId, long genStamp,
                long startOffset, long len, int bufferSize, boolean verifyChecksum) throws IOException {
            return newBlockReader(sock, file, blockId, genStamp, startOffset, len, bufferSize, verifyChecksum, "");
        }

        public static BlockReader newBlockReader(Socket sock, String file, long blockId, long genStamp,
                long startOffset, long len, int bufferSize, boolean verifyChecksum, String clientName)
                throws IOException {
            ...
            if (in.readShort() != DataTransferProtocol.OP_STATUS_SUCCESS) {
                throw new IOException("Got error in response to OP_READ_BLOCK " + "for file " + file + " for block "
                        + blockId);
            }
        ...
        /**
         * kind of like readFully(). Only reads as much as possible. And allows
         * use of protected readFully().
         */
    ...
    /****************************************************************
     * DFSInputStream provides bytes from a named file. It handles negotiation
     * of the namenode and various datanodes as necessary.
     ****************************************************************/
    class DFSInputStream extends FSInputStream {
        ...
        /*
         * XXX Use of CocurrentHashMap is temp fix. Need to fix parallel
         * accesses to DFSInputStream (through ptreads) properly
         */
        ...
        /**
         * Get block at the specified position. Fetch it from the namenode if
         * not cached.
         * 
         * @param offset
        ...
        /**
         * Get blocks in the specified range. Fetch them from the namenode if
         * not cached.
         * 
         * @param offset
         * @param length
        ...
        /**
         * Open a DataInputStream to a DataNode so that it can be read from. We
         * get block ID and the IDs of the destinations at startup, from the
         * namenode.
         */
        private synchronized DatanodeInfo blockSeekTo(long target) throws IOException {
            ...
            while (s == null) {
                ...
                try {
                    ...
                    blockReader = BlockReader.newBlockReader(s, src, blk.getBlockId(), blk.getGenerationStamp(),
                            offsetIntoBlock, blk.getNumBytes() - offsetIntoBlock, buffersize, verifyChecksum,
                            clientName);
                    return chosenNode;
        ...
        /*
         * This is a used by regular read() and handles ChecksumExceptions. name
         * readBuffer() is chosen to imply similarity to readBuffer() in
         * ChecksuFileSystem
         */
        private synchronized int readBuffer(byte buf[], int off, int len) throws IOException {
            ...
            /*
             * we retry current node only once. So this is set to true only
             * here. Intention is to handle one common case of an error that is
             * not a failure on datanode or client : when DataNode closes the
             * connection since client is idle. If there are other cases of
             * "non-errors" then then a datanode might be retried by setting
             * this to true again.
             */
            ...
            while (true) {
                ...
                try {
                    ...
                    LOG.warn("Found Checksum error for " + currentBlock + " from " + currentNode.getName() + " at "
                            + ce.getPos());
                    ...
                    if (!retryCurrentNode) {
                        LOG.warn("Exception while reading from " + currentBlock + " of " + src + " from " + currentNode
                ...
                if (retryCurrentNode) {
                    /*
                     * possibly retry the same node so that transient errors
                     * don't result in application level failures (e.g. Datanode
                     * could have closed the connection because the client is
                     * idle for too long).
                     */
                ...
                while (retries > 0) {
                    try {
                        ...
                        if (result >= 0) {
                            ...
                            // got a EOS from reader though we expect more data
                            // on it.
        ...
        private DNAddrPair chooseDataNode(LocatedBlock block) throws IOException {
            while (true) {
                ...
                try {
                    ...
                    deadNodes.clear(); // 2nd option is to remove only
                                       // nodes[blockId]
                    openInfo();
                    failures++;
                    continue;
        ...
        private void fetchBlockByteRange(LocatedBlock block, long start, long end, byte[] buf, int offset)
            ...
            while (dn == null && numAttempts-- > 0) {
                ...
                try {
                    ...
                    reader = BlockReader.newBlockReader(dn, src, block.getBlock().getBlockId(), block.getBlock()
                            .getGenerationStamp(), start, len, buffersize, verifyChecksum, clientName);
                    int nread = reader.readAll(buf, offset, len);
                    if (nread != len) {
                        throw new IOException("truncated return from reader.read(): " + "excpected " + len + ", got "
                                + nread);
                    }
                    return;
                } catch (ChecksumException e) {
                    ioe = e;
                    LOG.warn("fetchBlockByteRange(). Got a checksum exception for " + src + " at " + block.getBlock()
                            + ":" + e.getPos() + " from " + chosenNode.getName());
                    reportChecksumFailure(src, block.getBlock(), chosenNode);
                } catch (IOException e) {
                    ioe = e;
                    LOG.warn("Failed to connect to " + targetAddr + " for file " + src + " for block "
                            + block.getBlock().getBlockId() + ":" + StringUtils.stringifyException(e));
                } finally {
            ...
            if (pos <= targetPos && targetPos <= blockEnd) {
                ...
                if (diff <= TCP_WINDOW_SIZE) {
                    try {
                        ...
                        LOG.debug("Exception while seek to " + targetPos + " from " + currentBlock + " of " + src
                                + " from " + currentNode + ": " + StringUtils.stringifyException(e));
                    }
                }
            }
            if (!done) {
        ...
        /**
         * Same as {@link #seekToNewSource(long)} except that it does not
         * exclude the current datanode and might connect to the same node.
         */
        ...
        /**
         * Seek to given position on a node other than the current node. If a
         * node other than the current node is found, then returns true. If
         * another node could not be found, then returns false.
         */
        @Override
     ...
     * size. A packet comprises of chunks. Each chunk is typically 512 bytes and
     * has an associated checksum with it.
     * 
     * When a client application fills up the currentPacket, it is enqueued into
     * dataQueue. The DataStreamer thread picks up packets from the dataQueue,
     * sends it to the first datanode in the pipeline and moves it from the
     * dataQueue to the ackQueue. The ResponseProcessor receives acks from the
     * datanodes. When an successful ack for a packet is received from all
     * datanodes, the ResponseProcessor removes the corresponding packet from
     * the ackQueue.
     * 
    ...
    class DFSOutputStream extends FSOutputSummer implements Syncable {
        ...
        private DatanodeInfo[] nodes = null; // list of targets for current
                                             // block
        ...
        private class Packet {
            ...
            /**
             * Returns ByteBuffer that contains one full packet, including
             * header.
             */
            ByteBuffer getBuffer() {
                /*
                 * Once this is called, no more data can be added to the packet.
                 * setting 'buf' to null ensures that. This is called only when
                ...
                // prepare the header and close any gap between checksum and
                // data.
                ...
                if (checksumPos != dataStart) {
                    /*
                     * move the checksum to cover the gap. This can happen for
                     * the last packet.
                     */
                ...
                /*
                 * write the header and data length. The format is described in
                 * comment before DataNode.BlockSender
                 */
                ...
                // end of pkt header
                buffer.putInt(dataLen); // actual data length, excluding
                                        // checksum.
                    ...
                    synchronized (dataQueue) {
                        ...
                        try {
                            ...
                            if (one.lastPacketInBlock) {
                                blockStream.writeInt(0); // indicate
                                                         // end-of-block
                            }
                            blockStream.flush();
                            if (LOG.isDebugEnabled()) {
                                LOG.debug("DataStreamer block " + block + " wrote packet seqno:" + one.seqno + " size:"
                                        + buf.remaining() + " offsetInBlock:" + one.offsetInBlock
                    ...
                    if (one.lastPacketInBlock) {
                        synchronized (ackQueue) {
                            while (!hasError && ackQueue.size() != 0 && clientRunning) {
                                try {
                                    ackQueue.wait(); // wait for acks to arrive
                                                     // from datanodes
                                } catch (InterruptedException e) {
                                }
                            }
                ...
                while (!closed && clientRunning && !lastPacketInBlock) {
                    ...
                    try {
                        ...
                        for (int i = 0; i < targets.length && clientRunning; i++) {
                            ...
                            if (reply != DataTransferProtocol.OP_STATUS_SUCCESS) {
                                ...
                                throw new IOException("Bad response " + reply + " for block " + block
                                        + " from datanode " + targets[i].getName());
                            }
                        }
                        ...
                        if (!closed) {
                            ...
                            LOG.warn("DFSOutputStream ResponseProcessor exception " + " for block " + block
                                    + StringUtils.stringifyException(e));
                            closed = true;
                        }
                    }
        ...
        private boolean processDatanodeError(boolean hasError, boolean isAppend) {
            ...
            if (errorIndex >= 0) {
                LOG.warn("Error Recovery for block " + block + " bad datanode[" + errorIndex + "] "
            ...
            while (!success && clientRunning) {
                ...
                try {
                    // Pick the "least" datanode as the primary datanode to
                    // avoid deadlock.
                    ...
                    if (recoveryErrorCount > maxRecoveryErrorCount) {
                        if (nodes.length > 1) {
                            // if the primary datanode failed, remove it from
                            // the list.
                            // The original bad datanode is left in the list
                            // because it is
                            // conservative to remove only one datanode in one
                            // iteration.
                            for (int j = 0; j < nodes.length; j++) {
                            ...
                            LOG.warn("Error Recovery for block " + block + " failed "
                                    + " because recovery from primary datanode " + primaryNode + " failed "
                                    + recoveryErrorCount + " times. Marking primary datanode as bad.");
                            recoveryErrorCount = 0;
                            return true; // sleep when we return from here
                        }
                        String emsg = "Error Recovery for block " + block + " failed "
                                + " because recovery from primary datanode " + primaryNode + " failed "
                                + recoveryErrorCount + " times. Aborting...";
                        LOG.warn(emsg);
                    ...
                    LOG.warn("Error Recovery for block " + block + " failed "
                            + " because recovery from primary datanode " + primaryNode + " failed "
                            + recoveryErrorCount + " times. Will retry...");
                    return true; // sleep when we return from here
                } finally {
                ...
                // If the block recovery generated a new generation stamp, use
                // that
        ...
        private DFSOutputStream(String src, long blockSize, Progressable progress, int bytesPerChecksum)
                throws IOException {
         ...
         *      short, long)
         */
        DFSOutputStream(String src, FsPermission masked, boolean overwrite, short replication, long blockSize,
                Progressable progress, int buffersize, int bytesPerChecksum) throws IOException {
         ...
         *      short, long)
         */
        DFSOutputStream(String src, int buffersize, Progressable progress, LocatedBlock lastBlock, FileStatus stat,
            ...
            if (lastBlock != null) {
                ...
                if (nodes.length < 1) {
                    throw new IOException("Unable to retrieve blocks locations " + " for last block " + block
                            + "of file " + src);
        ...
        private void computePacketChunkSize(int psize, int csize) {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("computePacketChunkSize: src=" + src + ", chunkSize=" + chunkSize + ", chunksPerPacket="
        ...
        private boolean createBlockOutputStream(DatanodeInfo[] nodes, String client, boolean recoveryFlag) {
            ...
            try {
                ...
                //
                DataOutputStream out = new DataOutputStream(new BufferedOutputStream(NetUtils.getOutputStream(s,
                        writeTimeout), DataNode.SMALL_BUFFER_SIZE));
                    ...
                    try {
                        ...
                        if (--retries == 0 && !NotReplicatedYetException.class.getName().equals(e.getClassName())) {
                            ...
                            if (System.currentTimeMillis() - localstart > 5000) {
                                LOG.info("Waiting for replication for " + (System.currentTimeMillis() - localstart)
                                        / 1000 + " seconds");
                            }
                            try {
        ...
        protected synchronized void writeChunk(byte[] b, int offset, int len, byte[] checksum) throws IOException {
            ...
            if (checksum.length != this.checksum.getChecksumSize()) {
                throw new IOException("writeChunk() checksum size is supposed to be " + this.checksum.getChecksumSize()
            ...
            synchronized (dataQueue) {
                ...
                if (currentPacket == null) {
                    ...
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("DFSClient writeChunk allocating new packet seqno=" + currentPacket.seqno + ", src="
                                + src + ", packetSize=" + packetSize + ", chunksPerPacket=" + chunksPerPacket
                ...
                if (currentPacket.numChunks == currentPacket.maxChunks || bytesCurBlock == blockSize) {
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("DFSClient writeChunk packet full seqno=" + currentPacket.seqno + ", src=" + src
                                + ", bytesCurBlock=" + bytesCurBlock + ", blockSize=" + blockSize + ", appendChunk="
                                + appendChunk);
                    }
                    //
                    // if we allocated a new packet because we encountered a
                    // block
                    ...
                    // the above
                    // write filled up any partial chunk. Tell the summer to
                    // generate full
        ...
        /**
         * All data is written out to datanodes. It is not guaranteed that data
         * has been flushed to persistent store on the datanode. Block
         * allocations are persisted on namenode.
         */
        public synchronized void sync() throws IOException {
            try {
                /*
                 * Record current blockOffset. This might be changed inside
                 * flushBuffer() where a partial checksum chunk might be
                 * flushed. After the flush, reset the bytesCurBlock back to its
                 * previous value, any partial checksum chunk will be sent now
                 * and in next packet.
                 */
                ...
                LOG.debug("DFSClient flush() : saveOffset " + saveOffset + " bytesCurBlock " + bytesCurBlock
                ...
                if (lastFlushOffset != bytesCurBlock) {
                    ...
                    // just discard the current packet since it is already been
                    // sent.
                    currentPacket = null;
                }
        ...
        /**
         * Waits till all existing data is flushed and confirmations received
         * from datanodes.
         */
        ...
        private synchronized void closeInternal() throws IOException {
            ...
            try {
                ...
                synchronized (dataQueue) {
                    if (blockStream != null) {
                        blockStream.writeInt(0); // indicate end-of-block to
                                                 // datanode
                        blockStream.close();
                        blockReplyStream.close();
                    }
    ...
    // just reports checksum failure and ignores any exception during the
    // report.
    void reportChecksumFailure(String file, LocatedBlock lblocks[]) {
        try {
            ...
            LOG.info("Found corruption while reading " + file
                    + ".  Error repairing corrupt blocks.  Bad blocks remain. " + StringUtils.stringifyException(ie));
        }
    }
