...
public interface DataTransferProtocol {
    ...
    /**
     * Version for data transfers between clients and datanodes This should
     * change when serialization of DatanodeInfo, not just when protocol
     * changes. It is not very obvious.
     */
    /*
     * Version 14: OP_REPLACE_BLOCK is sent from the Balancer server to the
     * destination, including the block id, source, and proxy. OP_COPY_BLOCK is
     * sent from the destination to the proxy, which contains only the block id.
     * A reply to OP_COPY_BLOCK sends the block content. A reply to
     * OP_REPLACE_BLOCK includes an operation status.
     */
