 ...
 */
public class IndexUpdateMapper<K extends WritableComparable, V extends Writable> extends MapReduceBase implements
        Mapper<K, V, Shard, IntermediateForm> {
     ...
     */
    public void map(K key, V value, OutputCollector<Shard, IntermediateForm> output, Reporter reporter)
            throws IOException {
                ...
                if (doc.getOp() == DocumentAndOp.Op.INSERT) {
                    ...
                    if (insertToShard >= 0) {
                        if (insertToShard == deleteFromShard) {
                            ...
                            insertionForm.process(new DocumentAndOp(DocumentAndOp.Op.INSERT, doc.getDocument()),
                                    analyzer);
    ...
    public void configure(JobConf job) {
        ...
        localAnalysis = (ILocalAnalysis) ReflectionUtils.newInstance(iconf.getLocalAnalysisClass(), job);
        localAnalysis.configure(job);

        shards = Shard.getIndexShards(iconf);

        distributionPolicy = (IDistributionPolicy) ReflectionUtils.newInstance(iconf.getDistributionPolicyClass(), job);
