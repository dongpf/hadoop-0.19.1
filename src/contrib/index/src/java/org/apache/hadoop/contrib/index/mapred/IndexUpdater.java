...
public class IndexUpdater implements IIndexUpdater {
     ...
     * 
     * @see
     * org.apache.hadoop.contrib.index.mapred.IIndexUpdater#run(org.apache.hadoop
     * .conf.Configuration, org.apache.hadoop.fs.Path[],
     * org.apache.hadoop.fs.Path, int,
     * org.apache.hadoop.contrib.index.mapred.Shard[])
     */
    public void run(Configuration conf, Path[] inputPaths, Path outputPath, int numMapTasks, Shard[] shards)
            throws IOException {
        JobConf jobConf = createJob(conf, inputPaths, outputPath, numMapTasks, shards);
        JobClient.runJob(jobConf);
    }

    JobConf createJob(Configuration conf, Path[] inputPaths, Path outputPath, int numMapTasks, Shard[] shards)
            throws IOException {
        ...
        // MapTask.MapOutputBuffer uses "io.sort.mb" to decide its max buffer
        // size
