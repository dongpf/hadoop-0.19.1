...
/**
 * This class tests that the Fuse module for DFS can mount properly and does a
 * few simple commands: mkdir rmdir ls cat
 * 
...
public class TestFuseDFS extends TestCase {
     ...
     * /usr/local/lib or somewhere else on your pre-existing LD_LIBRARY_PATH
     * 
     */
    ...
    static private void mount(String mountpoint, URI dfs) throws IOException, InterruptedException {
        ...
        String cmd[] = { fuse_cmd, "dfs://" + dfs.getHost() + ":" + String.valueOf(dfs.getPort()), mountpoint,
                "-obig_writes", "-odebug", "-oentry_timeout=1", "-oattribute_timeout=1", "-ousetrash", "rw",
     ...
     * First, just check raw size reading is ok and then check with smaller
     * reads including checking the validity of the data read.
     * 
     */
