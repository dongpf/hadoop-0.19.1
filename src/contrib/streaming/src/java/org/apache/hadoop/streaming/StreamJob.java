...
import org.apache.hadoop.mapred.PostJobHook;
...
public class StreamJob implements Tool {
    ...
    private MultiPropertyOption jobconf = new MultiPropertyOption("-jobconf",
            "(n=v) Optional. Add or override a JobConf property.", 'D');
    private MultiPropertyOption cmdenv = new MultiPropertyOption("-cmdenv",
            "(n=v) Pass env.var to streaming commands.", 'E');
    ...
    private Option createOption(String name, String desc, String argName, int max, boolean required) {
        Argument argument = argBuilder.withName(argName).withMinimum(1).withMaximum(max).create();
        return builder.withLongName(name).withArgument(argument).withDescription(desc).withRequired(required).create();
    }

    private Option createOption(String name, String desc, String argName, int max, boolean required, Validator validator) {

        Argument argument = argBuilder.withName(argName).withMinimum(1).withMaximum(max).withValidator(validator)
                .create();
    ...
    private void setupOptions() {
        ...
        Option file = createOption("file", "File/dir to be shipped in the Job jar file", "file", Integer.MAX_VALUE,
                false, execValidator);
        ...
        allOptions = new GroupBuilder().withOption(input).withOption(output).withOption(mapper).withOption(combiner)
                .withOption(reducer).withOption(file).withOption(dfs).withOption(jt).withOption(additionalconfspec)
                .withOption(inputformat).withOption(outputformat).withOption(partitioner).withOption(numReduceTasks)
                .withOption(inputreader).withOption(mapDebug).withOption(reduceDebug).withOption(jobconf)
                .withOption(cmdenv).withOption(cacheFile).withOption(cacheArchive).withOption(verbose).withOption(info)
                .withOption(debug).withOption(inputtagged).withOption(help).create();
    ...
    public void exitUsage(boolean detailed) {
        ...
        System.out.println("   setenv HSTREAMING \"$HADOOP_HOME/bin/hadoop jar $HADOOP_HOME/hadoop-streaming.jar\"");
        System.out.println();
    ...
    protected void setJobConf() throws IOException {
        ...
        if (inReaderSpec_ == null && inputFormatSpec_ == null) {
            ...
            if (inputFormatSpec_.equals(TextInputFormat.class.getName())
                ...
                fmt = TextInputFormat.class;
            } else if (inputFormatSpec_.equals(KeyValueTextInputFormat.class.getName())
                    ...
                    || inputFormatSpec_.equals(org.apache.hadoop.mapred.SequenceFileInputFormat.class
                            .getCanonicalName())
                    || inputFormatSpec_.equals(org.apache.hadoop.mapred.SequenceFileInputFormat.class.getSimpleName())) {
    ...
    public int submitAndMonitorJob() throws IOException {
        ...
        boolean enabledPostHook = jobConf_.getBoolean("mapred.client.post.hook.enabled", false);
        if (enabledPostHook) {
            // Call the post job hook list
            try {
                if (running_ != null && jobId_ != null)
                    runPostJobHooks(jobId_, jobConf_, running_);
            } catch (Exception e) {
    ...
    class MultiPropertyOption extends PropertyOption {
        ...
        public void process(final WriteableCommandLine commandLine, final ListIterator arguments)
            ...
            // add to any existing values (support specifying args multiple
            // times)
