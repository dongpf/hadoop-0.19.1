...
public class StreamXmlRecordReader extends StreamBaseRecordReader {
    ...
    public StreamXmlRecordReader(FSDataInputStream in, FileSplit split, Reporter reporter, JobConf job, FileSystem fs)
            throws IOException {
    ...
    public void init() throws IOException {
        LOG.info("StreamBaseRecordReader.init: " + " start_=" + start_ + " end_=" + end_ + " length_=" + length_
                + " start_ > in_.getPos() =" + (start_ > in_.getPos()) + " " + start_ + " > " + in_.getPos());
    ...
    private boolean slowReadUntilMatch(Pattern markPattern, boolean includePat, DataOutputBuffer outBufOrNull)
            throws IOException {
        ...
        if (matched) {
            ...
            bin_.reset();
            skippedBytes = bin_.skip(endPos); // Skip succeeds as we have
                                              // already read this is buffer
            pos_ += endPos;
