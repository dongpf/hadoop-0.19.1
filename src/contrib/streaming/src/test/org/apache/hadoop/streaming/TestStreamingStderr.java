...
/**
 * Test that streaming consumes stderr from the streaming process (before,
 * during, and after the main processing of mapred input), and that stderr
 * messages count as task progress.
 */
public class TestStreamingStderr extends TestCase {
    ...
    protected String[] genArgs(File input, File output, int preLines, int duringLines, int postLines) {
        return new String[] {
                ...
                "-mapper",
                StreamUtil.makeJavaCommand(
                        StderrApp.class,
                        new String[] { Integer.toString(preLines), Integer.toString(duringLines),
                                Integer.toString(postLines) }), "-reducer", StreamJob.REDUCE_NONE, "-jobconf",
                "keep.failed.task.files=true", "-jobconf", "mapred.task.timeout=5000", "-jobconf",
    ...
    public void runStreamJob(String baseName, boolean hasInput, int preLines, int duringLines, int postLines) {
        try {
