...
/**
 * This class tests hadoopStreaming with customized separator in MapReduce local
 * mode.
 */
public class TestStreamingSeparator extends TestCase {
    ...
    protected String[] genArgs() {
        return new String[] {
                ...
                "stream.tmpdir=" + System.getProperty("test.build.data", "/tmp"), "-inputformat",
                "KeyValueTextInputFormat", "-jobconf", "key.value.separator.in.input.line=1", "-jobconf",
                "stream.map.input.field.separator=2", "-jobconf", "stream.map.output.field.separator=3", "-jobconf",
                "stream.reduce.input.field.separator=3", "-jobconf", "stream.reduce.output.field.separator=4",
