...
/**
 * This class tests if hadoopStreaming fails a job when the mapper or reducers
 * have non-zero exit status and the stream.non.zero.exit.status.is.failure
 * jobconf is set.
 */
public class TestStreamingExitStatus extends TestCase {
    ...
    protected String[] genArgs(boolean exitStatusIsFailure, boolean failMap) {
        return new String[] { "-input", INPUT_FILE.getAbsolutePath(), "-output", OUTPUT_DIR.getAbsolutePath(),
                "-mapper", (failMap ? failingTask : echoTask), "-reducer", (failMap ? echoTask : failingTask),
                "-jobconf", "keep.failed.task.files=true", "-jobconf",
                "stream.non.zero.exit.is.failure=" + exitStatusIsFailure, "-jobconf",
    ...
    public void runStreamJob(boolean exitStatusIsFailure, boolean failMap) {
        try {
            ...
            if (exitStatusIsFailure) {
                assertEquals("Streaming Job failure code expected", /*
                                                                     * job not
                                                                     * successful
                                                                     * :
                                                                     */1, returnStatus);
