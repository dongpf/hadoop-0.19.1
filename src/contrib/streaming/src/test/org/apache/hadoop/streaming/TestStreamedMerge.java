...
/**
 * This JUnit test is not pure-Java and is not run as part of the standard ant
 * test* targets. Two ways to run this:
 * 
 * <pre>
...
public class TestStreamedMerge extends TestCase {
    ...
    String createInputs(boolean tag) throws IOException {
        ...
        String expect = i18n(unt(">1\tk1\tv1\n", tag) + unt(">2\tk1\tv2\n", tag) + unt(">3\tk1\tv3\n", tag)
                + unt(">2\tk2\tv4\n", tag) + unt(">1\tk3\tv5\n", tag));
        return expect;
    }
    ...
    String i18n(String c) {
        ...
        c = c.replace('v', '\u00a2'); // Cent sign, in UTF-8: C2A2 ; UTF-16
                                      // contains null
    ...
    void callStreaming(String argSideOutput, boolean inputTagged) throws IOException {
        String[] testargs = new String[] {
                ...
                "-verbose", "-jobconf", "stream.testmerge=1", "-input",
                "+/input/part-00 | /input/part-01 | /input/part-02", "-mapper", StreamUtil.localizeBin("/bin/cat"),
                "-reducer", "NONE", "-output", "/my.output", "-mapsideoutput", argSideOutput, "-dfs",
                conf_.get("fs.default.name"), "-jt", "local", "-jobconf", "stream.sideoutput.localfs=true", "-jobconf",
    ...
    void goSocketTagged(boolean socket, boolean inputTagged) throws IOException {
        ...
        if (socket) {
            ...
            String userOut = StreamUtil.getBoundAntProperty("hadoop.test.localoutputfile", null);
            if (userOut != null) {
                f = new File(userOut);
