...
/**
 * This class tests hadoopStreaming in MapReduce local mode by giving empty
 * input to mapper and the mapper generates nonempty output. Since map() is not
 * called at all, output thread was not getting created and the mapper was
 * hanging forever. Now this issue is solved. Similarly reducer is also checked
 * for task completion with empty input and nonempty output.
 */
public class TestStreamingEmptyInpNonemptyOut extends TestCase {
    ...
    protected void createInputAndScript() throws IOException {
        ...
        out = new DataOutputStream(new FileOutputStream(SCRIPT_FILE.getAbsoluteFile()));
        out.write(script.getBytes("UTF-8"));
        out.close();
    }

    protected String[] genArgs() {
        return new String[] { "-input", INPUT_FILE.getAbsolutePath(), "-output", OUTPUT_DIR.getAbsolutePath(),
                "-mapper", map, "-reducer",
                reduce,
                // "-verbose",
                // "-jobconf", "stream.debug=set"
                "-jobconf", "keep.failed.task.files=true", "-jobconf",
    ...
    public void testEmptyInputNonemptyOutput() throws IOException {
        try {
            ...
            // First let us test if mapper doesn't hang for empty i/p and
            // nonempty o/p
            ...
            // Now let us test if reducer doesn't hang for empty i/p and
            // nonempty o/p
