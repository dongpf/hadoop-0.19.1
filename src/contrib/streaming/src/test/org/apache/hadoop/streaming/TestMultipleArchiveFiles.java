...
/**
 * This class tests cacheArchive option of streaming The test case creates 2
 * archive files, ships it with hadoop streaming and compares the output with
 * expected output
 */
public class TestMultipleArchiveFiles extends TestStreaming {
    ...
    protected String[] genArgs() {
        ...
        try {
            cacheArchiveString1 = fileSys.getUri().toString() + fileSys.getWorkingDirectory().toString() + "/"
                    + CACHE_ARCHIVE_1 + "#symlink1";
            cacheArchiveString2 = fileSys.getUri().toString() + fileSys.getWorkingDirectory().toString() + "/"
                    + CACHE_ARCHIVE_2 + "#symlink2";
        } catch (Exception e) {
            e.printStackTrace();
        }

        return new String[] { "-input", INPUT_FILE.toString(), "-output", OUTPUT_DIR, "-mapper", "xargs cat",
                "-reducer", "cat", "-jobconf", "mapred.reduce.tasks=1", "-cacheArchive", cacheArchiveString1,
                "-cacheArchive", cacheArchiveString2, "-jobconf", strNamenode, "-jobconf", strJobTracker, "-jobconf",
