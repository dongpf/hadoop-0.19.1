...
/**
 * A {@link JobInProgressListener} that maintains the jobs being managed in one
 * or more queues.
 */
class JobQueuesManager extends JobInProgressListener {

    /*
     * If a queue supports priorities, jobs must be sorted on priorities, and
     * then on their start times (technically, their insertion time. If a queue
     * doesn't support priorities, jobs are sorted based on their start time.
     */

    // comparator for jobs in queues that don't support priorities
    private static final Comparator<JobSchedulingInfo> STARTTIME_JOB_COMPARATOR = new Comparator<JobSchedulingInfo>() {
    ...
    private static class QueueInfo {
        ...
        QueueInfo(boolean prio) {
            ...
            if (supportsPriorities) {
                ...
                this.waitingJobs = new TreeMap<JobSchedulingInfo, JobInProgress>(
                        JobQueueJobInProgressListener.FIFO_JOB_QUEUE_COMPARATOR);
                this.runningJobs = new TreeMap<JobSchedulingInfo, JobInProgress>(
                        JobQueueJobInProgressListener.FIFO_JOB_QUEUE_COMPARATOR);
            } else {
                this.waitingJobs = new TreeMap<JobSchedulingInfo, JobInProgress>(STARTTIME_JOB_COMPARATOR);
                this.runningJobs = new TreeMap<JobSchedulingInfo, JobInProgress>(STARTTIME_JOB_COMPARATOR);
            }
        }
    }

    // we maintain a hashmap of queue-names to queue info
    private Map<String, QueueInfo> jobQueues = new HashMap<String, QueueInfo>();
    ...
    public void jobAdded(JobInProgress job) {
        ...
        if (null == qi) {
            ...
            LOG.warn("Invalid queue " + job.getProfile().getQueueName() + " specified for job"
                    + job.getProfile().getJobID() + ". Ignoring job.");
            return;
        }
    ...
    private void jobCompleted(JobInProgress job, JobSchedulingInfo oldInfo, QueueInfo qi) {
        LOG.info("Job " + job.getJobID().toString() + " submitted to queue " + job.getProfile().getQueueName()
                + " has completed");
    ...
    private void reorderJobs(JobInProgress job, JobSchedulingInfo oldInfo, QueueInfo qi) {
    ...
    private void makeJobRunning(JobInProgress job, JobSchedulingInfo oldInfo, QueueInfo qi) {
    ...
    public void jobUpdated(JobChangeEvent event) {
        ...
        if (null == qi) {
            ...
            LOG.warn("Could not find queue " + job.getProfile().getQueueName() + " when updating job "
