...
/**
 * A {@link TaskScheduler} that implements the requirements in HADOOP-3421 and
 * provides a HOD-less way to share large clusters. This scheduler provides the
 * following features: * support for queues, where a job is submitted to a
 * queue. * Queues are guaranteed a fraction of the capacity of the grid (their
 ...
 * access to the capacity guaranteed to the Org. * Free resources can be
 * allocated to any queue beyond its guaranteed capacity. These excess allocated
 * resources can be reclaimed and made available to another queue in order to
 * meet its capacity guarantee. * The scheduler guarantees that excess resources
 * taken from a queue will be restored to it within N minutes of its need for
 * them. * Queues optionally support job priorities (disabled by default). *
 ...
 * will not be preempted for a higher priority job. * In order to prevent one or
 * more users from monopolizing its resources, each queue enforces a limit on
 * the percentage of resources allocated to a user at any given time, if there
...
class CapacityTaskScheduler extends TaskScheduler {
    ...
    /**
     * For keeping track of reclaimed capacity. Whenever slots need to be
     * reclaimed, we create one of these objects. As the queue gets slots, the
     * amount to reclaim gets decremented. if we haven't reclaimed enough within
     * a certain time, we need to kill tasks. This object 'expires' either if
     * all resources are reclaimed before the deadline, or the deadline passes .
     */
    ...
    /**
     * This class keeps track of scheduling info for each queue for either Map
     * or Reduce tasks. . This scheduling information is used by the JT to
     * decide how to allocate tasks, redistribute capacity, etc.
     */
    private static class QueueSchedulingInfo {
        ...
        /**
         * the actual gc, which depends on how many slots are available in the
         * cluster at any given time.
         */
        int guaranteedCapacity = 0;

        /**
         * we also keep track of how many tasks are running for all jobs in the
         * queue, and how many overall tasks there are. This info is available
         * for each job, but keeping a sum makes our algos faster.
         */
        ...
        Map<String, Integer> numRunningTasksByUser = new HashMap<String, Integer>();

        /** min value of user limit (same for all users) */
        int ulMin;

        /**
         * We need to keep track of resources to reclaim. Whenever a queue is
         * under capacity and has tasks pending, we offer it an SLA that gives
         * it free slots equal to or greater than the gap in its capacity,
         * within a period of time (reclaimTime). To do this, we periodically
         * check if queues need to reclaim capacity. If they do, we create a
         * ResourceReclaim object. We also periodically check if a queue has
         * received enough free slots within, say, 80% of its reclaimTime. If
         * not, we kill enough tasks to make up the difference. We keep two
         * queues of ResourceReclaim objects. when an object is created, it is
         * placed in one queue. Once we kill tasks to recover resources for that
         * object, it is placed in an expiry queue. we need to do this to
         * prevent creating spurious ResourceReclaim objects. We keep a count of
         * total resources that are being reclaimed. This count is decremented
         * when an object expires.
         */

        /**
         * reclaim time limit (in msec). This time represents the SLA we offer a
         * queue - a queue gets back any lost capacity withing this period of
         * time.
         */
        long reclaimTime;
        /**
         ...
         */
        LinkedList<ReclaimedResource> reclaimList = new LinkedList<ReclaimedResource>();
        /**
         ...
         */
        LinkedList<ReclaimedResource> reclaimExpireList = new LinkedList<ReclaimedResource>();
        /**
         * sum of all resources that are being reclaimed. We keep this to
         * prevent unnecessary ReclaimResource objects from being created.
         */
        int numReclaimedResources = 0;

        public QueueSchedulingInfo(String queueName, float guaranteedCapacity, int ulMin, long reclaimTime) {
    ...
    private static class SchedulingInfo {
        ...
        public SchedulingInfo(QueueSchedulingInfo mqsi, QueueSchedulingInfo rqsi, boolean supportsPriority) {
            this.mqsi = mqsi;
            this.rqsi = rqsi;
    ...
    /**
     * This class handles the scheduling algorithms. The algos are the same for
     * both Map and Reduce tasks. There may be slight variations later, in which
     * case we can make this an abstract base class and have derived classes for
     * Map and Reduce.
     */
    private static abstract class TaskSchedulingMgr {

        /** quick way to get qsi object given a queue name */
        private Map<String, QueueSchedulingInfo> queueInfoMap = new HashMap<String, QueueSchedulingInfo>();
        ...
        abstract Task obtainNewTask(TaskTrackerStatus taskTracker, JobInProgress job) throws IOException;
        ...
        /**
         * List of QSIs for assigning tasks. This list is ordered such that
         * queues that need to reclaim capacity sooner, come before queues that
         * don't. For queues that don't, they're ordered by a ratio of (# of
         * running tasks)/Guaranteed capacity, which indicates how much 'free
         * space' the queue has, or how much it is over capacity. This ordered
         * list is iterated over, when assigning tasks.
         */
        private List<QueueSchedulingInfo> qsiForAssigningTasks = new ArrayList<QueueSchedulingInfo>();

        /** comparator to sort queues */
        private static final class QueueComparator implements Comparator<QueueSchedulingInfo> {
            public int compare(QueueSchedulingInfo q1, QueueSchedulingInfo q2) {
                // if one queue needs to reclaim something and the other one
                // doesn't,
                // the former is first
                if ((0 == q1.reclaimList.size()) && (0 != q2.reclaimList.size())) {
                    ...
                    // neither needs to reclaim. If either doesn't have a
                    // capacity yet,
                    // it comes at the end of the queue.
                    if ((q1.guaranteedCapacity == 0) && (q2.guaranteedCapacity != 0)) {
                        return 1;
                    } else if ((q1.guaranteedCapacity != 0) && (q2.guaranteedCapacity == 0)) {
                        return -1;
                    } else if ((q1.guaranteedCapacity == 0) && (q2.guaranteedCapacity == 0)) {
                    ...
                    // both have to reclaim. Look at which one needs to reclaim
                    // earlier
        ...
        /**
         * Periodically, we walk through our queues to do the following: a.
         * Check if a queue needs to reclaim any resources within a period of
         * time (because it's running below capacity and more tasks are waiting)
        ...
        private synchronized void reclaimCapacity() {
            ...
            QueueSchedulingInfo lastQsi = qsiForAssigningTasks.get(qsiForAssigningTasks.size() - 1);
            ...
            for (QueueSchedulingInfo qsi : queueInfoMap.values()) {
                ...
                if ((!qsi.reclaimList.isEmpty())
                        && (qsi.reclaimList.getFirst().whenToKill < currentTime
                                + CapacityTaskScheduler.RECLAIM_CAPACITY_INTERVAL)) {
                ...
                if (qsi.numRunningTasks < qsi.guaranteedCapacity) {
                    ...
                    // see if we have remaining capacity and if we have enough
                    // pending
                    // tasks to use up remaining capacity
                    if ((usedCap < qsi.guaranteedCapacity) && ((qsi.numPendingTasks - qsi.numReclaimedResources) > 0)) {
                        ...
                        // create a rsource object that needs to be reclaimed
                        // some time
                        // in the future
                        long whenToKill = qsi.reclaimTime
                                - (CapacityTaskScheduler.HEARTBEATS_LEFT_BEFORE_KILLING * scheduler.taskTrackerManager
                                        .getNextHeartbeatInterval());
                        if (whenToKill < 0)
                            whenToKill = 0;
                        qsi.reclaimList.add(new ReclaimedResource(amt, currentTime + qsi.reclaimTime, currentTime
                                + whenToKill));
                        qsi.numReclaimedResources += amt;
                        LOG.debug("Queue " + qsi.queueName + " needs to reclaim " + amt + " resources");
                    }
                }
            }
        ...
        private void killTasks(int tasksToKill) {
             ...
             * tasks killed is equal to 'tasksToKill'. Maybe the best way is to
             * keep a global ordering of running tasks and kill the ones that
             * ran last, irrespective of what queue or job they belong to. What
             * we do here is look at how many tasks is each queue running over
             * capacity, and use that as a weight to decide how many tasks to
             * kill from that queue.
             */
            ...
            // if some queue needs to reclaim cap, there must be at least one
            // queue
            ...
            for (int i = loc; i < qsiForAssigningTasks.size(); i++) {
                ...
                killTasksFromQueue(
                        qsi,
                        (int) Math.round(((double) (qsi.numRunningTasks - qsi.guaranteedCapacity)) * tasksToKill
                                / (double) tasksOverCap));
            }
        }
        ...
        private void killTasksFromQueue(QueueSchedulingInfo qsi, int tasksToKill) {
            // we start killing as many tasks as possible from the jobs that
            // started
            // last. This way, we let long-running jobs complete faster.
            int tasksKilled = 0;
            JobInProgress jobs[] = scheduler.jobQueuesManager.getRunningJobQueue(qsi.queueName).toArray(
                    new JobInProgress[0]);
        ...
        /**
         * Update individual QSI objects. We don't need exact information for
         * all variables, just enough for us to make scheduling decisions. For
         * example, we don't need an exact count of numRunningTasks. Once we
         * count upto the grid capacity (gcSum), any number beyond that will
         * make no difference.
         * */
        private synchronized void updateQSIObjects() {
            ...
            for (QueueSchedulingInfo qsi : queueInfoMap.values()) {
                ...
                if (slotsDiff != 0) {
                    qsi.guaranteedCapacity = (int) (qsi.guaranteedCapacityPercent * numSlots / 100);
                }
                ...
                for (JobInProgress j : scheduler.jobQueuesManager.getRunningJobQueue(qsi.queueName)) {
                    ...
                    qsi.numRunningTasksByUser.put(j.getProfile().getUser(), i + getRunningTasks(j));
                    qsi.numPendingTasks += getPendingTasks(j);
                    LOG.debug("updateQSI: job " + j.toString() + ": run(m) = " + j.runningMaps() + ", run(r) = "
                            + j.runningReduces() + ", finished(m) = " + j.finishedMaps() + ", finished(r)= "
                            + j.finishedReduces() + ", failed(m) = " + j.failedMapTasks + ", failed(r) = "
                            + j.failedReduceTasks + ", spec(m) = " + j.speculativeMapTasks + ", spec(r) = "
                            + j.speculativeReduceTasks + ", total(m) = " + j.numMapTasks + ", total(r) = "
                            + j.numReduceTasks);
                    /*
                     * it's fine walking down the entire list of running jobs -
                     * there probably will not be many, plus, we may need to go
                     * through the list to compute numRunningTasksByUser. If
                     * this is expensive, we can keep a list of running jobs per
                     * user. Then we only need to consider the first few jobs
                     * per user.
                     */
                }
                // update stats on waiting jobs
                for (JobInProgress j : scheduler.jobQueuesManager.getWaitingJobQueue(qsi.queueName)) {
                    // pending tasks
        ...
        void jobAdded(JobInProgress job) {
            // update qsi
            QueueSchedulingInfo qsi = queueInfoMap.get(job.getProfile().getQueueName());
            ...
            LOG.debug("Job " + job.getJobID().toString() + " is added under user " + job.getProfile().getUser()
                    + ", user now has " + i + " jobs");
        }
        ...
        void jobRemoved(JobInProgress job) {
            ...
            if (0 == i.intValue()) {
                ...
                qsi.numJobsByUser.put(job.getProfile().getUser(), i);
                LOG.debug("User still has " + i + " jobs, number of users = " + qsi.numJobsByUser.size());
            }
        }
        ...
        private boolean isUserOverLimit(JobInProgress j, QueueSchedulingInfo qsi) {
            ...
            int limit = Math.max((int) (Math.ceil((double) currentCapacity / (double) qsi.numJobsByUser.size())),
                    (int) (Math.ceil((double) (qsi.ulMin * currentCapacity) / 100.0)));
            if (qsi.numRunningTasksByUser.get(j.getProfile().getUser()) >= limit) {
                LOG.debug("User " + j.getProfile().getUser() + " is over limit, num running tasks = "
                        + qsi.numRunningTasksByUser.get(j.getProfile().getUser()) + ", limit = " + limit);
                return true;
            } else {
                return false;
            }
        ...
        private Task getTaskFromQueue(TaskTrackerStatus taskTracker, QueueSchedulingInfo qsi) throws IOException {
            Task t = null;
            ...
            for (JobInProgress j : scheduler.jobQueuesManager.getRunningJobQueue(qsi.queueName)) {
                ...
                if (t != null) {
                    LOG.debug("Got task from job " + j.getJobID().toStringWOPrefix() + " in queue " + qsi.queueName);
                    return t;
                }
            }

            // if we're here, we found nothing in the running jobs. Time to
            // look at waiting jobs. Get first job of a user that is not over
            // limit
            for (JobInProgress j : scheduler.jobQueuesManager.getWaitingJobQueue(qsi.queueName)) {
                // is this job's user over limit?
                ...
                t = obtainNewTask(taskTracker, j);
                if (t != null) {
                    LOG.debug("Getting task from job " + j.getJobID().toStringWOPrefix() + " in queue " + qsi.queueName);
                    return t;
                }
            }
            ...
            // enough tasks to satisfy his limit. If it's the later case, look
            // at
            ...
            if (usersOverLimit.size() > 0) {
                for (JobInProgress j : scheduler.jobQueuesManager.getRunningJobQueue(qsi.queueName)) {
                    if ((j.getStatus().getRunState() == JobStatus.RUNNING)
                        ...
                        t = obtainNewTask(taskTracker, j);
                        if (t != null) {
                            LOG.debug("Getting task from job " + j.getJobID().toStringWOPrefix() + " in queue "
                                    + qsi.queueName);
                            return t;
                        }
                    }
        ...
        private List<Task> assignTasks(TaskTrackerStatus taskTracker) throws IOException {
            ...
            /*
             * update all our QSI objects. This involves updating each qsi
             * structure. This operation depends on the number of running jobs
             * in a queue, and some waiting jobs. If it becomes expensive, do it
             * once every few heartbeats only.
             */
            updateQSIObjects();
            LOG.debug("After updating QSI objects:");
            printQSIs();
            /*
             * sort list of queues first, as we want queues that need the most
             * to get first access. If this is expensive, sort every few
             * heartbeats. We're only sorting a collection of queues - there
             * shouldn't be many.
             */
        ...
        private void printQSIs() {
            ...
            for (QueueSchedulingInfo qsi : qsiForAssigningTasks) {
                Collection<JobInProgress> runJobs = scheduler.jobQueuesManager.getRunningJobQueue(qsi.queueName);
                Collection<JobInProgress> waitJobs = scheduler.jobQueuesManager.getWaitingJobQueue(qsi.queueName);
                s.append(" Queue '" + qsi.queueName + "'(" + this.type + "): run=" + qsi.numRunningTasks + ", gc="
                        + qsi.guaranteedCapacity + ", wait=" + qsi.numPendingTasks + ", run jobs=" + runJobs.size()
    ...
    private static class MapSchedulingMgr extends TaskSchedulingMgr {
        ...
        Task obtainNewTask(TaskTrackerStatus taskTracker, JobInProgress job) throws IOException {
            ClusterStatus clusterStatus = scheduler.taskTrackerManager.getClusterStatus();
        ...
        int killTasksFromJob(JobInProgress job, int tasksToKill) {
             ...
             * least progress. Ideally, each job would have a list of tasks,
             * sorted by start time or progress. That's a lot of state to keep,
             * however. For now, we do something a little different. We first
             * try and kill non-local tasks, as these can be run anywhere. For
             * each TIP, we kill the task that has made the least progress, if
             * the TIP has more than one active task. We then look at tasks in
             * runningMapCache.
             */
            int tasksKilled = 0;

            /*
             * For non-local running maps, we 'cheat' a bit. We know that the
             * set of non-local running maps has an insertion order such that
             * tasks that ran last are at the end. So we iterate through the set
             * in reverse. This is OK because even if the implementation
             * changes, we're still using generic set iteration and are no worse
             * of.
             */
            TaskInProgress[] tips = job.getNonLocalRunningMaps().toArray(new TaskInProgress[0]);
    ...
    private static class ReduceSchedulingMgr extends TaskSchedulingMgr {
        ...
        int killTasksFromJob(JobInProgress job, int tasksToKill) {
            /*
             * For reduces, we 'cheat' a bit. We know that the set of running
             * reduces has an insertion order such that tasks that ran last are
             * at the end. So we iterate through the set in reverse. This is OK
             * because even if the implementation changes, we're still using
             * generic set iteration and are no worse of.
             */
            int tasksKilled = 0;
            TaskInProgress[] tips = job.getRunningReduces().toArray(new TaskInProgress[0]);
            for (int i = tips.length - 1; i >= 0; i--) {
                // pick the tast attempt that has progressed least
                TaskAttemptID tid = getRunningTaskWithLeastProgress(tips[i]);
                if (null != tid) {
    ...
    public synchronized void start() throws IOException {
        ...
        for (String queueName : queues) {
            ...
            SchedulingInfo schedulingInfo = new SchedulingInfo(mapScheduler.getQueueSchedulingInfo(queueName),
    ...
    public synchronized void terminate() throws IOException {
        ...
        if (jobQueuesManager != null) {
            taskTrackerManager.removeJobInProgressListener(jobQueuesManager);
        }
    ...
    /**
     * provided for the test classes lets you update the QSI objects and sorted
     * collection
     */
    void updateQSIInfo() {
    ...
    /*
     * The grand plan for assigning a task. First, decide whether a Map or
     * Reduce task should be given to a TT (if the TT can accept either). Next,
     * pick a queue. We only look at queues that need a slot. Among these, we
     * first look at queues whose ac is less than gc (queues that gave up
     * capacity in the past). Next, we look at any other queue that needs a
     * slot. Next, pick a job in a queue. we pick the job at the front of the
     * queue unless its user is over the user limit. Finally, given a job, pick
     * a task from the job.
     */
    @Override
    public synchronized List<Task> assignTasks(TaskTrackerStatus taskTracker) throws IOException {
         ...
         * give it a Map or Reduce task. Number of ways to do this. For now,
         * base decision on how much is needed versus how much is used (default
         * to Map, if equal).
         */
        LOG.debug("TT asking for task, max maps=" + taskTracker.getMaxMapTasks() + ", run maps="
                + taskTracker.countMapTasks() + ", max reds=" + taskTracker.getMaxReduceTasks() + ", run reds="
                + taskTracker.countReduceTasks() + ", map cap=" + mapScheduler.getClusterCapacity() + ", red cap = "
    ...
    public synchronized Collection<JobInProgress> getJobs(String queueName) {
        ...
        Collection<JobInProgress> runningJobs = jobQueuesManager.getRunningJobQueue(queueName);
        if (runningJobs != null) {
            jobCollection.addAll(runningJobs);
        }
        Collection<JobInProgress> waitingJobs = jobQueuesManager.getWaitingJobQueue(queueName);
        if (waitingJobs != null) {
