...
/**********************************************************
 * This class can be used to anonymize logs independently of Hadoop and the
 * Executor. It parses the specified log file to create log records for it and
 * then passes them to the Anonymizer. After they are anonymized, they are
 * written to a local file, which is then compressed and stored locally.
 * 
 **********************************************************/
...
public class OfflineAnonymizer {
     ...
     * 
     * @param logtype
     *            the type of the log file. This can either be LogFile.HADOOP or
     *            LogFile.SYSTEM
     * @param filename
    ...
    /**
     * Performs anonymization for the log file. Log entries are read one by one
     * and EventRecords are created, which are then anonymized and written to
     * the output.
     * 
     */
    public void anonymize() throws Exception {
        ...
        System.out.println("Compressed output file written to " + logfile.getName() + ".anonymized"
                + LocalStore.COMPRESSION_SUFFIX);
    }
