...
/**********************************************************
 * An object of this class parses a Hadoop log file to create appropriate
 * EventRecords. The log file can either be the log of a NameNode or JobTracker
 * or DataNode or TaskTracker.
 * 
 **********************************************************/
...
public class HadoopLogParser extends LogParser {
    ...
    /**
     * Create a new parser object and try to find the hostname of the node that
     * generated the log
     */
    ...
    public EventRecord parseLine(String line) throws IOException {
        ...
        if (line != null) {
            ...
            if (matcher.find(0) && matcher.groupCount() >= 5) {
                retval = new EventRecord(hostname, ips, parseDate(matcher.group(1), matcher.group(2)), "HadoopLog",
     ...
     * This information can be found in the STARTUP_MSG lines of the Hadoop log,
     * which are emitted when the node starts.
     * 
     */
    private void findHostname() {
        String startupInfo = Environment.runCommand("grep --max-count=1 STARTUP_MSG:\\s*host " + file.getName())
                .toString();
