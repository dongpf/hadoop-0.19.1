...
/**********************************************************
 * Objects of this class parse the output of ifconfig to gather information
 * about present Network Interface Cards in the system. The list of NICs to poll
...
public class NICParser extends ShellParser {
    ...
    /**
     * Reads and parses the output of ifconfig for a specified NIC and creates
     * an appropriate EventRecord that holds the desirable information for it.
     * 
     * @param device
     *            the NIC device name to query
     * 
    ...
    public EventRecord query(String device) throws UnknownHostException {
        ...
        EventRecord retval = new EventRecord(InetAddress.getLocalHost().getCanonicalHostName(),
                InetAddress.getAllByName(InetAddress.getLocalHost().getHostName()), Calendar.getInstance(), "NIC",
                "Unknown", device, "-");
     ...
     * of the NICs specified in the configuration.
     * 
