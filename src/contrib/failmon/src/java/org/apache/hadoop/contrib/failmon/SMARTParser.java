...
/**********************************************************
 * Objects of this class parse the output of smartmontools to gather information
 * about the state of disks in the system. The smartmontools utility reads the
 * S.M.A.R.T. attributes from the disk devices and reports them to the user.
 * Note that since running smartctl requires superuser provileges, one should
 * grand sudo privileges to the running user for the command smartctl (without a
 * password). Alternatively, one can set up a cron job that periodically dumps
 * the output of smartctl into a user-readable file. See the configuration file
 * for details.
 * 
 **********************************************************/
...
public class SMARTParser extends ShellParser {
    ...
    /**
     * Reads and parses the output of smartctl for a specified disk and creates
     * an appropriate EventRecord that holds the desirable information for it.
     * Since the output of smartctl is different for different kinds of disks,
     * we try to identify as many attributes as posssible for all known output
     * formats.
     * 
     * @param device
    ...
    public EventRecord query(String device) throws Exception {
        ...
        EventRecord retval = new EventRecord(InetAddress.getLocalHost().getCanonicalHostName(),
                InetAddress.getAllByName(InetAddress.getLocalHost().getHostName()), Calendar.getInstance(), "SMART",
                "Unknown", (conf == null ? "sudo smartctl --all " + device : "file " + conf), "-");
        // IBM SCSI disks
        ...
        for (String s : var) {
            ...
            retval.set(s + "_uncorrected",
                    findPattern(s + "\\s*:\\s*(\\d+\\s+){5}(\\d+.?\\d*){1}\\s+(\\d+)", sb.toString(), 3));
        }
        ...
        retval.set("status",
                "PASSED".equalsIgnoreCase(findPattern("test\\s*result\\s*:\\s*(.*)", sb.toString(), 1)) ? "OK"
                        : "FAILED");
     ...
     * RAW_VALUE 3 Spin_Up_Time 0x0027 180 177 063 Pre-fail Always - 10265 4
     * Start_Stop_Count 0x0032 253 253 000 Old_age Always - 34 5
     * Reallocated_Sector_Ct 0x0033 253 253 063 Pre-fail Always - 0 6
     * Read_Channel_Margin 0x0001 253 253 100 Pre-fail Offline - 0 7
     * Seek_Error_Rate 0x000a 253 252 000 Old_age Always - 0 8
     * Seek_Time_Performance 0x0027 250 224 187 Pre-fail Always - 53894 9
     * Power_On_Minutes 0x0032 210 210 000 Old_age Always - 878h+00m 10
     * Spin_Retry_Count 0x002b 253 252 157 Pre-fail Always - 0 11
     * Calibration_Retry_Count 0x002b 253 252 223 Pre-fail Always - 0 12
     * Power_Cycle_Count 0x0032 253 253 000 Old_age Always - 49 192
     * PowerOff_Retract_Count 0x0032 253 253 000 Old_age Always - 0 193
     * Load_Cycle_Count 0x0032 253 253 000 Old_age Always - 0 194
     * Temperature_Celsius 0x0032 037 253 000 Old_age Always - 37 195
     * Hardware_ECC_Recovered 0x000a 253 252 000 Old_age Always - 2645
     * 
     * This format is mostly found in IDE and SATA disks.
     * 
     * @param er
     *            the EventRecord in which to store attributes found
     * @param sb
     *            the StringBuffer with the text to parse
     * 
     * @return the EventRecord in which new attributes are stored.
     */
    private EventRecord readColumns(EventRecord er, StringBuffer sb) {

        Pattern pattern = Pattern.compile("^\\s{0,2}(\\d{1,3}\\s+.*)$", Pattern.MULTILINE);
    ...
    /**
     * Invokes query() to do the parsing and handles parsing errors for each one
     * of the disks specified in the configuration.
     * 
