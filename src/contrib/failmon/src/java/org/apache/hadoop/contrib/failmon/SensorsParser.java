...
/**********************************************************
 * Objects of this class parse the output of the lm-sensors utility to gather
 * information about fan speed, temperatures for cpus and motherboard etc.
 * 
 **********************************************************/
...
public class SensorsParser extends ShellParser {
    ...
    /**
     * Reads and parses the output of the 'sensors' command and creates an
     * appropriate EventRecord that holds the desirable information.
     * 
     * @param s
     *            unused parameter
     * 
    ...
    public EventRecord query(String s) throws Exception {
        ...
        EventRecord retval = new EventRecord(InetAddress.getLocalHost().getCanonicalHostName(),
                InetAddress.getAllByName(InetAddress.getLocalHost().getHostName()), Calendar.getInstance(),
                "lm-sensors", "Unknown", "sensors -A", "-");
    ...
    /**
     * Reads and parses lines that provide the output of a group of sensors with
     ...
     *            the text to parse
     * @param prefix
     *            a String prefix specifying the common prefix of the sensors'
     *            names in the group (e.g. "fan", "in", "temp"
     * 
     * @return the EventRecord created
     */
    private EventRecord readGroup(EventRecord er, StringBuffer sb, String prefix) {

        Pattern pattern = Pattern.compile(".*(" + prefix + "\\s*\\d*)\\s*:\\s*(\\+?\\d+)", Pattern.MULTILINE);
