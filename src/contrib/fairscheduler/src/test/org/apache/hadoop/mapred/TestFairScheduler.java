...
public class TestFairScheduler extends TestCase {
    final static String TEST_DIR = new File(System.getProperty("test.build.data", "build/contrib/streaming/test/data"))
            .getAbsolutePath();
    final static String ALLOC_FILE = new File(TEST_DIR, "test-pools").getAbsolutePath();
    ...
    public void testAllocationFileParsing() throws Exception {
        ...
        assertEquals(5, poolManager.getPools().size()); // 4 in file + default
                                                        // pool
    ...
    public void testNonRunningJobsAreIgnored() throws IOException {
        ...
        assertNull(scheduler.assignTasks(tracker("tt1")));
        advanceTime(100); // Check that we still don't assign jobs after an
                          // update
        assertNull(scheduler.assignTasks(tracker("tt1")));
    }

    /**
     * This test contains two jobs with fewer required tasks than there are
     * slots. We check that all tasks are assigned, but job 1 gets them first
    ...
    public void testSmallJobs() throws IOException {
        ...
        // Advance time before submitting another job j2, to make j1 run before
        // j2
        // deterministically.
        advanceTime(100);
        ...
        // Check scheduler variables; the fair shares should now have been
        // allocated
        // equally between j1 and j2, but j1 should have (4 slots)*(100 ms)
        // deficit
        assertEquals(0, info1.runningMaps);
        assertEquals(0, info1.runningReduces);
        assertEquals(2, info1.neededMaps);
     ...
     * of 400 while job 1 is down to a deficit of 0. We then finish all tasks
     * and assign new ones, which should all be from job 2. These run for 50 ms,
     * which is not enough time for job 2 to make up its deficit (it only makes
     * up 100 ms of deficit). Finally we assign a new round of tasks, which
     * should all be from job 2 again.
     */
    public void testLargeJobs() throws IOException {
        ...
        // the JobInProgress's getMapTasks/getReduceTasks and think they
        // finished.
    ...
    /**
     * This test starts by submitting three large jobs: - job1 in the default
     * pool, at time 0 - job2 in poolA, with an allocation of 1 map / 2 reduces,
     * at time 200 - job3 in poolB, with an allocation of 2 maps / 1 reduce, at
     * time 200
     * 
     ...
     * job3 has more maps in its min share than job2, but job1 has been around a
     * long time at the beginning. The reduce deficits are similar, except job2
    ...
    /**
     * This test starts by submitting three large jobs: - job1 in the default
     * pool, at time 0 - job2 in poolA, with an allocation of 2 maps / 2
     * reduces, at time 200 - job3 in poolA, with an allocation of 2 maps / 2
     * reduces, at time 300
     * 
    ...
    /**
     * This test starts by submitting two jobs at time 0: - job1 in the default
     * pool - job2, with 1 map and 1 reduce, in poolA, which has an alloc of 4
     * maps and 4 reduces
     * 
     ...
     * running only one task, it accumulates deficit in case it will have
     * failures or need speculative tasks later. (TODO: This may not be a good
     * policy.)
     */
    ...
    /**
     * This test starts by submitting four jobs in the default pool. However,
     * the maxRunningJobs limit for this pool has been set to two. We should see
     * only the first two jobs get scheduled, each with half the total slots.
     */
    ...
    public void testSizeBasedWeight() throws Exception {
        ...
        assertTrue(scheduler.infos.get(job2).mapFairShare > scheduler.infos.get(job1).mapFairShare);
        assertTrue(scheduler.infos.get(job1).reduceFairShare > scheduler.infos.get(job2).reduceFairShare);
    }
    ...
    protected void checkAssignment(String taskTrackerName, String expectedTaskString) throws IOException {
    ...
    /**
     * Tests that max-running-tasks per node are set by assigning load equally
     * accross the cluster in CapBasedLoadManager.
     */
