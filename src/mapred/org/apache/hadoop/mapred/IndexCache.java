...
class IndexCache {
    ...
    /**
     * This method gets the index information for the given mapId and reduce. It
     * reads the index file into cache if it is not already present.
     * 
     * @param mapId
     * @param reduce
     * @param fileName
    ...
    public IndexRecord getIndexInformation(String mapId, int reduce, Path fileName) throws IOException {
        ...
        if (info.indexRecordArray.length == 0 || info.indexRecordArray.length < reduce) {
    ...
    /**
     * This method removes the map from the cache. It should be called when a
     * map output on this tracker is discarded.
     * 
     * @param mapId
    ...
    private static class IndexInformation {
        ...
        int getSize() {
            return ((indexRecordArray == null) ? 0 : indexRecordArray.length * MapTask.MAP_OUTPUT_INDEX_RECORD_LENGTH);
        }
    }
}
