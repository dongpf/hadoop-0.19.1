 ...
 * 
 * The Map/Reduce framework can be configured with one or more queues, depending
 * on the scheduler it is configured with. While some schedulers work only with
 * one queue, some schedulers support multiple queues.
 * 
 * Queues can be configured with various properties. Some of these properties
 * are common to all schedulers, and those are handled by this class. Schedulers
 * might also associate several custom properties with queues. Where such a case
 * exists, the queue name must be used to link the common properties with the
 * scheduler specific ones.
 */
class QueueManager {
    ...
    static enum QueueOperation {
        SUBMIT_JOB("acl-submit-job", false), ADMINISTER_JOBS("acl-administer-jobs", true);
    ...
    private static class ACL {
         ...
         * 
         * The String is a a comma separated list of users and groups. The user
         * list comes first and is separated by a space followed by the group
         * list. For e.g. "user1,user2 group1,group2"
         * 
         * @param aclString
     ...
     * 
     * @param conf
     *            Configuration object where queue configuration is specified.
     */
     ...
     * 
     * @param queueName
     *            Queue on which the operation needs to be performed.
     * @param oper
     *            The operation to perform
     * @param ugi
     *            The user and groups who wish to perform the operation.
     * 
     ...
     * performing the task, all users are provided access for this operation, or
     * if either the user or any of the groups specified is provided access.
     * 
     ...
     *            Queue on which the operation needs to be performed.
     * @param job
     *            The {@link JobInProgress} on which the operation is being
     *            performed.
     * @param oper
     *            The operation to perform
     * @param ugi
     ...
     * @return true if the operation is allowed, false otherwise.
     */
    public synchronized boolean hasAccess(String queueName, JobInProgress job, QueueOperation oper,
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("checking access for : " + toFullPropertyName(queueName, oper.getAclName()));
        }
        ...
        return ((acl.allUsersAllowed()) || (acl.isUserAllowed(ugi.getUserName())) || (acl.isAnyGroupAllowed(ugi
                .getGroupNames())));
    }

    /**
     * Set a generic Object that represents scheduling information relevant to a
     * queue.
     * 
     * A string representation of this Object will be used by the framework to
     * display in user facing applications like the JobTracker web UI and the
     * hadoop CLI.
     * 
     * @param queueName
     *            queue for which the scheduling information is to be set.
     * @param queueInfo
     *            scheduling information for this queue.
     */
    ...
    /**
     * Refresh information configured for queues in the system by reading it
     * from the passed in {@link org.apache.hadoop.conf.Configuration}.
     * 
     * Previously stored information about queues is removed and new information
     * populated from the configuration.
     * 
     * @param conf
     *            New configuration for the queues.
     */
    ...
    synchronized JobQueueInfo[] getJobQueueInfos() {
        ...
        return (JobQueueInfo[]) queueInfoList.toArray(new JobQueueInfo[queueInfoList.size()]);
    }
