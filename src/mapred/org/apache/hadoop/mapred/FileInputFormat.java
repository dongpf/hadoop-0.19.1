 ...
 * {@link #getSplits(JobConf, int)}. Subclasses of <code>FileInputFormat</code>
...
public abstract class FileInputFormat<K, V> implements InputFormat<K, V> {
    ...
    public static PathFilter getInputPathFilter(JobConf conf) {
        Class<? extends PathFilter> filterClass = conf
                .getClass("mapred.input.pathFilter.class", null, PathFilter.class);
        ...
        for (Path p : dirs) {
            ...
            if (matches == null) {
                ...
                for (FileStatus globStat : matches) {
                    for (LocatedFileStatus stat : fs.listLocatedStatus(globStat.getPath(), inputFilter)) {
                        result.add(stat);
                    }
                }
            }
