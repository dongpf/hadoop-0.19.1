...
abstract class TaskStatus implements Writable, Cloneable {
    ...
    public TaskStatus(TaskAttemptID taskid, float progress, State runState, String diagnosticInfo, String stateString,
    ...
    /**
     * Get task finish time. if shuffleFinishTime and sortFinishTime are not set
     * before, these are set to finishTime. It takes care of the case when
     * shuffle, sort and finish are completed with in the heartbeat interval and
     * are not reported separately. if task state is TaskStatus.FAILED then
     * finish time represents when the task failed.
     * 
    ...
    /**
     * Get shuffle finish time for the task. If shuffle finish time was not set
     * due to shuffle/sort/finish phases ending within same heartbeat interval,
     * it is set to finish time of next phase i.e. sort or task finish when
     * these are set.
     * 
     * @return 0 if shuffleFinishTime, sortFinishTime and finish time are not
     *         set. else it returns approximate shuffle finish time.
     */
    ...
    /**
     * Get sort finish time for the task,. If sort finish time was not set due
     * to sort and reduce phase finishing in same heartebat interval, it is set
     * to finish time, when finish time is set.
     * 
    ...
    /**
     * Sets sortFinishTime, if shuffleFinishTime is not set before then its set
     * to sortFinishTime.
     * 
    ...
    /**
     * Get current phase of this task. Phase.Map in case of map tasks, for
     * reduce one of Phase.SHUFFLE, Phase.SORT or Phase.REDUCE.
     * 
     * @return .
     */
     ...
     * 
     * @param l
     *            the number of map output bytes
     */
     ...
     * @param counters
     */
    synchronized void statusUpdate(State runState, float progress, String state, Counters counters) {
     ...
     * 
     * This update is done in JobTracker when a cleanup attempt of task reports
    ...
    /**
     * Clear out transient information after sending out a status-update from
     * either the {@link Task} to the {@link TaskTracker} or from the
    ...
    static TaskStatus createTaskStatus(DataInput in, TaskAttemptID taskId, float progress, State runState,
            String diagnosticInfo, String stateString, String taskTracker, Phase phase, Counters counters)
            throws IOException {
        boolean isMap = in.readBoolean();
        return createTaskStatus(isMap, taskId, progress, runState, diagnosticInfo, stateString, taskTracker, phase,
                counters);
    }

    static TaskStatus createTaskStatus(boolean isMap, TaskAttemptID taskId, float progress, State runState,
            String diagnosticInfo, String stateString, String taskTracker, Phase phase, Counters counters) {
        return (isMap) ? new MapTaskStatus(taskId, progress, runState, diagnosticInfo, stateString, taskTracker, phase,
                counters) : new ReduceTaskStatus(taskId, progress, runState, diagnosticInfo, stateString, taskTracker,
                phase, counters);
    }
