...
class MapTask extends Task {
    ...
    public MapTask(String jobFile, TaskAttemptID taskId, int partition, String splitClass, BytesWritable split) {
    ...
    class TrackedRecordReader<K, V> implements RecordReader<K, V> {
        ...
        TrackedRecordReader(RecordReader<K, V> raw, Counters counters) throws IOException {
            rawIn = raw;
    ...
    class SkippingRecordReader<K, V> extends TrackedRecordReader<K, V> {
        ...
        SkippingRecordReader(RecordReader<K, V> raw, Counters counters, TaskUmbilicalProtocol umbilical)
                throws IOException {
            super(raw, counters);
    ...
    public void run(final JobConf job, final TaskUmbilicalProtocol umbilical) throws IOException {
        ...
        RecordReader in = isSkipping() ? new SkippingRecordReader(rawIn, getCounters(), umbilical)
    ...
    interface MapOutputCollector<K, V> extends OutputCollector<K, V> {
    ...
    class MapOutputBuffer<K extends Object, V extends Object> implements MapOutputCollector<K, V>, IndexedSortable {
        ...
        public MapOutputBuffer(TaskUmbilicalProtocol umbilical, JobConf job, Reporter reporter) throws IOException {
            ...
            combineCollector = (null != combinerClass) ? new CombineOutputCollector<K, V>(combineOutputCounter) : null;
                ...
                do {
                    ...
                    if (kvfull) {
                        try {
                            ...
                            throw (IOException) new IOException("Collector interrupted while waiting for the writer")
                                    .initCause(e);
                        }
                    }
                } while (kvfull);
        ...
        private void sortAndSpill() throws IOException {
            ...
            try {
                ...
                if (totalIndexCacheMemory >= INDEX_CACHE_MEMORY_LIMIT) {
                    ...
                    Path indexFilename = mapOutputFile.getSpillIndexFileForWrite(getTaskID(), numSpills, partitions
        ...
        private void combineAndSpill(RawKeyValueIterator kvIter, Counters.Counter inCounter) throws IOException {
            ...
            try {
                CombineValuesIterator<K, V> values = new CombineValuesIterator<K, V>(kvIter, comparator, keyClass,
                        valClass, job, reporter, inCounter);
                while (values.more()) {
        ...
        private void mergeParts() throws IOException {
            ...
            if (numSpills == 1) { // the spill is the final output
                ...
                if (indexCacheList.size() == 0) {
                    rfs.rename(mapOutputFile.getSpillIndexFile(getTaskID(), 0), new Path(filename[0].getParent(),
                            "file.out.index"));
                } else {
            ...
            }
            {
                for (int parts = 0; parts < partitions; parts++) {
                    ...
                    for (int i = 0; i < numSpills; i++) {
                        final IndexRecord indexRecord = getIndexInformation(mapId, i, parts);

                        long segmentOffset = indexRecord.startOffset;
                        long segmentLength = indexRecord.partLength;

                        Segment<K, V> s = new Segment<K, V>(job, rfs, filename[i], segmentOffset, segmentLength, codec,
                                true);
                        segmentList.add(i, s);

                        if (LOG.isDebugEnabled()) {
                            long rawSegmentLength = indexRecord.rawLength;
                            LOG.debug("MapId=" + mapId + " Reducer=" + parts + "Spill =" + i + "(" + segmentOffset
                    ...
                    // merge
                    @SuppressWarnings("unchecked")
                    RawKeyValueIterator kvIter = Merger.merge(job, rfs, keyClass, valClass, segmentList,
        ...
        private void writeIndexRecord(IFileOutputStream indexOut, long start, Writer<K, V> writer) throws IOException {
            ...
            // file by doing this as opposed to writing VLong but it helps us
            // later on.
        ...
        /**
         * This function returns the index information for the given mapId,
         * Spill number and reducer combination. Index Information is obtained
         ...
         * @throws IOException
         */
        private IndexRecord getIndexInformation(TaskAttemptID mapId, int spillNum, int reducer) throws IOException {
            IndexRecord[] irArray = null;
        ...
        /**
         * This function writes index information from the indexMap to the index
         * file that could be used by mergeParts
         * 
         * @param mapId
         * @param finalName
         * @throws IOException
         */
        private void writeSingleSpillIndexToFile(TaskAttemptID mapId, Path finalName) throws IOException {
