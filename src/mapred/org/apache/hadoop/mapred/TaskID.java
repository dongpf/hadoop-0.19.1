 ...
 * part is the task number. <br>
 * An example TaskID is : <code>task_200707121733_0003_m_000005</code> , which
 * represents the fifth map task in the third job running at the jobtracker
 * started at <code>200707121733</code>.
 * <p>
 * Applications should never construct or parse TaskID strings , but rather use
 * appropriate constructors or {@link #forName(String)} method.
 * 
 * @see JobID
 * @see TaskAttemptID
 */
public class TaskID extends ID {
    ...
    public String toString() {
        ...
        return builder.append(TASK).append(UNDERSCORE).append(toStringWOPrefix()).toString();
    }

    StringBuilder toStringWOPrefix() {
        StringBuilder builder = new StringBuilder();
        builder.append(jobId.toStringWOPrefix()).append(isMap ? "_m_" : "_r_");
    ...
    /**
     * Returns a regex pattern which matches task IDs. Arguments can be given
     * null, in which case that part of the regex will be generic. For example
     * to obtain a regex matching <i>the first map task</i> of <i>any
     * jobtracker</i>, of <i>any job</i>, we would use :
     * 
     * <pre>
    ...
    public static String getTaskIDsPattern(String jtIdentifier, Integer jobId, Boolean isMap, Integer taskId) {
        StringBuilder builder = new StringBuilder(TASK).append(UNDERSCORE).append(
                getTaskIDsPatternWOPrefix(jtIdentifier, jobId, isMap, taskId));
        return builder.toString();
    }

    static StringBuilder getTaskIDsPatternWOPrefix(String jtIdentifier, Integer jobId, Boolean isMap, Integer taskId) {
        StringBuilder builder = new StringBuilder();
        builder.append(JobID.getJobIDsPatternWOPrefix(jtIdentifier, jobId)).append(UNDERSCORE)
