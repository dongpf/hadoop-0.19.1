...
/**
 * A class that allows a map/red job to work on a sample of sequence files. The
 * sample is decided by the filter class set by the job.
 * 
 */
...
public class SequenceFileInputFilter<K, V> extends SequenceFileInputFormat<K, V> {
    ...
    public static class RegexFilter extends FilterBase {
        ...
        /**
         * Filtering method If key matches the regex, return true; otherwise
         * return false
         * 
    ...
    /**
     * This class returns a percentage of records The percentage is determined
     * by a filtering frequency <i>f</i> using the criteria record# % f == 0.
    ...
    public static class PercentFilter extends FilterBase {
        ...
        /**
         * Filtering method If record# % frequency==0, return true; otherwise
         * return false
         * 
         * @see org.apache.hadoop.mapred.SequenceFileInputFilter.Filter#accept(Object)
         */
    ...
    /**
     * This class returns a set of records by examing the MD5 digest of its key
     * against a filtering frequency <i>f</i>. The filtering criteria is
     * MD5(key) % f == 0.
     */
    public static class MD5Filter extends FilterBase {
        ...
        /**
         * Filtering method If MD5(key) % frequency==0, return true; otherwise
         * return false
         * 
         * @see org.apache.hadoop.mapred.SequenceFileInputFilter.Filter#accept(Object)
         */
        ...
        synchronized private long MD5Hashcode(byte[] bytes, int start, int length) throws DigestException {
    ...
    private static class FilterRecordReader<K, V> extends SequenceFileRecordReader<K, V> {
        ...
        public FilterRecordReader(Configuration conf, FileSplit split) throws IOException {
            super(conf, split);
            // instantiate filter
            filter = (Filter) ReflectionUtils.newInstance(conf.getClass(FILTER_CLASS, PercentFilter.class), conf);
        }
