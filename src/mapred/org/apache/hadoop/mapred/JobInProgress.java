...
class JobInProgress {
    ...
    public JobInProgress(JobID jobid, JobTracker jobtracker, JobConf default_conf) throws IOException {
        ...
        this.profile = new JobProfile(conf.getUser(), jobid, jobFile.toString(), url, conf.getJobName(),
                conf.getQueueName());
    ...
    public synchronized Task obtainNewMapTask(TaskTrackerStatus tts, int clusterSize, int numUniqueHosts)
        ...
        int target = findNewMapTask(tts, clusterSize, numUniqueHosts, status.mapProgress());
        if (target == -1) {
            return null;
        }
     ...
     */
    public synchronized Task obtainTaskCleanupTask(TaskTrackerStatus tts, boolean isMapSlot) throws IOException {
     ...
     *            upon restart
     */
    synchronized void addRunningTaskToTIP(TaskInProgress tip, TaskAttemptID id, TaskTrackerStatus tts,
            boolean isScheduled) {
     ...
     */
    private synchronized TaskInProgress findTaskFromList(Collection<TaskInProgress> tips, TaskTrackerStatus ttStatus,
     ...
     */
    private synchronized int findNewMapTask(TaskTrackerStatus tts, int clusterSize, int numUniqueHosts,
            double avgProgress) {
        String taskTracker = tts.getTrackerName();
        TaskInProgress tip = null;
        ...
        if (availSpace < outSize) {
            LOG.warn("No room for map task. Node " + node + " has " + availSpace
    ...
    public synchronized boolean completedTask(TaskInProgress tip, TaskStatus status, JobTrackerInstrumentation metrics) {
        ...
        String taskType = tip.isJobCleanupTask() ? Values.CLEANUP.name() : tip.isJobSetupTask() ? Values.SETUP.name()
                : tip.isMapTask() ? Values.MAP.name() : Values.REDUCE.name();
        if (status.getIsMap()) {
            JobHistory.MapAttempt.logStarted(status.getTaskID(), status.getStartTime(), status.getTaskTracker(),
                    ttStatus.getHttpPort(), taskType);
            JobHistory.MapAttempt.logFinished(status.getTaskID(), status.getFinishTime(), trackerHostname, taskType,
                    status.getStateString(), status.getCounters());
        } else {
            JobHistory.ReduceAttempt.logStarted(status.getTaskID(), status.getStartTime(), status.getTaskTracker(),
                    ttStatus.getHttpPort(), taskType);
            JobHistory.ReduceAttempt.logFinished(status.getTaskID(), status.getShuffleFinishTime(),
                    status.getSortFinishTime(), status.getFinishTime(), trackerHostname, taskType,
                    status.getStateString(), status.getCounters());
        }
    ...
    private void jobComplete(JobTrackerInstrumentation metrics) {
        ...
        if (this.status.getRunState() == JobStatus.RUNNING) {
            ...
            JobHistory.JobInfo.logFinished(this.status.getJobID(), finishTime, this.finishedMapTasks,
                    this.finishedReduceTasks, failedMapTasks, failedReduceTasks, getCounters());
    ...
    private synchronized void terminateJob(int jobTerminationState) {
        if ((status.getRunState() == JobStatus.RUNNING) || (status.getRunState() == JobStatus.PREP)) {
            if (jobTerminationState == JobStatus.FAILED) {
                ...
                this.finishTime = System.currentTimeMillis();
                JobHistory.JobInfo.logFailed(this.status.getJobID(), finishTime, this.finishedMapTasks,
                        this.finishedReduceTasks);
            } else {
                this.status = new JobStatus(status.getJobID(), 1.0f, 1.0f, 1.0f, JobStatus.KILLED,
                        status.getJobPriority());
                this.finishTime = System.currentTimeMillis();
                JobHistory.JobInfo.logKilled(this.status.getJobID(), finishTime, this.finishedMapTasks,
                        this.finishedReduceTasks);
            }
            garbageCollect();
        }
    ...
    private void failedTask(TaskInProgress tip, TaskAttemptID taskid, TaskStatus status,
        ...
        String diagInfo = taskDiagnosticInfo == null ? "" : StringUtils.arrayToString(taskDiagnosticInfo
    ...
    public void failedTask(TaskInProgress tip, TaskAttemptID taskid, String reason, TaskStatus.Phase phase,
            TaskStatus.State state, String trackerName, JobTrackerInstrumentation metrics) {
        TaskStatus status = TaskStatus.createTaskStatus(tip.isMapTask(), taskid, 0.0f, state, reason, reason,
                trackerName, phase, null);
        status.setFinishTime(System.currentTimeMillis());
        updateTaskStatus(tip, status, metrics);
        JobHistory.Task.logFailed(
                tip.getTIPId(),
                tip.isJobCleanupTask() ? Values.CLEANUP.name() : tip.isJobSetupTask() ? Values.SETUP.name() : tip
                        .isMapTask() ? Values.MAP.name() : Values.REDUCE.name(), tip.getExecFinishTime(), reason,
                taskid);
    }
    ...
    synchronized void fetchFailureNotification(TaskInProgress tip, TaskAttemptID mapTaskId, String trackerName,
        ...
        if (fetchFailures >= MAX_FETCH_FAILURES_NOTIFICATIONS && isMapFaulty) {
            ...
            failedTask(tip, mapTaskId, "Too many fetch-failures", (tip.isMapTask() ? TaskStatus.Phase.MAP
                    : TaskStatus.Phase.REDUCE), TaskStatus.State.FAILED, trackerName, metrics);
