...
public class Parser {
    ...
    public abstract static class Node implements ComposableInputFormat {
         ...
         */
        protected static void addIdentifier(String ident, Class<?>[] mcstrSig, Class<? extends Node> nodetype,
                Class<? extends ComposableRecordReader> cl) throws NoSuchMethodException {
    ...
    static class WNode extends Node {
        ...
        static void addIdentifier(String ident, Class<? extends ComposableRecordReader> cl)
        ...
        public ComposableRecordReader getRecordReader(InputSplit split, JobConf job, Reporter reporter)
                throws IOException {
            try {
    ...
    static class CNode extends Node {
        ...
        public InputSplit[] getSplits(JobConf job, int numSplits) throws IOException {
            ...
            for (int i = 0; i < kids.size(); ++i) {
                ...
                if (i > 0 && splits[i - 1].length != tmp.length) {
                    throw new IOException("Inconsistent split cardinality from child " + i + " ("
                            + splits[i - 1].length + "/" + tmp.length + ")");
                }
                splits[i] = tmp;
            }
        ...
        public ComposableRecordReader getRecordReader(InputSplit split, JobConf job, Reporter reporter)
            ...
            try {
                ...
                ret = (CompositeRecordReader) rrCstrMap.get(ident).newInstance(id, job, capacity, cmpcl);
            } catch (IllegalAccessException e) {
                throw (IOException) new IOException().initCause(e);
            } catch (InstantiationException e) {
                throw (IOException) new IOException().initCause(e);
    ...
    static Node parse(String expr, JobConf job) throws IOException {
        ...
        Class<? extends WritableComparator> cmpcl = job.getClass("mapred.join.keycomparator", null,
