...
abstract class TaskRunner extends Thread {
    ...
    /**
     * Called when this task's output is no longer needed. This method is run in
     * the parent process after the child exits. It should not execute user
     * code, only system code.
     */
    ...
    public final void run() {
        try {
            ...
            File workDir = new File(lDirAlloc.getLocalPathToRead(
                    TaskTracker.getLocalTaskDir(t.getJobID().toString(), t.getTaskID().toString(),
                            t.isTaskCleanupTask())
            ...
            if ((archives != null) || (files != null)) {
                if (archives != null) {
                    ...
                    for (int i = 0; i < archives.length; i++) {
                        ...
                        p[i] = DistributedCache.getLocalCache(archives[i], conf, new Path(baseDir), fileStatus, true,
                                Long.parseLong(archivesTimestamps[i]), new Path(workDir.getAbsolutePath()), false);
                ...
                if ((files != null)) {
                    ...
                    for (int i = 0; i < files.length; i++) {
                        ...
                        baseDir = localPath.toString().replace(cacheId, "");
                        p[i] = DistributedCache.getLocalCache(files[i], conf, new Path(baseDir), fileStatus, false,
                                Long.parseLong(fileTimestamps[i]), new Path(workDir.getAbsolutePath()), false);
                    }
            ...
            //
            // The following symbols if present in mapred.child.java.opts value
            // are
            // replaced:
            ...
            // connect with jconsole and the likes to watch child memory,
            // threads
            ...
            // 2. We also add the 'cwd' of the task to it's java.library.path to
            // help
            ...
            String pidFile = lDirAlloc.getLocalPathForWrite(
                    (TaskTracker.getPidFile(t.getJobID().toString(), taskid.toString(), t.isTaskCleanupTask())),
                    this.conf).toString();
            ...
            jvmManager.launchJvm(this,
                    jvmManager.constructJvmEnv(setup, vargs, stdout, stderr, logSize, workDir, env, pidFile, conf));
            synchronized (lock) {
                while (!done) {
                    lock.wait();
                }
    ...
    // Mostly for setting up the symlinks. Note that when we setup the
    // distributed
