...
/**
 * A checksum input stream, used for IFiles. Used to validate the checksum of
 * files created by {@link IFileOutputStream}.
 */
...
class IFileInputStream extends InputStream {
    ...
    /**
     * Read bytes from the stream. At EOF, checksum is validated, but the
     * checksum bytes are not passed back in the buffer.
     */
    ...
    /**
     * Read bytes from the stream. At EOF, checksum is validated and sent back
     * as the last four bytes of the buffer. The caller should handle these
     * bytes appropriately
     */
