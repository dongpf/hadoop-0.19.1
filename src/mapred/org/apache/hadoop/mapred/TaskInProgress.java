...
class TaskInProgress {
     ...
     */
    public TaskInProgress(JobID jobid, String jobFile, RawSplit rawSplit, JobTracker jobtracker, JobConf conf,
     ...
     */
    public TaskInProgress(JobID jobid, String jobFile, int numMaps, int partition, JobTracker jobtracker, JobConf conf,
            JobInProgress job) {
        this.jobFile = jobFile;
    ...
    public boolean shouldClose(TaskAttemptID taskid) {
        ...
        if ((ts != null)
                ...
                && ((this.failed) || ((job.getStatus().getRunState() != JobStatus.RUNNING && (job.getStatus()
                        .getRunState() != JobStatus.PREP))))) {
            tasksReportedClosed.add(taskid);
            close = true;
        } else if (isComplete() && !(isMapTask() && !jobSetup && !jobCleanup && isComplete(taskid))
                && !tasksReportedClosed.contains(taskid)) {
            tasksReportedClosed.add(taskid);
            close = true;
     ...
     * status, plus reports for all the component task-threads that have ever
     * been started.
     */
    synchronized TaskReport generateSingleReport() {
        ...
        TaskReport report = new TaskReport(getTIPId(), (float) progress, state,
                diagnostics.toArray(new String[diagnostics.size()]), execStartTime, execFinishTime, counters);
    ...
    synchronized boolean updateStatus(TaskStatus status) {
        ...
        if (oldStatus != null) {
            ...
            if ((newState != TaskStatus.State.RUNNING && newState != TaskStatus.State.COMMIT_PENDING
                    && newState != TaskStatus.State.FAILED_UNCLEAN && newState != TaskStatus.State.KILLED_UNCLEAN && newState != TaskStatus.State.UNASSIGNED)
                    && (oldState == newState)) {
                LOG.warn("Recieved duplicate status update of '" + newState + "' for '" + taskid + "' of TIP '"
                        + getTIPId() + "'");
                return false;
            }

            // The task is not allowed to move from completed back to running.
            // We have seen out of order status messagesmoving tasks from
            // complete
            ...
            if ((newState == TaskStatus.State.RUNNING || newState == TaskStatus.State.UNASSIGNED)
                    && (oldState == TaskStatus.State.FAILED || oldState == TaskStatus.State.KILLED
                            ...
                            || oldState == TaskStatus.State.KILLED_UNCLEAN || oldState == TaskStatus.State.SUCCEEDED || oldState == TaskStatus.State.COMMIT_PENDING)) {
                return false;
            }
        ...
        if (!isCleanupAttempt(taskid)) {
            ...
            taskStatuses.get(taskid).statusUpdate(status.getRunState(), status.getProgress(), status.getStateString(),
                    status.getPhase(), status.getFinishTime());
        }
    ...
    public void incompleteSubTask(TaskAttemptID taskid, JobStatus jobStatus) {
        ...
        if (status != null) {
            ...
            if (taskState != TaskStatus.State.FAILED && taskState != TaskStatus.State.KILLED
                    && taskState != TaskStatus.State.FAILED_UNCLEAN && taskState != TaskStatus.State.KILLED_UNCLEAN) {
                LOG.info("Task '" + taskid + "' running on '" + trackerName + "' in state: '" + taskState
                        + "' being failed!");
        ...
        // should note this failure only for completed maps, only if this
        // taskid;
        ...
        if (this.isMapTask() && !jobSetup && !jobCleanup && isComplete(taskid)
            ...
            // Reset the successfulTaskId since we don't have a SUCCESSFUL task
            // now
    ...
    boolean killTask(TaskAttemptID taskId, boolean shouldFail) {
        ...
        if (st != null
                && (st.getRunState() == TaskStatus.State.RUNNING || st.getRunState() == TaskStatus.State.COMMIT_PENDING
    ...
    /**
     * This method is called whenever there's a status change for one of the
     * TIP's sub-tasks. It recomputes the overall progress for the TIP. We
     * examine all sub-tasks and find the one that's most advanced (and
     * non-failed).
     */
        ...
        if (isComplete()) {
            ...
            for (Iterator<TaskAttemptID> it = taskStatuses.keySet().iterator(); it.hasNext();) {
                ...
                if (status.getRunState() == TaskStatus.State.SUCCEEDED) {
                    ...
                    // for COMMIT_PENDING, we take the last state that we
                    // recorded
    ...
    boolean hasSpeculativeTask(long currentTime, double averageProgress) {
        ...
        if (!skipping && activeTasks.size() <= MAX_TASK_EXECS && (averageProgress - progress >= SPECULATIVE_GAP)
                && (currentTime - startTime >= SPECULATIVE_LAG) && completes == 0 && !isOnlyCommitPending()) {
            return true;
        }
        return false;
    }
    ...
    public Task getTaskToRun(String taskTracker) throws IOException {
        ...
        if (nextTaskId < (MAX_TASK_EXECS + maxTaskAttempts + numKilledTasks)) {
            ...
            LOG.warn("Exceeded limit of " + (MAX_TASK_EXECS + maxTaskAttempts) + " (plus " + numKilledTasks
                    + " killed)" + " attempts for the tip '" + getTIPId() + "'");
            return null;
        }
    ...
    public Task addRunningTask(TaskAttemptID taskid, String taskTracker, boolean taskCleanup) {
        ...
        Task t = null;
        if (isMapTask()) {
            LOG.debug("attemdpt " + numTaskFailures + " sending skippedRecords " + failedRanges.getIndicesCount());
            t = new MapTask(jobFile, taskid, partition, rawSplit.getClassName(), rawSplit.getBytes());
        } else {
     ...
     * based on failed records from all the previous attempts. It also narrow
     * down the skip records if it is more than the acceptable value by dividing
     * the failed range into half. In this case one half is executed in the next
     * attempt (test attempt). In the test attempt, only the test range gets
     * executed, others get skipped. Based on the success/failure of the test
     * attempt, the range is divided further.
     */
    private class FailedRanges {
        ...
        synchronized void add(Range failedRange) {
            ...
            if (divide != null) {
                LOG.warn("FailedRange:" + failedRange + "  test:" + divide.test + "  pass:" + divide.testPassed);
