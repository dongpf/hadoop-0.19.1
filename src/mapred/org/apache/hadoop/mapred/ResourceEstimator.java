...
class ResourceEstimator {
    ...
    /**
     * How much relative weight to put on the current estimate. Each completed
     * map has unit weight.
     */
    ...
    void updateWithCompletedTask(TaskStatus ts, TaskInProgress tip) {
        ...
        if (tip.isMapTask() && ts.getOutputSize() != -1) {
            double blowupOnThisTask = ts.getOutputSize() / ((double) tip.getMapInputSize() + 1);

            LOG.info("measured blowup on " + tip.getTIPId() + " was " + ts.getOutputSize() + "/"
                    + (tip.getMapInputSize() + 1) + " = " + blowupOnThisTask);

            double newEstimate;
            synchronized (this) {
                newEstimate = blowupOnThisTask / estimateWeight + ((estimateWeight - 1) / estimateWeight)
                        * getBlowupRatio();
                estimateWeight++;
            }
    ...
    protected long getEstimatedTotalMapOutputSize() {
        ...
        if (estWeight < threshholdToUse) {
            ...
            LOG.debug("estimate total map output will be " + estimate + " bytes. (blowup = 2*" + blowup + ")");
            return estimate;
        }
    }
