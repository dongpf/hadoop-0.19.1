 ...
 * 
 * <p>
 * The number of <code>Reducer</code>s for the job is set by the user via
 ...
 * 
 * <p>
 * <code>Reducer</code> is input the grouped output of a {@link Mapper}. In the
 * phase the framework, for each <code>Reducer</code>, fetches the relevant
 * partition of the output of all the <code>Mapper</code>s, via HTTP.
 * </p>
 * </li>
 * 
 * <li>
 ...
 * 
 * <p>
 * The framework groups <code>Reducer</code> inputs by <code>key</code>s (since
 * different <code>Mapper</code>s may have output the same key) in this stage.
 * </p>
 * 
 * <p>
 ...
 * from those for grouping keys before reduction, then one may specify a
 * <code>Comparator</code> via
 * {@link JobConf#setOutputValueGroupingComparator(Class)}.Since
 * {@link JobConf#setOutputKeyComparatorClass(Class)} can be used to control how
 ...
 * <p>
 * In this phase the
 * {@link #reduce(Object, Iterator, OutputCollector, Reporter)} method is called
 * for each <code>&lt;key, (list of values)></code> pair in the grouped inputs.
 * </p>
 * <p>
 * The output of the reduce task is typically written to the {@link FileSystem}
 * via {@link OutputCollector#collect(Object, Object)}.
 * </p>
 * </li>
 * </ol>
 * 
 ...
 * 
 * <pre>
 * public class MyReducer&lt;K extends WritableComparable, V extends Writable&gt; extends MapReduceBase implements
 *         Reducer&lt;K, V, K, V&gt; {
 * 
...
public interface Reducer<K2, V2, K3, V3> extends JobConfigurable, Closeable {
   ...
   * <a href="{@docRoot} provided to report progress or
     * just indicate that they are alive. In scenarios where the application
     * takes an insignificant amount of time to process individual key/value
     * pairs, this is crucial since the framework might assume that the task has
     * timed-out and kill that task. The other way of avoiding this is to set <a
     * href="{@docRoot}/../hadoop-default.html#mapred.task.timeout">
