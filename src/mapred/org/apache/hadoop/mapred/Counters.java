 ...
 * 
 * <p>
 * <code>Counters</code> represent global counters, defined either by the
 * Map-Reduce framework or applications. Each <code>Counter</code> can be of any
 * {@link Enum} type.
 * </p>
 * 
 * <p>
 * <code>Counters</code> are bunched into {@link Group}s, each comprising of
...
public class Counters implements Writable, Iterable<Counters.Group> {
    ...
    private static char[] charsToEscape = { GROUP_OPEN, GROUP_CLOSE, COUNTER_OPEN, COUNTER_CLOSE, UNIT_OPEN, UNIT_CLOSE };
    ...
    public static class Counter implements Writable {
        ...
        synchronized boolean contentEquals(Counter c) {
            return name.equals(c.getName()) && displayName.equals(c.getDisplayName()) && value == c.getCounter();
        }
    ...
    public static class Group implements Writable, Iterable<Counter> {
         ...
         * {(actual-name)(display-name)(value)[][][]} where [] are compact
         * strings for the counters within.
         */
        ...
        /**
         * Looks up key in the ResourceBundle and returns the corresponding
         * value. If the bundle or the key doesn't exist, returns the default
         * value.
         */
    ...
    /**
     * A cache from enum values to the associated counter. Dramatically speeds
     * up typical usage.
     */
