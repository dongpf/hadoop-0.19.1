...
public class TaskCompletionEvent implements Writable {
    ...
    /**
     * Constructor. eventId should be created externally and incremented per
     * event for each job.
     * 
     * @param eventId
     ...
     *            task tracker's host:port for http.
     */
     ...
     *            time (in millisec) the task took to complete
     */
    ...
    public boolean equals(Object o) {
        ...
        if (o.getClass().equals(TaskCompletionEvent.class)) {
            ...
            return this.isMap == event.isMapTask() && this.eventId == event.getEventId()
                    && this.idWithinJob == event.idWithinJob() && this.status.equals(event.getTaskStatus())
                    && this.taskId.equals(event.getTaskAttemptId()) && this.taskRunTime == event.getTaskRunTime()
