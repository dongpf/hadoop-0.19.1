...
/**
 * A {@link TaskScheduler} that limits the maximum number of tasks running for a
 * job. The limit is set by means of the
 * <code>mapred.jobtracker.scheduler.maxRunningTasksPerJob</code> property.
 */
class LimitTasksPerJobTaskScheduler extends JobQueueTaskScheduler {
    ...
    public synchronized void start() throws IOException {
        ...
        queueManager.setSchedulerInfo(queueName, "Maximum Tasks Per Job :: " + String.valueOf(maxTasksPerJob));
    }
    ...
    public synchronized void setConf(Configuration conf) {
        ...
        if (maxTasksPerJob <= 0) {
            String msg = MAX_TASKS_PER_JOB_PROPERTY + " is set to zero or a negative value. Aborting.";
            LOG.fatal(msg);
    ...
    public synchronized List<Task> assignTasks(TaskTrackerStatus taskTracker) throws IOException {
         ...
         * maxTasksPerJob When step == 1, this loop starts as many reduce tasks
         * it can wrt maxTasksPerJob When step == 2, this loop starts as many
         * map tasks it can When step == 3, this loop starts as many reduce
         * tasks it can
         * 
         * It may seem that we would improve this loop by queuing jobs we cannot
         * start in steps 0 and 1 because of maxTasksPerJob, and using that
         * queue in step 2 and 3. A first thing to notice is that the time with
         * the current algorithm is logarithmic, because it is the sum of (p^k)
         * for k from 1 to N, were N is the number of jobs and p is the
         * probability for a job to not exceed limits The probability for the
         * cache to be useful would be similar to p^N, that is 1/(e^N), whereas
         * its size and the time spent to manage it would be in ln(N). So it is
         * not a good idea.
         */
        ...
        for (int step = beginAtStep; step <= 3; ++step) {
            ...
            if ((step == 0 || step == 2) && mapTasksNumber >= maximumMapLoad || (step == 1 || step == 3)
                    && reduceTasksNumber >= maximumReduceLoad) {
                continue;
            }
    ...
    /**
     * Determine the maximum number of maps or reduces that we are willing to
     * run on a taskTracker which accept a maximum of localMaxMapLoad maps and
     * localMaxReduceLoad reduces
     * 
     * @param localMaxMapLoad
     *            The local maximum number of map tasks for a host
     * @param localMaxReduceLoad
     *            The local maximum number of reduce tasks for a host
