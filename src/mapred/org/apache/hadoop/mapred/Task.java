...
abstract class Task implements Writable, Configurable {
    ...
    protected static enum Counter {
        MAP_INPUT_RECORDS, MAP_OUTPUT_RECORDS, MAP_SKIPPED_RECORDS, MAP_INPUT_BYTES, MAP_OUTPUT_BYTES, COMBINE_INPUT_RECORDS, COMBINE_OUTPUT_RECORDS, REDUCE_INPUT_GROUPS, REDUCE_INPUT_RECORDS, REDUCE_OUTPUT_RECORDS, REDUCE_SKIPPED_GROUPS, REDUCE_SKIPPED_RECORDS
    }
    ...
    protected static enum FileSystemCounter {
        LOCAL_READ, LOCAL_WRITE, HDFS_READ, HDFS_WRITE, S3_READ, S3_WRITE, KFS_READ, KFSWRITE
    }
    ...
    public Task(String jobFile, TaskAttemptID taskId, int partition) {
        ...
        this.taskStatus = TaskStatus.createTaskStatus(isMapTask(), this.taskId, 0.0f, TaskStatus.State.UNASSIGNED, "",
                ...
                while (!taskDone.get()) {
                    try {
                        ...
                        if (sendProgress) {
                            ...
                            updateCounters();
                            taskStatus.statusUpdate(getState(), taskProgress.get(), taskProgress.toString(), counters);
    ...
    public void initialize(JobConf job, Reporter reporter) throws IOException {
        ...
        return new Reporter() {
            ...
            public void incrCounter(String group, String counter, long amount) {
                ...
                if (skipping
                        ...
                        && (SkipBadRecords.COUNTER_MAP_PROCESSED_RECORDS.equals(counter) || SkipBadRecords.COUNTER_REDUCE_PROCESSED_GROUPS
                                .equals(counter))) {
    ...
    {
        statisticUpdaters.add(new FileSystemStatisticUpdater(FileSystemCounter.LOCAL_READ,
                FileSystemCounter.LOCAL_WRITE, RawLocalFileSystem.class));
        statisticUpdaters.add(new FileSystemStatisticUpdater(FileSystemCounter.HDFS_READ, FileSystemCounter.HDFS_WRITE,
    ...
    private void commit(TaskUmbilicalProtocol umbilical, OutputCommitter committer) throws IOException {
        int retries = MAX_RETRIES;
        while (true) {
            try {
                ...
                if (--retries == 0) {
                    // if it couldn't commit a successfully then delete the
                    // output
                    discardOutput(taskContext, committer);
                    System.exit(68);
                }
    ...
    private void discardOutput(TaskAttemptContext taskContext, OutputCommitter committer) {
        try {
    ...
    protected void runTaskCleanupTask(TaskUmbilicalProtocol umbilical) throws IOException {
    ...
    protected void runJobCleanupTask(TaskUmbilicalProtocol umbilical) throws IOException {
        // set phase for this task
        setPhase(TaskStatus.Phase.CLEANUP);
        getProgress().setStatus("cleanup");
        // do the cleanup
    ...
    protected void runJobSetupTask(TaskUmbilicalProtocol umbilical) throws IOException {
        // do the setup
     ...
     */
    protected static class CombineOutputCollector<K extends Object, V extends Object> implements OutputCollector<K, V> {
    ...
    static class ValuesIterator<KEY, VALUE> implements Iterator<VALUE> {
        ...
        public ValuesIterator(RawKeyValueIterator in, RawComparator<KEY> comparator, Class<KEY> keyClass,
    ...
    protected static class CombineValuesIterator<KEY, VALUE> extends ValuesIterator<KEY, VALUE> {

        private final Counters.Counter combineInputCounter;

        public CombineValuesIterator(RawKeyValueIterator in, RawComparator<KEY> comparator, Class<KEY> keyClass,
                Class<VALUE> valClass, Configuration conf, Reporter reporter, Counters.Counter combineInputCounter)
                throws IOException {
