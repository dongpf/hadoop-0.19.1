...
public class TaskTracker implements MRConstants, TaskUmbilicalProtocol, Runnable {
    ...
    private long maxVirtualMemoryForTasks = JobConf.DISABLED_VIRTUAL_MEMORY_LIMIT;
    ...
    synchronized void initialize() throws IOException {
        ...
        this.jobClient = (InterTrackerProtocol) RPC.waitForProxy(InterTrackerProtocol.class,
        ...
        maxVirtualMemoryForTasks = fConf.getLong("mapred.tasktracker.tasks.maxmemory",
    ...
    public static Class<? extends TaskTrackerInstrumentation> getInstrumentationClass(Configuration conf) {
        return conf.getClass("mapred.tasktracker.instrumentation", TaskTrackerMetricsInst.class,
                ...
                try {
                    ...
                    for (FetchStatus f : fList) {
                        ...
                        try {
                            ...
                            LOG.warn("Ignoring exception that fetch for map completion" + " events threw for "
                                    + f.jobId + " threw: " + StringUtils.stringifyException(e));
                        }
                        if (!running) {
                            break;
                        }
    ...
    private class FetchStatus {
        ...
        /**
         * Check if the number of events that are obtained are more than
         * required. If yes then purge the extra ones.
         */
    ...
    private void localizeJob(TaskInProgress tip) throws IOException {
        ...
        Path localJobFile = lDirAlloc.getLocalPathForWrite(getLocalJobDir(jobId.toString()) + Path.SEPARATOR
                + "job.xml", jobFileSize, fConf);
        ...
        synchronized (rjob) {
            if (!rjob.localized) {
                ...
                Path workDir = lDirAlloc.getLocalPathForWrite(
                        (getLocalJobDir(jobId.toString()) + Path.SEPARATOR + "work"), fConf);
                ...
                if (jarFile != null) {
                    ...
                    // directory
                    localJarFile = new Path(lDirAlloc.getLocalPathForWrite(getLocalJobDir(jobId.toString())
                            + Path.SEPARATOR + "jars", 5 * jarFileSize, fConf), "job.jar");
     ...
     * @throws IOException
     */
    private List<TaskCompletionEvent> queryJobTracker(IntWritable fromEventId, JobID jobId,
            InterTrackerProtocol jobClient) throws IOException {
            ...
            try {
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Got heartbeatResponse from JobTracker with responseId: "
                            + heartbeatResponse.getResponseId() + " and " + ((actions != null) ? actions.length : 0)
                            + " actions");
                }
    ...
    private HeartbeatResponse transmitHeartBeat(long now) throws IOException {
        ...
        if (status == null) {
            synchronized (this) {
                status = new TaskTrackerStatus(taskTrackerName, localHostname, httpPort,
                        cloneAndResetRunningTaskStatuses(sendCounters), failures, maxCurrentMapTasks,
                        maxCurrentReduceTasks);
            }
        } else {
            LOG.info("Resending 'status' to '" + jobTrackAddr.getHostName() + "' with reponseId '"
                    + heartbeatResponseId);
        }
        ...
        synchronized (this) {
            askForNewTask = (status.countMapTasks() < maxCurrentMapTasks || status.countReduceTasks() < maxCurrentReduceTasks)
                    && acceptNewTasks;
        ...
        // Xmit the heartbeat
        //
        HeartbeatResponse heartbeatResponse = jobClient.heartbeat(status, justStarted, askForNewTask,
                heartbeatResponseId);
    ...
    /**
     * Return the maximum amount of memory available for all tasks on this
     * tracker
     * 
    ...
    /**
     * Find the minimum amount of virtual memory that would be available for a
     * new task.
     * 
     * The minimum amount of virtual memory is computed by looking at the
     * maximum amount of virtual memory that is allowed for all tasks in the
     * system, as per mapred.tasktracker.tasks.maxmemory, and the total amount
     * of maximum virtual memory that can be used by all currently running
     * tasks.
     * 
     * @return amount of free virtual memory that can be assured for new tasks
     */
     ...
     * returned. Else, the default memory that would be assigned for the task is
     * returned.
     * 
     * @param conf
    ...
    long getMemoryForTask(JobConf conf) {
        ...
        if (memForTask == JobConf.DISABLED_VIRTUAL_MEMORY_LIMIT) {
            memForTask = fConf.getLong("mapred.task.default.maxmemory", 512 * 1024 * 1024L);
        }
        return memForTask;
    }
        ...
        for (TaskInProgress tip : runningTasks.values()) {
            if (tip.getRunState() == TaskStatus.State.RUNNING || tip.getRunState() == TaskStatus.State.COMMIT_PENDING
                ...
                if (timeSinceLastReport > jobTaskTimeout && !tip.wasKilled) {
                    String msg = "Task " + tip.getTask().getTaskID() + " failed to report status for "
        ...
        if (rjob == null) {
            ...
            synchronized (rjob) {
                ...
                if (!rjob.keepJobFiles) {
                    directoryCleanupThread.addToQueue(getLocalFiles(fConf, getLocalJobDir(rjob.getJobID().toString())));
                }
                // Remove this job
                rjob.tasks.clear();
            }
            ...
            if ((tip.getRunState() == TaskStatus.State.RUNNING || tip.getRunState() == TaskStatus.State.COMMIT_PENDING)
                ...
                if (killMe == null) {
                    ...
                    if (killMe.getTask().isMapTask()
                            || (tip.getTask().getProgress().get() < killMe.getTask().getProgress().get())) {
    ...
    void addToMemoryManager(TaskAttemptID attemptId, JobConf conf, String pidFile) {
        if (isTaskMemoryManagerEnabled()) {
            taskMemoryManager.addTask(attemptId, getMemoryForTask(conf), pidFile);
        }
    }
     ...
     */
        ...
        public TaskInProgress(Task task, JobConf conf, TaskLauncher launcher) {
            ...
            localJobConf = null;
            taskStatus = TaskStatus.createTaskStatus(task.isMapTask(), task.getTaskID(), 0.0f, task.getState(),
                    diagnosticInfo.toString(), "initializing", getName(),
                    task.isTaskCleanupTask() ? TaskStatus.Phase.CLEANUP : task.isMapTask() ? TaskStatus.Phase.MAP
        ...
        private void localizeTask(Task task) throws IOException {
            ...
            Path localTaskDir = lDirAlloc.getLocalPathForWrite(
                    TaskTracker.getLocalTaskDir(task.getJobID().toString(), task.getTaskID().toString(),
                            task.isTaskCleanupTask()), defaultJobConf);
            ...
            String workDir = lDirAlloc.getLocalPathToRead(
                    TaskTracker.getLocalJobDir(task.getJobID().toString()) + Path.SEPARATOR + "work", defaultJobConf)
                    .toString();
            ...
            Path cwd = lDirAlloc.getLocalPathForWrite(
                    getLocalTaskDir(task.getJobID().toString(), task.getTaskID().toString(), task.isTaskCleanupTask())
                            + Path.SEPARATOR + MRConstants.WORKDIR, defaultJobConf);
     ...
     */
        ...
        public synchronized void launchTask() throws IOException {
            if (this.taskStatus.getRunState() == TaskStatus.State.UNASSIGNED
                ...
                LOG.info("Not launching task: " + task.getTaskID() + " since it's state is "
        ...
        public synchronized void reportProgress(TaskStatus taskStatus) {
            LOG.info(task.getTaskID() + " " + taskStatus.getProgress() + "% " + taskStatus.getStateString());
     ...
     */
            ...
            synchronized (this) {
                if (!done) {
                    if (!wasKilled) {
                         ...
                         * FAILED_UNCLEAN/FAILED FAILED_UNCLEAN -> FAILED
                        ...
                        if (debugCommand != null) {
                            ...
                            try {
                                ...
                                taskStdout = FileUtil.makeShellPath(TaskLog.getRealTaskLogFileLocation(
                                        task.getTaskID(), TaskLog.LogName.STDOUT));
                                // get task's stderr file
                                taskStderr = FileUtil.makeShellPath(TaskLog.getRealTaskLogFileLocation(
                                        task.getTaskID(), TaskLog.LogName.STDERR));
                                // get task's syslog file
                                taskSyslog = FileUtil.makeShellPath(TaskLog.getRealTaskLogFileLocation(
                            ...
                            try {
                                workDir = new File(lDirAlloc.getLocalPathToRead(
                                        TaskTracker.getLocalTaskDir(task.getJobID().toString(), task.getTaskID()
                                ...
                                LOG.warn("Working Directory of the task " + task.getTaskID()
                                        + "doesnt exist. Caught exception " + StringUtils.stringifyException(e));
                            }
                            // Build the command
                            File stdout = TaskLog
                                    .getRealTaskLogFileLocation(task.getTaskID(), TaskLog.LogName.DEBUGOUT);
                ...
                needCleanup = (taskStatus.getRunState() == TaskStatus.State.FAILED
                        ...
                        || taskStatus.getRunState() == TaskStatus.State.KILLED_UNCLEAN || taskStatus.getRunState() == TaskStatus.State.KILLED);
            }
        ...
        public void runScript(List<String> args, File dir) throws IOException {
            ShellCommandExecutor shexec = new ShellCommandExecutor(args.toArray(new String[0]), dir);
            shexec.execute();
        ...
        public void jobHasFinished(boolean wasFailure) throws IOException {
            ...
            synchronized (this) {
                if (getRunState() == TaskStatus.State.RUNNING || getRunState() == TaskStatus.State.UNASSIGNED
                        || getRunState() == TaskStatus.State.COMMIT_PENDING || isCleaningup()) {
                    kill(wasFailure);
                }
            }
        ...
        public synchronized void kill(boolean wasFailure) throws IOException {
             ...
             * FAILED_UNCLEAN/KILLED_UNCLEAN/FAILED/KILLED COMMIT_PENDING ->
             * FAILED_UNCLEAN/KILLED_UNCLEAN FAILED_UNCLEAN -> FAILED
             * KILLED_UNCLEAN -> KILLED UNASSIGNED -> FAILED/KILLED
             */
            if (taskStatus.getRunState() == TaskStatus.State.RUNNING
                    || taskStatus.getRunState() == TaskStatus.State.COMMIT_PENDING || isCleaningup()) {
                wasKilled = true;
                if (wasFailure) {
                    failures += 1;
                }
                ...
                if (task.isMapOrReduce()) {
                    taskStatus.setRunState((wasFailure) ? TaskStatus.State.FAILED_UNCLEAN
                            : TaskStatus.State.KILLED_UNCLEAN);
                } else {
                    // go FAILED_UNCLEAN -> FAILED and KILLED_UNCLEAN -> KILLED
                    // always
                    if (taskStatus.getRunState() == TaskStatus.State.FAILED_UNCLEAN) {
                        taskStatus.setRunState(TaskStatus.State.FAILED);
                    } else if (taskStatus.getRunState() == TaskStatus.State.KILLED_UNCLEAN) {
        ...
        void cleanup(boolean needCleanup) throws IOException {
            ...
            synchronized (this) {
                try {
                    String taskDir = getLocalTaskDir(task.getJobID().toString(), taskId.toString(),
                    ...
                    if (needCleanup) {
                        ...
                        if (localJobConf.getNumTasksToExecutePerJvm() == 1) {
                            directoryCleanupThread.addToQueue(getLocalFiles(defaultJobConf, taskDir));
                        }

                        else {
                            directoryCleanupThread.addToQueue(getLocalFiles(defaultJobConf, taskDir + "/job.xml"));
                        }
                    } else {
                        if (localJobConf.getNumTasksToExecutePerJvm() == 1) {
                            directoryCleanupThread.addToQueue(getLocalFiles(defaultJobConf, taskDir + "/work"));
                        }
                    }
                } catch (Throwable ie) {
        ...
        public boolean equals(Object obj) {
            return (obj instanceof TaskInProgress)
                    && task.getTaskID().equals(((TaskInProgress) obj).getTask().getTaskID());
        }
    ...
    public synchronized JvmTask getTask(JVMId jvmId) throws IOException {
        ...
        if (tasks.get(tip.getTask().getTaskID()) != null) { // is task still
            ...
            LOG.info("Killing JVM with ID: " + jvmId + " since scheduled task: " + tip.getTask().getTaskID() + " is "
    ...
    public synchronized MapTaskCompletionEventsUpdate getMapCompletionEvents(JobID jobId, int fromEventId, int maxLocs,
    ...
    public static void main(String argv[]) throws Exception {
        ...
        try {
            ...
            LOG.error("Can not start task tracker because " + StringUtils.stringifyException(e));
            System.exit(-1);
        }
    }
    ...
    public static class MapOutputServlet extends HttpServlet {
        ...
        public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException,
            ...
            try {
                ...
                // Index file
                Path indexFileName = lDirAlloc.getLocalPathToRead(TaskTracker.getIntermediateOutputDir(jobId, mapId)
                        + "/file.out.index", conf);

                // Map-output file
                Path mapOutputFileName = lDirAlloc.getLocalPathToRead(
                        TaskTracker.getIntermediateOutputDir(jobId, mapId) + "/file.out", conf);
                ...
                // The clever trick here to reduce the impact of the extra seek
                // for
                // logging the first key/value lengths is to read the lengths
                // before
                // the second seek for the actual shuffle. The second seek is
                // almost
                // a no-op since it is very short (go back length of two VInts)
                // and the
                ...
                int len = checksumInputStream.readWithChecksum(buffer, 0,
                        partLength < MAX_BYTES_TO_READ ? (int) partLength : MAX_BYTES_TO_READ);
                while (len > 0) {
                    try {
                    ...
                    len = checksumInputStream.readWithChecksum(buffer, 0,
                            (partLength - totalRead) < MAX_BYTES_TO_READ ? (int) (partLength - totalRead)
                                    : MAX_BYTES_TO_READ);
                }

                LOG.info("Sent out " + totalRead + " bytes for reduce: " + reduce + " from map: " + mapId + " given "
                        + partLength + "/" + rawPartLength + " from " + startOffset + " with (" + firstKeyLength + ", "
                        + firstValueLength + ")");
            } catch (IOException ie) {
                ...
                if (ClientTraceLog.isInfoEnabled()) {
                    ClientTraceLog.info(String.format(MR_CLIENTTRACE_FORMAT,
                            request.getLocalAddr() + ":" + request.getLocalPort(), request.getRemoteAddr() + ":"
                                    + request.getRemotePort(), totalRead, "MAPRED_SHUFFLE", mapId));
                }
            }
            outStream.close();
    ...
    private void setTaskMemoryManagerEnabledFlag() {
        ...
        if (tasksMaxMem == JobConf.DISABLED_VIRTUAL_MEMORY_LIMIT) {
            LOG.info("TaskTracker's tasksMaxMem is not set. TaskMemoryManager is " + "disabled.");
            taskMemoryManagerEnabled = false;
            return;
        }
     ...
     * @param diagnosticMsg
     */
    synchronized void cleanUpOverMemoryTask(TaskAttemptID tid, String diagnosticMsg) {
