 ...
 *       (using the JobConf.setMapRunnerClass method) and the number of thread
 *       the thread-pool can use with the
 *       <code>mapred.map.multithreadedrunner.threads</code> property, its
...
public class MultithreadedMapRunner<K1, V1, K2, V2> implements MapRunnable<K1, V1, K2, V2> {
    ...
    public void configure(JobConf jobConf) {
        ...
        executorService = new ThreadPoolExecutor(numberOfThreads, numberOfThreads, 0L, TimeUnit.MILLISECONDS,
                new BlockingArrayQueue(numberOfThreads));
    }

    /**
     * A blocking array queue that replaces offer and add, which throws on a
     * full queue, to a put, which waits on a full queue.
     */
    ...
    public void run(RecordReader<K1, V1> input, OutputCollector<K2, V2> output, Reporter reporter) throws IOException {
        try {
            ...
            while (input.next(key, value)) {
                ...
                // Allocate new key & value instances as mapper is running in
                // parallel
                key = input.createKey();
            ...
            try {
                ...
                while (!executorService.awaitTermination(100, TimeUnit.MILLISECONDS)) {
                    ...
                    // NOTE: while Mapper.map dispatching has concluded there
                    // are still
                ...
                // NOTE: it could be that a map call has had an exception after
                // the
                // call for awaitTermination() returing true. And edge case but
                // it
                // could happen.
                checkForExceptionsFromProcessingThreads();

            } catch (IOException ioEx) {
                // Forcing a shutdown of all thread of the threadpool and
                // rethrowing
                // the IOException
    ...
    private class MapperInvokeRunable implements Runnable {
        ...
        public void run() {
            try {
                ...
                // If there is an IOException during the call it is set in an
                // instance
                ...
                // If there is a RuntimeException during the call it is set in
                // an
                // instance variable of the MultithreadedMapRunner from where it
                // will be
                // rethrown.
                synchronized (MultithreadedMapRunner.this) {
