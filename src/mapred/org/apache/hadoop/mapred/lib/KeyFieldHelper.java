 ...
 * {@link KeyFieldBasedPartitioner}. Defines all the methods for parsing key
 * specifications. The key specification is of the form: -k pos1[,pos2], where
 * pos is of the form f[.c][opts], where f is the number of the field to use,
 * and c is the number of the first character from the beginning of the field.
 * Fields and character posns are numbered starting with 1; a character position
 * of zero in pos2 indicates the field's last character. If '.c' is omitted from
 * pos1, it defaults to 1 (the beginning of the field); if omitted from pos2, it
 * defaults to 0 (the end of the field). opts are ordering options (supported
 * options are 'nr').
 */
...
class KeyFieldHelper {
    ...
    public int getEndOffset(byte[] b, int start, int end, int[] lengthIndices, KeyDescription k) {
        ...
        // note that the [0]'th element is the number of fields in the key
        if (k.endFieldIdx == 0) {
            // there is no end field specified for this keyspec. So the
            // remaining
                    ...
                    do {
                        if (token.equals("n")) {
                            ...
                            throw new IllegalArgumentException("Invalid -k argument. "
                                    + "Must be of the form -k pos1,[pos2], where pos is of the form " + "f[.c]nr");
                        }
                        if (st.hasMoreTokens()) {
                            token = st.nextToken();
                        } else {
