...
/**
 * This comparator implementation provides a subset of the features provided by
 * the Unix/GNU Sort. In particular, the supported features are: -n, (Sort
 * numerically) -r, (Reverse the result of comparison) -k pos1[,pos2], where pos
 * is of the form f[.c][opts], where f is the number of the field to use, and c
 * is the number of the first character from the beginning of the field. Fields
 * and character posns are numbered starting with 1; a character position of
 * zero in pos2 indicates the field's last character. If '.c' is omitted from
 * pos1, it defaults to 1 (the beginning of the field); if omitted from pos2, it
 * defaults to 0 (the end of the field). opts are ordering options (any of 'nr'
 * as described above). We assume that the fields in the key are separated by
...
public class KeyFieldBasedComparator<K, V> extends WritableComparator implements JobConfigurable {
    ...
    public int compare(byte[] b1, int s1, int l1, byte[] b2, int s2, int l2) {
        ...
        for (KeyDescription keySpec : allKeySpecs) {
            ...
            int result;
            if ((result = compareByteSequence(b1, startCharFirst, endCharFirst, b2, startCharSecond, endCharSecond,
                    keySpec)) != 0) {
                return result;
            }
        }
    ...
    private int compareByteSequence(byte[] first, int start1, int end1, byte[] second, int start2, int end2,
            KeyDescription key) {
        if (start1 == -1) {
            if (key.reverse) {
                return 1;
    ...
    private int numericalCompare(byte[] a, int start1, int end1, byte[] b, int start2, int end2) {
        ...
        if (first_a == NEGATIVE) {
            if (first_b != NEGATIVE) {
                // check for cases like -0.0 and 0.0 (they should be declared
                // equal)
        ...
        while (i <= end1) {
            // if we encounter a non-digit treat the corresponding number as
            // being
            // smaller
            if (isdigit(a[i++])) {
                numRemainDigits_a++;
