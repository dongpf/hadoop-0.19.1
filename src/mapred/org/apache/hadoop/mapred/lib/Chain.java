...
class Chain {
     ...
     * <p/>
     * It creates a new JobConf using the chain job's JobConf as base and adds
     * to it the configuration properties for the chain element. The keys of the
     * chain element jobConf have precedence over the given JobConf.
     * 
     * @param jobConf
     *            the chain job's JobConf.
     * @param confKey
     *            the key for chain element configuration serialized in the
     *            chain job's JobConf.
    ...
    private static JobConf getChainElementConf(JobConf jobConf, String confKey) {
        ...
        // values set in the conf making not possible do do a new
        // JobConf(jobConf)
     ...
     * @param byValue
     *            indicates if key/values should be passed by value to the next
     *            Mapper in the chain, if any.
     * @param mapperConf
     *            a JobConf with the configuration for the Mapper class. It is
     *            recommended to use a JobConf without default values using the
     *            <code>JobConf(boolean loadDefaults)</code> constructor with
     *            FALSE.
     */
    public static <K1, V1, K2, V2> void addMapper(boolean isMap, JobConf jobConf,
            Class<? extends Mapper<K1, V1, K2, V2>> klass, Class<? extends K1> inputKeyClass,
            Class<? extends V1> inputValueClass, Class<? extends K2> outputKeyClass,
            Class<? extends V2> outputValueClass, boolean byValue, JobConf mapperConf) {
        ...
        if (!isMap) {
            if (jobConf.getClass(prefix + CHAIN_REDUCER_CLASS, Reducer.class) == null) {
                throw new IllegalStateException("A Mapper can be added to the chain only after the Reducer has "
                        + "been set");
            }
        }
        ...
        // if it is a reducer chain and the first Mapper is being added check
        // the
        ...
        if (!isMap && index == 0) {
            ...
            // check the that the new Mapper in the chain key and value input
            // classes
            // match those of the previous Mapper output.
            JobConf previousMapperConf = getChainElementConf(jobConf, prefix + CHAIN_MAPPER_CONFIG + (index - 1));
        ...
        // store in the private mapper conf the input/output classes of the
        // mapper
     ...
     *            Mapper in the chain, if any.
     * @param reducerConf
     *            a JobConf with the configuration for the Reducer class. It is
     *            recommended to use a JobConf without default values using the
     *            <code>JobConf(boolean loadDefaults)</code> constructor with
     *            FALSE.
     */
    public static <K1, V1, K2, V2> void setReducer(JobConf jobConf, Class<? extends Reducer<K1, V1, K2, V2>> klass,
            Class<? extends K1> inputKeyClass, Class<? extends V1> inputValueClass, Class<? extends K2> outputKeyClass,
            Class<? extends V2> outputValueClass, boolean byValue, JobConf reducerConf) {
        ...
        // store in the private reducer conf the input/output classes of the
        // reducer
        // and if it works by value or by reference
    ...
    public void configure(JobConf jobConf) {
        ...
        for (int i = 0; i < index; i++) {
            ...
            if (mConf.getBoolean(MAPPER_BY_VALUE, true)) {
                mappersKeySerialization.add(serializationFactory.getSerialization(mConf.getClass(
                        MAPPER_OUTPUT_KEY_CLASS, null)));
                mappersValueSerialization.add(serializationFactory.getSerialization(mConf.getClass(
        ...
        if (klass != null) {
            ...
            if (rConf.getBoolean(REDUCER_BY_VALUE, true)) {
                reducerKeySerialization = serializationFactory.getSerialization(rConf.getClass(
     ...
     * 
     * @throws IOException
     *             thrown if any of the chain elements threw an IOException
     *             exception.
     */
    ...
    private class ChainOutputCollector<K, V> implements OutputCollector<K, V> {
         ...
         */
        public ChainOutputCollector(int index, Serialization<K> keySerialization, Serialization<V> valueSerialization,
         ...
         */
        public ChainOutputCollector(Serialization<K> keySerialization, Serialization<V> valueSerialization,
                OutputCollector output, Reporter reporter) {
        ...
        public void collect(K key, V value) throws IOException {
            if (nextMapperIndex < mappers.size()) {
                ...
                nextMapper.map(key, value, new ChainOutputCollector(nextMapperIndex, nextKeySerialization,
                        nextValueSerialization, output, reporter), reporter);
            } else {
