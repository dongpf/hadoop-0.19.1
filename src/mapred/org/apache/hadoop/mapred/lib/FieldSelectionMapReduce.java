 ...
 * field spec: fieldSpec,fieldSpec,fieldSpec ... Each field spec can be a simple
 * number (e.g. 5) specifying a specific field, or a range (like 2-5) to specify
 * a range of fields, or an open range (like 3-) specifying all the fields
 * starting from field 3. The open range field spec applies value fields only.
 * They have no effect on the key fields.
 * 
 * Here is an example: "4,3,0,1:6,5,1-3,7-". It specifies to use fields 4,3,0
 * and 1 for keys, and use fields 6,5,1,2,3,7 and above for values.
 * 
...
public class FieldSelectionMapReduce<K, V> implements Mapper<K, V, Text, Text>, Reducer<Text, Text, Text, Text> {
    ...
    /**
     * Extract the actual field numbers from the given field specs. If a field
     * spec is in the form of "n-" (like 3-), then n will be the return value.
