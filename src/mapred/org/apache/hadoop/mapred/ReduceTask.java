...
class ReduceTask extends Task {
    ...
    private Counters.Counter reduceInputKeyCounter = getCounters().findCounter(Counter.REDUCE_INPUT_GROUPS);
    private Counters.Counter reduceInputValueCounter = getCounters().findCounter(Counter.REDUCE_INPUT_RECORDS);
    private Counters.Counter reduceOutputCounter = getCounters().findCounter(Counter.REDUCE_OUTPUT_RECORDS);
    private Counters.Counter reduceCombineInputCounter = getCounters().findCounter(Counter.COMBINE_INPUT_RECORDS);
    private Counters.Counter reduceCombineOutputCounter = getCounters().findCounter(Counter.COMBINE_OUTPUT_RECORDS);
    ...
    @Override
    public TaskRunner createRunner(TaskTracker tracker, TaskInProgress tip) throws IOException {
    ...
    private class ReduceValuesIterator<KEY, VALUE> extends ValuesIterator<KEY, VALUE> {
        public ReduceValuesIterator(RawKeyValueIterator in, RawComparator<KEY> comparator, Class<KEY> keyClass,
                Class<VALUE> valClass, Configuration conf, Progressable reporter) throws IOException {
    ...
    private class SkippingReduceValuesIterator<KEY, VALUE> extends ReduceValuesIterator<KEY, VALUE> {
        ...
        public SkippingReduceValuesIterator(RawKeyValueIterator in, RawComparator<KEY> comparator, Class<KEY> keyClass,
                Class<VALUE> valClass, Configuration conf, Progressable reporter, TaskUmbilicalProtocol umbilical)
                throws IOException {
            super(in, comparator, keyClass, valClass, conf, reporter);
            this.umbilical = umbilical;
            this.skipGroupCounter = getCounters().findCounter(Counter.REDUCE_SKIPPED_GROUPS);
            this.skipRecCounter = getCounters().findCounter(Counter.REDUCE_SKIPPED_RECORDS);
        ...
        private void writeSkippedRec(KEY key, VALUE value) throws IOException {
            if (skipWriter == null) {
                ...
                skipWriter = SequenceFile.createWriter(skipFile.getFileSystem(conf), conf, skipFile, keyClass,
                        valClass, CompressionType.BLOCK, getReporter(umbilical));
            }
    ...
    public void run(JobConf job, final TaskUmbilicalProtocol umbilical) throws IOException {
        ...
        RawKeyValueIterator rIter = isLocal ? Merger.merge(job, rfs, job.getMapOutputKeyClass(),
                job.getMapOutputValueClass(), codec, getMapFiles(rfs, true), !conf.getKeepFailedTaskFiles(),
                job.getInt("io.sort.factor", 100), new Path(getTaskID().toString()), job.getOutputKeyComparator(),
                reporter) : reduceCopier.createKVIterator(job, rfs, reporter);
        ...
        try {
            ...
            ReduceValuesIterator values = isSkipping() ? new SkippingReduceValuesIterator(rIter,
                    job.getOutputValueGroupingComparator(), keyClass, valClass, job, reporter, umbilical)
                    : new ReduceValuesIterator(rIter, job.getOutputValueGroupingComparator(), keyClass, valClass, job,
                            reporter);
            ...
            while (values.more()) {
                ...
                if (incrProcCount) {
                    reporter.incrCounter(SkipBadRecords.COUNTER_GROUP, SkipBadRecords.COUNTER_REDUCE_PROCESSED_GROUPS,
                            1);
                }
                values.nextKey();
                values.informReduceProgress();
    ...
    class ReduceCopier<K, V> implements MRConstants {
        ...
        private class MapOutputCopier extends Thread {
                ...
                while (true) {
                    try {
                        ...
                        try {
                            ...
                            LOG.warn(reduceTask.getTaskID() + " copy failed: " + loc.getTaskAttemptId() + " from "
                                    + loc.getHost());
                        ...
                        try {
                            ...
                            LOG.error("Could not notify TT of FSError: " + StringUtils.stringifyException(io));
                        }
                    } catch (Throwable th) {
                        LOG.error("Map output copy failure: " + StringUtils.stringifyException(th));
                    }
                }
            }
            ...
            private long copyOutput(MapOutputLocation loc) throws IOException, InterruptedException {
                ...
                if (copiedMapOutputs.contains(loc.getTaskId()) || obsoleteMapIds.contains(loc.getTaskAttemptId())) {
                ...
                if (mapOutput == null) {
                    throw new IOException("Failed to fetch map-output for " + loc.getTaskAttemptId() + " from "
                            + loc.getHost());
                }
             ...
             */
            private MapOutput getMapOutput(MapOutputLocation mapOutputLoc, Path filename) throws IOException,
                ...
                if (shuffleInMemory) {
                    LOG.info("Shuffling " + decompressedLength + " bytes (" + compressedLength + " raw bytes) "
            ...
            private MapOutput shuffleInMemory(MapOutputLocation mapOutputLoc, URLConnection connection,
                    InputStream input, int mapOutputLength, int compressedLength) throws IOException,
                    InterruptedException {
                ...
                if (bytesRead != mapOutputLength) {
                    ...
                    throw new IOException("Incomplete map output received for " + mapOutputLoc.getTaskAttemptId()
                            + " from " + mapOutputLoc.getOutputLocation() + " (" + bytesRead + " instead of "
                            + mapOutputLength + ")");
                }
                ...
                if (mapOutputLength > 0) {
                    ...
                    LOG.info("Rec #1 from " + mapOutputLoc.getTaskAttemptId() + " -> (" + WritableUtils.readVInt(dib)
                            + ", " + WritableUtils.readVInt(dib) + ") from " + mapOutputLoc.getHost());
                }

                return mapOutput;
            }

            private MapOutput shuffleToDisk(MapOutputLocation mapOutputLoc, InputStream input, Path filename,
                ...
                MapOutput mapOutput = new MapOutput(mapOutputLoc.getTaskId(), mapOutputLoc.getTaskAttemptId(), conf,
        ...
        public ReduceCopier(TaskUmbilicalProtocol umbilical, JobConf conf) throws IOException {
            ...
            combineCollector = (null != combinerClass) ? new CombineOutputCollector(reduceCombineOutputCounter) : null;
            ...
            this.maxInMemReduce = (int) Math.min(Runtime.getRuntime().maxMemory() * maxRedPer, Integer.MAX_VALUE);
            ...
            long randomSeed = System.nanoTime()
                    + (long) Math.pow(this.reduceTask.getPartition(), (this.reduceTask.getPartition() % 10));
        ...
        public boolean fetchOutputs() throws IOException {
            ...
            HashMap<String, List<MapOutputLocation>> mapLocations = new HashMap<String, List<MapOutputLocation>>();
            int totalFailures = 0;
            ...
            while (copiedMapOutputs.size() < numMaps && mergeThrowable == null) {
                ...
                if (logNow) {
                    LOG.info(reduceTask.getTaskID() + " Need another " + (numMaps - copiedMapOutputs.size())
                            + " map output(s) " + "where " + numInFlight + " is already in progress");
                }
                ...
                try {
                    ...
                    // The call getMapCompletionEvents will update fromEventId
                    // to
                    // used for the next call to getMapCompletionEvents

                    int currentNumObsoleteMapIds = obsoleteMapIds.size();

                    int numNewOutputs = getMapCompletionEvents(fromEventId, mapLocations, hostList);

                    if (numNewOutputs > 0 || logNow) {
                        LOG.info(reduceTask.getTaskID() + ": " + "Got " + numNewOutputs + " new map-outputs");
                    }

                    int numNewObsoleteMaps = obsoleteMapIds.size() - currentNumObsoleteMapIds;

                    if (numNewObsoleteMaps > 0) {
                        LOG.info(reduceTask.getTaskID() + ": " + "Got " + numNewObsoleteMaps
                    ...
                    LOG.warn(reduceTask.getTaskID() + " Problem locating map outputs: "
                ...
                if (numScheduled > 0 || logNow) {
                    LOG.info(reduceTask.getTaskID() + " Scheduled " + numScheduled + " outputs (" + penaltyBox.size()
                ...
                if (penaltyBox.size() > 0 && logNow) {
                    ...
                    for (String host : penaltyBox.keySet()) {
                        LOG.info(host + " Will be considered after: " + ((penaltyBox.get(host) - currentTime) / 1000)
                                + " seconds.");
                    }
                }
                ...
                while (numInFlight > 0 && mergeThrowable == null) {
                    ...
                    if (cr.getSuccess()) { // a successful copy
                        ...
                        // ignore
                        LOG.info(reduceTask.getTaskID() + " Ignoring obsolete copy result for Map Task: "
                        ...
                        if (noFailedFetches == maxFetchRetriesPerMap) {
                            ...
                            if ((fetchFailedMaps.size() >= maxFailedUniqueFetches || fetchFailedMaps.size() == (numMaps - copiedMapOutputs
                                    .size())) && !reducerHealthy && (!reducerProgressedEnough || reducerStalled)) {
                        ...
                        LOG.warn(reduceTask.getTaskID() + " adding host " + cr.getHost()
                                + " to penalty box, next contact in " + (currentBackOff / 1000) + " seconds");
                    }
            ...
            if (mergeThrowable == null) {
                try {
                    ...
                    LOG.info("Interleaved on-disk merge complete: " + mapOutputFilesOnDisk.size() + " files left.");

                    // wait for an ongoing merge (if it is in flight) to
                    // complete
                    inMemFSMergeThread.join();
                    LOG.info("In-memory merge complete: " + mapOutputsFilesInMemory.size() + " files left.");
                } catch (Throwable t) {
                    LOG.warn(reduceTask.getTaskID() + " Final merge of the inmemory files threw an exception: "
        ...
        private long createInMemorySegments(List<Segment<K, V>> inMemorySegments, long leaveBytes) throws IOException {
            long totalSize = 0L;
            synchronized (mapOutputsFilesInMemory) {
                ...
                while (fullSize > leaveBytes) {
                    ...
                    fullSize -= mo.data.length;
                    Reader<K, V> reader = new InMemoryReader<K, V>(ramManager, mo.mapAttemptId, mo.data, 0,
                            mo.data.length);
        ...
        private RawKeyValueIterator createKVIterator(JobConf job, FileSystem fs, Reporter reporter) throws IOException {
            ...
            if (mapOutputsFilesInMemory.size() > 0) {
                ...
                if (numMemDiskSegments > 0 && ioSortFactor > mapOutputFilesOnDisk.size()) {
                    ...
                    // intermediate merge
                    final Path outputPath = mapOutputFile.getInputFileForWrite(mapId, reduceTask.getTaskID(),
                            inMemToDiskBytes);
                    final RawKeyValueIterator rIter = Merger.merge(job, fs, keyClass, valueClass, memDiskSegments,
            ...
            if (0 != onDiskBytes) {
                ...
                memDiskSegments.clear();
                RawKeyValueIterator diskMerge = Merger.merge(job, fs, keyClass, valueClass, diskSegments, ioSortFactor,
            ...
            return Merger.merge(job, fs, keyClass, valueClass, finalSegments, finalSegments.size(), tmpDir, comparator,
                    reporter);
        }
         ...
         * 
         * @param fromEventId
         *            the first event ID we want to start from, this is modified
         *            by the call to this method
         * @param mapLocations
         *            the hash map of map locations by host
         * @param hostsList
         *            the list that contains unique hosts having map outputs,
         *            will be updated on the return of this method
         * @return the number of new map-completion events from the given event
         *         ID
         * @throws IOException
         */
        private int getMapCompletionEvents(IntWritable fromEventId,
                HashMap<String, List<MapOutputLocation>> mapLocations, List<String> hostsList) throws IOException {
            ...
            MapTaskCompletionEventsUpdate update = umbilical.getMapCompletionEvents(reduceTask.getJobID(),
                    fromEventId.get(), MAX_EVENTS_TO_FETCH, reduceTask.getTaskID());
            ...
            // stop fetching
            // from those maps.
            // 3. Remove TIPFAILED maps from neededOutputs since we don't need
            // their
            // outputs at all.
            for (TaskCompletionEvent event : events) {
                ...
                case SUCCEEDED: {
                    ...
                    URL mapOutputLocation = new URL(event.getTaskTrackerHttp() + "/mapOutput?job=" + taskId.getJobID()
                ...
                try {
                    ...
                    while (!exitLocalFSMerge) {
                        ...
                        LOG.info(reduceTask.getTaskID() + "We have  " + mapOutputFilesOnDisk.size()
                                + " map outputs on disk. " + "Triggering merge of " + ioSortFactor + " files");
                        ...
                        Path outputPath = lDirAlloc.getLocalPathForWrite(mapFiles.get(0).toString(), approxOutputSize,
                                conf).suffix(".merged");
                        ...
                        final Reporter reporter = getReporter(umbilical);
                        try {
                            iter = Merger.merge(conf, rfs, conf.getMapOutputKeyClass(), conf.getMapOutputValueClass(),
                                    codec, mapFiles.toArray(new Path[mapFiles.size()]), true, ioSortFactor, tmpDir,
                    ...
                    LOG.warn(reduceTask.getTaskID() + " Merging of the local FS files threw an exception: "
                            + StringUtils.stringifyException(t));
                    if (mergeThrowable == null) {
                        mergeThrowable = t;
                    }
        ...
        private class InMemFSMergeThread extends Thread {
            ...
            public void run() {
                ...
                try {
                    ...
                    LOG.warn(reduceTask.getTaskID() + " Merge of the inmemory files threw an exception: "
                            + StringUtils.stringifyException(t));
            ...
            private void doInMemMerge() throws IOException {
                ...
                Path outputPath = mapOutputFile.getInputFileForWrite(mapId, reduceTask.getTaskID(), mergeOutputSize);
                ...
                try {
                    ...
                    rIter = Merger.merge(conf, rfs, (Class<K>) conf.getMapOutputKeyClass(),
                            (Class<V>) conf.getMapOutputValueClass(), inMemorySegments, inMemorySegments.size(),
                            new Path(reduceTask.getTaskID().toString()), conf.getOutputKeyComparator(), reporter);
        ...
        private void combineAndSpill(RawKeyValueIterator kvIter, Counters.Counter inCounter) throws IOException {
            ...
            try {
                CombineValuesIterator values = new CombineValuesIterator(kvIter, comparator, keyClass, valClass, job,
                        Reporter.NULL, inCounter);
                while (values.more()) {
