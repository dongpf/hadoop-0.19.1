...
/**
 * Keeps the Ranges sorted by startIndex. The added ranges are always ensured to
 * be non-overlapping. Provides the SkipRangeIterator, which skips the Ranges
...
class SortedRanges implements Writable {
    ...
    /**
     * Get Iterator which skips the stored ranges. The Iterator.next() call
    ...
    /**
     * Add the range indices. It is ensured that the added range doesn't overlap
     * the existing ranges. If it overlaps, the existing overlapping ranges are
     * removed and a single range having the superset of all the removed ranges
     * and this range is added. If the range is of 0 length, doesn't do
     * anything.
     * 
     * @param range
     *            Range to be added.
    ...
    /**
     * Remove the range indices. If this range is found in existing ranges, the
     * existing ranges are shrunk. If range is of 0 length, doesn't do anything.
     * 
     * @param range
     *            Range to be removed.
     */
    ...
    /**
     * Index Range. Comprises of start index and length. A Range can be of 0
     * length also. The Range stores indices of type long.
     */
    ...
    static class SkipRangeIterator implements Iterator<Long> {
         ...
         * 
         * @param rangeIterator
         *            the iterator which gives the ranges.
         */
         ...
         * 
         * @return <code>true</code> next index exists. <code>false</code>
         *         otherwise.
         */
