 ...
 * 
 * <p>
 * Hadoop provides an optional mode of execution in which the bad records are
 * detected and skipped in further attempts.
 * 
 * <p>
 ...
 * perhaps the bug is in third party libraries for which the source code is not
 * available. Due to this, the task never reaches to completion even with
 * multiple attempts and complete data for that task is lost.
 * </p>
 * 
 * <p>
 * With this feature, only a small portion of data is lost surrounding the bad
 * record, which may be acceptable for some user applications. see
 * {@link SkipBadRecords#setMapperMaxSkipRecords(Configuration, long)}
 * </p>
 * 
 * <p>
 * The skipping mode gets kicked off after certain no of failures see
 * {@link SkipBadRecords#setAttemptsToStartSkipping(Configuration, int)}
 * </p>
 * 
 * <p>
 * In the skipping mode, the map/reduce task maintains the record range which is
 * getting processed at all times. Before giving the input to the map/reduce
 * function, it sends this record range to the Task tracker. If task crashes,
 * the Task tracker knows which one was the last reported range. On further
 * attempts that range get skipped.
 * </p>
 */
public class SkipBadRecords {

    /**
     * Special counters which are written by the application and are used by the
     * framework for detecting bad records. For detecting bad records these
     * counters must be incremented by the application.
     */
     ...
     * those are skipped. Default value is 2.
     * 
     * @param conf
     *            the configuration
     ...
     * {@link SkipBadRecords#COUNTER_MAP_PROCESSED_RECORDS} is incremented by
     * MapRunner after invoking the map function. This value must be set to
     ...
     * should increment this counter on their own. Default value is true.
     * 
     * @param conf
     *            the configuration
    ...
    /**
     * Get the flag which if set to true,
     * {@link SkipBadRecords#COUNTER_REDUCE_PROCESSED_GROUPS} is incremented by
     ...
     * the sub directory of the output _logs directory. User can stop writing
     * skipped records by setting the value null.
     * 
     * @param conf
     *            the configuration.
     ...
     * bad record in mapper. The number includes the bad record as well. To turn
     * the feature of detection/skipping of bad records off, set the value to 0.
     * The framework tries to narrow down the skipped range by retrying until
     * this threshold is met OR all attempts get exhausted for this task. Set
     * the value to Long.MAX_VALUE to indicate that framework need not try to
     * narrow down. Whatever records(depends on application) get skipped are
     * acceptable. Default value is 0.
     * 
     * @param conf
     *            the configuration
     ...
     * bad group in reducer. The number includes the bad group as well. To turn
     * the feature of detection/skipping of bad groups off, set the value to 0.
     * The framework tries to narrow down the skipped range by retrying until
     * this threshold is met OR all attempts get exhausted for this task. Set
     * the value to Long.MAX_VALUE to indicate that framework need not try to
