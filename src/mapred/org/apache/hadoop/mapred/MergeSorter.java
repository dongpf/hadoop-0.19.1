 ...
 * MergeSort. Note that this class is really a wrapper over the actual mergesort
 * implementation that is there in the util package. The main intent of
 * providing this class is to setup the input data for the util.MergeSort algo
 * so that the latter doesn't need to bother about the various data structures
 * that have been created for the Map output but rather concentrate on the core
 * algorithm (thereby allowing easy integration of a mergesort implementation).
 * The bridge between this class and the util.MergeSort class is the Comparator.
 */
class MergeSorter extends BasicTypeSorterBase implements Comparator<IntWritable> {
    ...
    public RawKeyValueIterator sort() {
        ...
        return new MRSortResultIterator(super.keyValBuffer, pointersCopy, super.startOffsets, super.keyLengths,
                super.valueLengths);
    }
    ...
    public int compare(IntWritable i, IntWritable j) {
        ...
        return comparator.compare(keyValBuffer.getData(), startOffsets[i.get()], keyLengths[i.get()],
                keyValBuffer.getData(), startOffsets[j.get()], keyLengths[j.get()]);
    }
