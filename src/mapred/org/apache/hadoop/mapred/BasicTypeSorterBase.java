...
/**
 * This class implements the sort interface using primitive int arrays as the
 * data structures (that is why this class is called 'BasicType'SorterBase)
 */
abstract class BasicTypeSorterBase implements BufferSorter {
    ...
    protected OutputBuffer keyValBuffer; // the buffer used for storing
                                         // key/values
    protected int[] startOffsets; // the array used to store the start offsets
                                  // of
    ...
    protected int[] pointers; // the array of startOffsets's indices. This will
                              // be sorted at the end to contain a sorted array
                              // of
                              // indices to offsets
    ...
    public void setInputBuffer(OutputBuffer buffer) {
        // store a reference to the keyValBuffer that we need to read during
        // sort
        this.keyValBuffer = buffer;
    }

    public long getMemoryUtilized() {
        // the total length of the arrays + the max{Key,Val}Length (this will be
        // the
    ...
    public void close() {
        // set count to 0; also, we don't reuse the arrays since we want to
        // maintain
...
class MRSortResultIterator implements RawKeyValueIterator {
    ...
    public MRSortResultIterator(OutputBuffer keyValBuffer, int[] pointers, int[] startOffsets, int[] keyLengths,
            int[] valLengths) {
