...
/**
 * This tests timout out from SocketInputStream and SocketOutputStream using
 * pipes.
 * 
 * Normal read and write using these streams are tested by pretty much every DFS
 * unit test.
 */
public class TestSocketIOWithTimeout extends TestCase {
    ...
    private void doIO(InputStream in, OutputStream out) throws IOException {
        /*
         * Keep on writing or reading until we get SocketTimeoutException. It
         * expects this exception to occur within 100 millis of TIMEOUT.
         */
    ...
    static class ReadRunnable implements Runnable {
        ...
        public void run() {
            try {
                ...
                LOG.info("Got expection while reading as expected : " + e.getMessage());
                return;
            }
            assertTrue(false);
        }
    ...
    public void testSocketIOWithTimeout() throws IOException {
        ...
        try {
            ...
            /*
             * Verify that it handles interrupted threads properly. Use a large
             * timeout and expect the thread to return quickly.
             */
