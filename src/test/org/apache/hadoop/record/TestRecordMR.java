...
public class TestRecordMR extends TestCase {
     ...
     * instructed.
     */
    static public class RandomGenMapper implements Mapper<RecInt, RecInt, RecInt, RecString> {
        Random r = new Random();

        public void configure(JobConf job) {
        }

        public void map(RecInt key, RecInt val, OutputCollector<RecInt, RecString> out, Reporter reporter)
                throws IOException {
            int randomVal = key.getData();
            int randomCount = val.getData();

            for (int i = 0; i < randomCount; i++) {
                out.collect(new RecInt(Math.abs(r.nextInt())), new RecString(Integer.toString(randomVal)));
            }
        }
    ...
    /**
   */
    static public class RandomGenReducer implements Reducer<RecInt, RecString, RecInt, RecString> {
        public void configure(JobConf job) {
        }

        public void reduce(RecInt key, Iterator<RecString> it, OutputCollector<RecInt, RecString> out, Reporter reporter)
                throws IOException {
            int keyint = key.getData();
            while (it.hasNext()) {
                String val = it.next().getData();
                out.collect(new RecInt(Integer.parseInt(val)), new RecString(""));
            }
        }
     ...
     * emitted.
     */
    static public class RandomCheckMapper implements Mapper<RecInt, RecString, RecInt, RecString> {
        public void configure(JobConf job) {
        }

        public void map(RecInt key, RecString val, OutputCollector<RecInt, RecString> out, Reporter reporter)
                throws IOException {
            int pos = key.getData();
    ...
    /**
   */
    static public class RandomCheckReducer implements Reducer<RecInt, RecString, RecInt, RecString> {
        public void configure(JobConf job) {
        }
     ...
     */
    static public class MergeMapper implements Mapper<RecInt, RecString, RecInt, RecInt> {
        public void configure(JobConf job) {
        }

        public void map(RecInt key, RecString val, OutputCollector<RecInt, RecInt> out, Reporter reporter)
                throws IOException {
            int keyint = key.getData();
    ...
    static public class MergeReducer implements Reducer<RecInt, RecInt, RecInt, RecInt> {
        public void configure(JobConf job) {
        }

        public void reduce(RecInt key, Iterator<RecInt> it, OutputCollector<RecInt, RecInt> out, Reporter reporter)
                throws IOException {
            int keyint = key.getData();
            int total = 0;
            while (it.hasNext()) {
   ...
   */
    public static void launch() throws Exception {
        ...
        SequenceFile.Writer out = SequenceFile.createWriter(fs, conf, answerkey, RecInt.class, RecInt.class,
            ...
            for (int i = 0; i < range; i++) {
                ...
                if (!in.next(key, val)) {
                    ...
                    if (!((key.getData() == i) && (val.getData() == dist[i]))) {
                        System.err.println("Mismatch!  Pos=" + key.getData() + ", i=" + i + ", val=" + val.getData()
                                + ", dist[i]=" + dist[i]);
                        success = false;
                    }
    ...
    public static void main(String[] argv) throws Exception {
        if (argv.length < 2) {
            ...
            System.err.println();
            System.err
                    .println("Note: a good test will have a <counts> value that is substantially larger than the <range>");
            return;
        }
