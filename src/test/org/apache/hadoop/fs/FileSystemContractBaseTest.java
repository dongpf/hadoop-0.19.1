...
public abstract class FileSystemContractBaseTest extends TestCase {
    ...
    private void writeReadAndDelete(int len) throws IOException {
        ...
        FSDataOutputStream out = fs.create(path, false, fs.getConf().getInt("io.file.buffer.size", 4096), (short) 1,
                getBlockSize());
    ...
    public void testRenameDirectoryMoveToExistingDirectory() throws Exception {
        ...
        assertTrue("Renamed nested file1 exists", fs.exists(path("/test/new/newdir/file1")));
        assertTrue("Renamed nested exists", fs.exists(path("/test/new/newdir/subdir/file2")));
    }
    ...
    private void rename(Path src, Path dst, boolean renameSucceeded, boolean srcExists, boolean dstExists)
            throws IOException {
