 ...
 * constraints: 1. The number of subdirectories is a random number in [minWidth,
 * maxWidth]. 2. The maximum depth of each subdirectory is a random number
 * [2*maxDepth/3, maxDepth]. 3. Files are randomly placed in the empty
 * directories. The size of each file follows Gaussian distribution. The
 * generated namespace structure is described by two files in the output
 ...
 * 
 * The synopsis of the command is java StructureGenerator -maxDepth <maxDepth> :
 * maximum depth of the directory tree; default is 5. -minWidth <minWidth> :
 * minimum number of subdirectories per directories; default is 1 -maxWidth
 ...
 * -outDir <outDir>: output directory; default is the current directory. -seed
...
public class StructureGenerator {
    ...
    /**
     * The main function first parses the command line arguments, then generates
     * in-memory directory structure and outputs to a file, last generates
     * in-memory files and outputs them to a file.
     */
    ...
    private static class INode {
        ...
        /**
         * Output the subtree rooted at the current node. Only the leaves are
         * printed.
         */
    ...
    /**
     * Generate a directory tree rooted at <code>rootName</code> The number of
     * subtree is in the range of [minWidth, maxWidth]. The maximum depth of
     * each subtree is in the range of [2*maxDepth/3, maxDepth].
     */
    ...
    /**
     * Decides where to place all the files and its length. It first collects
     * all empty directories in the tree. For each file, it randomly chooses an
     * empty directory to place the file. The file's length is generated using
    ...
    private void genFileStructure() {
        ...
        for (int i = 0; i < numOfFiles; i++) {
            ...
            leaves.get(leaveNum).addChild(new FileINode(FILE_NAME_PREFIX + i, fileSize));
        }
    }

    /**
     * Output directory structure to a file, each line of the file contains the
     * directory name. Only empty directory names are printed.
     */
