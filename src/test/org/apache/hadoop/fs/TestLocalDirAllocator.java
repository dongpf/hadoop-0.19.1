...
/**
 * This test LocalDirAllocator works correctly; Every test case uses different
 * buffer dirs to enforce the AllocatorPerContext initialization. This test does
 * not run on Cygwin because under Cygwin a directory can be created in a
 * read-only directory which breaks this test.
 */
public class TestLocalDirAllocator extends TestCase {
    ...
    final static private String BUFFER_DIR[] = new String[] { BUFFER_DIR_ROOT + "/tmp0", BUFFER_DIR_ROOT + "/tmp1",
            BUFFER_DIR_ROOT + "/tmp2", BUFFER_DIR_ROOT + "/tmp3", BUFFER_DIR_ROOT + "/tmp4", BUFFER_DIR_ROOT + "/tmp5",
            BUFFER_DIR_ROOT + "/tmp6" };
    final static private Path BUFFER_PATH[] = new Path[] { new Path(BUFFER_DIR[0]), new Path(BUFFER_DIR[1]),
            new Path(BUFFER_DIR[2]), new Path(BUFFER_DIR[3]), new Path(BUFFER_DIR[4]), new Path(BUFFER_DIR[5]),
            new Path(BUFFER_DIR[6]) };
    final static private String CONTEXT = "dfs.client.buffer.dir";
    final static private String FILENAME = "block";
    final static private LocalDirAllocator dirAllocator = new LocalDirAllocator(CONTEXT);
    ...
    /**
     * Two buffer dirs. The first dir exists & is on a read-only disk; The
     * second dir exists & is RW
     * 
     * @throws Exception
     */
    ...
    /**
     * Two buffer dirs. Both do not exist but on a RW disk. Check if tmp dirs
    ...
    /**
     * Two buffer dirs. Both exists and on a R/W disk. Later disk1 becomes
     * read-only.
     * 
     * @throws Exception
     */
    public void test3() throws Exception {
        ...
        try {
            ...
            // create the first file with size, and then figure the round-robin
            // sequence
            createTempFile(SMALL_FILE_SIZE);
     ...
     * 
     * Try to create a whole bunch of files. Verify that they do indeed all get
     * created where they should.
     * 
     * Would ideally check statistical properties of distribution, but we don't
     * have the nerve to risk false-positives here.
     * 
     * @throws Exception
     */
