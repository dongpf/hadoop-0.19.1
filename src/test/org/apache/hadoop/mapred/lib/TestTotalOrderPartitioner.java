...
public class TestTotalOrderPartitioner extends TestCase {
    ...
    private static final ArrayList<Check<Text>> testStrings = new ArrayList<Check<Text>>();
    static {
    ...
    private static <T extends WritableComparable> Path writePartitionFile(String testname, JobConf conf, T[] splits)
            throws IOException {
        ...
        try {
            ...
            w = SequenceFile.createWriter(fs, conf, p, splits[0].getClass(), NullWritable.class,
    ...
    public void testTotalOrderMemCmp() throws Exception {
        ...
        Path p = TestTotalOrderPartitioner.<Text> writePartitionFile("totalordermemcmp", job, splitStrings);
    ...
    public void testTotalOrderBinarySearch() throws Exception {
        TotalOrderPartitioner<Text, NullWritable> partitioner = new TotalOrderPartitioner<Text, NullWritable>();
        JobConf job = new JobConf();
        Path p = TestTotalOrderPartitioner.<Text> writePartitionFile("totalorderbinarysearch", job, splitStrings);
    ...
    public void testTotalOrderCustomComparator() throws Exception {
        ...
        Path p = TestTotalOrderPartitioner
                .<Text> writePartitionFile("totalordercustomcomparator", job, revSplitStrings);
        job.setBoolean("total.order.partitioner.natural.order", false);
        job.setMapOutputKeyClass(Text.class);
