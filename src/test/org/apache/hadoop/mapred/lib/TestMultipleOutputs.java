...
public class TestMultipleOutputs extends HadoopTestCase {
    ...
    protected void _testMultipleOutputs(boolean withCounters) throws Exception {
        ...
        MultipleOutputs.addMultiNamedOutput(conf, "sequence", SequenceFileOutputFormat.class, LongWritable.class,
                Text.class);
        ...
        for (FileStatus status : statuses) {
            if (status.getPath().getName().equals("text-m-00000") || status.getPath().getName().equals("text-m-00001")
        ...
        BufferedReader reader = new BufferedReader(new InputStreamReader(fs.open(new Path(FileOutputFormat
                .getOutputPath(conf), "text-r-00000"))));
        int count = 0;
        ...
        SequenceFile.Reader seqReader = new SequenceFile.Reader(fs, new Path(FileOutputFormat.getOutputPath(conf),
