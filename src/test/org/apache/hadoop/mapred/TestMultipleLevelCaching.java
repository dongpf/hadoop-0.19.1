...
public class TestMultipleLevelCaching extends TestCase {
     ...
     * for the rack. For id = 2, level = 2 we get /a/b2/c2 id = 1, level = 3 we
     * get /a/b1/c1/d1 NOTE There should always be one shared node i.e /a
     * 
     * @param id
     *            Unique Id for the rack
     * @param level
     *            The level in the topology where the separation starts
     */
    ...
    private void testCachingAtLevel(int level) throws IOException {
        ...
        try {
            ...
            // cache-level = level (unshared levels) + 1(topmost shared node i.e
            // /a)
            // + 1 (for host)
             ...
             * Since the datanode is running under different subtree, there is
             * no node-level data locality but there should be topological
             * locality.
             */
