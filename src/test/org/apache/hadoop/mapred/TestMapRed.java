...
public class TestMapRed extends TestCase {
   ...
   */
    static class RandomGenReducer implements Reducer<IntWritable, IntWritable, IntWritable, IntWritable> {
        ...
        public void reduce(IntWritable key, Iterator<IntWritable> it, OutputCollector<IntWritable, IntWritable> out,
                Reporter reporter) throws IOException {
            while (it.hasNext()) {
    ...
    static class RandomCheckMapper implements Mapper<WritableComparable, Text, IntWritable, IntWritable> {
        ...
        public void map(WritableComparable key, Text val, OutputCollector<IntWritable, IntWritable> out,
                Reporter reporter) throws IOException {
   ...
   */
     ...
     * IOException { this.range = range; this.counts = counts; this.conf = conf;
     * }
     **/
    ...
    public static class NullMapper implements Mapper<NullWritable, Text, NullWritable, Text> {
        public void map(NullWritable key, Text val, OutputCollector<NullWritable, Text> output, Reporter reporter)
                throws IOException {
    ...
    public void testNullKeys() throws Exception {
        ...
        SequenceFile.Writer w = SequenceFile.createWriter(fs, conf, inFile, NullWritable.class, Text.class,
    ...
    private void checkCompression(boolean compressMapOutputs, CompressionType redCompression, boolean includeCombine)
            throws Exception {
        JobConf conf = new JobConf(TestMapRed.class);
        ...
        try {
            ...
            assertEquals("is reduce output compressed " + output, redCompression != CompressionType.NONE,
                    rdr.isCompressed());
            rdr.close();
        } finally {
            fs.delete(testdir, true);
   ...
   */
    public static void launch() throws Exception {
        ...
        SequenceFile.Writer out = SequenceFile.createWriter(fs, conf, answerkey, IntWritable.class, IntWritable.class,
                SequenceFile.CompressionType.NONE);
        try {
            for (int i = 0; i < range; i++) {
