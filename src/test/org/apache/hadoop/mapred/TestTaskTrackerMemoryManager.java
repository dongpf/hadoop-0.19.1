...
public class TestTaskTrackerMemoryManager extends TestCase {
    ...
    private void runWordCount(JobConf conf) throws Exception {
        ...
        ToolRunner.run(WordCount, new String[] { input.toString(), output.toString() });
    }
    ...
    public void testNormalTaskAndLimitedTT() throws Exception {
        ...
        try {
            if (!ProcfsBasedProcessTree.isAvailable()) {
                LOG.info("Currently ProcessTree has only one implementation "
                        + "ProcfsBasedProcessTree, which is not available on this " + "system. Not testing");
                return;
            }
        } catch (Exception e) {
        ...
        Pattern diagMsgPattern = Pattern.compile("TaskTree \\[pid=[0-9]*,tipID=.*\\] is running beyond "
        ...
        fConf.setLong("mapred.tasktracker.tasks.maxmemory", Long.valueOf(10000000000L)); // Fairly
                                                                                         // large
                                                                                         // value
                                                                                         // for
                                                                                         // WordCount
        ...
        conf.set("fs.default.name", "hdfs://" + nn.getNameNodeAddress().getHostName() + ":"
        ...
        for (TaskCompletionEvent tce : taskComplEvents) {
            ...
            if (diagnostics != null) {
                for (String str : diagnostics) {
                    ...
                    // The error pattern shouldn't be there in any TIP's
                    // diagnostics
    ...
    public void testOOMTaskAndLimitedTT() throws Exception {
        ...
        Pattern diagMsgPattern = Pattern.compile("TaskTree \\[pid=[0-9]*,tipID=.*\\] is running beyond "
                + "memory-limits. Current usage : [0-9]*kB. Limit : " + PER_TASK_LIMIT + "kB. Killing task.");
        Matcher mat = null;
        ...
        for (TaskCompletionEvent tce : taskComplEvents) {
            ...
            for (String str : diagnostics) {
                mat = diagMsgPattern.matcher(str);
                // Every task HAS to spit out the out-of-memory errors in the
                // same
