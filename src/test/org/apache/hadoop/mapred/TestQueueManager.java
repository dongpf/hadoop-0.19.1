...
public class TestQueueManager extends TestCase {
    ...
    public void testUserDisabledACLForJobSubmission() throws IOException {
        JobConf conf = setupConf("mapred.queue.default.acl-submit-job", "3698-non-existent-user");
        verifyJobSubmission(conf, false);
    }
    ...
    public void testUserEnabledACLForJobSubmission() throws IOException, LoginException {
        ...
        UserGroupInformation ugi = UnixUserGroupInformation.login();
        String userName = ugi.getUserName();
        JobConf conf = setupConf("mapred.queue.default.acl-submit-job", "3698-junk-user," + userName
    ...
    public void testGroupsEnabledACLForJobSubmission() throws IOException, LoginException {
        ...
        JobConf conf = setupConf("mapred.queue.default.acl-submit-job", "3698-junk-user1,3698-junk-user2 "
                + groups[groups.length - 1] + ",3698-junk-group");
        verifyJobSubmission(conf, true);
    }
    ...
    public void testOwnerAllowedForJobKill() throws IOException {
        JobConf conf = setupConf("mapred.queue.default.acl-administer-jobs", "junk-user");
        verifyJobKill(conf, true);
    }

    public void testUserDisabledACLForJobKill() throws IOException {
        // setup a cluster allowing a user to submit
        JobConf conf = setupConf("mapred.queue.default.acl-administer-jobs", "dummy-user");
        verifyJobKillAsOtherUser(conf, false, "dummy-user,dummy-user-group");
    }
    ...
    public void testUserEnabledACLForJobKill() throws IOException, LoginException {
        ...
        UserGroupInformation ugi = UnixUserGroupInformation.login();
        String userName = ugi.getUserName();
        JobConf conf = setupConf("mapred.queue.default.acl-administer-jobs", "dummy-user," + userName);
    ...
    public void testUserDisabledForJobPriorityChange() throws IOException {
        JobConf conf = setupConf("mapred.queue.default.acl-administer-jobs", "junk-user");
        verifyJobPriorityChangeAsOtherUser(conf, false, "junk-user,junk-user-group");
    }
    ...
    private void verifyJobSubmission(JobConf conf, boolean shouldSucceed, String queue) throws IOException {
        ...
        try {
            ...
            if (shouldSucceed) {
                ...
                assertTrue(ioe.getMessage().contains("cannot perform operation " + "SUBMIT_JOB on queue " + queue));
            }
        } finally {
            tearDownCluster();
        }
    ...
    private void verifyJobKill(JobConf conf, boolean shouldSucceed) throws IOException {
        ...
        try {
            ...
            if (shouldSucceed) {
                ...
                LOG.info("exception while submitting job: " + ioe.getMessage());
                assertTrue(ioe.getMessage().contains("cannot perform operation " + "ADMINISTER_JOBS on queue default"));
            }
        } finally {
            tearDownCluster();
        }
    ...
    private void verifyJobPriorityChangeAsOtherUser(JobConf conf, boolean shouldSucceed, String otherUserInfo)
            throws IOException {
        setUpCluster(conf);
        try {
    ...
    private RunningJob submitSleepJob(int numMappers, int numReducers, long mapSleepTime, long reduceSleepTime,
            boolean shouldComplete) throws IOException {
        return submitSleepJob(numMappers, numReducers, mapSleepTime, reduceSleepTime, shouldComplete, null);
    }

    private RunningJob submitSleepJob(int numMappers, int numReducers, long mapSleepTime, long reduceSleepTime,
            boolean shouldComplete, String userInfo) throws IOException {
        return submitSleepJob(numMappers, numReducers, mapSleepTime, reduceSleepTime, shouldComplete, userInfo, null);
    }

    private RunningJob submitSleepJob(int numMappers, int numReducers, long mapSleepTime, long reduceSleepTime,
            boolean shouldComplete, String userInfo, String queueName) throws IOException {
        JobConf clientConf = new JobConf();
        clientConf.set("mapred.job.tracker", "localhost:" + miniMRCluster.getJobTrackerPort());
        SleepJob job = new SleepJob();
        job.setConf(clientConf);
        clientConf = job.setupJobConf(numMappers, numReducers, mapSleepTime, (int) mapSleepTime / 100, reduceSleepTime,
                (int) reduceSleepTime / 100);
        if (queueName != null) {
