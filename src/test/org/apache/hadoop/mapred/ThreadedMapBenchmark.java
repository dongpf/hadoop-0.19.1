 ...
 * <p>
 * This benchmark generates random data per map and tests the performance of
 * having multiple spills (using multiple threads) over having just one spill.
 * Following are the parameters that can be specified
...
public class ThreadedMapBenchmark extends Configured implements Tool {
    ...
    private static Path BASE_DIR = new Path(System.getProperty("test.build.data", File.separator + "benchmarks"
            + File.separator + "ThreadedMapBenchmark"));
     ...
     * each map. Using {@link RandomWriter} code.
     */
    ...
    public static class Map extends MapReduceBase implements
        ...
        public void map(WritableComparable key, Writable value, OutputCollector<BytesWritable, BytesWritable> output,
     ...
     */
    public static void generateInputData(int dataSizePerMap, int numSpillsPerMap, int numMapsPerHost, JobConf masterConf)
            throws Exception {
    ...
    public int run(String[] args) throws Exception {
        ...
        String usage = "Usage: threadedmapbenchmark "
        ...
        if (dataSizePerMap < 1 || // verify arguments
                numSpillsPerMap < 1 || numMapsPerHost < 1) {
            System.err.println(usage);
            System.exit(-1);
        }
