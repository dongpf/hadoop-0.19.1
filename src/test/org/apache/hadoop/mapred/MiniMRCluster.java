...
/**
 * This class creates a single-process Map-Reduce cluster for junit testing. One
 * thread is created for each server.
 */
public class MiniMRCluster {
    ...
    class TaskTrackerRunner implements Runnable {
        ...
        /**
         * Get the local dir for this TaskTracker. This is there so that we do
     ...
     * 
     * @param taskTracker
     *            the index of the task tracker to check
    ...
    public JobConf createJobConf(JobConf conf) {
        ...
        if (ugi != null) {
            ...
            UnixUserGroupInformation.saveToConf(result, UnixUserGroupInformation.UGI_PROPERTY_NAME, ugi);
        }
     ...
     * @throws IOException
     */
    public MiniMRCluster(int numTaskTrackers, String namenode, int numDir, String[] racks, String[] hosts)
            throws IOException {
     ...
     *            Array of racks
     * @param hosts
     *            Array of hosts in the corresponding racks
     * @param conf
     *            Default conf for the jobtracker
     * @throws IOException
     */
    public MiniMRCluster(int numTaskTrackers, String namenode, int numDir, String[] racks, String[] hosts, JobConf conf)
            throws IOException {
    ...
    public MiniMRCluster(int jobTrackerPort, int taskTrackerPort, int numTaskTrackers, String namenode, int numDir,
            String[] racks, String[] hosts, UnixUserGroupInformation ugi) throws IOException {
        this(jobTrackerPort, taskTrackerPort, numTaskTrackers, namenode, numDir, racks, hosts, ugi, null);
    }

    public MiniMRCluster(int jobTrackerPort, int taskTrackerPort, int numTaskTrackers, String namenode, int numDir,
            String[] racks, String[] hosts, UnixUserGroupInformation ugi, JobConf conf) throws IOException {
        ...
        for (int idx = 0; idx < numTaskTrackers; idx++) {
            ...
            taskTracker = new TaskTrackerRunner(idx, numDir, hosts == null ? null : hosts[idx], conf);
