 ...
 * This utility program has 2 main parts: 1. Checking the records' statistics a)
 * Validates the no. of bytes and records in sort's input & output. b) Validates
 * the xor of the md5's of each key/value pair. c) Ensures same key/value is
 * present in both input and output. 2. Check individual records to ensure each
 * record is present in both the input and the output of the sort (expensive on
 * large data-sets).
 * 
 * To run: bin/hadoop jar build/hadoop-examples.jar sortvalidate [-m
 * <i>maps</i>] [-r <i>reduces</i>] [-deep] -sortInput <i>sort-in-dir</i>
 * -sortOutput <i>sort-out-dir</i>
 */
public class SortValidator extends Configured implements Tool {
    ...
    /**
     * A simple map-reduce job which checks consistency of the MapReduce
     * framework's sort by checking: a) Records are sorted correctly b) Keys are
     * partitioned correctly c) The input and output have same no. of bytes and
     * records. d) The input and output have the correct 'checksum' by xor'ing
    ...
    public static class RecordStatsChecker {
        ...
        static class Raw {
             ...
             * 
             * @param writable
             *            {@link Writable} object from whom to get the raw data
        ...
        public static class Map extends MapReduceBase implements
                Mapper<WritableComparable, Writable, IntWritable, RecordStatsWritable> {
            ...
            public void configure(JobConf job) {
                // 'key' == sortInput for sort-input; key == sortOutput for
                // sort-output
                ...
                if (key == sortOutput) {
                    ...
                    // Figure the 'current' partition and no. of reduces of the
                    // 'sort'
                    try {
                        URI inputURI = new URI(job.get("map.input.file"));
                        String inputFile = inputURI.getPath();
                        partition = Integer.valueOf(inputFile.substring(inputFile.lastIndexOf("part") + 5)).intValue();
            ...
            public void map(WritableComparable key, Writable value,
                    OutputCollector<IntWritable, RecordStatsWritable> output, Reporter reporter) throws IOException {
                ...
                if (this.key == sortOutput) {
                    ...
                    if (prevKey == null) {
                        ...
                        if (keyClass != key.getClass()) {
                            throw new IOException("Type mismatch in key: expected " + keyClass.getName()
                    ...
                    if (partition != keyPartition) {
                        throw new IOException("Partitions do not match for record# " + recordId + " ! - '" + partition
                                + "' v/s '" + keyPartition + "'");
                    }
                }

                // Construct the record-stats and output (this.key,
                // record-stats)
        ...
        public static class Reduce extends MapReduceBase implements
                Reducer<IntWritable, RecordStatsWritable, IntWritable, RecordStatsWritable> {
        ...
        static void checkRecords(Configuration defaults, Path sortInput, Path sortOutput) throws IOException {
            ...
            System.out.println("\nSortValidator.RecordStatsChecker: Validate sort " + "from " + inputPaths[0] + " ("
                    + noSortInputpaths + " files), " + inputPaths[1] + " (" + noSortReduceTasks + " files) into "
                    + FileOutputFormat.getOutputPath(jobConf) + " with 1 reducer.");
    ...
    /**
     * A simple map-reduce task to check if the input and the output of the
     * framework's sort is consistent by ensuring each record is present in both
    ...
    public static class RecordChecker {
        ...
        public static class Map extends MapReduceBase implements
                Mapper<BytesWritable, BytesWritable, BytesWritable, IntWritable> {
            ...
            public void map(BytesWritable key, BytesWritable value, OutputCollector<BytesWritable, IntWritable> output,
        ...
        static void checkRecords(Configuration defaults, int noMaps, int noReduces, Path sortInput, Path sortOutput)
                throws IOException {
            ...
            // job_conf.set("mapred.job.tracker", "local");
            Path[] inputPaths = FileInputFormat.getInputPaths(jobConf);
            System.out.println("\nSortValidator.RecordChecker: Running on " + cluster.getTaskTrackers()
                    + " nodes to validate sort from " + inputPaths[0] + ", " + inputPaths[1] + " into "
                    + FileOutputFormat.getOutputPath(jobConf) + " with " + noReduces + " reduces.");
            Date startTime = new Date();
            System.out.println("Job started: " + startTime);
            JobClient.runJob(jobConf);
            Date end_time = new Date();
    ...
    /**
     * The main driver for sort-validator program. Invoke this method to submit
     * the map/reduce job.
     * 
     * @throws IOException
