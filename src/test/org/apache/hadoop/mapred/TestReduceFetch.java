...
public class TestReduceFetch extends TestCase {
    ...
    public static class MapMB implements Mapper<NullWritable, NullWritable, Text, Text> {
        ...
        public void map(NullWritable nk, NullWritable nv, OutputCollector<Text, Text> output, Reporter reporter)
                throws IOException {
            Text key = new Text();
            Text val = new Text();
    ...
    public void testReduceFromDisk() throws Exception {
        ...
        assertTrue(
                "Expected more bytes read from local (" + localRead + ") than written to HDFS (" + hdfsWritten + ")",
    ...
    public void testReduceFromPartialMem() throws Exception {
        ...
        final long hdfsWritten = c.findCounter(HDFS_WRITE).getCounter();
        final long localRead = c.findCounter(LOCAL_READ).getCounter();
        assertTrue("Expected at least 1MB fewer bytes read from local (" + localRead + ") than written to HDFS ("
                + hdfsWritten + ")", hdfsWritten >= localRead + 1024 * 1024);
    }
