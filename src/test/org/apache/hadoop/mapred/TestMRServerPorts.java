...
public class TestMRServerPorts extends TestCase {
    ...
    private JobTracker startJobTracker(JobConf conf, JTRunner runner) throws IOException {
        ...
        try {
            ...
            conf.set("mapred.job.tracker.http.address", "0.0.0.0:" + jt.getInfoPort());
    ...
    public void testJobTrackerPorts() throws Exception {
        ...
        try {
            ...
            conf2.set("mapred.job.tracker", FileSystem.getDefaultUri(hdfs.getConfig()).toString());
            conf2.set("mapred.job.tracker.http.address", TestHDFSServerPorts.NAME_NODE_HTTP_HOST + 0);
            boolean started = canStartJobTracker(conf2);
            assertFalse(started); // should fail

            // bind http server to the same port as name-node
            conf2.set("mapred.job.tracker", TestHDFSServerPorts.NAME_NODE_HOST + 0);
            conf2.set("mapred.job.tracker.http.address", hdfs.getConfig().get("dfs.http.address"));
