 ...
 * during the Map and Reduce phases, to sort/merge key-value pairs. Another is
 * used to group values for a particular key, when calling the user's reducer. A
 * user can override both of these two. This class has tests for making sure we
 * use the right comparators at the right places. See Hadoop issues 485 and
 * 1535. Our tests: 1. Test that the same comparator is used for all sort/merge
 * operations during the Map and Reduce phases. 2. Test the common use case
 * where values are grouped by keys but values within each key are grouped by a
 * secondary key (a timestamp, for example).
 */
public class TestComparators extends TestCase {
    ...
    /**
     * RandomGen is a mapper that generates 5 random values for each key in the
     * input. The values are in the range [0-4]. The mapper also generates a
     * composite key. If the input key is x and the generated value is y, the
     * composite key is x0y (x-zero-y). Therefore, the inter- mediate key value
     * pairs are ordered by {input key, value}. Think of the random value as a
    ...
    static class RandomGenMapper implements Mapper<IntWritable, Writable, IntWritable, IntWritable> {
        ...
        public void map(IntWritable key, Writable value, OutputCollector<IntWritable, IntWritable> out,
    ...
    static class AscendingKeysReducer implements Reducer<IntWritable, Writable, IntWritable, Text> {
        ...
        public void reduce(IntWritable key, Iterator<Writable> values, OutputCollector<IntWritable, Text> out,
                Reporter reporter) throws IOException {
    ...
    static class AscendingGroupReducer implements Reducer<IntWritable, IntWritable, IntWritable, Text> {
        ...
        public void reduce(IntWritable key, Iterator<IntWritable> values, OutputCollector<IntWritable, Text> out,
                Reporter reporter) throws IOException {
            // check key order
            int currentKey = key.get();
            if (currentKey < lastKey) {
    ...
    /**
     * Grouping function for values based on the composite key. This comparator
     * strips off the secondary key part from the x0y composite and only
     * compares the primary key value (x).
     */
    ...
    /**
     * Test the default comparator for Map/Reduce. Use the identity mapper and
     * see if the keys are sorted at the end
     * 
     * @throws Exception
     */
    ...
    /**
     * Test user-defined comparator for Map/Reduce. We provide our own
     * comparator that is the reverse of the default int comparator. Keys should
     * be sorted in reverse order in the reducer.
     * 
     * @throws Exception
     */
    ...
    /**
     * Test user-defined grouping comparator for grouping values in Reduce. We
     * generate composite keys that contain a random number, which acts as a
     * timestamp associated with the record. In our Reduce function, values for
     * a key should be sorted by the 'timestamp'.
     * 
     * @throws Exception
     */
    ...
    /**
     * Test all user comparators. Super-test of all tests here. We generate
     * composite keys that contain a random number, which acts as a timestamp
     * associated with the record. In our Reduce function, values for a key
     * should be sorted by the 'timestamp'. We also provide our own comparators
     * that reverse the default sorting order. This lets us make sure that the
     * right comparators are used.
     * 
     * @throws Exception
     */
    ...
    /**
     * Test a user comparator that relies on deserializing both arguments for
     * each compare.
     */
