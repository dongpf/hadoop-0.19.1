...
public class TestEmptyJobWithDFS extends TestCase {
    private static final Log LOG = LogFactory.getLog(TestEmptyJobWithDFS.class.getName());

    /**
     * Simple method running a MapReduce job with no input data. Used to test
     * that such a job is successful.
     * 
     * @param fileSys
     * @param jobTracker
     * @param conf
     ...
     * @throws IOException
     */
    public static boolean launchEmptyJob(String fileSys, String jobTracker, JobConf conf, int numMaps, int numReduces)
            throws IOException {
     ...
     * map task to execute) is successful.
     * 
     * @throws IOException
     */
    public void testEmptyJobWithDFS() throws IOException {
        ...
        try {
            ...
            boolean result;
            result = launchEmptyJob(namenode, jobTrackerName, jobConf, 3, 1);
            assertTrue(result);
