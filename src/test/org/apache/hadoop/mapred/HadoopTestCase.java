...
public abstract class HadoopTestCase extends TestCase {
     ...
     * 
     * @param taskTrackers
     *            number of task trackers to start when using cluster
     * 
     * @param dataNodes
     *            number of data nodes to start when using DFS
     * 
     * @throws IOException
    ...
    public HadoopTestCase(int mrMode, int fsMode, int taskTrackers, int dataNodes) throws IOException {
        ...
        /*
         * JobConf conf = new JobConf(); fsRoot = conf.get("hadoop.tmp.dir");
         * 
         * if (fsRoot == null) { throw new IllegalArgumentException(
         * "hadoop.tmp.dir is not defined"); }
         * 
         * fsRoot = fsRoot.replace(' ', '+') + "/fs";
         * 
         * File file = new File(fsRoot); if (!file.exists()) { if
         * (!file.mkdirs()) { throw new
         * RuntimeException("Could not create FS base path: " + file); } }
         */
    ...
    /**
     * Destroys Hadoop instance based on constructor configuration after a test
     * case is run.
     * 
     * @throws Exception
     */
     ...
     * 
     * TestCases should use this Filesystem as it is properly configured with
     * the workingDir for relative PATHs.
     * 
