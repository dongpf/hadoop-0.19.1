...
public class TestRackAwareTaskPlacement extends TestCase {
     ...
     * @param numMaps
     *            Number of maps
     * @param otherLocalMaps
     *            Expected value of other local maps
     * @param datalocalMaps
     *            Expected value of data(node) local maps
     * @param racklocalMaps
     *            Expected value of rack local maps
     */
    static void launchJobAndTestCounters(String jobName, MiniMRCluster mr, FileSystem fileSys, Path in, Path out,
            int numMaps, int otherLocalMaps, int dataLocalMaps, int rackLocalMaps) throws IOException {
        ...
        assertEquals("Number of local maps", counters.getCounter(JobInProgress.Counter.OTHER_LOCAL_MAPS),
                otherLocalMaps);
        assertEquals("Number of Data-local maps", counters.getCounter(JobInProgress.Counter.DATA_LOCAL_MAPS),
                dataLocalMaps);
        assertEquals("Number of Rack-local maps", counters.getCounter(JobInProgress.Counter.RACK_LOCAL_MAPS),
                rackLocalMaps);
        mr.waitUntilIdle();
        mr.shutdown();
    }
    ...
    public void testTaskPlacement() throws IOException {
        ...
        try {
             ...
             * files (splits). 1) file1, just after starting the datanode on r1,
             * with a repl factor of 1, and, 2) file2 & file3 after starting the
             * other two datanodes, with a repl factor of 3. At the end, file1
             * will be present on only datanode1, and, file2 and file3, will be
             ...
             * of the tasktracker will determine how many data/rack local maps
             * it runs. The hostname of the tasktracker is set to same as one of
             * the datanodes.
             */
             ...
             * present on all the datanodes, in particular, the datanodes on
             * rack2. The third input file is pulled from rack1.
             */
            ...
            /*
             * The job is configured with three maps since there are three
             * (non-splittable) files. On rack1, because of the way in which
             * repl was setup while creating the files, we will have all the
             * three files. Thus, a tasktracker will find all inputs in this
             * rack.
             */
    ...
    static void writeFile(NameNode namenode, Configuration conf, Path name, short replication) throws IOException {
        ...
        SequenceFile.Writer writer = SequenceFile.createWriter(fileSys, conf, name, BytesWritable.class,
    ...
    static RunningJob launchJob(JobConf jobConf, Path inDir, Path outputPath, int numMaps, String jobName)
            throws IOException {
