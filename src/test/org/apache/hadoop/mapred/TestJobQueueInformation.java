...
public class TestJobQueueInformation extends TestCase {
    ...
    private JobConf configureWaitingJob(JobConf conf) throws IOException {
        ...
        return TestJobTrackerRestart.getJobs(conf, priority, new int[] { 2 }, new int[] { 0 }, outputDir, inDir,
    ...
    public static class TestTaskScheduler extends LimitTasksPerJobTaskScheduler {
        ...
        public synchronized List<Task> assignTasks(TaskTrackerStatus taskTracker) throws IOException {
            Collection<JobInProgress> jips = jobQueueJobInProgressListener.getJobQueue();
    ...
    protected void setUp() throws Exception {
        ...
        jc = new JobConf();
        jc.setClass("mapred.jobtracker.taskScheduler", TestTaskScheduler.class, TaskScheduler.class);
        jc.setLong("mapred.jobtracker.taskScheduler.maxRunningTasksPerJob", 10L);
        mrCluster = new MiniMRCluster(0, 0, taskTrackers, dfsCluster.getFileSystem().getUri().toString(), 1, null,
                null, null, jc);
    }
