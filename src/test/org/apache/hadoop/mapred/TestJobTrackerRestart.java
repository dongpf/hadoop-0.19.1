...
/**
 * TestJobTrackerRestart checks if the jobtracker can restart. JobTracker should
 * be able to continue running the previously running jobs and also recover
 * previosuly submitted jobs.
 */
public class TestJobTrackerRestart extends TestCase {
     ...
     *            The default conf
     * @param priorities
     *            priorities for the jobs
     * @param numMaps
     *            number of maps for the jobs
     * @param numReds
     *            number of reducers for the jobs
     * @param outputDir
     *            output dir
     * @param inDir
     *            input dir
     * @param mapSignalFile
     *            filename thats acts as a signal for maps
     * @param reduceSignalFile
     *            filename thats acts as a signal for reducers
     * @return a array of jobconfs configured as needed
     * @throws IOException
     */
    static JobConf[] getJobs(JobConf conf, JobPriority[] priorities, int[] numMaps, int[] numReds, Path outputDir,
            Path inDir, String mapSignalFile, String reduceSignalFile) throws IOException {
        ...
        for (int i = 0; i < jobs.length; ++i) {
            ...
            configureWaitingJobConf(jobs[i], inDir, newOutputDir, numMaps[i], numReds[i], "jt-restart-test-job",
     ...
     */
    static void signalTasks(MiniDFSCluster dfs, FileSystem fileSys, boolean isMap, String mapSignalFile,
            String reduceSignalFile) throws IOException {
        // signal the maps to complete
        TestRackAwareTaskPlacement.writeFile(dfs.getNameNode(), fileSys.getConf(), isMap ? new Path(mapSignalFile)
     ...
     * Preparation : - Configure 3 jobs as follows [format {prio, maps,
     * reducers}] - job1 : {normal, 50, 1} - job2 : {low, 1, 1} - job3 : {high,
     * 1, 1} - Configure the cluster to run 1 reducer - Lower the history file
     * block size and buffer
     * 
     * Submit these 3 jobs but make sure that job1's priority is changed and
     * job1 is RUNNING before submitting other jobs The order in which the jobs
     * will be executed will be job1, job3 and job2.
     * 
     * Above ordering makes sure that job1 runs before everyone else. Wait for
     * job1 to complete 50%. Note that all the jobs are configured to use
     * {@link HalfWaitingMapper} and {@link WaitingReducer}. So job1 will
     * eventually wait on 50%
     * 
     * Make a note of the following things - Job start times
     * 
     * Restart the jobtracker
     * 
     * Wait for job1 to finish all the maps and note the TaskCompletion events
     * at the tracker.
     * 
     * Wait for all the jobs to finish
     * 
     * Also make sure that the order in which the jobs were sorted before
     * restart remains same. For this check the follwoing job1.start-time <
     * job2.start-time < job3.start-time and job1.finish-time < job3.finish-time
     * < job2.finish-time This ordering makes sure that the change of priority
     * is logged and recovered back
     */
    public void testRecoveryWithMultipleJobs(MiniDFSCluster dfs, MiniMRCluster mr) throws IOException {
        ...
        // Configure the jobs
        JobConf[] jobs = getJobs(jobConf, priorities, numMaps, numReds, outputDir, inDir, getMapSignalFile(shareDir),
        ...
        assertEquals("Restart failed as previously submitted job was missing", true,
                getJobStatus(jobClient, id) != null);

        // check if the job's priority got changed
        assertEquals("Restart failed as job's priority did not match", true,
                mr.getJobPriority(id).equals(masterJobNewPriority));
        ...
        boolean jobOrder = newStatuses[0].getJobID().getId() == 1 && newStatuses[1].getJobID().getId() == 2
        ...
        boolean startTimeOrder = newStatuses[0].getStartTime() > 0
                && newStatuses[0].getStartTime() < newStatuses[1].getStartTime()
                && newStatuses[1].getStartTime() < newStatuses[2].getStartTime();
        assertTrue("Job start-times are out of order", startTimeOrder);

        boolean finishTimeOrder = mr.getJobFinishTime(newStatuses[0].getJobID()) > 0
                && mr.getJobFinishTime(newStatuses[0].getJobID()) < mr.getJobFinishTime(newStatuses[2].getJobID())
                && mr.getJobFinishTime(newStatuses[2].getJobID()) < mr.getJobFinishTime(newStatuses[1].getJobID());
    ...
    /**
     * Tests the jobtracker with restart-recovery turned off. Submit a job with
     * normal priority, maps = 2, reducers = 0}
     * 
    ...
    public void testRestartWithoutRecovery(MiniDFSCluster dfs, MiniMRCluster mr) throws IOException {
        ...
        JobConf newConf = getJobs(mr.createJobConf(), new JobPriority[] { JobPriority.NORMAL }, new int[] { 2 },
                new int[] { 0 }, outputDir, inDir, getMapSignalFile(shareDir), getReduceSignalFile(shareDir))[0];
        ...
        assertTrue("Submitted job was detected with recovery disabled", getJobStatus(jobClient, id) == null);
    }

    /**
     * Tests a job on jobtracker with restart-recovery turned on. Preparation :
     * - Configure a job with - num-maps : 50 - num-reducers : 1 - Configure the
     * cluster to run 1 reducer - Lower the history file block size and buffer
     * 
     ...
     * eventually wait on 50%
     * 
     * Make a note of the following things - Task completion events - Cluster
     * status - Task Reports - Job start time
     * 
     * Restart the jobtracker
     * 
     ...
     * the tracker.
     * 
     * Wait for all the jobs to finish and note the following - New task
     * completion events at the jobtracker - Task reports - Cluster status
     * 
     * Check for the following - Task completion events for recovered tasks
     * should match - Task completion events at the tasktracker and the
     * restarted jobtracker should be same - Cluster status should be fine. -
     * Task Reports for recovered tasks should match Checks - start time -
     * finish time - counters - http-location - task-id - Job start time should
     * match - Check if the counters can be accessed - Check if the history
     * files are (re)named properly
     */
    public void testTaskEventsAndReportsWithRecovery(MiniDFSCluster dfs, MiniMRCluster mr) throws IOException {
        ...
        JobConf newConf = getJobs(mr.createJobConf(), new JobPriority[] { JobPriority.NORMAL }, new int[] { numMaps },
                new int[] { numReducers }, outputDir, inDir, getMapSignalFile(shareDir), getReduceSignalFile(shareDir))[0];
        ...
        while (true) {
            // Since we are using a half waiting mapper, maps should be stuck at
            // 50%
            TaskCompletionEvent[] trackerEvents = mr.getMapTaskCompletionEventsUpdates(0, id, numMaps)
     ...
     *            job conf
     */
    private void testJobHistoryFiles(JobID id, JobConf conf) throws IOException {
        // Get the history files for users
        String logFileName = JobHistory.JobInfo.getJobHistoryFileName(conf, id);
        String tempLogFileName = JobHistory.JobInfo.getSecondaryJobHistoryFile(logFileName);

        // I. User files
        Path logFile = JobHistory.JobInfo.getJobHistoryLogLocationForUser(logFileName, conf);
        ...
        Path tempLogFile = JobHistory.JobInfo.getJobHistoryLogLocationForUser(tempLogFileName, conf);
     ...
     * 
     * @param source
     *            the reports to be matched
     * @param target
     *            reports to match with
     * @param numToMatch
     *            num reports to match
     * @param mismatchSet
     *            reports that should not match
     */
    private void testTaskReports(TaskReport[] source, TaskReport[] target, int numToMatch) {
        for (int i = 0; i < numToMatch; ++i) {
            ...
            assertTrue("Task reports for same attempt has changed", source[i].equals(target[i]));
        }
    }
     ...
     * 
     * @param source
     *            the events to be matched
     * @param target
     *            events to match with
     * @param fullMatch
     *            whether to match the events completely or partially
     * @param numToMatch
     *            number of events to match in case full match is not desired
     * @param ignoreSet
     *            a set of taskids to ignore
     */
    private void testTaskCompletionEvents(TaskCompletionEvent[] source, TaskCompletionEvent[] target,
            boolean fullMatch, int numToMatch) {
        // Check if the event list size matches
        // The lengths should match only incase of full match
        if (fullMatch) {
            assertEquals("Map task completion events mismatch", source.length, target.length);
            numToMatch = source.length;
        }
        // Check if the events match
        for (int i = 0; i < numToMatch; ++i) {
            if (source[i].getTaskAttemptId().equals(target[i].getTaskAttemptId())) {
                assertTrue("Map task completion events ordering mismatch", source[i].equals(target[i]));
            }
        }
    }

    private boolean checkClusterStatusOnCompletion(ClusterStatus status, ClusterStatus prevStatus) {
        return status.getJobTrackerState() == prevStatus.getJobTrackerState() && status.getMapTasks() == 0
    ...
    static class HalfWaitingMapper extends MapReduceBase implements
         ...
         */
        public void map(WritableComparable key, Writable val, OutputCollector<WritableComparable, Writable> output,
    ...
    /**
     * Reduce that just waits for a file to be created on the dfs. The file
     * creation is a signal to the reduce.
     */

    static class WaitingReducer extends MapReduceBase implements
        ...
        /**
         * The waiting function. The reduce exits once it gets a signal. Here
         * the signal is the file existence.
         */
    ...
    static void configureWaitingJobConf(JobConf jobConf, Path inDir, Path outputPath, int numMaps, int numRed,
            String jobName, String mapSignalFilename, String redSignalFilename) throws IOException {
