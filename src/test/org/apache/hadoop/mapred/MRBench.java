...
public class MRBench {
    ...
    /**
     * Takes input format as text lines, runs some processing on it and writes
     * out data as text again.
     */
    public static class Map extends MapReduceBase implements Mapper<WritableComparable, Text, UTF8, UTF8> {

        public void map(WritableComparable key, Text value, OutputCollector<UTF8, UTF8> output, Reporter reporter)
                throws IOException {
     ...
     * The generated data are string representations of numbers. Each line is
     * the same length, which is achieved by padding each number with an
     * appropriate number of leading '0' (zero) characters. The order of
     * generated data is one of ascending, descending, or random.
     */
    public static void generateTextFile(FileSystem fs, Path inputFile, long numLines, Order sortOrder)
            throws IOException {
    ...
    /**
     * Convert the given number to a string and pad the number with leading '0'
     * (zero) characters so that the string is exactly the given length.
     */
    ...
    /**
     * Runs a MapReduce task, given number of times. The input to each run is
     * the same file.
     */
    private static ArrayList<Long> runJobInSequence(JobConf masterJobConf, int numRuns) throws IOException {
        ...
        for (int i = 0; i < numRuns; i++) {
            ...
            LOG.info("Running job " + i + ":" + " input=" + FileInputFormat.getInputPaths(jobConf)[0] + " output="
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (numRuns < 1 || // verify args
                numMaps < 1 || numReduces < 1 || inputLines < 0 || inputSortOrder == null) {
            System.err.println(usage);
            System.exit(-1);
        }
