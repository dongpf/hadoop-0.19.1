 ...
 * 
 * Starts up hadoop on Local or Cluster mode (by extending of the HadoopTestCase
 * class) and it starts a servlet engine that hosts a servlet that will receive
 * the notification of job finalization.
 * 
 * The notification servlet returns a HTTP 400 the first time is called and a
 * HTTP 200 the second time, thus testing retry.
 * 
 * In both cases local file system is used (this is irrelevant for the tested
 * functionality)
 * 
 * 
 */
public abstract class NotificationTestCase extends HadoopTestCase {
    ...
    public static class NotificationServlet extends HttpServlet {
        ...
        protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
            if (counter == 0) {
                stdPrintln((new Date()).toString() + "Receiving First notification for [" + req.getQueryString()
                        + "], returning error");
                res.sendError(HttpServletResponse.SC_BAD_REQUEST, "forcing error");
            } else {
                stdPrintln((new Date()).toString() + "Receiving Second notification for [" + req.getQueryString()
                        + "], returning OK");
    ...
    private String launchWordCount(JobConf conf, String input, int numMaps, int numReduces) throws IOException {
        ...
        // Hack for local FS that does not have the concept of a 'mounting
        // point'
        if (isLocalFS()) {
