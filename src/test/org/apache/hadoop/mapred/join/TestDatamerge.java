...
public class TestDatamerge extends TestCase {
    ...
    private static SequenceFile.Writer[] createWriters(Path testdir, Configuration conf, int srcs, Path[] src)
            throws IOException {
        ...
        for (int i = 0; i < srcs; ++i) {
            out[i] = new SequenceFile.Writer(testdir.getFileSystem(conf), conf, src[i], IntWritable.class,
                    IntWritable.class);
        }
        return out;
    }
    ...
    private static abstract class SimpleCheckerBase<V extends Writable> implements
            Mapper<IntWritable, V, IntWritable, IntWritable>, Reducer<IntWritable, IntWritable, Text, Text> {
        ...
        public abstract void map(IntWritable key, V val, OutputCollector<IntWritable, IntWritable> out,
                Reporter reporter) throws IOException;

        public void reduce(IntWritable key, Iterator<IntWritable> values, OutputCollector<Text, Text> output,
                Reporter reporter) throws IOException {
            int seen = 0;
    ...
    private static class InnerJoinChecker extends SimpleCheckerBase<TupleWritable> {
        public void map(IntWritable key, TupleWritable val, OutputCollector<IntWritable, IntWritable> out,
                Reporter reporter) throws IOException {
            int k = key.get();
    ...
    private static class OverrideChecker extends SimpleCheckerBase<IntWritable> {
        public void map(IntWritable key, IntWritable val, OutputCollector<IntWritable, IntWritable> out,
                Reporter reporter) throws IOException {
            int k = key.get();
    ...
    private static void joinAs(String jointype, Class<? extends SimpleCheckerBase> c) throws Exception {
        ...
        job.set("mapred.join.expr", CompositeInputFormat.compose(jointype, SequenceFileInputFormat.class, src));
    ...
    public void testEmptyJoin() throws Exception {
        ...
        job.set("mapred.join.expr", CompositeInputFormat.compose("outer", Fake_IF.class, src));
        job.setInputFormat(CompositeInputFormat.class);
        FileOutputFormat.setOutputPath(job, new Path(base, "out"));
    ...
    public static class Fake_IF<K, V> implements InputFormat<K, V>, JobConfigurable {
        ...
        public void configure(JobConf job) {
            keyclass = (Class<? extends K>) job.getClass("test.fakeif.keyclass", IncomparableKey.class,
                    WritableComparable.class);
            valclass = (Class<? extends V>) job.getClass("test.fakeif.valclass", NullWritable.class,
                    WritableComparable.class);
        }
