...
public class TestJobInProgressListener extends TestCase {
    ...
    private JobConf configureJob(JobConf conf, int m, int r, Path inDir, Path outputDir, String mapSignalFile,
            String redSignalFile) throws IOException {
        TestJobTrackerRestart.configureWaitingJobConf(conf, inDir, outputDir, m, r, "job-listener-test", mapSignalFile,
                redSignalFile);
        return conf;
    }
     ...
     * undesirable effects Test is as follows - submit 2 jobs of normal
     * priority. job1 is a waiting job which waits and blocks the cluster -
    ...
    public void testJobQueueChanges() throws IOException {
        ...
        // add the listener
        mr.getJobTrackerRunner().getJobTracker().addJobInProgressListener(myListener);

        // big blocking job
        Path outputDir = new Path(testDir, "output");
        Path newOutputDir = outputDir.suffix("0");
        JobConf job1 = configureJob(mr.createJobConf(), 10, 0, inDir, newOutputDir, mapSignalFile, redSignalFile);

        // short blocked job
        newOutputDir = outputDir.suffix("1");
        JobConf job2 = configureJob(mr.createJobConf(), 1, 0, inDir, newOutputDir, mapSignalFile, redSignalFile);
        ...
        assertTrue("Job completion garbles the queue", myListener.getJobQueue().size() == 0);
    }
    ...
    public void testJobSuccess() throws Exception {
        ...
        assertFalse("Missing event notification for a running job", myListener.contains(rJob.getID(), true));
        ...
        assertFalse("Missing event notification for a successful job", myListener.contains(rJob.getID(), false));
    }
}