...
public class TestDataTransferProtocol extends TestCase {
    ...
    private void sendRecvData(String testDescription, boolean eofExpected) throws IOException {
        /*
         * Opens a socket to datanode sends the data in sendBuf. If there is
         * data in expectedBuf, expects to receive the data from datanode that
         * matches expectedBuf. If there is an exception while recieving, throws
         * it only if exceptionExcepted is false.
         */
    ...
    public void testDataTransferProtocol() throws IOException {
        ...
        // negative length is ok. Datanode assumes we want to read the whole
        // block.
        recvBuf.reset();
        recvOut.writeShort((short) DataTransferProtocol.OP_STATUS_SUCCESS);
        sendBuf.reset();
        ...
        // At the end of all this, read the file to make sure that succeeds
        // finally.
        sendBuf.reset();
        sendOut.writeShort((short) DataTransferProtocol.DATA_TRANSFER_VERSION);
        sendOut.writeByte((byte) DataTransferProtocol.OP_READ_BLOCK);
