...
/**
 * A JUnit test for corrupted file handling. This test creates a bunch of
 * files/directories with replication factor of 2. Then verifies that a client
 * can automatically access the remaining valid replica inspite of the following
 * types of simulated errors:
 * 
 * 1. Delete meta file on one replica 2. Truncates meta file on one replica 3.
 * Corrupts the meta file header on one replica 4. Corrupts any random offset
 * and portion of the meta file 5. Swaps two meta files, i.e the format of the
 * meta files are valid but their CRCs do not match with their corresponding
 * data blocks The above tests are run for varied values of
 * io.bytes.per.checksum and dfs.block.size. It tests for the case when the meta
 * file is multiple blocks.
 * 
 ...
 * replication factor from 2 to 1. Validate that the remaining replica is the
 * good one. 2. Create file with 2 replica, corrupt the meta file of one
 * replica, increase replication factor of file to 3. verify that the new
        ...
        try {
            ...
            for (int idx = 0; idx < blocks.length; idx++) {
                if (blocks[idx].getName().startsWith("blk_") && blocks[idx].getName().endsWith(".meta")) {
                    ...
                    if (num % 3 == 0) {
                        ...
                        System.out.println("Deliberately truncating file " + blocks[idx].getName() + " to size "
                                + newsize + " bytes.");
                        ...
                        long position = 0;
                        //
                        // The very first time, corrupt the meta header at
                        // offset 0
                        //
                        if (num != 2) {
                        ...
                        System.out.println("Deliberately corrupting file " + blocks[idx].getName() + " at offset "
            ...
            for (int idx = 0; idx < blocks.length; idx++) {
                if (blocks[idx].getName().startsWith("blk_") && blocks[idx].getName().endsWith(".meta")) {
                    ...
                    count++;
                    if (count % 2 == 0) {
                        System.out.println("Deliberately insertimg bad crc into files " + blocks[idx].getName() + " "
                                + previous.getName());
                        assertTrue("Cannot remove file.", blocks[idx].delete());
            ...
            //
            // Only one replica is possibly corrupted. The other replica should
            // still
            // be good. Verify.
            //
            ...
            // the corrupted replica generated above is the one that gets
            // deleted.
