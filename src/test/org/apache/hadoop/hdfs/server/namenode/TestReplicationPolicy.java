...
public class TestReplicationPolicy extends TestCase {
    ...
    private static DatanodeDescriptor dataNodes[] = new DatanodeDescriptor[] {
    ...
    private final static DatanodeDescriptor NODE = new DatanodeDescriptor(new DatanodeID("h7:5020"), "/d2/r4");
    ...
    static {
        ...
        for (int i = 0; i < NUM_OF_DATANODES; i++) {
            dataNodes[i].updateHeartbeat(2 * FSConstants.MIN_BLOCKS_FOR_WRITE * BLOCK_SIZE, 0L, 2
     ...
     * placed on dataNodes[0], the 2nd replica should be placed on different
     * rack and third should be placed on different node of rack chosen for 2nd
     * node. The only excpetion is when the <i>numOfReplicas</i> is 2, the 1st
     * is on dataNodes[0] and the 2nd is on a different rack.
     * 
     * @throws Exception
     */
    ...
    /**
     * In this testcase, client is dataNodes[0], but the dataNodes[1] is not
     * allowed to be chosen. So the 1st replica should be placed on
     * dataNodes[0], the 2nd replica should be placed on a different rack, the
     * 3rd should be on same rack as the 2nd replica, and the rest should be
     * placed on a third rack.
     * 
     * @throws Exception
     */
     ...
     * dataNodes[1], the 2nd replica should be placed on a different rack, the
     * 3rd replica should be placed on the same rack as the 2nd replica, and the
     * rest should be placed on the third rack.
     * 
     * @throws Exception
     */
    public void testChooseTarget3() throws Exception {
        ...
        dataNodes[0].updateHeartbeat(2 * FSConstants.MIN_BLOCKS_FOR_WRITE * BLOCK_SIZE, 0L,
                (FSConstants.MIN_BLOCKS_FOR_WRITE - 1) * BLOCK_SIZE, 0); // no
                                                                         // space
     ...
     * rack 2 or rack 3. the 2nd replica should be placed on a different rack,
    ...
    /**
     * In this testcase, client is is a node outside of file system. So the 1st
     * replica can be placed on any node. the 2nd replica should be placed on a
     * different rack, the 3rd replica should be placed on the same rack as the
     * 2nd replica,
     * 
     * @throws Exception
     */
    public void testChooseTarget5() throws Exception {
        DatanodeDescriptor[] targets;
        targets = replicator.chooseTarget(0, NODE, null, BLOCK_SIZE);
        assertEquals(targets.length, 0);

        targets = replicator.chooseTarget(1, NODE, null, BLOCK_SIZE);
        assertEquals(targets.length, 1);

        targets = replicator.chooseTarget(2, NODE, null, BLOCK_SIZE);
        assertEquals(targets.length, 2);
        assertFalse(cluster.isOnSameRack(targets[0], targets[1]));

        targets = replicator.chooseTarget(3, NODE, null, BLOCK_SIZE);
        assertEquals(targets.length, 3);
        assertTrue(cluster.isOnSameRack(targets[1], targets[2]));
        assertFalse(cluster.isOnSameRack(targets[0], targets[1]));
    }
     ...
     * So the 1st replica can be placed on random rack. the 2nd replica should
     * be placed on different node by same rack as the 1st replica. The 3rd
     * replica can be placed randomly.
     * 
     * @throws Exception
     */
    public void testRereplicate1() throws Exception {
        ...
        targets = replicator.chooseTarget(3, dataNodes[0], chosenNodes, null, BLOCK_SIZE);
        assertEquals(targets.length, 3);
        assertTrue(cluster.isOnSameRack(dataNodes[0], targets[0]));
        assertFalse(cluster.isOnSameRack(targets[0], targets[2]));
    }
    ...
    /**
     * This testcase tests re-replication, when dataNodes[0] and dataNodes[1]
     * are already chosen. So the 1st replica should be placed on a different
     * rack than rack 1. the rest replicas can be placed randomly,
     * 
     * @throws Exception
     */
    ...
    /**
     * This testcase tests re-replication, when dataNodes[0] and dataNodes[2]
     * are already chosen. So the 1st replica should be placed on the rack that
     * the writer resides. the rest replicas can be placed randomly,
     * 
     * @throws Exception
     */
