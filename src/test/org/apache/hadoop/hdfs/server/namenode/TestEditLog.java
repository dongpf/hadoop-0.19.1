...
public class TestEditLog extends TestCase {
    ...
    static class Transactions implements Runnable {
        ...
        public void run() {
            ...
            for (int i = 0; i < numTransactions; i++) {
                try {
                    INodeFileUnderConstruction inode = new INodeFileUnderConstruction(p, replication, blockSize, 0, "",
                            "", null);
    ...
    public void testEditLog() throws IOException {
        ...
        for (Iterator<StorageDirectory> it = fsimage.dirIterator(NameNodeDirType.EDITS); it.hasNext();) {
            ...
            assertTrue("Verification for " + editFile + " failed. " + "Expected "
                    + (numThreads * 2 * numberTransactions) + " transactions. " + "Found " + numEdits
                    + " transactions.", numEdits == numThreads * 2 * numberTransactions);
