...
/**
 * This class tests various combinations of dfs.name.dir and dfs.name.edits.dir
 * configurations.
 */
public class TestNameEditsConfigs extends TestCase {
     ...
     * The test creates files and restarts cluster with different configs. 1.
     * Starts cluster with shared name and edits dirs 2. Restarts cluster by
     * adding additional (different) name and edits dirs 3. Restarts cluster by
     * removing shared name and edits dirs by allowing to start using separate
     * name and edits dirs 4. Restart cluster by adding shared directory again,
     * but make sure we do not read any stale image or edits. All along the
     * test, we create and delete files at reach restart to make sure we are
     * reading proper edits and image.
     */
    public void testNameEditsConfigs() throws IOException {
        ...
        cluster = new MiniDFSCluster(0, conf, NUM_DATA_NODES, true, false, true, null, null, null, null);
        cluster.waitActive();
        ...
        cluster = new MiniDFSCluster(0, conf, NUM_DATA_NODES, false, false, true, null, null, null, null);
        cluster.waitActive();
        secondary = startSecondaryNameNode(conf);
        fileSys = cluster.getFileSystem();
        ...
        // Add old name_and_edits dir. File system should not read image or
        // edits
        // from old dir
    ...
    /**
     * Test various configuration options of dfs.name.dir and dfs.name.edits.dir
     * This test tries to simulate failure scenarios. 1. Start cluster with
     * shared name and edits dir 2. Restart cluster by adding separate name and
     * edits dirs 3. Restart cluster by removing shared name and edits dir 4.
     * Restart cluster with old shared name and edits dir, but only latest name
     * dir. This should fail since we dont have latest edits dir 5. Restart
     * cluster with old shared name and edits dir, but only latest edits dir.
     * This should fail since we dont have latest name dir
     */
