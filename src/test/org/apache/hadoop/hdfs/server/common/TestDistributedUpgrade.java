...
public class TestDistributedUpgrade extends TestCase {
    ...
    void startNameNodeShouldFail(StartupOption operation) {
        try {
            ...
            // we set manage dirs to true as NN has to start from untar'ed image
            // with
            // nn dirs set to name1 and name2
            cluster = new MiniDFSCluster(0, conf, 0, false, true, operation, null); // Should
                                                                                    // fail
   ...
   */
    public void testDistributedUpgrade() throws Exception {
        ...
        if (System.getProperty("test.build.data") == null) { // to test to be
                                                             // run outside of
                                                             // ant
...
class UO_Datanode extends UpgradeObjectDatanode {
    ...
    public void doUpgrade() throws IOException {
        ...
        getDatanode().namenode.processUpgradeCommand(new UpgradeCommand(UpgradeCommand.UC_ACTION_REPORT_STATUS,
