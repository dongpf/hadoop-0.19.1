...
public class TestBalancer extends TestCase {
    ...
    /*
     * fill up a cluster with <code>numNodes</code> datanodes whose used space
     * to be <code>size</code>
     */
    ...
    Block[][] distributeBlocks(Block[] blocks, short replicationFactor, final long[] distribution) {
        // make a copy
    ...
    /*
     * we first start a cluster and fill the cluster up to a certain size. then
     * redistribute blocks according the required distribution. Afterwards a
     * balancer is running to balance the cluster.
     */
    private void testUnevenDistribution(long distribution[], long capacities[], String[] racks) throws Exception {
    ...
    /*
     * This test start a one-node cluster, fill the node to be 30% full; It then
     * adds an empty node and start balancing.
     * 
     * @param newCapacity new node's capacity
     * 
     * @param new
     */
    private void test(long[] capacities, String[] racks, long newCapacity, String newRack) throws Exception {
        ...
        try {
            ...
            // start up an empty node with the same capacity and on the same
            // rack
    ...
    private void runBalancer(long totalUsedSpace, long totalCapacity) throws Exception {
        ...
        do {
            ...
            for (DatanodeInfo datanode : datanodeReport) {
                if (Math.abs(avgUtilization - ((double) datanode.getDfsUsed()) / datanode.getCapacity() * 100) > 10) {
                    balanced = false;
                    try {
                        Thread.sleep(100);
                    } catch (InterruptedException ignored) {
    ...
    public void testBalancer0() throws Exception {
        ...
        test(new long[] { CAPACITY, CAPACITY }, new String[] { RACK0, RACK1 }, CAPACITY, RACK2);
    }

    /** Test unevenly distributed cluster */
    public void testBalancer1() throws Exception {
        testUnevenDistribution(new long[] { 50 * CAPACITY / 100, 10 * CAPACITY / 100 },
