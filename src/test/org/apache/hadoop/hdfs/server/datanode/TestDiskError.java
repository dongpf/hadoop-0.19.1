...
public class TestDiskError extends TestCase {
    public void testShutdown() throws Exception {
        if (System.getProperty("os.name").startsWith("Windows")) {
            /**
             * This test depends on OS not allowing file creations on a
             * directory that does not have write permissions for the user.
             * Apparently it is not the case on Windows (at least under Cygwin),
    ...
    public void testReplicationError() throws Exception {
        ...
        try {
            ...
            LocatedBlocks blocks = cluster.getNameNode().namesystem.getBlockLocations(fileName.toString(), 0,
                    (long) fileLen);
