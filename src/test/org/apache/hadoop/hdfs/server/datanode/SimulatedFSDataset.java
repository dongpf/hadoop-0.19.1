 ...
 * discarded. Fixed data is returned when blocks are read; a null CRC meta file
 * is created for such data.
 * 
 * This FSDataset does not remember any block information across its restarts;
 * it does however offer an operation to inject blocks (See the
 * TestInectionForSImulatedStorage() for a usage example of injection.
 * 
...
public class SimulatedFSDataset implements FSConstants, FSDatasetInterface, Configurable {
    ...
    {
        DataChecksum checksum = DataChecksum.newDataChecksum(DataChecksum.CHECKSUM_NULL, 16 * 1024);
    ...
    private class BInfo { // information about a single block
        Block theBlock;
        private boolean finalized = false; // if not finalized => ongoing
                                           // creation
        ...
        BInfo(Block b, boolean forWriting) throws IOException {
            ...
            if (!storage.alloc(theBlock.getNumBytes())) { // expected length -
                                                          // actual length may
    ...
    public void setConf(Configuration iconf) {
        conf = iconf;
        storageId = conf.get("StorageId", "unknownStorageId" + new Random().nextInt());
        registerMBean(storageId);
        storage = new SimulatedStorage(conf.getLong(CONFIG_PROPERTY_CAPACITY, DEFAULT_CAPACITY));
        // DataNode.LOG.info("Starting Simulated storage; Capacity = " +
        // getCapacity() +
    ...
    public synchronized void injectBlocks(Block[] injectBlocks) throws IOException {
        if (injectBlocks != null) {
            for (Block b : injectBlocks) { // if any blocks in list is bad,
                                           // reject list
                if (b == null) {
    ...
    public synchronized BlockWriteStreams writeToBlock(Block b, boolean isRecovery) throws IOException {
        if (isValidBlock(b)) {
    ...
    public synchronized InputStream getBlockInputStream(Block b) throws IOException {
        ...
        // DataNode.LOG.info("Opening block(" + b.blkid + ") of length " +
        // b.len);
     ...
     * 
     * @param b
     *            - the block for which the metadata is desired
     * @return metaData of block b as an input stream
     * @throws IOException
     *             - block does not exist or problems accessing the meta file
     */
    ...
    public MetaDataInputStream getMetaDataInputStream(Block b) throws IOException {
        ...
        return new MetaDataInputStream(getMetaDataInStream(b), getMetaDataLength(b));
    }
    ...
    public synchronized long getChannelPosition(Block b, BlockWriteStreams stream) throws IOException {
        BInfo binfo = blockMap.get(b);
        if (binfo == null) {
            throw new IOException("No such Block " + b);
        }
    ...
    public synchronized void setChannelPosition(Block b, BlockWriteStreams stream, long dataOffset, long ckOffset)
            throws IOException {
        BInfo binfo = blockMap.get(b);
        if (binfo == null) {
            throw new IOException("No such Block " + b);
     ...
     * @param storageId
     * 
     *            We use storage id for MBean name since a minicluster within a
     *            single Java VM may have multiple Simulated Datanodes.
     */
    void registerMBean(String storageId) {
        ...
        try {
            ...
            mbeanName = MBeanUtil.registerMBean("DataNode-" + storageId, "FSDatasetStatus", bean);
