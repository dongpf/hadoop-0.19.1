...
public class TestSimulatedFSDataset extends TestCase {
    ...
    int addSomeBlocks(FSDatasetInterface fsdataset, int startingBlockId) throws IOException {
        ...
        for (int i = startingBlockId; i < startingBlockId + NUMBLOCKS; ++i) {
            Block b = new Block(i, 0, 0); // we pass expected len as zero, -
                                          // fsdataset should use the sizeof
                                          // actual data written
            OutputStream dataOut = fsdataset.writeToBlock(b, false).dataOut;
            assertEquals(0, fsdataset.getLength(b));
            for (int j = 1; j <= blockIdToLen(i); ++j) {
                dataOut.write(j);
                assertEquals(j, fsdataset.getLength(b)); // correct length even
                                                         // as we write
                bytesAdded++;
            }
            dataOut.close();
    ...
    void checkBlockDataAndSize(FSDatasetInterface fsdataset, Block b, long expectedLen) throws IOException {
    ...
    public void testInjectionNonEmpty() throws IOException {
        ...
        // Now test that the dataset cannot be created if it does not have
        // sufficient cap
