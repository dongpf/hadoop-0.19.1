...
public class TestDFSShell extends TestCase {
    ...
    public void testPut() throws IOException {
        ...
        try {
            ...
            // use SecurityManager to pause the copying of f1 and begin copying
            // f2
    ...
    public void testErrOutPut() throws Exception {
        ...
        try {
            ...
            argv[1] = "/nonexistentfile";
            ret = ToolRunner.run(shell, argv);
            assertTrue(" -lsr should fail ", (ret < 0));
            out.reset();
            ...
            ret = ToolRunner.run(shell, argv);
            assertTrue(" -ls on nonexistent glob returns -1", (ret < 0));
            out.reset();
            argv[0] = "-mkdir";
            argv[1] = "/testdir";
            ret = ToolRunner.run(shell, argv);
    ...
    public void testCopyToLocal() throws IOException {
        Configuration conf = new Configuration();
        /*
         * This tests some properties of ChecksumFileSystem as well. Make sure
         * that we create ChecksumDFS
         */
    ...
    private void runCount(String path, long dirs, long files, Configuration conf) throws IOException {
    ...
    void testChmod(Configuration conf, FileSystem fs, String chmodDir) throws IOException {
        ...
        try {
            ...
            assertEquals("rwxrw----", fs.getFileStatus(dir).getPermission().toString());
            ...
            assertEquals("rw-r--r--", fs.getFileStatus(file).getPermission().toString());
    ...
    public void testDFSShell() throws IOException {
        ...
        try {
            ...
            // Verify that we can get with and without crc
            {
                ...
                File checksumFile = new File(fileSys.getChecksumFile(new Path(testFile.getAbsolutePath())).toString());
                testFile.delete();
                checksumFile.delete();
            ...
            // Verify that we get an error while trying to read an nonexistent
            // file
            {
                String[] args = new String[2];
                args[0] = "-cat";
            ...
            // Verify that we get an error while trying to delete an nonexistent
            // file
            {
                String[] args = new String[2];
                args[0] = "-rm";
    ...
    public void testLsr() throws Exception {
        ...
        try {
            ...
            UnixUserGroupInformation tmpUGI = new UnixUserGroupInformation(tmpusername, new String[] { tmpusername });
            UnixUserGroupInformation.saveToConf(conf, UnixUserGroupInformation.UGI_PROPERTY_NAME, tmpUGI);
