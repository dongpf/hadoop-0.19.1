...
/**
 * This class defines a number of static helper methods used by the DFS Upgrade
 * unit tests. By default, a singleton master populated storage directory is
 * created for a Namenode (contains edits, fsimage, version, and time files) and
 * a Datanode (contains version and block files). The master directories are
 * lazily created. They are then copied by the createStorageDirs() method to
 * create new storage directories of the appropriate type (Namenode or
 * Datanode).
 */
public class UpgradeUtilities {
    ...
    /**
     * Initialize the data structures used by this class. IMPORTANT NOTE: This
     * method must be called once before calling any other public method on this
     * class.
     * <p>
     * Creates a singleton master populated storage directory for a Namenode
     * (contains edits, fsimage, version, and time files) and a Datanode
     * (contains version and block files). This can be a lengthy operation.
     */
    ...
    /**
     * Create empty directories. If a specified directory already exists then it
     * is first removed.
     */
    ...
    /**
     * Return the checksum for the singleton master storage directory of the
     * given node type.
     */
    ...
    /**
     * Simulate the <code>dfs.name.dir</code> or <code>dfs.data.dir</code> of a
     * populated DFS filesystem.
     * 
     * This method creates and populates the directory specified by
     * <code>parent/dirName</code>, for each parent directory. The contents of
     * the new directories will be appropriate for the given node type. If the
     * directory does not exist, it will be created. If the directory already
     * exists, it will first be deleted.
     * 
     * By default, a singleton master populated storage directory is created for
     * a Namenode (contains edits, fsimage, version, and time files) and a
     * Datanode (contains version and block files). These directories are then
     * copied by this method to create new storage directories of the
     * appropriate type (Namenode or Datanode).
     * 
    ...
    public static File[] createStorageDirs(NodeType nodeType, String[] parents, String dirName) throws Exception {
        ...
        for (int i = 0; i < parents.length; i++) {
            ...
            switch (nodeType) {
            case NAME_NODE:
                localFS.copyToLocalFile(new Path(namenodeStorage.toString(), "current"), new Path(newDir.toString()),
                        false);
                ...
                break;
            case DATA_NODE:
                localFS.copyToLocalFile(new Path(datanodeStorage.toString(), "current"), new Path(newDir.toString()),
                        false);
    ...
    /**
     * Create a <code>version</code> file inside the specified parent directory.
     * If such a file already exists, it will be overwritten. The given version
     * string will be written to the file as the layout version. None of the
     ...
     */
    public static File[] createVersionFile(NodeType nodeType, File[] parent, StorageInfo version) throws IOException {
    ...
    /**
     * Corrupt the specified file. Some random bytes within the file will be
     * changed to some random values.
     * 
     * @throws IllegalArgumentException
    ...
    /**
     * Return the layout version inherent in the current version of the
     * Namenode, whether it is running or not.
     */
    ...
    /**
     * Return the namespace ID inherent in the currently running Namenode. If no
     * Namenode is running, return the namespace ID of the master Namenode
     * storage directory.
     * 
    ...
    /**
     * Return the File System State Creation Timestamp (FSSCTime) inherent in
     * the currently running Namenode. If no Namenode is running, return the
     * FSSCTime of the master Namenode storage directory.
     * 
     * The UpgradeUtilities.initialize() method must be called once before
     * calling this method.
     */
