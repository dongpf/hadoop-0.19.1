...
public class TestDatanodeDeath extends TestCase {
     ...
     * last kill of a datanode. This guarantees that all three replicas of a
     * block do not get killed (otherwise the file will be corrupt and the test
     * will fail).
     */
    ...
    private void complexTest() throws IOException {
        ...
        try {
            ...
            // Create a thread that kills existing datanodes and creates new
            // ones.
            ...
            for (int i = 0; i < numThreads; i++) {
                try {
                    ...
                    // if most of the threads are done, then stop restarting
                    // datanodes.
                    if (i >= numThreads / 2) {
                        modThread.close();
                    }
    ...
    /**
     * Write to one file, then kill one datanode in the pipeline and then close
     * the file.
     */
