 ...
 * system when the system is upgraded under various storage state and version
 * conditions.
 */
public class TestDFSUpgrade extends TestCase {
     ...
     * containing files with their original checksum. It is assumed that the
    ...
    /**
     * Attempts to start a NameNode with the given operation. Starting the
     * NameNode should throw an exception.
     */
    ...
    /**
     * Attempts to start a DataNode with the given operation. Starting the
     * DataNode should throw an exception.
     */
    void startDataNodeShouldFail(StartupOption operation) {
        try {
            cluster.startDataNodes(conf, 1, false, operation, null); // should
                                                                     // fail
    ...
    /**
     * This test attempts to upgrade the NameNode and DataNode under a number of
    ...
    public void testUpgrade() throws Exception {
        ...
        for (int numDirs = 1; numDirs <= 2; numDirs++) {
            ...
            UpgradeUtilities.createVersionFile(DATA_NODE, baseDirs,
                    new StorageInfo(Integer.MIN_VALUE, UpgradeUtilities.getCurrentNamespaceID(cluster),
            ...
            UpgradeUtilities.createVersionFile(
                    DATA_NODE,
                    baseDirs,
                    new StorageInfo(UpgradeUtilities.getCurrentLayoutVersion(), UpgradeUtilities
                            .getCurrentNamespaceID(cluster), Long.MAX_VALUE));
            startDataNodeShouldFail(StartupOption.REGULAR);
            cluster.shutdown();
            UpgradeUtilities.createEmptyDirs(nameNodeDirs);
            UpgradeUtilities.createEmptyDirs(dataNodeDirs);
            ...
            baseDirs = UpgradeUtilities.createStorageDirs(NAME_NODE, nameNodeDirs, "current");
            UpgradeUtilities.createVersionFile(NAME_NODE, baseDirs, new StorageInfo(
                    Storage.LAST_UPGRADABLE_LAYOUT_VERSION + 1, UpgradeUtilities.getCurrentNamespaceID(null),
            ...
            UpgradeUtilities.createVersionFile(
                    NAME_NODE,
                    baseDirs,
                    new StorageInfo(Integer.MIN_VALUE, UpgradeUtilities.getCurrentNamespaceID(null), UpgradeUtilities
