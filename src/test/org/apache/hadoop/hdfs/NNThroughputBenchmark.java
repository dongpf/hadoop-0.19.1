...
public class NNThroughputBenchmark {
    ...
    class CreateFileStats extends OperationStatsBase {
        ...
        long executeOp(int daemonId, int inputIdx, String clientName) throws IOException {
            ...
            nameNode.create(fileNames[daemonId][inputIdx], FsPermission.getDefault(), clientName, true, replication,
                    BLOCK_SIZE);
            long end = System.currentTimeMillis();
            for (boolean written = !closeUponCreate; !written; written = nameNode.complete(
                    fileNames[daemonId][inputIdx], clientName))
                ;
            return end - start;
        }
    ...
    class OpenFileStats extends CreateFileStats {
        ...
        void generateInputs(int[] opsPerThread) throws IOException {
            ...
            String[] createArgs = new String[] { "-op", "create", "-threads", String.valueOf(this.numThreads),
                    "-files", String.valueOf(numOpsRequired), "-filesPerDir",
            ...
            if (nameNode.getFileInfo(opCreate.getBaseDir()) != null && nameNode.getFileInfo(getBaseDir()) == null) {
    ...
    class DeleteFileStats extends OpenFileStats {
        ...
        static final String OP_DELETE_USAGE = "-op " + OP_DELETE_NAME + OP_USAGE_ARGS;
    ...
    class RenameFileStats extends OpenFileStats {
        ...
        static final String OP_RENAME_USAGE = "-op " + OP_RENAME_NAME + OP_USAGE_ARGS;
        ...
        long executeOp(int daemonId, int inputIdx, String ignore) throws IOException {
            long start = System.currentTimeMillis();
            nameNode.rename(fileNames[daemonId][inputIdx], destNames[daemonId][inputIdx]);
            long end = System.currentTimeMillis();
            return end - start;
        }
    }
    ...
    private static class TinyDatanode implements Comparable<String> {
        ...
        void sendHeartbeat() throws IOException {
            // register datanode
            DatanodeCommand[] cmds = nameNode.sendHeartbeat(dnRegistration, DF_CAPACITY, DF_USED,
                    DF_CAPACITY - DF_USED, 0, 0);
            if (cmds != null) {
        ...
        private int transferBlocks(Block blocks[], DatanodeInfo xferTargets[][]) throws IOException {
            for (int i = 0; i < blocks.length; i++) {
                ...
                for (int t = 0; t < blockTargets.length; t++) {
                    ...
                    nameNode.blockReceived(receivedDNReg, new Block[] { blocks[i] },
    ...
    class BlockReportStats extends OperationStatsBase {
        ...
        void generateInputs(int[] ignore) throws IOException {
            ...
            for (int idx = 0; idx < nrFiles; idx++) {
                ...
                nameNode.create(fileName, FsPermission.getDefault(), clientName, true, replication, BLOCK_SIZE);
        ...
        private void addBlocks(String fileName, String clientName) throws IOException {
            for (int jdx = 0; jdx < blocksPerFile; jdx++) {
                ...
                for (DatanodeInfo dnInfo : loc.getLocations()) {
                    ...
                    nameNode.blockReceived(datanodes[dnIdx].dnRegistration, new Block[] { loc.getBlock() },
                            new String[] { "" });
                }
            }
        }
        ...
        long executeOp(int daemonId, int inputIdx, String ignore) throws IOException {
            ...
            long start = System.currentTimeMillis();
            nameNode.blockReport(dn.dnRegistration, BlockListAsLongs.convertToArrayLongs(dn.blocks));
            long end = System.currentTimeMillis();
            return end - start;
        }
    ...
    class ReplicationStats extends OperationStatsBase {
        ...
        ReplicationStats(List<String> args) {
            ...
            numOpsRequired = (totalBlocks * replication * nodesToDecommission * 2) / (numDatanodes * numDatanodes);

            String[] blkReportArgs = { "-op", "blockReport", "-datanodes", String.valueOf(numDatanodes),
                    "-blocksPerReport", String.valueOf(totalBlocks * replication / numDatanodes), "-blocksPerFile",
                    String.valueOf(numDatanodes) };
    ...
    public static void main(String[] args) throws Exception {
        runBenchmark(new Configuration(), new ArrayList<String>(Arrays.asList(args)));
    }
}
