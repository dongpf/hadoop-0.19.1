...
public class TestQuota extends TestCase {
    ...
    /**
     * Test quota related commands: setQuota, clrQuota, setSpaceQuota,
     * clrSpaceQuota, and count
     */
    public void testQuotaCommands() throws Exception {
        ...
        try {
            ...
            // 10.s: but writing fileLen bytes should result in an quota
            // exception
            hasException = false;
            try {
    ...
    /**
     * Test commands that change the size of the name space: mkdirs, rename, and
     * delete
     */
    ...
    public void testSpaceCommands() throws Exception {
        ...
        try {
            ...
            // Create /nqdir0/qdir1/qdir21 and set its space quota to 2 *
            // fileSpace
            ...
            // Reverse: Move /nqdir0/qdir1/qdir20/nqdir30 to
            // /nqdir0/qdir1/qdir21/
            hasException = false;
            try {
            ...
            /*
             * NOTE: append() is not supported in 0.18. OutputStream out =
             * dfs.append(file2); // appending 1 fileLen should succeed
             * out.write(new byte[fileLen]); out.close();
             * 
             * file2Len += fileLen; // after append
             * 
             * // verify space after append; c = dfs.getContentSummary(dstPath);
             * assertEquals(c.getSpaceConsumed(), 4 * fileSpace);
             * 
             * // now increase the quota for quotaDir1 dfs.setQuota(quotaDir1,
             * FSConstants.QUOTA_DONT_SET, 5 * fileSpace); // Now, appending
             * more than 1 fileLen should result in an error out =
             * dfs.append(file2); hasException = false; try { out.write(new
             * byte[fileLen + 1024]); out.flush(); out.close(); } catch
             * (QuotaExceededException e) { hasException = true;
             * IOUtils.closeStream(out); } assertTrue(hasException);
             * 
             * file2Len += fileLen; // after partial append
             * 
             * // verify space after partial append c =
             * dfs.getContentSummary(dstPath);
             * assertEquals(c.getSpaceConsumed(), 5 * fileSpace); == end of
             * append test ==
             */
