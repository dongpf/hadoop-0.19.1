...
public class TestDatanodeBlockScanner extends TestCase {
    ...
    private static String urlGet(URL url) {
        try {
            ...
            LOG.warn("Failed to fetch " + url.toString() + " : " + e.getMessage());
        }
        return "";
    }
    ...
    /**
     * This connects to datanode and fetches block verification data. It repeats
     * this until the given block has a verification time > 0.
     */
    ...
    /**
     * testBlockCorruptionRecoveryPolicy. This tests recovery of corrupt
     * replicas, first for one corrupt replica then for two. The test invokes
     * blockCorruptionRecoveryPolicy which 1. Creates a block with desired
     * number of replicas 2. Corrupts the desired number of replicas and
     * restarts the datanodes containing the corrupt replica. Additionaly we
     * also read the block in case restarting does not report corrupt replicas.
     * Restarting or reading from the datanode would trigger reportBadBlocks to
     * namenode. NameNode adds it to corruptReplicasMap and neededReplication 3.
     * Test waits until all corrupt replicas are reported, meanwhile
     * Re-replciation brings the block back to healthy state 4. Test again waits
     * until the block is reported with expected number of good replicas.
     */
    ...
    private void blockCorruptionRecoveryPolicy(int numDataNodes, short numReplicas, int numCorruptReplicas)
            throws IOException {
        Configuration conf = new Configuration();
        ...
        while (corruptReplicaSize != numCorruptReplicas) {
            ...
            try {
                LOG.info("Looping until expected " + numCorruptReplicas + " are " + "reported. Current reported "
                        + corruptReplicaSize);
                Thread.sleep(1000);
            } catch (InterruptedException ignore) {
            }
    ...
    public void testTruncatedBlockReport() throws Exception {
        ...
        try {
            ...
            // restart the cluster
            cluster = new MiniDFSCluster(0, conf, REPLICATION_FACTOR, false, true, null, null, null);
            cluster.startDataNodes(conf, 1, true, null, null);
            cluster.waitActive(); // now we have 3 datanodes

            // wait for truncated block be detected and the block to be
            // replicated
