...
public class DataNodeCluster {
    ...
    static final String USAGE = "Usage: datanodecluster " + " -n <numDataNodes> " + " [-racks <numRacks>] "
            + " [-simulated] " + " [-inject startingBlockId numBlocksPerDN]"
            + " [-r replicationFactorForInjectedBlocks]" + " [-d dataNodeDirs]\n"
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Starting " + numDataNodes + (simulated ? " Simulated " : " ")
        ...
        try {
            ...
            if (inject) {
                long blockSize = 10;
                System.out.println("Injecting " + numBlocksPerDNtoInject + " blocks in each DN starting at blockId "
                ...
                for (int i_dn = 0; i_dn < numDataNodes; ++i_dn) {
                    ...
                    for (int i = 1; i <= replication; ++i) {
                        // inject blocks for dn_i into dn_i and replica in
                        // dn_i's neighbors
    ...
    /*
     * There is high probability that the rack id generated here will not
     * conflict with those of other data node cluster. Not perfect but mostly
     * unique rack ids are good enough
     */
