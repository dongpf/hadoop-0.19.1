...
/**
 * This test ensures the appropriate response from the system when the system is
 * finalized.
 */
public class TestDFSFinalize extends TestCase {
     ...
     * does not exist. Verify that current hasn't been modified by comparing the
     * checksum of all it's containing files with their original checksum. Note
     * that we do not check that previous is removed on the DataNode because its
     * removal is asynchronous therefore we have no reliable way to know when it
     * will happen.
     */
    void checkResult(String[] nameNodeDirs, String[] dataNodeDirs) throws IOException {
        ...
        for (int i = 0; i < dataNodeDirs.length; i++) {
            assertEquals(UpgradeUtilities.checksumContents(DATA_NODE, new File(dataNodeDirs[i], "current")),
    ...
    public void testFinalize() throws Exception {
        ...
        for (int numDirs = 1; numDirs <= 2; numDirs++) {
            /*
             * This test requires that "current" directory not change after the
             * upgrade. Actually it is ok for those contents to change. For now
             * disabling block verification so that the contents are not
             * changed.
             */
