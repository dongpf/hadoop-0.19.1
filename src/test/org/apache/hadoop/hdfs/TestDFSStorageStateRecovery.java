...
/**
 * This test ensures the appropriate response (successful or failure) from the
 * system when the system is started under various storage state and version
 * conditions.
 */
public class TestDFSStorageStateRecovery extends TestCase {
    ...
    /**
     * The test case table. Each row represents a test case. This table is taken
     * from the table in Apendix A of the HDFS Upgrade Test Plan
     * (TestPlan-HdfsUpgrade.html) attached to
     * http://issues.apache.org/jira/browse/HADOOP-702 The column meanings are:
     * 0) current directory exists 1) previous directory exists 2) previous.tmp
     * directory exists 3) removed.tmp directory exists 4) node should recover
     * and startup 5) current directory should exist after recovery but before
     * startup 6) previous directory should exist after recovery but before
     * startup
     */
    static boolean[][] testCases = new boolean[][] { new boolean[] { true, false, false, false, true, true, false }, // 1
    ...
    /**
     * Writes an INFO log message containing the parameters. Only the first 4
     * elements of the state array are included in the message.
     */
    void log(String label, int numDirs, int testCaseNum, boolean[] state) {
        LOG.info("============================================================");
        LOG.info("***TEST " + (testCounter++) + "*** " + label + ":" + " numDirs=" + numDirs + " testCase="
                + testCaseNum + " current=" + state[0] + " previous=" + state[1] + " previous.tmp=" + state[2]
                + " removed.tmp=" + state[3]);
    }
     ...
     * of the <code>state</code> array will be created and populated. See
     * UpgradeUtilities.createStorageDirs().
     * 
     * @param nodeType
     *            the type of node that storage should be created for. Based on
     *            this parameter either dfs.name.dir or dfs.data.dir is used
     *            from the global conf.
     * @param state
     *            a row from the testCases table which indicates which
     *            directories to setup for the node
    ...
    String[] createStorageState(NodeType nodeType, boolean[] state) throws Exception {
        String[] baseDirs = (nodeType == NAME_NODE ? conf.getStrings("dfs.name.dir") : conf.getStrings("dfs.data.dir"));
    ...
    /**
     * Verify that the current and/or previous exist as indicated by the method
     * parameters. If previous exists, verify that it hasn't been modified by
     * comparing the checksum of all it's containing files with their original
     * checksum. It is assumed that the server has recovered.
     */
    void checkResult(NodeType nodeType, String[] baseDirs, boolean currentShouldExist, boolean previousShouldExist)
            throws IOException {
        switch (nodeType) {
        case NAME_NODE:
        ...
        case DATA_NODE:
            if (currentShouldExist) {
                for (int i = 0; i < baseDirs.length; i++) {
                    assertEquals(UpgradeUtilities.checksumContents(nodeType, new File(baseDirs[i], "current")),
        ...
        if (previousShouldExist) {
            for (int i = 0; i < baseDirs.length; i++) {
                ...
                assertEquals(UpgradeUtilities.checksumContents(nodeType, new File(baseDirs[i], "previous")),
                        UpgradeUtilities.checksumMasterContents(nodeType));
            }
        }
    }
    ...
    /**
     * This test iterates over the testCases table and attempts to startup the
     * NameNode and DataNode normally.
     */
        ...
        for (int numDirs = 1; numDirs <= 2; numDirs++) {
            ...
            for (int i = 0; i < testCases.length; i++) {
                ...
                if (!testCase[0] && !testCase[1] && !testCase[2] && !testCase[3]) {
                    // DataNode will create and format current if no directories
                    // exist
