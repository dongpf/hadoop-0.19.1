...
/**
 * This class tests the replication and injection of blocks of a DFS file for
 * simulated storage.
 */
public class TestInjectionForSimulatedStorage extends TestCase {
    ...
    private void waitForBlockReplication(String filename, ClientProtocol namenode, int expected, long maxWaitSec)
        ...
        for (int i = 0; i < numBlocks; ++i) {
            ...
            while (true) { // Loop to check for block i (usually when 0 is done
                           // all will be done
                ...
                LOG.info("Not enough replicas for " + (i + 1) + "th block " + block.getBlock() + " yet. Expecting "
                        + expected + ", got " + actual + ".");
     ...
     * under replicated blocks. This test uses simulated storage and one of its
     * features to inject blocks,
     * 
     * It creates a file with several blocks and replication of 4. The cluster
     * is then shut down - NN retains its state but the DNs are all simulated
     * and hence loose their blocks. The blocks are then injected in one of the
     * DNs. The expected behaviour is that the NN will arrange for themissing
     * replica will be copied from a valid source.
     */
    public void testInjection() throws IOException {
        ...
        try {
            ...
            /*
             * Start the MiniDFSCluster with more datanodes since once a
             * writeBlock to a datanode node fails, same block can not be
             * written to it immediately. In our case some replication attempts
             * will fail.
             */
