...
public class TestDFSPermission extends TestCase {
    ...
    final static private int NUM_TEST_PERMISSIONS = conf.getInt("test.dfs.permission.num", 10) * (MAX_PERMISSION + 1)
            / 100;
    ...
    static {
        try {
            ...
            USER1 = new UnixUserGroupInformation(USER1_NAME, new String[] { GROUP1_NAME, GROUP2_NAME });
            USER2 = new UnixUserGroupInformation(USER2_NAME, new String[] { GROUP2_NAME, GROUP3_NAME });
            USER3 = new UnixUserGroupInformation(USER3_NAME, new String[] { GROUP3_NAME, GROUP4_NAME });
    ...
    /**
     * This tests if permission setting in create, mkdir, and setPermission
     * works correctly
     */
    ...
    private void create(OpType op, Path name, short umask, FsPermission permission) throws IOException {
        ...
        switch (op) {
        case CREATE:
            FSDataOutputStream out = fs.create(name, permission, true, conf.getInt("io.file.buffer.size", 4096),
     ...
     * checks if the permission is set correctly; If the delete flag is true,
     * delete the file afterwards; otherwise leave it in the file system.
     */
    private void createAndCheckPermission(OpType op, Path name, short umask, FsPermission permission, boolean delete)
            throws Exception {
        // create the file/directory
        ...
        short expectedPermission = (op == OpType.CREATE) ? (short) (~umask & permissionNum & FILE_MASK)
                : (short) (~umask & permissionNum);
    ...
    /*
     * Check if the permission of a file/directory is the same as the expected
     * permission; If the delete flag is true, delete the file/directory
     * afterwards.
     */
    ...
    /*
     * change a file/directory's owner and group. if expectDeny is set, expect
    ...
    private enum OpType {
        CREATE, MKDIRS, OPEN, SET_REPLICATION, GET_FILEINFO, IS_DIR, EXISTS, GET_CONTENT_LENGTH, LIST, RENAME, DELETE
    };

    /*
     * Check if namenode performs permission checking correctly for superuser,
     * file owner, group owner, and other users
     */
    public void testPermissionChecking() throws Exception {
        ...
        try {
            ...
            // create a directory hierarchy and sets random permission for each
            // inode
            ...
            /* file owner */
            testPermissionCheckingPerUser(USER1, ancestorPermissions, parentPermissions, permissions, parentPaths,
                    filePaths, dirPaths);
            /* group owner */
            testPermissionCheckingPerUser(USER2, ancestorPermissions, parentPermissions, permissions, parentPaths,
                    filePaths, dirPaths);
            /* other owner */
            testPermissionCheckingPerUser(USER3, ancestorPermissions, parentPermissions, permissions, parentPaths,
                    filePaths, dirPaths);
            /* super owner */
            testPermissionCheckingPerUser(SUPERUSER, ancestorPermissions, parentPermissions, permissions, parentPaths,
                    filePaths, dirPaths);
        } finally {
            fs.close();
            cluster.shutdown();
    ...
    /*
     * Check if namenode performs permission checking correctly for the given
     * user for operations mkdir, open, setReplication, getFileInfo,
     * isDirectory, exists, getContentLength, list, rename, and delete
     */
    private void testPermissionCheckingPerUser(UnixUserGroupInformation ugi, short[] ancestorPermission,
            short[] parentPermission, short[] filePermission, Path[] parentDirs, Path[] files, Path[] dirs)
            throws Exception {
        login(SUPERUSER);
        for (int i = 0; i < NUM_TEST_PERMISSIONS; i++) {
        ...
        for (int i = 0; i < NUM_TEST_PERMISSIONS; i++) {
            ...
            testRename(ugi, files[i], files[next], ancestorPermission[i], parentPermission[i],
                    ancestorPermission[next], parentPermission[next]);
    ...
    /*
     * A random permission generator that guarantees that each permission value
    ...
    static private class PermissionGenerator {
        ...
        short next() throws IOException {
            ...
            // swap the chosen permission with last available permission in the
            // array
    ...
    /*
     * A base class that verifies the permission checking is correct for an
     * operation
     */
    abstract class PermissionVerifier {
        ...
        void verifyPermission(UnixUserGroupInformation ugi) throws LoginException, IOException {
            if (this.ugi != ugi) {
        ...
        protected boolean expectPermissionDeny() {
            return (requiredPermission & permission) != requiredPermission
                    || (requiredParentPermission & parentPermission) != requiredParentPermission
                    || (requiredAncestorPermission & ancestorPermission) != requiredAncestorPermission;
        }

        /* Set the permissions required to pass the permission checking */
        protected void setRequiredPermissions(UnixUserGroupInformation ugi) throws IOException {
    ...
    private void testCreateMkdirs(UnixUserGroupInformation ugi, Path path, short ancestorPermission,
            short parentPermission) throws Exception {
    ...
    private void testOpen(UnixUserGroupInformation ugi, Path path, short ancestorPermission, short parentPermission,
            short filePermission) throws Exception {
    ...
    private void testSetReplication(UnixUserGroupInformation ugi, Path path, short ancestorPermission,
            short parentPermission, short filePermission) throws Exception {
        replicatorVerifier.set(path, ancestorPermission, parentPermission, filePermission);
        replicatorVerifier.verifyPermission(ugi);
    }

    /*
     * A class that verifies the permission checking is correct for setTimes
     */
    ...
    /* test if the permission checking of setReplication is correct */
    private void testSetTimes(UnixUserGroupInformation ugi, Path path, short ancestorPermission,
            short parentPermission, short filePermission) throws Exception {
    ...
    /*
     * test if the permission checking of isDirectory, exist, getFileInfo,
     * getContentSummary is correct
     */
    private void testStats(UnixUserGroupInformation ugi, Path path, short ancestorPermission, short parentPermission)
            throws Exception {
    ...
    private void testList(UnixUserGroupInformation ugi, Path file, Path dir, short ancestorPermission,
            short parentPermission, short filePermission) throws Exception {
    ...
    private class RenamePermissionVerifier extends PermissionVerifier {
        ...
        /* initialize */
        void set(Path src, short srcAncestorPermission, short srcParentPermission, Path dst,
                short dstAncestorPermission, short dstParentPermission) {
        ...
        protected boolean expectPermissionDeny() {
            return super.expectPermissionDeny()
                    || (requiredParentPermission & dstParentPermission) != requiredParentPermission
    ...
    private void testRename(UnixUserGroupInformation ugi, Path src, Path dst, short srcAncestorPermission,
            short srcParentPermission, short dstAncestorPermission, short dstParentPermission) throws Exception {
    ...
    /*
     * A class that verifies the permission checking is correct for directory
     * deletion
     */
    ...
    private void testDeleteFile(UnixUserGroupInformation ugi, Path file, short ancestorPermission,
            short parentPermission) throws Exception {
    ...
    private void testDeleteDir(UnixUserGroupInformation ugi, Path path, short ancestorPermission,
            short parentPermission, short permission, short[] childPermissions) throws Exception {
        dirDeletionVerifier.set(path, ancestorPermission, parentPermission, permission, childPermissions);
