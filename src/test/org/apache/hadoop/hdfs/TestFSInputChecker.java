...
public class TestFSInputChecker extends TestCase {
    ...
    private void writeFile(FileSystem fileSys, Path name) throws IOException {
        ...
        FSDataOutputStream stm = fileSys.create(name, new FsPermission((short) 0777), true,
                fileSys.getConf().getInt("io.file.buffer.size", 4096), (short) NUM_OF_DATANODES, BLOCK_SIZE, null);
        stm.write(expected);
        stm.close();
    }
    ...
    private void checkAndEraseData(byte[] actual, int from, byte[] expected, String message) throws Exception {
        for (int idx = 0; idx < actual.length; idx++) {
            assertEquals(message + " byte " + (from + idx) + " differs. expected " + expected[from + idx] + " actual "
                    + actual[idx], actual[idx], expected[from + idx]);
            actual[idx] = 0;
        }
    }
     ...
     */
    private void testChecker(ChecksumFileSystem fileSys, boolean readCS) throws Exception {
    ...
    private void testSeekAndRead(ChecksumFileSystem fileSys) throws IOException {
        Path file = new Path("try.dat");
        writeFile(fileSys, file);
        stm = fileSys.open(file, fileSys.getConf().getInt("io.file.buffer.size", 4096));
        checkSeekAndRead();
        stm.close();
        cleanupFile(fileSys, file);
    }
