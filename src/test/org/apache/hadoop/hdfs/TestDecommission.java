...
public class TestDecommission extends TestCase {
    ...
    private void checkFile(FileSystem fileSys, Path name, int repl, String downnode) throws IOException {
        //
        // sleep an additional 10 seconds for the blockreports from the
        // datanodes
        // to arrive.
        //
        // need a raw stream
        ...
        for (LocatedBlock blk : dinfo) { // for each block
            ...
            for (int j = 0; j < nodes.length; j++) { // for each replica
                if (nodes[j].getName().equals(downnode)) {
                    hasdown++;
                    System.out.println("Block " + blk.getBlock() + " replica " + nodes[j].getName()
                            + " is decommissioned.");
                }
            }
            System.out.println("Block " + blk.getBlock() + " has " + hasdown + " decommissioned replica.");
            assertEquals("Number of replicas for block" + blk.getBlock(), Math.min(numDatanodes, repl + hasdown),
                    nodes.length);
        }
    }
     ...
     */
    private String decommissionNode(NameNode namenode, Configuration conf, DFSClient client, FileSystem localFileSys)
            throws IOException {
    ...
    private void waitNodeState(FileSystem filesys, String node, NodeState state) throws IOException {
        ...
        while (!done) {
            System.out.println("Waiting for node " + node + " to change state to " + state);
            try {
                Thread.sleep(1000);
    ...
    public void testDecommission() throws IOException {
        ...
        try {
            for (int iteration = 0; iteration < numDatanodes - 1; iteration++) {
                ...
                String downnode = decommissionNode(cluster.getNameNode(), conf, client, localFileSys);
