 ...
 * NameNode. Possible operations include create/writing files, opening/reading
 * files, renaming files, and deleting files.
 * 
...
public class NNBenchWithoutMR {
    ...
    /**
     * Create and write to a given number of files. Repeat each remote operation
     * until is suceeds (does not throw an exception).
     * 
    ...
    /**
     * Rename a given number of files. Repeat each remote operation until is
     * suceeds (does not throw an exception).
     * 
     * @return the number of exceptions caught
     */
    static int rename() {
        ...
        for (int index = 0; index < numFiles; index++) {
            ...
            do { // rename file until is succeeds
                try {
                    boolean result = fileSys.rename(new Path(taskDir, "" + index), new Path(taskDir, "A" + index));
                    success = true;
                } catch (IOException ioe) {
                    success = false;
                    totalExceptions++;
    ...
    /**
     * Delete a given number of files. Repeat each remote operation until is
     * suceeds (does not throw an exception).
     * 
     * @return the number of exceptions caught
     */
     ...
     * starting at a given time (<code>-startTime</code>). The files used by the
     * openRead, rename, and delete operations are the same files created by the
     * createWrite operation. Typically, the program would be run four times,
     * once for each operation in this order: createWrite, openRead, rename,
     * delete.
     * 
     * <pre>
     * Usage: nnbench 
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (operation == null
                || // verify args
                baseDir == null || numFiles < 1 || blocksPerFile < 1 || bytesPerBlock < 0
