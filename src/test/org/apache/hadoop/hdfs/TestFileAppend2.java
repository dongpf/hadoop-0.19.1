...
public class TestFileAppend2 extends TestCase {
    ...
    /***
     * int numberOfFiles = 1; int numThreads = 1; int numAppendsPerThread =
     * 2000;
     ****/
    ...
    /**
     * Creates one file, writes a few bytes to it and then closed it. Reopens
     * the same file for appending, write all blocks and then close. Verify that
     * all data exists in file.
     */
    public void testSimpleAppend() throws IOException {
        ...
        try {
            ...
            { // test append permission.
                ...
                UnixUserGroupInformation appenduser = UnixUserGroupInformation.createImmutable(new String[] { username,
                        group });
        ...
        public void run() {
            ...
            for (int i = 0; i < numAppendsPerThread; i++) {
                ...
                try {
                    ...
                    System.out.println("Workload thread " + id + " appending " + sizeToAppend + " bytes " + " to file "
                            + testfile + " of size " + len);
                    FSDataOutputStream stm = fs.append(testfile);
                    stm.write(fileContents, (int) len, sizeToAppend);
                    stm.close();

                    // wait for the file size to be reflected in the namenode
                    // metadata
                    while (fs.getFileStatus(testfile).getLen() != (len + sizeToAppend)) {
                        try {
                            System.out.println("Workload thread " + id + " file " + testfile + " size "
                                    + fs.getFileStatus(testfile).getLen() + " expected size " + (len + sizeToAppend)
                    ...
                    assertTrue("File " + testfile + " size is " + fs.getFileStatus(testfile).getLen()
                    ...
                    assertTrue("Workload exception " + id + " testfile " + testfile + " expected size "
                            + (len + sizeToAppend), false);
                }
