...
public class MiniDFSCluster {
     ...
     *            the operation with which to start the servers. If null or
     ...
     * 
     * @param nameNodePort
     *            suggestion for which rpc port to use. caller should use
     ...
     *            if true, format the NameNode and DataNodes before starting up
     * @param manageDfsDirs
     *            if true, the data directories for servers will be created and
     *            dfs.name.dir and dfs.data.dir will be set in the conf
     * @param operation
     *            the operation with which to start the servers. If null or
     *            StartupOption.FORMAT, then StartupOption.REGULAR will be used.
     * @param racks
     ...
     */
    public MiniDFSCluster(int nameNodePort, Configuration conf, int numDataNodes, boolean format,
            boolean manageDfsDirs, StartupOption operation, String[] racks, long[] simulatedCapacities)
            throws IOException {
        this(nameNodePort, conf, numDataNodes, format, manageDfsDirs, manageDfsDirs, operation, racks, null,
                simulatedCapacities);
    }
     ...
     *            if true, the data directories for datanodes will be created
     *            and dfs.data.dir set to same in the conf
     * @param operation
     *            the operation with which to start the servers. If null or
     *            StartupOption.FORMAT, then StartupOption.REGULAR will be used.
     * @param racks
    ...
    public MiniDFSCluster(int nameNodePort, Configuration conf, int numDataNodes, boolean format,
        ...
        if (manageNameDfsDirs) {
            conf.set("dfs.name.dir",
                    new File(base_dir, "name1").getPath() + "," + new File(base_dir, "name2").getPath());
            conf.set("fs.checkpoint.dir", new File(base_dir, "namesecondary1").getPath() + ","
                    + new File(base_dir, "namesecondary2").getPath());
        }
     ...
     * @param manageDfsDirs
     *            if true, the data directories for DataNodes will be created
     *            and dfs.data.dir will be set in the conf
     * @param operation
     *            the operation with which to start the DataNodes. If null or
     *            StartupOption.FORMAT, then StartupOption.REGULAR will be used.
     * @param racks
     *            array of strings indicating the rack that each DataNode is on
     * @param hosts
    ...
    public synchronized void startDataNodes(Configuration conf, int numDataNodes, boolean manageDfsDirs,
            StartupOption operation, String[] racks, String[] hosts, long[] simulatedCapacities) throws IOException {
        ...
        String[] args = (operation == null || operation == StartupOption.FORMAT || operation == StartupOption.REGULAR) ? null
                : new String[] { "-" + operation.toString() };
        ...
        for (int i = curDatanodesNum; i < curDatanodesNum + numDataNodes; i++) {
            ...
            if (manageDfsDirs) {
                ...
                if (!dir1.isDirectory() || !dir2.isDirectory()) {
                    throw new IOException("Mkdirs failed to create directory for DataNode " + i + ": " + dir1 + " or "
                            + dir2);
                }
            ...
            if (hosts != null) {
                ...
                System.out
                        .println("Starting DataNode " + i + " with hostname set to: " + dnConf.get("slave.host.name"));
            }
            if (racks != null) {
     ...
     * this method return, the NameNode should be finalized, but DataNodes may
     * not be since that occurs asynchronously.
     * 
     * @throws IllegalStateException
    ...
    /**
     * Gets the rpc port used by the NameNode, because the caller supplied port
     * is not necessarily the actual port used.
     */
    ...
    /**
     * Shutdown all DataNodes started by this class. The NameNode is left
     * running so that new DataNodes may be started.
     */
     ...
     * 
     * @param dataNodeIndex
     *            - data node whose block report is desired - the index is same
     *            as for getDataNodes()
    ...
    /**
     * 
     * @return block reports from all data nodes Block[] is indexed in the same
     *         order as the list of datanodes returned by getDataNodes()
     */
     ...
     *            - the blocks
     * @throws IOException
     *             if not simulatedFSDataset if any of blocks already exist in
     *             the data node
     * 
     */
     ...
     * 
     * @param blocksToInject
     *            - blocksToInject[] is indexed in the same order as the list of
     *            datanodes returned by getDataNodes()
     * @throws IOException
     *             if not simulatedFSDataset if any of blocks already exist in
     *             the data nodes Note the rest of the blocks are not injected.
     */
