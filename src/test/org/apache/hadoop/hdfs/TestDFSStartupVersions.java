...
/**
 * This test ensures the appropriate response (successful or failure) from a
 * Datanode when the system is started with differing version combinations.
 */
public class TestDFSStartupVersions extends TestCase {
    ...
    private static Path TEST_ROOT_DIR = new Path(System.getProperty("test.build.data", "/tmp").toString()
            .replace(' ', '+'));
    ...
    void log(String label, NodeType nodeType, Integer testCase, StorageInfo version) {
        ...
        LOG.info("***TEST*** " + label + ":" + testCaseLine + " nodeType=" + nodeType + " layoutVersion="
                + version.getLayoutVersion() + " namespaceID=" + version.getNamespaceID() + " fsscTime="
                + version.getCTime());
    }

    /**
     * Initialize the versions array. This array stores all combinations of
     * cross product:
    ...
    private StorageInfo[] initializeVersions() throws Exception {
        ...
        return new StorageInfo[] { new StorageInfo(layoutVersionOld, namespaceIdCur, fsscTimeOld), // 0
    ...
    /**
     * Determines if the given Namenode version and Datanode version are
     * compatible with each other. Compatibility in this case mean that the
     * Namenode and Datanode will successfully start up and will work together.
     * The rules for compatibility, taken from the DFS Upgrade Design, are as
     * follows:
     * 
     * <pre>
    ...
    boolean isVersionCompatible(StorageInfo namenodeVer, StorageInfo datanodeVer) {
        ...
        // check #1
        int softwareLV = FSConstants.LAYOUT_VERSION; // will also be Namenode's
                                                     // LV
    ...
    public void testVersions() throws Exception {
        ...
        UpgradeUtilities.createStorageDirs(NAME_NODE, conf.getStrings("dfs.name.dir"), "current");
        cluster = new MiniDFSCluster(conf, 0, StartupOption.REGULAR);
        StorageInfo nameNodeVersion = new StorageInfo(UpgradeUtilities.getCurrentLayoutVersion(),
                UpgradeUtilities.getCurrentNamespaceID(cluster), UpgradeUtilities.getCurrentFsscTime(cluster));
        log("NameNode version info", NAME_NODE, null, nameNodeVersion);
        for (int i = 0; i < versions.length; i++) {
            File[] storage = UpgradeUtilities.createStorageDirs(DATA_NODE, conf.getStrings("dfs.data.dir"), "current");
