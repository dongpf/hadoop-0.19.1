...
/**
 * This tests data transfer protocol handling in the Datanode. It sends various
 * forms of wrong data and verifies that Datanode handles it well.
 * 
 * This test uses the following two file from src/test/.../dfs directory : 1)
 * hadoop-version-dfs-dir.tgz : contains DFS directories. 2) hadoop-dfs-dir.txt
 * : checksums that are compared in this test. Please read hadoop-dfs-dir.txt
...
public class TestDFSUpgradeFromImage extends TestCase {
    ...
    public void unpackStorage() throws IOException {
        ...
        BufferedReader reader = new BufferedReader(new FileReader(System.getProperty("test.cache.data",
                "build/test/cache") + "/hadoop-dfs-dir.txt"));
        String line;
    ...
    private void verifyFileSystem(DFSClient client) throws IOException {
        ...
        if (printChecksum) {
            throw new IOException("Checksums are written to log as requested. "
                    + "Throwing this exception to force an error " + "for this test.");
        }
    }
    ...
    public void testUpgradeFromImage() throws IOException {
        ...
        try {
            ...
            if (System.getProperty("test.build.data") == null) { // to allow
                                                                 // test to be
                                                                 // run outside
                                                                 // of Ant
                System.setProperty("test.build.data", "build/test/data");
            }
            conf.setInt("dfs.datanode.scan.period.hours", -1); // block scanning
                                                               // off
            cluster = new MiniDFSCluster(0, conf, numDataNodes, false, true, StartupOption.UPGRADE, null);
            cluster.waitActive();
