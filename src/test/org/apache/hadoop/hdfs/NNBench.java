...
/**
 * This program executes a specified operation that applies load to the
 * NameNode.
 * 
 * When run simultaneously on multiple nodes, this program functions as a
 * stress-test and benchmark for namenode, especially when the number of bytes
 * written to each file is small.
 * 
 * Valid operations are: create_write open_read rename delete
 * 
...
public class NNBench {
    ...
    public static long startTime = System.currentTimeMillis() + (120 * 1000); // default
                                                                              // is
                                                                              // 'now'
                                                                              // +
                                                                              // 2min
    ...
    /**
     * Create control files before a test run. Number of files created is equal
     * to the number of maps specified
     * 
     * @throws IOException
     *             on error
     */
    private static void createControlFiles() throws IOException {
        ...
        for (int i = 0; i < numberOfMaps; i++) {
            ...
            try {
                writer = SequenceFile.createWriter(tempFS, config, filePath, Text.class, LongWritable.class,
    ...
    private static void displayUsage() {
        String usage = "Usage: nnbench <options>\n" + "Options:\n" + "\t-operation <Available operations are "
                + OP_CREATE_WRITE + " " + OP_OPEN_READ + " " + OP_RENAME + " " + OP_DELETE + ". "
    ...
    private static void analyzeResults() throws IOException {
        ...
        // The time it takes for the longest running map is measured. Using
        // that,
        ...
        double totalTimeTPS = (longestMapTimeTPmS == 0) ? (1000 * successfulFileOps)
        ...
        String resultLines[] = { "-------------- NNBench -------------- : ",
                "                               Version: " + NNBENCH_VERSION,
                "                           Date & time: " + sdf.format(new Date(System.currentTimeMillis())), "",
                "                        Test Operation: " + operation,
                "                            Start time: " + sdf.format(new Date(startTime)),
                ...
                "                          # exceptions: " + numOfExceptions, "", resultTPSLine1, resultTPSLine2,
                resultALLine1, resultALLine2, "", "                 RAW DATA: AL Total #1: " + totalTimeAL1,
    ...
    public static void validateInputs() {
        ...
        if (!operation.equals(OP_CREATE_WRITE) && !operation.equals(OP_OPEN_READ) && !operation.equals(OP_RENAME)
     ...
     * 
     * @throws IOException
     *             indicates a problem with test startup
     */
    ...
    static class NNBenchMapper extends Configured implements Mapper<Text, LongWritable, Text, Text> {
         ...
         * multiple instances of this program, running on clock synchronized
         * nodes, to start at roughly the same time.
         */
         ...
         */
        private void doCreateWriteOp(String name, OutputCollector<Text, Text> output, Reporter reporter) {
         ...
         * Open operation
         */
        private void doOpenReadOp(String name, OutputCollector<Text, Text> output, Reporter reporter) {
            ...
            for (long l = 0l; l < numberOfFiles; l++) {
                ...
                while (!successfulOp && numOfExceptions < MAX_OPERATION_EXCEPTIONS) {
                    try {
                        ...
                        // If the file needs to be read (specified at command
                        // line)
                        if (readFile) {
                            startTimeAL = System.currentTimeMillis();
         ...
         * Rename operation
         */
        private void doRenameOp(String name, OutputCollector<Text, Text> output, Reporter reporter) {
            for (long l = 0l; l < numberOfFiles; l++) {
                Path filePath = new Path(new Path(baseDir, dataDirName), name + "_" + l);
         ...
         * Delete operation
         */
        private void doDeleteOp(String name, OutputCollector<Text, Text> output, Reporter reporter) {
            for (long l = 0l; l < numberOfFiles; l++) {
                Path filePath = new Path(new Path(baseDir, dataDirName), name + "_" + l);
