...
public class DataChecksum implements Checksum {
    ...
    public static DataChecksum newDataChecksum(int type, int bytesPerChecksum) {
        ...
        switch (type) {
        case CHECKSUM_NULL:
            return new DataChecksum(CHECKSUM_NULL, new ChecksumNull(), CHECKSUM_NULL_SIZE, bytesPerChecksum);
        case CHECKSUM_CRC32:
            return new DataChecksum(CHECKSUM_CRC32, new CRC32(), CHECKSUM_CRC32_SIZE, bytesPerChecksum);
        default:
            return null;
        }
    }
    ...
    public static DataChecksum newDataChecksum(byte bytes[], int offset) {
        ...
        // like readInt():
        int bytesPerChecksum = ((bytes[offset + 1] & 0xff) << 24) | ((bytes[offset + 2] & 0xff) << 16)
