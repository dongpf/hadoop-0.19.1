...
public class UnixUserGroupInformation extends UserGroupInformation {
    ...
    /**
     * Constructor with parameters user name and its group names. The first
     * entry in the groups list is the default group.
     * 
     * @param userName
     *            a user's name
     * @param groupNames
     ...
     * 
     * @param ugi
     *            an array containing user/group names, the first element of
     *            which is the user name, the second of which is the default
     *            group name.
     * @exception IllegalArgumentException
     *                if the array size is less than 2 or any element is null.
     */
    ...
    /**
     * Deserialize this object First check if this is a UGI in the string
     * format. If no, throw an IOException; otherwise set this object's fields
     * by reading them from the given data input
     * 
     * @param in
     *            input stream
     * @exception IOException
     *                is thrown if encounter any error when reading
     */
    public void readFields(DataInput in) throws IOException {
        // read UGI type first
    ...
    /**
     * Serialize this object First write a string marking that this is a UGI in
     * the string format, then write this object's serialized form to the given
     * data output
     * 
     * @param out
     *            output stream
     * @exception IOException
     *                if encounter any error during writing
     */
    ...
    /*
     * The following two methods deal with transferring UGI through conf. In
     * this pass of implementation we store UGI as a string in conf. Later we
     * may change it to be a more general approach that stores it as a byte
     * array
     */
    /**
     ...
     * as a comma separated string that starts with the user name followed by
     * group names. If the property name is not defined, return null. It's
     * assumed that there is only one UGI per user. If this user already has a
     * UGI in the ugi map, return the ugi in the map. Otherwise, construct a UGI
     * from the configuration, store it in the ugi map and return it.
     * 
     * @param conf
     *            configuration
     * @param attr
     ...
     *             if the stored string is ill-formatted.
     */
    ...
    /**
     * Get current user's name and the names of all its groups from Unix. It's
     * assumed that there is only one UGI per user. If this user already has a
     * UGI in the ugi map, return the ugi in the map. Otherwise get the current
     * user's information from Unix, store it in the map, and return it.
     */
    public static UnixUserGroupInformation login() throws LoginException {
        try {
            ...
            /*
             * get groups list from UNIX. It's assumed that the first group is
             * the default group.
             */
    ...
    /**
     * Get a user's name & its group names from the given configuration; If it
     * is not defined in the configuration, get the current user's information
     * from Unix. If the user has a UGI in the ugi map, return the one in the
     * UGI map.
     * 
     * @param conf
     *            either a job configuration or client's configuration
     * @param save
     *            saving it to conf?
     * @return UnixUserGroupInformation a user/group information
     * @exception LoginException
     *                if not able to get the user/group information
     */
    ...
    /*
     * Return a string representation of a string array. Two strings are
     ...
     * @throws IOException
     *             if encounter any error while running the command
     */
    static String getUnixUserName() throws IOException {
        String[] result = executeShellCommand(new String[] { Shell.USER_NAME_COMMAND });
        if (result.length != 1) {
            throw new IOException("Expect one token as the result of " + Shell.USER_NAME_COMMAND + ": "
                    + toString(result));
        }
        return result[0];
    }
    ...
    public boolean equals(Object other) {
        ...
        return new TreeSet<String>(Arrays.asList(groupNames)).equals(new TreeSet<String>(Arrays
                .asList(otherUGI.groupNames)));
    }

    /**
     * Returns a hash code for this UGI. The hash code for a UGI is the hash
     * code of its user name string.
     * 
