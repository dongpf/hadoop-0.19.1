 ...
 * 
 * Type information for a record comprises metadata about the record, as well as
 * a collection of type information for each field in the record.
 */
public class RecordTypeInfo extends org.apache.hadoop.record.Record {
    ...
    // A RecordTypeInfo object is just a collection of TypeInfo objects for each
    // of its fields.
    // private ArrayList<FieldTypeInfo> typeInfos = new
    // ArrayList<FieldTypeInfo>();
    // we keep a hashmap of struct/record names and their type information, as
    // we need it to
    // set filters when reading nested structs. This map is used during
    // deserialization.
    // private Map<String, RecordTypeInfo> structRTIs = new HashMap<String,
    // RecordTypeInfo>();
    ...
    /**
     * Return the type info of a nested record. We only consider nesting to one
     * level.
     * 
     * @param name
     *            Name of the nested record
     */
    ...
    /**
     * This class doesn't implement Comparable as it's not meant to be used for
     * anything besides de/serializing. So we always throw an exception. Not
     * implemented. Always returns 0 if another RecordTypeInfo is passed in.
     */
