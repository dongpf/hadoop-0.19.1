...
public class JRecord extends JCompType {
    ...
    class JavaRecord extends JavaCompType {
        ...
        String getTypeIDObjectString() {
            return "new org.apache.hadoop.record.meta.StructTypeID(" + fullName + ".getTypeInfo())";
        }
        ...
        void genSetRTIFilter(CodeBuffer cb, Map<String, Integer> nestedStructMap) {
            ...
            if (!nestedStructMap.containsKey(fullName)) {
                ...
                cb.append(fullName + ".setTypeFilter(rti.getNestedStructTypeInfo(\"" + name + "\"));\n");
                nestedStructMap.put(fullName, null);
            }
        }

        // for each typeInfo in the filter, we see if there's a similar one in
        // the record.
        // Since we store typeInfos in ArrayLists, thsi search is O(n squared).
        // We do it faster
        // if we also store a map (of TypeInfo to index), but since
        // setupRtiFields() is called
        // only once when deserializing, we're sticking with the former, as the
        // code is easier.
        void genSetupRtiFields(CodeBuffer cb) {
            ...
            cb.append(Consts.RTI_FILTER_FIELDS + " = new int [" + Consts.RIO_PREFIX
                    + "rtiFilter.getFieldTypeInfos().size()];\n");
            cb.append("for (" + Consts.RIO_PREFIX + "i=0; " + Consts.RIO_PREFIX + "i<" + Consts.RTI_FILTER_FIELDS
                    + ".length; " + Consts.RIO_PREFIX + "i++) {\n");
            cb.append(Consts.RTI_FILTER_FIELDS + "[" + Consts.RIO_PREFIX + "i] = 0;\n");
            cb.append("}\n");
            cb.append("java.util.Iterator<org.apache.hadoop.record.meta." + "FieldTypeInfo> " + Consts.RIO_PREFIX
                    + "itFilter = " + Consts.RIO_PREFIX + "rtiFilter.getFieldTypeInfos().iterator();\n");
            cb.append(Consts.RIO_PREFIX + "i=0;\n");
            cb.append("while (" + Consts.RIO_PREFIX + "itFilter.hasNext()) {\n");
            cb.append("org.apache.hadoop.record.meta.FieldTypeInfo " + Consts.RIO_PREFIX + "tInfoFilter = "
                    + Consts.RIO_PREFIX + "itFilter.next();\n");
            cb.append("java.util.Iterator<org.apache.hadoop.record.meta." + "FieldTypeInfo> " + Consts.RIO_PREFIX
                    + "it = " + Consts.RTI_VAR + ".getFieldTypeInfos().iterator();\n");
            cb.append(Consts.RIO_PREFIX + "j=1;\n");
            cb.append("while (" + Consts.RIO_PREFIX + "it.hasNext()) {\n");
            cb.append("org.apache.hadoop.record.meta.FieldTypeInfo " + Consts.RIO_PREFIX + "tInfo = "
                    + Consts.RIO_PREFIX + "it.next();\n");
            ...
            cb.append("}\n");
            /*
             * int ct = 0; for (Iterator<JField<JavaType>> i =
             * fields.iterator(); i.hasNext();) { ct++; JField<JavaType> jf =
             * i.next(); JavaType type = jf.getType(); String name =
             * jf.getName(); if (ct != 1) { cb.append("else "); }
             * type.genRtiFieldCondition(cb, name, ct); } if (ct != 0) {
        ...
        void genCode(String destDir, ArrayList<String> options) throws IOException {
            ...
            // To prevent setting up the type filter for the same struct more
            // than once,
            ...
            // serialize()
            cb.append("public void serialize(" + "final org.apache.hadoop.record.RecordOutput " + Consts.RECORD_OUTPUT
                    + ", final String " + Consts.TAG + ")\n" + "throws java.io.IOException {\n");
            ...
            // deserialize()
            cb.append("public void deserialize(final " + "org.apache.hadoop.record.RecordInput " + Consts.RECORD_INPUT
                    + ", final String " + Consts.TAG + ")\n" + "throws java.io.IOException {\n");
            ...
            cb.append("for (int " + Consts.RIO_PREFIX + "i=0; " + Consts.RIO_PREFIX + "i<" + Consts.RTI_FILTER
            ...
            if (0 != ct) {
                ...
                cb.append("org.apache.hadoop.record.meta.Utils.skip(" + Consts.RECORD_INPUT + ", " + "typeInfos.get("
                        + Consts.RIO_PREFIX + "i).getFieldID(), typeInfos.get(" + Consts.RIO_PREFIX
                        + "i).getTypeID());\n");
                cb.append("}\n");
            }
            cb.append("}\n");
            ...
            for (Iterator<JField<JavaType>> i = fields.iterator(); i.hasNext();) {
                ...
                cb.append(Consts.RIO_PREFIX + "result = 37*" + Consts.RIO_PREFIX + "result + " + Consts.RIO_PREFIX
                        + "ret;\n");
            }
    ...
    class CppRecord extends CppCompType {
        ...
        String getTypeIDObjectString() {
            return "new ::hadoop::StructTypeID(" + fullName + "::getTypeInfo().getFieldTypeInfos())";
        }
        ...
        void genSetupRTIFields(CodeBuffer cb) {
            ...
            cb.append("p" + Consts.RTI_FILTER_FIELDS + " = new int[p" + Consts.RTI_FILTER
                    + "->getFieldTypeInfos().size()];\n");
            cb.append("for (unsigned int " + Consts.RIO_PREFIX + "i=0; " + Consts.RIO_PREFIX + "i<p"
                    + Consts.RTI_FILTER + "->getFieldTypeInfos().size(); " + Consts.RIO_PREFIX + "i++) {\n");
            cb.append("p" + Consts.RTI_FILTER_FIELDS + "[" + Consts.RIO_PREFIX + "i] = 0;\n");
            cb.append("}\n");
            cb.append("for (unsigned int " + Consts.RIO_PREFIX + "i=0; " + Consts.RIO_PREFIX + "i<p"
                    + Consts.RTI_FILTER + "->getFieldTypeInfos().size(); " + Consts.RIO_PREFIX + "i++) {\n");
            cb.append("for (unsigned int " + Consts.RIO_PREFIX + "j=0; " + Consts.RIO_PREFIX + "j<p" + Consts.RTI_VAR
                    + "->getFieldTypeInfos().size(); " + Consts.RIO_PREFIX + "j++) {\n");
            cb.append("if (*(p" + Consts.RTI_FILTER + "->getFieldTypeInfos()[" + Consts.RIO_PREFIX + "i]) == *(p"
                    + Consts.RTI_VAR + "->getFieldTypeInfos()[" + Consts.RIO_PREFIX + "j])) {\n");
        ...
        void genCode(FileWriter hh, FileWriter cc, ArrayList<String> options) throws IOException {
            ...
            hb.append("virtual void deserializeWithoutFilter(::hadoop::IArchive& " + Consts.RECORD_INPUT
            ...
            hb.append("virtual void serialize(::hadoop::OArchive& " + Consts.RECORD_OUTPUT + ", const char* "
                    + Consts.TAG + ") const;\n");
            hb.append("virtual void deserialize(::hadoop::IArchive& " + Consts.RECORD_INPUT + ", const char* "
                    + Consts.TAG + ");\n");
            ...
            cb.append("::hadoop::RecordTypeInfo* " + fullName + "::p" + Consts.RTI_VAR + " = " + fullName
                    + "::setupTypeInfo();\n");
            ...
            // set RTIFilter for nested structs. We may end up with multiple
            // lines that
            // do the same thing, if the same struct is nested in more than one
            // field,
            // but that's OK.
            for (Iterator<JField<CppType>> i = fields.iterator(); i.hasNext();) {
                JField<CppType> jf = i.next();
            ...
            // serialize()
            cb.append("void " + fullName + "::serialize(::hadoop::OArchive& " + Consts.RECORD_OUTPUT + ", const char* "
                    + Consts.TAG + ") const {\n");
            cb.append(Consts.RECORD_OUTPUT + ".startRecord(*this," + Consts.TAG + ");\n");
            for (Iterator<JField<CppType>> i = fields.iterator(); i.hasNext();) {
                JField<CppType> jf = i.next();
            ...
            // deserializeWithoutFilter()
            cb.append("void " + fullName + "::deserializeWithoutFilter(::hadoop::IArchive& " + Consts.RECORD_INPUT
                    + ", const char* " + Consts.TAG + ") {\n");
            ...
            // deserialize()
            cb.append("void " + fullName + "::deserialize(::hadoop::IArchive& " + Consts.RECORD_INPUT
                    + ", const char* " + Consts.TAG + ") {\n");
            cb.append("if (NULL == p" + Consts.RTI_FILTER + ") {\n");
            cb.append("deserializeWithoutFilter(" + Consts.RECORD_INPUT + ", " + Consts.TAG + ");\n");
            cb.append("return;\n");
            ...
            if (0 != ct) {
                cb.append("else {\n");
                cb.append("const std::vector< ::hadoop::FieldTypeInfo* >& typeInfos = p" + Consts.RTI_FILTER
                        + "->getFieldTypeInfos();\n");
                cb.append("::hadoop::Utils::skip(" + Consts.RECORD_INPUT + ", typeInfos[" + Consts.RIO_PREFIX
                        + "i]->getFieldID()->c_str()" + ", *(typeInfos[" + Consts.RIO_PREFIX + "i]->getTypeID()));\n");
                cb.append("}\n");
            }
            cb.append("}\n");
            cb.append(Consts.RECORD_INPUT + ".endRecord(*this, " + Consts.TAG + ");\n");
