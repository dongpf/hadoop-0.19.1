...
public class JMap extends JCompType {
    ...
    class JavaMap extends JavaCompType {
        ...
        String getTypeIDObjectString() {
            return "new org.apache.hadoop.record.meta.MapTypeID(" + key.getTypeIDObjectString() + ", "
        ...
        void genCompareTo(CodeBuffer cb, String fname, String other) {
            ...
            cb.append(iterType + getId(Consts.RIO_PREFIX + "miter1") + " = " + getId(Consts.RIO_PREFIX + "set1")
                    + ".iterator();\n");
            cb.append(iterType + getId(Consts.RIO_PREFIX + "miter2") + " = " + getId(Consts.RIO_PREFIX + "set2")
                    + ".iterator();\n");
            ...
            cb.append("}\n");
            cb.append(Consts.RIO_PREFIX + "ret = (" + getId(Consts.RIO_PREFIX + "set1") + ".size() - "
                    + getId(Consts.RIO_PREFIX + "set2") + ".size());\n");
            cb.append("}\n");
        }
        ...
        void genReadMethod(CodeBuffer cb, String fname, String tag, boolean decl) {
            ...
            cb.append("{\n");
            incrLevel();
            cb.append("org.apache.hadoop.record.Index " + getId(Consts.RIO_PREFIX + "midx") + " = "
                    + Consts.RECORD_INPUT + ".startMap(\"" + tag + "\");\n");
            cb.append(fname + "=new " + getType() + "();\n");
            cb.append("for (; !" + getId(Consts.RIO_PREFIX + "midx") + ".done(); " + getId(Consts.RIO_PREFIX + "midx")
                    + ".incr()) {\n");
        ...
        void genWriteMethod(CodeBuffer cb, String fname, String tag) {
            ...
            String iterType = "java.util.Iterator<java.util.Map.Entry<" + key.getWrapperType() + ","
            ...
            cb.append("for(" + iterType + getId(Consts.RIO_PREFIX + "midx") + " = " + getId(Consts.RIO_PREFIX + "es")
                    + ".iterator(); " + getId(Consts.RIO_PREFIX + "midx") + ".hasNext();) {\n");
            cb.append(entryType + getId(Consts.RIO_PREFIX + "me") + " = " + getId(Consts.RIO_PREFIX + "midx")
                    + ".next();\n");
            cb.append(key.getType() + " " + getId(Consts.RIO_PREFIX + "k") + " = " + getId(Consts.RIO_PREFIX + "me")
                    + ".getKey();\n");
            cb.append(value.getType() + " " + getId(Consts.RIO_PREFIX + "v") + " = " + getId(Consts.RIO_PREFIX + "me")
                    + ".getValue();\n");
        ...
        void genSlurpBytes(CodeBuffer cb, String b, String s, String l) {
            cb.append("{\n");
            incrLevel();
            cb.append("int " + getId("mi") + " = org.apache.hadoop.record.Utils.readVInt(" + b + ", " + s + ");\n");
            cb.append("int " + getId("mz") + " = org.apache.hadoop.record.Utils.getVIntSize(" + getId("mi") + ");\n");
            cb.append(s + "+=" + getId("mz") + "; " + l + "-=" + getId("mz") + ";\n");
            cb.append("for (int " + getId("midx") + " = 0; " + getId("midx") + " < " + getId("mi") + "; "
                    + getId("midx") + "++) {");
        ...
        void genCompareBytes(CodeBuffer cb) {
            ...
            cb.append("s1+=" + getId("mz1") + "; s2+=" + getId("mz2") + "; l1-=" + getId("mz1") + "; l2-="
                    + getId("mz2") + ";\n");
            cb.append("for (int " + getId("midx") + " = 0; " + getId("midx") + " < " + getId("mi1") + " && "
                    + getId("midx") + " < " + getId("mi2") + "; " + getId("midx") + "++) {");
            key.genCompareBytes(cb);
            value.genSlurpBytes(cb, "b1", "s1", "l1");
            value.genSlurpBytes(cb, "b2", "s2", "l2");
            cb.append("}\n");
            cb.append("if (" + getId("mi1") + " != " + getId("mi2") + ") { return (" + getId("mi1") + "<"
                    + getId("mi2") + ")?-1:0; }\n");
            decrLevel();
            cb.append("}\n");
        }
