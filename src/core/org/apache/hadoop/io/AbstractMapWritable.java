 ...
 * </p>
 * 
 * Class ids range from 1 to 127 so there can be at most 127 distinct classes in
 * any specific map instance.
 */
public abstract class AbstractMapWritable implements Writable, Configurable {
    ...
    private synchronized void addToMap(Class clazz, byte id) {
        if (classToIdMap.containsKey(clazz)) {
            ...
            if (b != id) {
                throw new IllegalArgumentException("Class " + clazz.getName() + " already registered but maps to " + b
                        + " and not " + id);
            }
        }
        if (idToClassMap.containsKey(id)) {
            ...
            if (!c.equals(clazz)) {
                throw new IllegalArgumentException("Id " + id + " exists but maps to " + c.getName() + " and not "
                        + clazz.getName());
            }
        }
