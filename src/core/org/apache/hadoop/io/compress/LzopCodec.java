...
public class LzopCodec extends LzoCodec {
    ...
    private static final byte[] LZO_MAGIC = new byte[] { -119, 'L', 'Z', 'O', 0, '\r', '\n', '\032', '\n' };
    ...
    public CompressionOutputStream createOutputStream(OutputStream out, Compressor compressor) throws IOException {
        ...
        LzoCompressor.CompressionStrategy strategy = LzoCompressor.CompressionStrategy.valueOf(getConf().get(
                "io.compression.codec.lzo.compressor", LzoCompressor.CompressionStrategy.LZO1X_1.name()));
        int bufferSize = getConf().getInt("io.compression.codec.lzo.buffersize", 64 * 1024);
    ...
    public CompressionInputStream createInputStream(InputStream in, Decompressor decompressor) throws IOException {
        ...
        return new LzopInputStream(in, decompressor, getConf()
                .getInt("io.compression.codec.lzo.buffersize", 256 * 1024));
    }
    ...
    public Decompressor createDecompressor() {
        ...
        return new LzopDecompressor(getConf().getInt("io.compression.codec.lzo.buffersize", 256 * 1024));
    }
    ...
    protected static class LzopOutputStream extends BlockCompressorStream {
        ...
        public LzopOutputStream(OutputStream out, Compressor compressor, int bufferSize,
            ...
            super(out, compressor, bufferSize, strategy.name().contains("LZO1") ? (bufferSize >> 4) + 64 + 3
                    : (bufferSize >> 3) + 128 + 3);
            writeLzopHeader(out, strategy);
        }

        /**
         * Close the underlying stream and write a null word to the output
         * stream.
         */
    ...
    protected static class LzopInputStream extends BlockDecompressorStream {
        ...
        /**
         * Read len bytes into buf, st LSB of int returned is the last byte of
         * the first word read.
         */
        ...
        /**
         * Read bytes, update checksums, return first four bytes as an int,
         * first byte read in the MSB.
         */
        ...
        /**
         * Read and verify an lzo header, setting relevant block checksum
         * options and ignoring most everything else.
         */
        protected void readHeader(InputStream in) throws IOException {
            ...
            int hitem = readHeaderItem(in, buf, 2, adler, crc32); // lzop
                                                                  // version
            ...
            hitem = readHeaderItem(in, buf, 2, adler, crc32); // lzo library
                                                              // version
            if (hitem > LzoDecompressor.LZO_LIBRARY_VERSION) {
                throw new IOException("Compressed with incompatible lzo version: 0x" + Integer.toHexString(hitem)
                        + " (expected 0x" + Integer.toHexString(LzoDecompressor.LZO_LIBRARY_VERSION) + ")");
            }
            hitem = readHeaderItem(in, buf, 2, adler, crc32); // lzop extract
                                                              // version
            if (hitem > LZOP_VERSION) {
                throw new IOException("Compressed with incompatible lzop version: 0x" + Integer.toHexString(hitem)
                        + " (expected 0x" + Integer.toHexString(LZOP_VERSION) + ")");
            }
        ...
        protected void getCompressedData() throws IOException {
            ...
            for (CChecksum chk : ccheck.keySet()) {
                // NOTE: if the compressed size is not less than the
                // uncompressed
    ...
    protected static class LzopDecompressor extends LzoDecompressor {
        ...
        private EnumMap<DChecksum, Checksum> chkDMap = new EnumMap<DChecksum, Checksum>(DChecksum.class);
        private EnumMap<CChecksum, Checksum> chkCMap = new EnumMap<CChecksum, Checksum>(CChecksum.class);
