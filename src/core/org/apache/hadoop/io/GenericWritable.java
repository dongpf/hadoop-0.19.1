 ...
 * because <code>ObjectWritable</code> will append the class declaration as a
 * String into the output file in every Key-Value pair.
 * </p>
 * 
 * <p>
 * Generic Writable implements {@link Configurable} interface, so that it will
 * be configured by the framework. The configuration is passed to the wrapped
 * objects implementing {@link Configurable} interface <i>before
 * deserialization</i>.
 * </p>
 * 
 * how to use it: <br>
 ...
 * GenericWritable.<br>
 * 2. Implements the abstract method <code>getTypes()</code>, defines the
 * classes which will be wrapped in GenericObject in application. Attention:
 * this classes defined in <code>getTypes()</code> method, must implement
 * <code>Writable</code> interface. <br>
 * <br>
 * 
 * The code looks like this: <blockquote>
 * 
 * <pre>
 * public class GenericObject extends GenericWritable {
 * 
 *     private static Class[] CLASSES = { ClassType1.class, ClassType2.class, ClassType3.class, };
 * 
...
// NOTE
// reduce中同一个key的value必须为同一类型。当需要处理不同类型的value时，可以通过继承GenericWritable实现。实现的子类中需要提供类型数组，通过type记录当前实例所属的class在类型数组中的index。
