...
public class SequenceFile {
     ...
     */
    public static Writer createWriter(FileSystem fs, Configuration conf, Path name, Class keyClass, Class valClass,
            int bufferSize, short replication, long blockSize, CompressionType compressionType, CompressionCodec codec,
        ...
        if (compressionType == CompressionType.NONE) {
            ...
            writer = new RecordCompressWriter(fs, conf, name, keyClass, valClass, bufferSize, replication, blockSize,
    ...
    private static Writer createWriter(FileSystem fs, Configuration conf, Path file, Class keyClass, Class valClass,
            boolean compress, boolean blockCompress, CompressionCodec codec, Progressable progress, Metadata metadata)
            throws IOException {
        if (codec != null && (codec instanceof GzipCodec) && !NativeCodeLoader.isNativeCodeLoaded()
                && !ZlibFactory.isNativeZlibLoaded(conf)) {
            throw new IllegalArgumentException("SequenceFile doesn't work with "
    ...
    public static class Writer implements java.io.Closeable {
        ...
        public Writer(FileSystem fs, Configuration conf, Path name, Class keyClass, Class valClass,
                Progressable progress, Metadata metadata) throws IOException {
            this(fs, conf, name, keyClass, valClass, fs.getConf().getInt("io.file.buffer.size", 4096), fs
                    .getDefaultReplication(), fs.getDefaultBlockSize(), progress, metadata);
        }
        ...
        public Writer(FileSystem fs, Configuration conf, Path name, Class keyClass, Class valClass, int bufferSize,
            ...
            init(name, conf, fs.create(name, true, bufferSize, replication, blockSize, progress), keyClass, valClass,
                    false, null, metadata);
        ...
        private Writer(Configuration conf, FSDataOutputStream out, Class keyClass, Class valClass, Metadata metadata)
                throws IOException {
    ...
    static class RecordCompressWriter extends Writer {
        ...
        public RecordCompressWriter(FileSystem fs, Configuration conf, Path name, Class keyClass, Class valClass,
            ...
            this(fs, conf, name, keyClass, valClass, fs.getConf().getInt("io.file.buffer.size", 4096), fs
                    .getDefaultReplication(), fs.getDefaultBlockSize(), codec, progress, metadata);
        }
    ...
    static class BlockCompressWriter extends Writer {
        ...
        public BlockCompressWriter(FileSystem fs, Configuration conf, Path name, Class keyClass, Class valClass,
                CompressionCodec codec) throws IOException {
            this(fs, conf, name, keyClass, valClass, fs.getConf().getInt("io.file.buffer.size", 4096), fs
                    .getDefaultReplication(), fs.getDefaultBlockSize(), codec, null, new Metadata());
        }
    ...
    public static class Sorter {
            ...
            public int run(boolean deleteInput) throws IOException {
                ...
                while (!atEof) {
                    ...
                    while (!atEof && bytesProcessed < memoryLimit && count < recordLimit) {
                        ...
                        ValueBytes rawValue = (count == keyOffsets.length || rawValues[count] == null) ? in
                                .createValueBytes() : rawValues[count];
        ...
        private class MergeQueue extends PriorityQueue implements RawKeyValueIterator {
            ...
            protected boolean lessThan(Object a, Object b) {
                ...
                return comparator.compare(msa.getKey().getData(), 0, msa.getKey().getLength(), msb.getKey().getData(),
                        0, msb.getKey().getLength()) < 0;
            }
            ...
            public RawKeyValueIterator merge() throws IOException {
                ...
                do {
                    ...
                    if (numSegments <= factor) {
                        ...
                        Path outputFile = lDirAlloc
                                .getLocalPathForWrite(tmpFilename.toString(), approxOutputSize, conf);
                        LOG.debug("writing intermediate results to " + outputFile);
                        Writer writer = cloneFileAttributes(fs.makeQualified(segmentsToMerge.get(0).segmentPathName),
