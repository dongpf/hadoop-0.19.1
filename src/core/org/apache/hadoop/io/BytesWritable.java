...
/**
 * A byte sequence that is usable as a key or value. It is resizable and
 * distinguishes between the size of the seqeunce and the current capacity. The
 * hash function is the front of the md5 of the buffer. The sort order is the
 * same as memcmp.
 */
public class BytesWritable extends BinaryComparable implements WritableComparable<BinaryComparable> {
     ...
     * 
     * @param newData
     *            the value to set this BytesWritable to.
     */
    ...
    // NOTE
    // 每次修改不重写byte[]内容，只设置size大小。序列化反序列化时stream中首先存储size，然后是bytes。从stream中取size长度的bytes来获得有效内容。
    // inherit javadoc
