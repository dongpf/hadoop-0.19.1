 ...
 * locally-connected disk. The local version exists for small Hadoop instances
 * and for testing.
 * 
 * <p>
 * 
 * All user code that may potentially use the Hadoop Distributed File System
 * should be written to use a FileSystem object. The Hadoop DFS is a
 * multi-machine system that appears as a single disk. It's useful because of
 * its fault tolerance and potentially very large capacity.
 * 
 * <p>
...
public abstract class FileSystem extends Configured implements Closeable {
    ...
    /**
     * Parse the cmd-line args, starting at i. Remove consumed args from array.
     * We expect param in the form: '-local | -dfs <namenode:port>'
     * 
    ...
    public static FileSystem parseArgs(String argv[], int i, Configuration conf) throws IOException {
        /**
         * if (argv.length - i < 1) { throw new
         * IOException("Must indicate filesystem type for DFS"); }
         */
        int orig = i;
        FileSystem fs = null;
        String cmd = argv[i];
    ...
    /**
     * Close all cached filesystems. Be sure those filesystems are not used
     * anymore.
     * 
     * @throws IOException
     */
    ...
    /**
     * create a file with the provided permission The permission of the file is
     * set to be the provided permission as in setPermission, not
     * permission&~umask
     * 
     * It is implemented using two RPCs. It is understood that it is
     * inefficient, but the implementation is thread-safe. The other option is
     * to change the value of umask in configuration to be 0, but it is not
     * thread-safe.
     * 
     * @param fs
     *            file system handle
    ...
    /**
     * create a directory with the provided permission The permission of the
     * directory is set to be the provided permission as in setPermission, not
     * permission&~umask
     * 
    ...
    protected void checkPath(Path path) {
        ...
        if (thisScheme.equalsIgnoreCase(thatScheme)) {// schemes match
            ...
            if (thatAuthority == null && // path's authority is null
                ...
                URI defaultUri = getDefaultUri(getConf()); // & is the default
                                                           // fs
    ...
    static final String RCFILE_FLAG = "org.apache.hadoop.hive.ql.io.RCFile";

    /**
     * Opens an InputStream for read at given path. Compatible for
     * 
     * TextFile(Compressed or uncompressed), SequenceFile, RCFile
     * 
     */
    public InputStream open2(Path f, int bufferSize) throws IOException {
        FSDataInputStream i = open(f, bufferSize);
        try {
            switch (i.readShort()) {
            case 0x1f8b: // RFC 1952
                i.seek(0);
                return new BufferedInputStream(new GZIPInputStream(i));
            case 0x5345: // 'S' 'E'
                if (i.readByte() == 'Q') {
                    if (readString(i, RCFILE_FLAG.length()).equals(RCFILE_FLAG)) {
                        i.close();
                        return new RCFileRecordInputStream(this, f, getConf());
                    } else {
                        i.close();
                        return new SequenceFileInputStream(this, f, getConf());
                    }
                }
                break;
            default:
                ...
                CompressionCodec codec = compressionCodecs.getCodec(f);
                if (codec != null)
    ...
    /**
     * Read head string from stream
     * 
     */
    private String readString(FSDataInputStream i, int size) throws IOException {
        i.skipBytes(2);
    ...
    /**
     * Creates the given Path as a brand-new zero-length file. If create fails,
     * or if it already existed, return false.
     */
    ...
    /**
     * Append to an existing file (optional operation). Same as append(f,
     * getConf().getInt("io.file.buffer.size", 4096), null)
     * 
     * @param f
    ...
    /**
     * Mark a path to be deleted when FileSystem is closed. When the JVM shuts
     * down, all FileSystem objects will be closed automatically. Then, the
     * marked path will be deleted as a result of closing the FileSystem.
     * 
    ...
    /**
     * Listing a directory The returned results include its block location if it
     ...
     */
    public void listLocatedStatus(ArrayList<LocatedFileStatus> results, Path f, PathFilter filter) throws IOException {
        FileStatus listing[] = listStatus(f, filter);
        if (listing != null) {
            for (int i = 0; i < listing.length; i++) {
                BlockLocation[] locs = listing[i].isDir() ? null : getFileBlockLocations(listing[i], 0,
                        listing[i].getLen());
     ...
     *             if encounter any problem while fetching the status
     */
    public LocatedFileStatus[] listLocatedStatus(Path f, PathFilter filter) throws IOException {
        ArrayList<LocatedFileStatus> results = new ArrayList<LocatedFileStatus>();
     ...
     * their path names. Return null if pathPattern has no glob and the path
     * does not exist. Return an empty array if pathPattern has a glob and no
     * path matches it.
     * 
     * @param pathPattern
    ...
    private FileStatus[] globStatusInternal(Path pathPattern, PathFilter filter) throws IOException {
        ...
        Path[] parentPaths = globPathsLevel(parents, components, level, filter, hasGlob);
        FileStatus[] results;
     ...
     */
    private Path[] globPathsLevel(Path[] parents, String[] filePattern, int level, PathFilter filter, boolean[] hasGlob)
            throws IOException {
    ...
    /**
     * @return an iterator over the corrupt files under the given path (may
     *         contain duplicates if a file has more than one corrupt block)
     * @throws IOException
     */
    public RemoteIterator<Path> listCorruptFileBlocks(Path path) throws IOException {
        throw new UnsupportedOperationException(getClass().getCanonicalName() + " does not support"
    ...
    /**
     * Return the current user's home directory in this filesystem. The default
     * implementation returns "/user/$USER/".
     */
    ...
    /**
     * The src files is on the local disk. Add it to FS at the given dst name,
    ...
    /**
     * The src file is under FS, and the dst is on the local disk. Copy it from
     * FS control to the local dst name. Remove the source afterwards
     */
    ...
    /**
     * The src file is under FS, and the dst is on the local disk. Copy it from
     * FS control to the local dst name. delSrc indicates if the src will be
     * removed or not.
     */
     ...
     * @param f
     *            The file path
     * @return The file checksum. The default return value is null, which
     *         indicates that no checksum algorithm is implemented in the
    ...
    /**
     * Return a list of file status objects that corresponds to the list of
     * paths excluding those non-existent paths.
     * 
     * @param paths
    ...
    public static final class Statistics {
        ...
        public String toString() {
            return bytesRead + " bytes read and " + bytesWritten + " bytes written";
        }
    }
