...
public class ContentSummary implements Writable {
    ...
    /**
     * Output format: <----12----> <----12----> <-------18-------> DIR_COUNT
     * FILE_COUNT CONTENT_SIZE FILE_NAME
     */
    private static final String STRING_FORMAT = "%12d %12d %18d ";
    /**
     * Output format: <----12----> <----15----> <----15----> <----15---->
     * <----12----> <----12----> <-------18-------> QUOTA REMAINING_QUATA
     * SPACE_QUOTA SPACE_QUOTA_REM DIR_COUNT FILE_COUNT CONTENT_SIZE FILE_NAME
     */
    ...
    private static final String HEADER = String
            .format(STRING_FORMAT.replace('d', 's'), "directories", "files", "bytes");

    private static final String QUOTA_HEADER = String.format(QUOTA_STRING_FORMAT + SPACE_QUOTA_STRING_FORMAT, "quota",
            "remaining quota", "space quota", "reamaining quota") + HEADER;

    /**
     * Return the header of the output. if qOption is false, output directory
     * count, file count, and content size; if qOption is true, output quota and
     * remaining quota as well.
     * 
     * @param qOption
    ...
    /**
     * Return the string representation of the object in the output format. if
     * qOption is false, output directory count, file count, and content size;
    ...
    public String toString(boolean qOption) {
        ...
        if (qOption) {
            ...
            prefix = String.format(QUOTA_STRING_FORMAT + SPACE_QUOTA_STRING_FORMAT, quotaStr, quotaRem, spaceQuotaStr,
                    spaceQuotaRem);
        }
