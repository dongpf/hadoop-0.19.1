...
public class FsShell extends Configured implements Tool {
   ...
   */
    ...
    /**
     * Obtain the indicated files that match the file pattern <i>srcf</i> and
     * copy them to the local name. srcf is kept. When copying multiple files,
     * the destination must be a directory. Otherwise, IOException is thrown.
     * 
     * @param argv
     *            : arguments
     * @param pos
     *            : Ignore everything before argv[pos]
    ...
    /**
     * The prefix for the tmp file used in copyToLocal. It must be at least
     * three characters long, required by
     ...
     * and merge and sort them to only one file on local fs srcf is kept.
     * 
     * @param srcf
     ...
     *            : a destination local file/directory
     * @param endline
     *            : if an end of line character is added to a text file
     * @exception: IOException
     * @see org.apache.hadoop.fs.FileSystem.globStatus
     */
    ...
    /**
     * Obtain the indicated file and copy to the local name. srcf is removed.
     */
    ...
    /**
     * Fetch all files that match the file pattern <i>srcf</i> and display their
     * content on stdout.
     * 
     * @param srcf
     *            : a file pattern specifying source files
    ...
    private InputStream forMagic(Path p, FileSystem srcFs) throws IOException {
        if (srcFs.getFileStatus(p).getLen() == 0) {
            ...
            return srcFs.open2(p, getConf().getInt("io.file.buffer.size", 4096));
        }
    }
     ...
     * @param cmd
     * @param pos
     *            ignore anything before this pos in cmd
     * @throws IOException
     */
    ...
    /**
     * Set the replication for files that match file pattern <i>srcf</i> if it's
     * a directory and recursive is true, set replication for all the subdirs
     * and those files too.
     * 
     * @param newRep
    ...
    /**
     * Actually set the replication for this file If it fails either throw
     * IOException or print an error msg
     * 
     * @param file
     *            : a file/directory
     * @param newRep
     ...
     */
    private int ls(Path src, FileSystem srcFs, boolean recursive, boolean printHeader) throws IOException {
    ...
    void du(String src) throws IOException {
        ...
        if ((items == null) || ((items.length == 0) && (!srcFs.exists(srcPath)))) {
            ...
            for (int i = 0; i < items.length; i++) {
                String size = String.valueOf(srcFs.getContentSummary(items[i].getPath()).getLength());
                if (size.length() > maxLength)
                    maxLength = size.length();
            }
            for (int i = 0; i < items.length; i++) {
                System.out.printf("%-" + (maxLength + BORDER) + "d", srcFs.getContentSummary(items[i].getPath())
                        .getLength());
    ...
    /**
     * Show the summary disk usage of each dir/file that matches the file
     * pattern <i>src</i>
     * 
     * @param src
     *            a file pattern specifying source files
    ...
    /**
     * Print statistics about path in specified format. Format sequences: %b:
     * Size of file in blocks %n: Filename %o: Block size %r: replication %y:
     * UTC date as &quot;yyyy-MM-dd HH:mm:ss&quot; %Y: Milliseconds since
     * January 1, 1970 UTC
     */
    ...
    /**
     * Move files that match the file pattern <i>srcf</i> to a destination file.
    ...
    /**
     * Move/rename file(s) to a destination file. Multiple source files can be
     * specified. The destination is the last element of the argvp[] array. If
     * multiple source files are specified, then the destination must be a
     * directory. Otherwise, IOException is thrown.
     * 
     * @exception: IOException
     */
    private int rename(String argv[], Configuration conf) throws IOException {
        ...
        if (argv.length > 3) {
            ...
            if (!dstFs.isDirectory(dst)) {
                throw new IOException("When moving multiple files, " + "destination " + dest
    ...
    /**
     * Copy files that match the file pattern <i>srcf</i> to a destination file.
    ...
    /**
     * Copy file(s) to a destination file. Multiple source files can be
     * specified. The destination is the last element of the argvp[] array. If
     * multiple source files are specified, then the destination must be a
     * directory. Otherwise, IOException is thrown.
     * 
    ...
    private int copy(String argv[], Configuration conf) throws IOException {
        ...
        if (argv.length > 3) {
            ...
            if (!fs.isDirectory(dst)) {
                throw new IOException("When copying multiple files, " + "destination " + dest
                        + " should be a directory.");
            }
        }
        //
    ...
    private static FileStatus[] shellListStatus(String cmd, FileSystem srcFs, Path path) {
        try {
    ...
    /**
     * Runs the command on a given file with the command handler. If recursive
     * is set, command is run recursively.
     */
    private static int runCmdHandler(CmdHandler handler, FileStatus stat, FileSystem srcFs, boolean recursive)
        ...
        if (recursive && stat.isDir() && handler.okToContinue()) {
            FileStatus[] files = shellListStatus(handler.getName(), srcFs, stat.getPath());
            if (files == null) {
                return 1;
            }
    ...
    private void printHelp(String cmd) {
        String summary = "hadoop fs is the command to execute fs commands. " + "The full syntax is: \n\n"
                ...
                + "[-rmi <src>] [-put <localsrc> ... <dst>] [-copyFromLocal <localsrc> ... <dst>]\n\t"
        ...
        String rmi = "-rmi <src>: \tRemove all directories which match the specified file \n"
                + "\t\tpattern. Equivlent to the Unix command \"rm -rfi <src>\"\n";
        ...
        /*
         * String text =
         * "-text <path>: Attempt to decode contents if the first few bytes\n" +
        ...
        for (; i < argv.length; i++) {
            try {
                ...
                if ("-cat".equals(cmd)) {
                    ...
                    System.out.println("delete path: " + argv[i]);
    ...
    private static void printUsage(String cmd) {
        ...
        if ("-fs".equals(cmd)) {
            ...
            System.err.println("Usage: java FsShell" + " [-D <[property=value>]");
                ...
                || "-rm".equals(cmd) || "-rmr".equals(cmd) || "-rmi".equals(cmd) || "-touchz".equals(cmd)
                || "-mkdir".equals(cmd) || "-text".equals(cmd)) {
    ...
    public int run(String argv[]) throws Exception {
        ...
        } else if ("-rm".equals(cmd) || "-rmr".equals(cmd) || "-rmi".equals(cmd) || "-cat".equals(cmd)
                || "-mkdir".equals(cmd) || "-touchz".equals(cmd) || "-stat".equals(cmd) || "-text".equals(cmd)) {
            if (argv.length < 2) {
                printUsage(cmd);
                return exitCode;
            }
