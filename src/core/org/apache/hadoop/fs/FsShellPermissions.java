...
/**
 * This class is the home for file permissions related commands. Moved to this
 * seperate class since FsShell is getting too large.
 */
class FsShellPermissions {
    ...
    /*
     * The pattern is alsmost as flexible as mode allowed by chmod shell
     * command. The main restriction is that we recognize only rwxX. To reduce
     * errors we also enforce 3 digits for octal mode.
     */
    ...
    private static class ChmodHandler extends CmdHandler {
        ...
        public void run(FileStatus file, FileSystem srcFs) throws IOException {
            ...
            int newperms = (applyChmod(userType, userMode, (existing >>> 6) & 7, exeOk) << 6
                    | applyChmod(groupType, groupMode, (existing >>> 3) & 7, exeOk) << 3 | applyChmod(othersType,
                    othersMode, existing & 7, exeOk));

            if (existing != newperms) {
                try {
                    srcFs.setPermission(file.getPath(), new FsPermission((short) newperms));
                } catch (IOException e) {
                    System.err.println(getName() + ": changing permissions of '" + file.getPath() + "':"
                            + e.getMessage());
                }
            }
        }
    ...
    static private Pattern chownPattern = Pattern.compile("^\\s*(" + allowedChars + "+)?" + "([:](" + allowedChars
            + "*))?\\s*$");
    static private Pattern chgrpPattern = Pattern.compile("^\\s*(" + allowedChars + "+)\\s*$");
    ...
    private static class ChownHandler extends CmdHandler {
        ...
        ChownHandler(FileSystem fs, String ownerStr) throws IOException {
            ...
            if (owner == null && group == null) {
                throw new IOException("'" + ownerStr + "' does not specify " + " onwer or group.");
            }
        }
        ...
        public void run(FileStatus file, FileSystem srcFs) throws IOException {
            ...
            String newOwner = (owner == null || owner.equals(file.getOwner())) ? null : owner;
            String newGroup = (group == null || group.equals(file.getGroup())) ? null : group;
            ...
            if (newOwner != null || newGroup != null) {
                try {
                    ...
                    System.err
                            .println(getName() + ": changing ownership of '" + file.getPath() + "':" + e.getMessage());
    ...
    static void changePermissions(FileSystem fs, String cmd, String argv[], int startIndex, FsShell shell)
            throws IOException {
        CmdHandler handler = null;
        boolean recursive = false;

        // handle common arguments, currently only "-R"
        for (; startIndex < argv.length && argv[startIndex].equals("-R"); startIndex++) {
            recursive = true;
        }
