...
abstract public class FSInputChecker extends FSInputStream {
     ...
     * <code>offset</code> and checksum into <code>checksum</code>. The method
     * is used for implementing read, therefore, it should be optimized for
     * sequential reading
     * 
     * @param pos
     *            chunkPos
    ...
    /**
     * Read checksum verified bytes from this byte-input stream into the
     ...
     * @exception IOException
     *                if an I/O error occurs. ChecksumException if any checksum
     *                error occurs
     */
    ...
    /**
     * Fills the buffer with a chunk data. No mark is supported. This method
     * assumes that all data in the buffer has already been read in, hence pos >
     * count.
     */
    ...
    /*
     * Read up one checksum chunk to array <i>b</i> at pos <i>off</i> It
     * requires a checksum chunk boundary in between <cur_pos, cur_pos+len> and
     * it stops reading at the boundary or at the end of the stream; Otherwise
     * an IllegalArgumentException is thrown. This makes sure that all data read
    ...
    private int readChecksumChunk(byte b[], int off, int len) throws IOException {
        ...
        do {
            ...
            try {
                ...
                LOG.info(
                        "Found checksum error: b[" + off + ", " + (off + read) + "]="
                ...
                if (seekToNewSource(chunkPos)) {
                    ...
                    // Neither the data stream nor the checksum stream are being
                    // read
                    // from different sources, meaning we'll still get a
                    // checksum error
                    // if we try to do the read again. We throw an exception
                    // instead.
                    throw ce;
                }
            }
     ...
     * @exception IOException
     *                if an I/O error occurs. ChecksumException if the chunk to
     *                skip to is corrupted
     */
    ...
    /**
     * Seek to the given position in the stream. The next read() will be from
     * that position.
     * 
     * <p>
     * This method may seek past the end of the file. This produces no exception
     * and an attempt to read from the stream will result in -1 indicating the
     * end of the file.
     * 
     * @param pos
     ...
     *            checksum size
     */
    final protected synchronized void set(Checksum sum, int maxChunkSize, int checksumSize) {
        this.sum = sum;
