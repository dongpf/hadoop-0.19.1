 ...
 * files. The way it works is that it is kept track what disk was last allocated
 * for a file write. For the current request, the next disk from the set of
 * disks would be allocated if the free space on the disk is sufficient enough
 * to accomodate the file that is being considered for creation. If the space
 * requirements cannot be met, the next disk in order would be tried and so on
 * till a disk is found with sufficient capacity. Once a disk with sufficient
 * space is identified, a check is done to make sure that the disk is writable.
 * Also, there is an API provided that doesn't take the space requirements into
 * consideration but just checks whether the disk under consideration is
 * writable (this should be used for cases where the file size is not known
 * apriori). An API is provided to read a path that was created earlier. That
 * API works by doing a scan of all the disks for the input pathname. This
 * implementation also provides the functionality of having multiple allocators
 * per JVM (one for each unique functionality or context, like mapred,
 * dfs-client, etc.). It ensures that there is only one instance of an allocator
 * per context per JVM. Note: 1. The contexts referred above are actually the
 * configuration items defined in the Configuration class like
 * "mapred.local.dir" (for which we want to control the dir allocations). The
 * context-strings are exactly those configuration items. 2. This implementation
 * does not take into consideration cases where a disk becomes read-only or goes
 * out of space while a file is being written to (disks are shared between
 * multiple processes, and so the latter situation is probable). 3. In the class
 * implementation, "Disk" is referred to as "Dir", which actually points to the
 * configured directory on the Disk which will be the parent for all file
 * write/read allocations.
 */
public class LocalDirAllocator {
     ...
     * defined in the Configuration object for which we want to control the dir
     * allocations (e.g., <code>mapred.local.dir</code>). The method will create
     ...
     * (via the configured dirs) and return the first complete path where we
     * could create the parent directory of the passed path.
     * 
     * @param pathStr
     ...
     * configured dirs for the file's existence and return the complete path to
     * the file when we find one
     * 
     * @param pathStr
     *            the requested file (this will be searched)
     * @param conf
     *            the Configuration object
     * @return the complete path to the file on a local disk
     * @throws IOException
     ...
     * created on this directory. The file is guaranteed to go away when the JVM
     * exits.
     * 
     * @param pathStr
    ...
    /**
     * We search through all the configured dirs for the file's existence and
     * return true when we find
     * 
     * @param pathStr
     *            the requested file (this will be searched)
     * @param conf
    ...
    private static class AllocatorPerContext {
         ...
         * 
         * It will use roulette selection, picking directories with probability
         ...
         * over the set of disks (via the configured dirs) and return the first
         * complete path which has enough space.
         * 
         * If size is not known, use roulette selection -- pick directories with
         * probability proportional to their available space.
         */
        public synchronized Path getLocalPathForWrite(String pathStr, long size, Configuration conf) throws IOException {
            ...
            if (size == -1) { // do roulette selection: pick dir with
                              // probability
         ...
         * We round-robin over the set of disks (via the configured dirs) and
         * return a file on the first path which has enough space. The file is
         * guaranteed to go away when the JVM exits.
         */
        public File createTmpFileForWrite(String pathStr, long size, Configuration conf) throws IOException {
        ...
        /**
         * Get a path from the local FS for reading. We search through all the
         * configured dirs for the file's existence and return the complete path
         * to the file when we find one
         */
        ...
        public synchronized boolean ifExists(String pathStr, Configuration conf) {
            try {
                int numDirs = localDirs.length;
                int numDirsSearched = 0;
                // remove the leading slash from the path (to make sure that the
                // uri
                // resolution results in a valid path on the dir being checked)
                if (pathStr.startsWith("/")) {
                    pathStr = pathStr.substring(1);
