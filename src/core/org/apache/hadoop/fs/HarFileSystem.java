...
/**
 * This is an implementation of the Hadoop Archive Filesystem. This archive
 * Filesystem has index files of the form _index* and has contents of the form
 * part-*. The index files store the indexes of the real files. The index files
 * are of the form _masterindex and _index. The master index is a level of
 * indirection in to the index file to make the look ups faster. the index file
 * is sorted with hash code of the paths that it contains and the master index
 * contains pointers to the positions in index for ranges of hashcodes.
 */
...
public class HarFileSystem extends FilterFileSystem {
    ...
    /**
     * Initialize a Har filesystem per har archive. The archive home directory
     * is the top level directory in the filesystem that contains the HAR
     * archive. Be careful with this method, you do not want to go on creating
     * new Filesystem instances per call to path.getFileSystem(). the uri of Har
     * is har://underlyingfsscheme-host:port/archivepath. or har:///archivepath.
     * This assumes the underlying filesystem to be used in case not specified.
     */
    ...
    /*
     * find the parent path that is the archive path in the path. The last path
     * segment that ends with .har is the path that will be returned.
     */
     ...
     * 
     * @param underLyingURI
     *            the uri of underlying filesystem
    ...
    /**
     * Returns the uri of this filesystem. The uri is of the form
    ...
    /**
     * this method returns the path inside the har filesystem. this is relative
     * path inside the har filesystem.
     * 
     * @param path
     *            the fully qualified path in the har filesystem.
    ...
    /*
     * this makes a path qualified in the har filesystem (non-Javadoc)
     * 
     ...
     * 
     * @param file
     *            the input filestatus to get block locations
     * @param start
     *            the start in the file
     * @param len
    ...
    /**
     * fake the rawblocks since map reduce uses the block offsets to fo some
     * computations regarding the blocks
     * 
     * @param rawBlocks
     *            the raw blocks returned by the filesystem
    ...
    /**
     * return the filestatus of files in har archive. The permission returned
     * are that of the archive index files. The permissions are not persisted
    ...
    public FileStatus getFileStatus(Path f) throws IOException {
        ...
        return new FileStatus(hstatus.isDir() ? 0 : hstatus.getLength(), hstatus.isDir(),
                ...
                archiveStatus.getModificationTime(), archiveStatus.getAccessTime(), new FsPermission(
                        archiveStatus.getPermission()), archiveStatus.getOwner(), archiveStatus.getGroup(),
                makeRelative(this.uri.toString(), new Path(hstatus.name)));
    }

    /**
     * Returns a har input stream which fakes end of file. It reads the index
     * files to get the part file name and the size and start of the file.
     */
    @Override
    ...
    /*
     * create throws an exception in Har filesystem. The archive once created
     * cannot be changed.
     */
    ...
    /**
     * liststatus returns the children of a directory after looking up the index
     * files.
     */
    @Override
    public FileStatus[] listStatus(Path f) throws IOException {
        ...
        HarStatus hstatus = new HarStatus(readStr);
        if (!hstatus.isDir())
            statuses.add(new FileStatus(hstatus.getLength(), hstatus.isDir(), archiveStatus.getReplication(),
                    archiveStatus.getBlockSize(), archiveStatus.getModificationTime(), archiveStatus.getAccessTime(),
                    new FsPermission(archiveStatus.getPermission()), archiveStatus.getOwner(),
                    archiveStatus.getGroup(), makeRelative(this.uri.toString(), new Path(hstatus.name))));
        else
    ...
    /**
     * Hadoop archives input stream. This input stream fakes EOF since archive
     * files are part of bigger part files.
     */
    private static class HarFSDataInputStream extends FSDataInputStream {
       ...
       */
         ...
         * @param p
         *            The path in the underlying filesystem
         * @param start
         *            the start position in the part file
         * @param length
