...
public class FTPFileSystem extends FileSystem {
    ...
    public FSDataInputStream open(Path file, int bufferSize) throws IOException {
        ...
        // file
        // on the server by opening up an InputStream. As a side effect the
        // working
        // directory on the server is changed to the parent directory of the
        // file.
    ...
    public FSDataOutputStream create(Path file, FsPermission permission, boolean overwrite, int bufferSize,
        ...
        client.allocate(bufferSize);
        // Change to parent directory on the server. Only then can we write to
        // the
        // file on the server by opening up an OutputStream. As a side effect
        // the
        // working directory on the server is changed to the parent directory of
        // the
        // file. The FTP client connection is closed when close() is called on
        // the
        // FSDataOutputStream.
        client.changeWorkingDirectory(parent.toUri().getPath());
    ...
    private FileStatus getFileStatus(FTPFile ftpFile, Path parentPath) {
        ...
        int blockReplication = 1;
        // Using default block size since there is no way in FTP client to know
        // of
        ...
        return new FileStatus(length, isDir, blockReplication, blockSize, modTime, accessTime, permission, user, group,
