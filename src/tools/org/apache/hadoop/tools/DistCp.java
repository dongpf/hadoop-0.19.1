...
public class DistCp implements Tool {
    ...
    private static final String usage = NAME + " [OPTIONS] <srcurl>* <desturl>" + "\n\nOPTIONS:"
            ...
            + NAME + " -p -update \"hdfs://A:8020/user/foo/bar\" " + "\"hdfs://B:8020/user/foo/baz\"\n"
    ...
    static enum Options {
        DELETE("-delete", NAME + ".delete"), FILE_LIMIT("-filelimit", NAME + ".limit.file"), SIZE_LIMIT("-sizelimit",
                NAME + ".limit.size"), IGNORE_READ_FAILURES("-i", NAME + ".ignore.read.failures"), PRESERVE_STATUS(
                "-p", NAME + ".preserve.status"), OVERWRITE("-overwrite", NAME + ".overwrite.always"), UPDATE(
                "-update", NAME + ".overwrite.ifnewer");
            ...
            for (char c : s.toCharArray()) {
                ...
                if (i < attributes.length) {
                    if (!set.contains(attributes[i])) {
                        ...
                        throw new IllegalArgumentException("There are more than one '" + attributes[i].symbol + "' in "
                                + s);
                    }
                } else {
        ...
        public InputSplit[] getSplits(JobConf job, int numSplits) throws IOException {
            ...
            try {
                ...
                for (; sl.next(key, value); last = sl.getPosition()) {
                    // if adding this split would put this split past the target
                    // size,
                    // cut the last split and put this next file in the next
                    // split.
    ...
    static class CopyFilesMapper implements Mapper<LongWritable, FilePair, WritableComparable<?>, Text> {
        ...
        /**
         * Return true if dst should be replaced by src and the update flag is
         * set. Right now, this merely checks that the src and dst len are not
         * equal. This should be improved on once modification times, CRCs, etc.
         * can be meaningful in this context.
         * 
         * @throws IOException
         */
        ...
        private FSDataOutputStream create(Path f, Reporter reporter, FileStatus srcstat) throws IOException {
            ...
            short replication = preseved.contains(FileAttribute.REPLICATION) ? srcstat.getReplication() : destFileSys
                    .getDefaultReplication();
            long blockSize = preseved.contains(FileAttribute.BLOCK_SIZE) ? srcstat.getBlockSize() : destFileSys
                    .getDefaultBlockSize();
        ...
        private void copy(FileStatus srcstat, Path relativedst, OutputCollector<WritableComparable<?>, Text> outc,
            ...
            if (srcstat.isDir()) {
                ...
                // TODO: when modification times can be set, directories should
                // be
                // emitted to reducers so they might be preserved. Also, mkdirs
                // does
                // not currently return an error when the directory already
                // exists;
                // if this changes, all directory work might as well be done in
                // reduce
                return;
            }
            ...
            if (cbcopied != srcstat.getLen()) {
                throw new IOException("File size not matched: copied " + bytesString(cbcopied) + " to tmpfile (="
                        + tmpfile + ") but expected " + bytesString(srcstat.getLen()) + " from " + srcstat.getPath());
            } else {
                if (totfiles == 1) {
        ...
        private void rename(Path tmp, Path dst) throws IOException {
            try {
                ...
                throw (IOException) new IOException("Fail to rename tmp file (=" + tmp + ") to destination file (="
                        + dst + ")").initCause(cause);
            }
        }
         ...
         * well as top-level paths on source and destination directories. Gets
         * the named file systems, to be used later in map.
         */
         ...
         *            src len
         * @param value
         *            FilePair (FileStatus src, Path dst)
         * @param out
         *            Log of failed copies
         * @param reporter
         */
    ...
    public static void copy(Configuration conf, String srcPath, String destPath, Path logPath, boolean srcAsList,
        ...
        EnumSet<Options> flags = ignoreReadFailures ? EnumSet.of(Options.IGNORE_READ_FAILURES) : EnumSet
                .noneOf(Options.class);

        final Path dst = new Path(destPath);
        copy(conf, new Arguments(tmp, dst, logPath, flags, null, Long.MAX_VALUE, Long.MAX_VALUE));
    }
    ...
    static private void finalize(Configuration conf, JobConf jobconf, final Path destPath, String presevedAttributes)
            throws IOException {
    ...
    static private class Arguments {
         ...
         *            Size limit
         */
        Arguments(List<Path> srcs, Path dst, Path log, EnumSet<Options> flags, String preservedAttributes,
        ...
        static Arguments valueOf(String[] args, Configuration conf) throws IOException {
            ...
            if (isDelete && !isOverwrite && !isUpdate) {
                throw new IllegalArgumentException(Options.DELETE.cmd + " must be specified with " + Options.OVERWRITE
                        + " or " + Options.UPDATE + ".");
            }
    ...
    /**
     * Make a path relative with respect to a root path. absPath is always
     * assumed to descend from root. Otherwise returned path is null.
     */
     ...
     * BYTES_PER_MAP or -m on the command line) and at most
     * (distcp.max.map.tasks, default MAX_MAPS_PER_NODE * nodes in the cluster).
     * 
     * @param totalBytes
     *            Count of total bytes for job
     * @param job
     ...
     * 
     * @param conf
     *            : The dfs/mapred configuration.
     * @param jobConf
     *            : The handle to the jobConf object to be initialized.
     * @param args
     *            Arguments
     */
    private static void setup(Configuration conf, JobConf jobConf, final Arguments args) throws IOException {
        ...
        SequenceFile.Writer dst_writer = SequenceFile.createWriter(jobfs, jobConf, dstfilelist, Text.class, Text.class,
                ...
                for (pathstack.push(srcfilestat); !exceededlimit && !pathstack.empty();) {
                    ...
                    for (int i = 0; !exceededlimit && i < children.length; i++) {
                        ...
                        if (child.isDir()) {
                            ...
                            // skip file if the src and the dst files are the
                            // same.
                        ...
                        if (!skipfile && !exceededlimit) {
                            src_writer.append(new LongWritable(child.isDir() ? 0 : child.getLen()), new FilePair(child,
                                    dst));
        ...
        Path tmpDir = new Path((dstExists && !dstIsDir) || (!dstExists && srcCount == 1) ? args.dst.getParent()
                : args.dst, "_distcp_tmp_" + randomId);
     ...
     * 
     * When file checksum is not supported in any of file systems, two files are
    ...
    static private void deleteNonexisting(FileSystem dstfs, FileStatus dstroot, Path dstsorted, FileSystem jobfs,
        ...
        if (!dstroot.isDir()) {
            throw new IOException("dst must be a directory when option " + Options.DELETE.cmd + " is set, but dst (= "
                    + dstroot.getPath() + ") is not a directory.");
        }
        ...
        try {
            ...
            for (; in.next(curdst, cursrc);) {
                if (prevdst != null && curdst.equals(prevdst)) {
                    throw new DuplicationException("Invalid input, there are duplicated files in the sources: "
                            + prevsrc + ", " + cursrc);
                }
                prevdst = curdst;
                curdst = new Text();
